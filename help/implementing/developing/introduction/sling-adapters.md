...
* 按 [`AdapterFactory`](https://sling.apache.org/apidocs/sling5/org/apache/sling/api/adapter/AdapterFactory.html)，可映射任意对象。
   ...
   对象仍必须实现 `Adaptable` 界面和必须 [`SlingAdaptable`](https://www.adobe.io/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/adapter/SlingAdaptable.html) (通过 `adaptTo` 调用中央适配器管理器)。

   这允许将挂接到 `adaptTo` 现有类的机制，例如 `Resource`.

* 两者兼有。

对于第一种情况，javaoc可以说明 `adaptTo-targets` 是可能的。 但是，对于特定子类（如基于JCR的资源），通常不可能执行此操作。 在后一种情况下，实施 `AdapterFactory` 通常是包的私有类的一部分，因此不会在客户端API中公开，也不会在javaoc中列出。 理论上，可以访问所有 `AdapterFactory` 实施 [OSGi](/help/implementing/deploying/configuring-osgi.md) 服务运行时并查看其“适配器”（源和目标）配置，但不要将它们相互映射。 最后，这取决于必须记录的内部逻辑。 因此，该参考。

## 引用 {#reference}

### Sling {#sling}

[**资源**](https://www.adobe.io/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/api/resource/Resource.html) 调整为：
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>如果这是基于JCR节点的资源（或其他资源支持值映射），则返回属性的方便使用映射。 也可以（更简单）使用<br /> <code><a href="https://www.adobe.io/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/api/resource/ResourceUtil.html#getvaluemap%28org.apache.sling.api.resource.resource%29">ResourceUtil.getValueMap(Resource)</a></code> （处理空大小写等）</td>
  </tr>
  <tr>
   ...
   <td>扩展 <a href="https://www.adobe.io/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/api/resource/ValueMap.html">ValueMap</a> 这允许在查找属性时考虑资源的层次结构。</td>
  </tr>
  <tr>
   ...
   <td>的扩展 <a href="https://www.adobe.io/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/api/resource/ValueMap.html">ValueMap</a>，用于修改该节点上的属性。</td>
  </tr>
  <tr>
...
[**ResourceResolver**](https://www.adobe.io/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/api/resource/ResourceResolver.html) 调整为：
...
[**SlingHttpServletRequest**](https://www.adobe.io/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/api/SlingHttpServletRequest.html) 调整为：

尚无目标，但实施了“适应性”，可在自定义AdapterFactory中用作源。

[**SlingHttpServletResponse**](https://www.adobe.io/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/api/SlingHttpServletResponse.html) 调整为：
...
**[页面](https://www.adobe.io/experience-manager/reference-materials/cloud-service/javadoc/com/day/cq/wcm/api/Page.html)** 调整为：
...
**[组件](https://www.adobe.io/experience-manager/reference-materials/cloud-service/javadoc/com/day/cq/wcm/api/components/Component.html)** 调整为：
...
**[模板](https://www.adobe.io/experience-manager/reference-materials/cloud-service/javadoc/com/day/cq/wcm/api/Template.html)** 调整为：
