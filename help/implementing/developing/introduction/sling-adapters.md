...
* 快捷方式创建需要传递内部上下文对象的对象。
  ...
  例如，基于JCR [`ResourceResolver`](https://sling.apache.org/apidocs/sling5/org/apache/sling/api/resource/ResourceResolver.html) 包含对请求的 [`JCR Session`](https://developer.adobe.com/experience-manager/reference-materials/spec/jsr170/javadocs/jcr-2.0/javax/jcr/Session.html)，而基于请求会话工作的许多对象又需要这些服务，例如 [`PageManager`](https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/com/day/cq/wcm/api/PageManager.html) 或 [`UserManager`](https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/com/day/cq/security/UserManager.html).
...
但是，没有一般规则 — 对象可以是新实例或现有实例。 因此，这意味着您无法依赖任何一种行为。 因此，它很重要，尤其是在内部 `AdapterFactory`，在这种情况下，对象可重用。
...
* 按 [`AdapterFactory`](https://sling.apache.org/apidocs/sling5/org/apache/sling/api/adapter/AdapterFactory.html)，可以映射任意对象。
  ...
  对象仍必须实施 `Adaptable` 接口，并且必须扩展 [`SlingAdaptable`](https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/adapter/SlingAdaptable.html) (通过 `adaptTo` 调用中央适配器管理器)。

  此方法允许挂接进入 `adaptTo` 用于现有类的机制，例如 `Resource`.

* 两者的组合。

对于第一种情况，Java™文档可以说明 `adaptTo-targets` 是可能的。 但是，对于特定的子类（如基于JCR的资源），此语句通常是不可能的。 在后一种情况下，实施 `AdapterFactory` 通常是捆绑包的私有类的一部分，因此不会在客户端API中公开，也不会在Java™文档中列出。 从理论上讲，可以访问 `AdapterFactory` 实施来自 [osgi](/help/implementing/deploying/configuring-osgi.md) 服务运行时并查看其“可适应的”（源和目标）配置，但不会将它们相互映射。 最后，它取决于内部逻辑，必须记录该逻辑。 因此，此参考内容。

## 引用 {#reference}

### Sling {#sling}

[**资源**](https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/api/resource/Resource.html) 可适应：
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>如果是基于JCR节点的资源（或其他支持值的资源映射），则返回便于使用的属性映射。 也可使用（更简单地）实现<br /> <code><a href="https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/api/resource/ResourceUtil.html">ResourceUtil.getValueMap(Resource)</a></code> （处理null大小写等）</td>
  </tr>
  <tr>
   ...
   <td>扩展 <a href="https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/api/resource/ValueMap.html">值映射</a> 这允许在查找资产时考虑资源的层次结构</td>
  </tr>
  <tr>
   ...
   <td>的扩展 <a href="https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/api/resource/ValueMap.html">值映射</a>，可修改该节点的属性</td>
  </tr>
  <tr>
   <td><a href="https://docs.oracle.com/javase/1.5.0/docs/api/java/io/InputStream.html">输入流</a></td>
   <td>返回文件资源的二进制内容(如果它是基于JCR节点的资源，并且节点类型为 <code>nt:file</code> 或 <code>nt:resource</code>；如果是捆绑资源，则为文件内容；如果是文件系统资源，则为文件内容)。 或者，返回二进制JCR属性资源的数据。</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>如果它是基于JCR节点的资源，并且节点是 <code>cq:Page</code> (或 <code>cq:PseudoPage</code>)</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>如果它是一个设计节点(<code>cq:Page</code>)</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>如果它低于 <code>cq:ContentSyncConfig</code> 节点资源</td>
  </tr>
 </tbody>
</table>

[**ResourceResolver**](https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/api/resource/ResourceResolver.html) 可适应：
...
<table>
 <tbody>
  ...
  <tr>
   <td><a href="https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/com/day/cq/wcm/api/designer/Designer.html">设计器</a></td>
   <td> </td>
  </tr>
  <tr>
...
[**SlingHttpServletRequest**](https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/api/SlingHttpServletRequest.html) 可适应：

还没有目标，但实施了Adaptable，并且可以在自定义AdapterFactory中用作源。

[**SlingHttpServletResponse**](https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/org/apache/sling/api/SlingHttpServletResponse.html) 可适应：
...
**[页面](https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/com/day/cq/wcm/api/Page.html)** 可适应：
...
**[组件](https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/com/day/cq/wcm/api/components/Component.html)** 可适应：
...
|---|---|
| [LabeledResource](https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/com/day/cq/commons/LabeledResource.html) | 已标记的资源(==此)。 |
...
**[模板](https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/com/day/cq/wcm/api/Template.html)** 可适应：
...
|---|---|
| [复制状态](https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/com/day/cq/replication/ReplicationStatus.html) | 返回用户/组主节点的复制状态。 |
...
此外，Sling/JCR/OCM还提供 [`AdapterFactory`](https://sling.apache.org/documentation/the-sling-engine/adapters.html) 用于自定义OCM ([对象内容映射](https://jackrabbit.apache.org/jcr/object-content-mapping.html))对象。
