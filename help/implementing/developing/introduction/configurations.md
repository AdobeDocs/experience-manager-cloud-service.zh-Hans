...
>[!NOTE]
>
>仅当您的用户具有 `admin` 权限。 此类 `admin` 此外，还需要具有相应权限才能为配置分配访问权限或修改配置。
...
内容通过 `cq:conf` 属性。 AEM根据内容及其上下文执行查找 `cq:conf` 属性以查找相应的配置。
...
所有配置查找的起点是下的某个内容资源 `/content`. 这可以是页面、页面内的组件、资产或DAM文件夹。 这是您正在为其查找适用于此上下文的正确配置的实际内容。
...
跳入 `/conf`，按照引用操作，查看其中的 `/conf/wknd` 节点。 这是配置。 它的查找对应用程序代码是透明的。 此示例代码从未有过专用引用，它隐藏在 `Conf` 对象。 要应用哪种配置，可通过JCR内容进行控制。

您会看到配置中包含一个名为的固定名称 `settings` 包含实际项的节点，包括 `dam/imageserver` 你在这个案子中需要。 此类项目可视为“设置文档”，并由 `cq:Page` 包括 `jcr:content` 保存实际内容。

最后，您会看到属性 `bgkcolor` 此示例代码需要的。 此 `ValueMap` 你从这里回来 `getItem` 基于页面的 `jcr:content` 节点。
...
单击 **解决** 因此，您可以查看已解析的配置，并获得有助于解析这些配置的代码示例。
