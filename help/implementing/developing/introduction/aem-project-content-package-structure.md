---
title: AEM 项目结构
description: 了解如何定义包结构以部署到Adobe Experience Manager Cloud Service。
...
AEM要求将&#x200B;**content**&#x200B;和&#x200B;**code**&#x200B;分开，这意味着单个内容包&#x200B;**无法**&#x200B;部署到存储库的&#x200B;**同时** `/apps`和运行时可写区域（例如，`/content`、`/conf`、`/home`或非`/apps`区域）。 相反，应用程序必须将代码和内容分离到离散包中，以部署到 AEM 中。
...
AEM的`/apps`和`/libs`区域是&#x200B;**不可变的**。 AEM启动后，您无法在运行时创建、更新或删除这些区域中的内容。 在运行时更改不可变区域的任何尝试都将失败。

存储库中的其他所有内容，`/content`、`/conf`、`/var`、`/etc`、`/oak:index`、`/system`、`/tmp`等都是&#x200B;**可变的**&#x200B;区域，这意味着可在运行时更改这些区域。

>[!WARNING]
>
>与AEM的早期版本一样，不应修改`/libs`。 只有AEM产品代码可以部署到`/libs`。

### Oak索引 {#oak-indexes}

AEM as a Cloud Service部署过程管理Oak索引(`/oak:index`)。 这是因为Cloud Manager必须等到部署任何新索引并完全重新索引后，才能切换到新代码图像。
...
>[!NOTE]
>
>必须将相同的代码部署到所有环境中。 此代码可确保将处于暂存环境中的验证也用于生产环境的置信度。 有关详细信息，请参阅[运行模式](/help/implementing/deploying/overview.md#runmodes)部分。


### 内容包

+ `ui.content`包包含所有内容和配置。 内容包包含不在`ui.apps`或`ui.config`包中的所有节点定义，换句话说，包含不在`/apps`或`/oak:index`中的所有节点定义。 `ui.content`包的常见元素包括但不限于：
   + 上下文感知配置
      + `/conf`
   ...
   + 受控制的标记分类
      + `/content/cq:tags`
   + 旧版`etc`节点（理想情况下，将这些节点迁移到非/etc位置）
      + `/etc`

### 容器包

+ `all`包是容器包。 它仅包含可部署工件作为嵌入： OSGi捆绑包JAR文件以及`ui.apps`、`ui.config`和`ui.content`包。 `all`包不得具有&#x200B;**自己的任何内容或代码**，而是将所有部署委派到存储库的子包或OSGi捆绑包Jar文件。

  包现在使用Maven [FileVault Package Maven插件的嵌入配置](#embeddeds)而非`<subPackages>`配置包含。

  对于复杂的Experience Manager部署，可能需要在AEM中创建表示特定网站或租户的多个`ui.apps`、`ui.config`和`ui.content`项目/包。 如果使用此方法，请遵循可变内容与不可变内容之间的拆分。 还将所需的内容包和OSGi捆绑JAR文件作为子包嵌入到`all`容器内容包中。

  例如，复杂的部署内容包结构可能如下所示：

   + `all`内容包嵌入以下包，以创建单个部署构件
      + `common.ui.apps`部署&#x200B;**站点A和站点B均所需的代码**
      + 站点A所需的`site-a.core` OSGi包Jar
...
+ `ui.config`包包含所有[OSGi配置](/help/implementing/deploying/configuring-osgi.md)：
   + 考虑使用属于OSGi捆绑包但不包含常规内容节点的代码。 因此，它被标记为容器包装
   ...
   + Repo Init OSGi配置脚本
      + 推荐使用[Repo Init](#repo-init)来部署（可变）内容，这些内容在逻辑上是AEM应用程序的一部分。 Repo Init OSGi配置应放置在如上所述的相应`config.<runmode>`文件夹中，并用于定义：
         + 基线内容结构
         + 用户
         + 服务用户
         + 组
...
## 将包标记为由Adobe Cloud Manager部署 {#marking-packages-for-deployment-by-adoube-cloud-manager}

默认情况下，Adobe Cloud Manager会收集由Maven内部版本生成的所有包。 但是，由于容器(`all`)包是包含所有代码和内容包的单个部署对象，因此您必须确保仅&#x200B;**部署**&#x200B;容器(`all`)包。 要确保这一点，Maven 内部版本生成的其他包必须使用 `<properties><cloudManagerTarget>none</cloudManageTarget></properties>` 的 FileVault Content Package Maven Plug-In 配置进行标记。
...
Repo Init脚本存储为`scripts` OSGi工厂配置的`RepositoryInitializer`个条目。 运行模式可以隐式定位它们。 通过此定位，您可以在AEM Author和AEM Publish服务之间，甚至在Dev、Stage和Prod环境之间区分Repo Init脚本。
...
在定义用户和组时，只有组被视为应用程序的一部分，并且是其功能的组成部分。 您仍然可以在运行时在AEM中定义“组织用户和组”。 例如，如果自定义工作流将工作分配给指定的组，请在AEM应用程序中通过Repo Init定义该组。 但是，如果分组仅是组织性的，例如“Wendy&#39;s Team”和“Sean&#39;s Team”，则最好在AEM运行时定义和管理这些组。
...
内容或代码包将放置在特殊的侧边栏文件夹中。 使用FileVault Maven插件的`<embeddeds>`配置定向它们，以将其安装在AEM Author和/或AEM Publish上。 不要使用`<subPackages>`配置。
...
要定位AEM作者，AEM发布该包，或同时发布两者，请按照以下格式将该包嵌入到特殊文件夹位置的`all`容器包中：
...
+ 第一级文件夹&#x200B;**必须为** `/apps`。
+ 第二级文件夹表示应用程序，该应用程序已将`-packages`后固定到文件夹名称。 通常，所有子包都只嵌入一个第二级文件夹，但可以创建任意数量的第二级文件夹来表示应用程序的最佳逻辑结构：
  ...
  >[!WARNING]
  >
  >按照惯例，子包嵌入式文件夹的名称带有后缀`-packages`。 此命名可确保部署代码和内容包&#x200B;**不会**&#x200B;部署到`/apps/<app-name>/...`下任何子包的目标文件夹。 这种做法可以防止破坏性和循环安装行为。

+ 第三级文件夹必须为
  `application`、`content`或`container`
   + `application`文件夹包含代码包
   + `content`文件夹包含内容包
   + `container`文件夹包含AEM应用程序可能未包含的任何[额外应用程序包](#extra-application-packages)。
此文件夹名称对应于它包含的[包类型](#package-types)。
+ 第4级文件夹包含子包，并且必须是以下包之一：
   + `install`以便您同时在&#x200B;**上安装** AEM作者和AEM发布
   + `install.author`，以便您仅在AEM作者上安装&#x200B;**2&rbrace;**
   + `install.publish`，以便在AEM Publish上安装&#x200B;**only**
仅`install.author`和`install.publish`受支持的目标。 不支持其 **他运行模式** 。

例如，包含AEM创作和发布特定包的部署可能如下所示：

+ `all`容器包嵌入以下包，以创建单个部署对象
   + 嵌入到`ui.apps`中的`/apps/my-app-packages/application/install`将代码部署到AEM作者和AEM发布
   + 嵌入到`ui.apps.author`中的`/apps/my-app-packages/application/install.author`将代码仅部署到AEM作者
   + `ui.content`中嵌入的`/apps/my-app-packages/content/install`将内容和配置部署到AEM作者和AEM发布
   + `ui.content.publish`中嵌入的`/apps/my-app-packages/content/install.publish`将内容和配置仅部署到AEM发布
...
## 嵌入第三方软件包 {#embedding-3rd-party-packages}

所有包必须可通过[Adobe的公共Maven对象存储库](https://repo1.maven.org/maven2/com/adobe/)或可访问的公共、可引用的第三方Maven对象存储库使用。

如果第三方包位于&#x200B;**Adobe的公共Maven对象存储库**&#x200B;中，则Adobe Cloud Manager无需进一步配置即可解析对象。
...
## `ui.apps`包中`ui.content`之间的包依赖关系 {#package-dependencies}
...
复杂的部署会扩展简单的案例，并设置相应可变内容和不可变代码包之间的依赖关系。 根据需要，还可以在不可变代码包之间建立依赖关系。
...
在`ui.apps/pom.xml`中，`<packageType>application</packageType>`插件声明的`filevault-package-maven-plugin`生成配置指令声明了其包类型。
...
在`ui.content/pom.xml`中，`<packageType>content</packageType>`插件声明的`filevault-package-maven-plugin`生成配置指令声明了其包类型。
...
### 标记用于Adobe Cloud Manager部署的包 {#cloud-manager-target}

在生成包&#x200B;**除容器(**)项目的`all`之外的每个项目中，将`<cloudManagerTarget>none</cloudManagerTarget>`添加到`<properties>`插件声明的`filevault-package-maven-plugin`配置中，以确保Adobe Cloud Manager不**部署它们。 容器(`all`)包应该是通过Cloud Manager部署的单个包，该包又嵌入了所有必需的代码和内容包。
...
包含Repo Init脚本的Repo Init脚本是通过`RepositoryInitializer`属性在`scripts` OSGi工厂配置中定义的。 由于您在OSGi配置中定义这些脚本，因此可以使用常用的`../config.<runmode>`文件夹语义通过运行模式轻松限定其范围。
...
在`ui.apps/pom.xml`以及声明代码包(`pom.xml`)的任何其他`<packageType>application</packageType>`中，将以下存储库结构包配置添加到FileVault Maven插件。 您可以[为您的项目](repository-structure-package.md)创建自己的存储库结构包。
...
### `ui.apps`包中`ui.content`之间的包依赖关系 {#xml-package-dependencies}
...
+ [使用 Maven 管理包](/help/implementing/developing/tools/maven-plugin.md)
