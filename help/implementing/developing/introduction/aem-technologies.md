---
title: AEM 技术基础
description: 概述AEM的技术基础，包括AEM的结构方式以及基础技术，如JCR、Sling和OSGi。
...
AEM是一个基于经验证、可扩展且灵活的技术而构建的强大平台。 本文档详细概述了AEM的各个组成部分，旨在作为全栈AEM开发人员的技术附录。 本指南并非旨在作为入门指南。 如果您是AEM开发的新手，请参阅作为第一步[AEM Sites开发入门 — WKND教程](develop-wknd-tutorial.md)。
...
Java™ Content Repository (JCR)标准[JSR 283](https://developer.adobe.com/experience-manager/reference-materials/spec/jcr/2.0/index.html)指定了一种独立于供应商且独立于实施的方法，用于在内容存储库内的粒度级别双向访问内容。 规范牵头机构为Adobe Research （瑞士） AG。

[JCR API 2.0](https://developer.adobe.com/experience-manager/reference-materials/spec/javax.jcr/javadocs/jcr-2.0/index.html)包`javax.jcr.*`用于直接访问和处理存储库内容。

AEM基于JCR构建。

## Apache Jackrabbit Oak {#jackrabbit-oak}

[Apache Jackrabbit Oak](https://jackrabbit.apache.org/oak/docs/)是可扩展的高性能分层内容存储库的实现，用于现代世界级网站和其他要求苛刻的内容应用程序的基础，符合JCR标准。

Jackrabbit Oak(也简称为Oak)是实施JCR标准，AEM构建基于此标准。

## Sling请求处理 {#sling-request-processing}

AEM是使用基于REST原则的Web应用程序框架[Sling](https://sling.apache.org/index.html)构建的，该框架为开发面向内容的应用程序提供了便利。 Sling使用Apache Jackrabbit Oak等JCR存储库作为其数据存储。 Sling已加入到Apache Software Foundation — 有关详细信息，请访问Apache。
...
下图说明了可与`SlingPostServlet`（所有POST请求的默认处理程序）一起使用的隐藏但功能强大的请求参数。 该处理程序为您提供了在存储库中创建、修改、删除、复制和移动节点的无限选项。
...
找到相应的资源（内容节点）后，将提取&#x200B;**sling资源类型**。 此路径将查找用于呈现内容的脚本。
...
所有Sling脚本都存储在`/apps`（可变用户脚本）或`/libs`（不可变，系统脚本）的子文件夹中，将按此顺序搜索这些子文件夹。

其他几点需要注意的是：

* 当需要方法(GET、POST)时，将按照HTTP规范以大写形式指定该方法，例如`jobs.POST.esp`
* 虽然支持各种脚本引擎，但常见的推荐脚本是HTL和JavaScript。

Felix管理控制台(`http://<host>:<port>/system/console/slingscripting`)上列出了给定的AEM实例支持的脚本引擎列表。

使用上一个示例，如果`sling:resourceType`为`hr/jobs`，则对于：

* 以`.html`结尾的GET/HEAD请求和URL（默认请求类型，默认格式）
   * 脚本是`/apps/hr/jobs/jobs.esp`；`sling:resourceType`的最后一个部分构成了文件名。
...
* 带有选择器的URL
   ...
   * 如果您查看打印机友好版本，其中选择器可能是`print`；如在`../content/corporate/jobs/developer.print.html`中
   * 脚本为`/apps/hr/jobs/jobs.print.esp`；选择器已添加到脚本名称中。
* 如果不是，则定义`sling:resourceType`，然后：
   * 内容路径用于搜索适当的脚本（如果基于路径的`ResourceTypeProvider`处于活动状态）。
   * 例如，`../content/corporate/jobs/developer.html`的脚本将在`/apps/content/corporate/jobs/`中生成搜索。
   * 使用主节点类型。
* 如果根本找不到脚本，则使用默认脚本。
   * 支持以纯文本(`.txt`)、HTML (`.html`)和JSON (`.json`)格式呈现默认演绎版，所有这些演绎版都列出了节点的属性（格式适当）。 扩展`.res`或没有请求扩展名的请求的默认演绎版是假脱机资源（如果可能）。
...
除了资源类型（主要由`sling:resourceType`属性定义）之外，还有资源超级类型。 `sling:resourceSuperType`属性指示此类型。 在尝试查找脚本时，也会考虑这些超类型。 资源超级类型的优点在于，它们可以形成资源的层次结构，其中默认资源类型`sling/servlet/default`（由默认servlet使用）有效地作为根。

可以通过两种方式定义资源的资源超级类型：

* 按资源的`sling:resourceSuperType`属性。
* `sling:resourceSuperType`指向的节点的`sling:resourceType`属性。
...
原因在于`/y`具有`sling:resourceSuperType`属性，而`/x`没有，因此其超类型取自其资源类型。
...
* 自动处理GET以外的http方法，包括：
   ...
   * `POST.jsp`位置中的`sling:resourceType`脚本
...
此架构允许您通过应用程序特定的模块扩展Sling。 Sling以及AEM使用OSGi的[Apache Felix](https://felix.apache.org/documentation/index.html)实现。 它们都是在OSGi框架内运行的OSGi捆绑包的集合。
...
有关详细信息，请参阅[为AEM as a Cloud Service配置OSGi](/help/implementing/deploying/configuring-osgi.md)。

## 存储库中的结构 {#structure-within-the-repository}

以下列表概述了您在存储库中看到的结构。

* `/apps` — 与应用程序相关；包含特定于您网站的组件定义。 您开发的组件可以基于`/libs/core/wcm/components`提供的现成组件。
* `/content` — 为您的网站创建的内容。
* `/etc`
* `/home` — 用户和组信息。
* `/libs` — 属于AEM核心的库和定义。 `/libs`中的子文件夹表示现成的AEM功能。 无法修改`/libs`中的内容。 您网站的特定功能应在`/apps`下创建。
* `/tmp` — 临时工作区。
