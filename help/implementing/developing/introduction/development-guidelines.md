...
Adobe建议使用提供 [的Apache HttpComponents Client 4.x库进行HTTP](https://hc.apache.org/httpcomponents-client-ga/) 连接。
...
## 没有流二进制文件通过AEM作为Cloud Service {#no-streaming-binaries}
...
>[!NOTE]
>
>要执行下面列出的配置更改，您需要在本地开发环境创建这些更改，然后将它们作为Cloud Service实例推送到AEM。 有关如何执行此操作的详细信息，请参 [阅将AEM部署为Cloud Service](/help/implementing/deploying/overview.md)。
...
对于本地开发，开发人员可以完全访`/crx/de`问CRXDE Lite()和AEM Web Console(`/system/console`)。
...
应 [使用Day CQ Mail Service](https://docs.adobe.com/content/help/en/experience-manager-65/administering/operations/notification.html#configuring-the-mail-service) OSGI服务，电子邮件必须发送到支持请求中指明的邮件服务器，而不是直接发送给收件人。
...
AEM中的电子邮件应使用Day CQ Mail [Service OSGi服务发送](https://docs.adobe.com/content/help/en/experience-manager-65/administering/operations/notification.html#configuring-the-mail-service)。

有关 [配置电子邮件设置的详细信息](https://docs.adobe.com/content/help/en/experience-manager-65/administering/operations/notification.html) ，请参阅AEM 6.5文档。 对于AEM CS，必须对服务进行以下调 `com.day.cq.mailer.DefaultMailService OSGI` 整：

如果已请求端口465:

* 设置 `smtp.port` 为 `465`
* 设置 `smtp.ssl` 为 `true`
* 设置 `smtp.starttls` 为 `false`

如果请求了端口587（仅当邮件服务器不支持端口465时才允许）:

* 设置 `smtp.port` 为 `587`
* 设置 `smtp.ssl` 为 `false`
* 设置 `smtp.starttls` 为 `true`