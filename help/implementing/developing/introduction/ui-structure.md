...
布局遵循响应式设计样式，并适应您所使用的设备、窗口或两者的大小。

例如，当分辨率低于1024像素时（如在移动设备上一样），显示器将相应地调整：
...
工具栏与您的位置相关，并且会显示与控制以下页面中的视图或资产相关的工具。 工具栏是特定于产品的，但是这些元素有一些共同之处。
...
包含三个默认选项卡。 此 **资产** 和 **组件** 选项卡允许您选择此类元素，并将其从面板中拖放到页面上。 此 **内容树** 选项卡可让您检查页面上内容的层次结构。

默认情况下，侧面板处于隐藏状态。 选中后，它将显示在左侧，或者当窗口宽度小于1024像素时，它会滑过以覆盖整个窗口，例如在移动设备上一样。

![侧面板](assets/ui-side-panel.png)

### 侧面板 — 资产 {#side-panel-assets}

在“资源”选项卡中，您可以从资源范围中进行选择。 此外，您还可以根据特定术语进行筛选，或选择组。
...
在“组件”选项卡中，您可以从组件范围中进行选择。 此外，您还可以根据特定术语进行筛选，或选择组。
...
覆盖内容框架，并由 [图层](#layer) 实现如何与组件及其内容透明交互的机制。
...
>[!NOTE]
>
>层是一个强大的概念，它影响用户对页面内容的查看以及与页面内容的交互。 在开发自己的图层时，请确保该图层在退出时进行清理。
...
欲了解更多技术信息，请参见 [JS文档集](https://developer.adobe.com/experience-manager/reference-materials/6-5/jsdoc/ui-touch/editor-core/index.html) （对于页面编辑器）。
