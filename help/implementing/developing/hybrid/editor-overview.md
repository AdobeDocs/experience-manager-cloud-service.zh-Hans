...
* [SPA Blueprint](blueprint.md) 的技术要求
* [AEM SPA使用React快速入门](getting-started-react.md) 快速浏览使用React的简单SPA
* [AEM SPA快速入门(使用Angular)](getting-started-angular.md) 快速浏览使用Angular的简单SPA
...
要使作者能够使用页面编辑器编辑SPA的内容，必须实施SPA应用程序才能与AEM SPA Editor SDK进行交互。 请参阅 [AEM SPA使用React快速入门](getting-started-react.md) 文档，以便您运行。
...
可以实施其他SPA框架以与AEM SPA Editor SDK一起使用。 请参阅 [SPA Blueprint](blueprint.md) 记录框架必须满足的要求，以便创建由模块、组件和服务组成的特定于框架的层，以便与AEM SPA Editor一起使用。

### 使用多个选择器 {#multiple-selectors}

其他自定义选择器可以定义，并用作为为AEM SPA SDK开发的SPA的一部分。 但是，此支持要求 `model` 选择器是第一个选择器，扩展是 `.json` JSON导出程序的要求。
