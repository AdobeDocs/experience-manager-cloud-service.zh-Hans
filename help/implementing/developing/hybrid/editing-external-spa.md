...
在决定时 [什么级别的集成](/help/implementing/developing/headful-headless.md) 您希望在外部SPA和AEM之间切换，请考虑您必须通常能够在AEM中编辑和查看SPA。
...
* 使用以下方式创建基本AEM SPA项目 [AEM项目原型](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/overview.html?#available-properties).
   ...
   * 对于本文档中的示例，Adobe使用 [WKND SPA项目](https://experienceleague.adobe.com/docs/experience-manager-learn/sites/spa-editor/spa-editor-framework-feature-video-use.html#spa-editor).
...
在应用程序呈现之前， [`ModelManager`](/help/implementing/developing/hybrid/blueprint.md#pagemodelmanager) 必须初始化才能处理AEM的创建 `ModelStore`.

此初始化必须在 `src/index.js` 应用程序的文件或呈现应用程序的根目录的位置。
...
* `path`  — 初始化时，将获取定义的路径上的模型并将其存储在 `ModelStore`. 此路径可用于获取 `rootModel` 初始化时（如果需要）。
...
1. 确定要显示的节点的路径。
   ...
   * `itemPath`：页面中节点的路径，在本例中为 `root/responsivegrid/text`
      * 由页面上包含项目的名称组成。
...
1. 从运行以下Maven命令 `aem-guides-wknd-spa` 目录，以便生成项目并将其部署到AEM。
...
1. 创建文件(例如， `Page.js`)对于可授权的页面组件。 使用中提供的页面组件 `@adobe/cq-react-editable-components`.
...
1. 如前所述，应用 `MapTo` 页中所有子组件的AEM资源类型。
   ...
   >[!NOTE]
   >
   >在此示例中，使用未包装的React文本组件，而不是包装的 `AEMText` 之前创建。 原因在于，当组件是页面/容器的一部分并且不是单独时，容器会负责递归映射组件。 并且，每个孩子不需要启用创作功能和其他包装器。
...
在此示例中，您将重用 `AEMText` 之前创建的组件。 您希望在WKND主页上的现有文本组件下方添加新文本。 添加组分与添加普通叶组分相同。 但是， `itemPath` 可更新到必须添加新组件的路径。
...
例如，如果容器存在于 `/root/responsivegrid`，且开发人员想要添加一个子容器：
...
* 要创建的容器的直接父级必须存在于AEM中。
   * 如果容器 `root/responsivegrid` 存在于AEM容器中，则通过提供路径可以创建新容器 `root/responsivegrid/newContainer`.
...
默认情况下，您可以假设React应用程序呈现在内部， `div` 元素ID的 `spa-root`. 如有必要，可以自定义此语法。

例如，假设您有一个SPA，其中应用程序在 `div` 元素ID的 `root`. 此语法必须反映在三个文件中。
...
在以下示例中，您有一个包含两个页面的简单React应用程序。 要呈现的页面是通过将提供给路由器的路径与活动URL进行匹配来确定的。 例如，如果您在 `mydomain.com/test`， `TestPage` 已呈现。
...
1. 在所需级别创建页面。
   ...
   * 在此示例中，要编辑的页面为 `mydomain.com/test`. `test` 位于应用程序的根路径中。 在AEM中创建页面时，也必须保留此根路径。 因此，您可以在上一步中定义的根级别创建页面。
   * 创建的新页面必须与要编辑的页面具有相同的名称。 在此示例中，对于 `mydomain.com/test`，创建的新页面必须 `/path/to/aem/root/test`.
...
1. 在SPA路由中添加帮助程序。
   ...
   * 此 `toAEMPath` 帮助程序提供者 `@adobe/cq-spa-page-model-manager` 可以使用。 当在AEM实例上打开应用程序时，它会转换为路由提供的路径，使其包括AEM特定的部分。 它接受三个参数：
      * 路由所需的路径
      * 编辑SPA的AEM实例的源URL
      * 第一步中确定的AEM上的项目根目录
