...
在决定[要在外部SPA和AEM之间具有的集成级别](/help/implementing/developing/headful-headless.md)时，您通常需要能够在AEM中编辑和查看SPA。
...
在呈现应用程序之前，需要对[`ModelManager`](/help/implementing/developing/hybrid/blueprint.md#pagemodelmanager)进行初始化，以处理AEM `ModelStore`的创建。
...
* `model`  — 使用SSR `model` 时通常作为参数传递的 [对象。](/help/implementing/developing/hybrid/ssr.md)
...
如果外部React SPA应用程序具有多个页面，则[它可以使用路由来确定要渲染的页面/组件。](/help/implementing/developing/hybrid/routing.md) 基本用例是将当前活动的URL与为路由提供的路径进行匹配。要在此类启用路由的应用程序上启用编辑功能，需要转换要与之匹配的路径，以适应特定于AEM的信息。
...
1. 确定将作为AEM根的级别。
   ...
   * 对于我们的示例，我们考虑将wknd-spa-react/us/en作为SPA的根。 这意味着该路径之前的所有内容都仅AEM页面/内容。
