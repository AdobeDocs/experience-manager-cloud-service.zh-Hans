...
`ModelRouter`自动获取模型的片段。 但是，作为任何自动化工具集，它都有局限性。 如果需要，可以禁用`ModelRouter`或将&lt;a0/>配置为使用元属性忽略路径(请参阅[SPA页面组件](page-component.md)文档的元属性部分)。 然后，前端开发者可以使用`getData()`函数请求`PageModelManager`加载任何给定的模型片段，从而实现自己的模型路由层。
