...
在Adobe Experience Manager中，内容片段作为云服务扩展标准资产； 请参阅：
...
#### 与标准资产一样，内容片段位于以下位置：{#asset-location}
...
功能集成 {#feature-integration}](/help/assets/content-fragments/content-fragments-delete.md)

#### 要与Assets核心集成，请执行以下操作：{#feature-integration}
...
>内 [容片段组件是核心组件的一部分[#$tu71]。 有关更 [多详细信息[#$tu72] ，请参阅开发核心组件。
>
>



* 组件允许您选择要显示的变量。`fragmentPath``fragmentPath`
...
### 内容片段可与以下内容集成：{#integration-with-other-frameworks}
...
* **内容片段与AEM翻译工作流程完全集成。 在建筑层面，这意味着：**
   ...
   * 它们位于不同的语言根系下； 但在相关语言根目录下共享完全相同的相对路径：
      ...
      * `/content/dam/<path>/en/<to>/<fragment>`
         ...
         `/content/dam/<path>/en/<to>/<fragment>`与

         `/content/dam/<path>/de/<to>/<fragment>`

         `/content/dam/<path>/de/<to>/<fragment>`除了基于规则的路径之外，内容片段的不同语言版本之间没有进一步的联系； 它们作为两个单独的片段处理，但UI提供了在语言变体之间导航的方法。
   * [!NOTE]
...
* 内容片段（重新）使用元 [数据模式](/help/assets/metadata-schemas.md)，这些可以使用标准资产进行定义。
   ...
   * CFM提供其自己的特定模式:[](/help/assets/metadata-schemas.md)

   * `/libs/dam/content/schemaeditors/forms/contentfragment`

      `/libs/dam/content/schemaeditors/forms/contentfragment`如果需要，可以扩展此功能。

      相应的模式表单与片段编辑器集成。

   * 内容片段管理API —— 服务器端 {#the-content-fragment-management-api-server-side}

## 您可以使用服务器端API访问您的内容片段； 请参阅：{#the-content-fragment-management-api-server-side}

[com.adobe.cq.dam.cfm[#$tu111]



>[!CAUTION]强烈建议使用服务器端API，而不是直接访问内容结构。
>
>关键接口 {#key-interfaces}

### 以下三个接口可用作入口点：{#key-interfaces}

**内容片段** ([ContentFragment[#$tu117])
...
* 
   ...
   * 列表元素
      ...
      * 访问元素数据(请参 `ContentElement`阅)](#caveats)

      * 为片段定义的列表变量`ContentElement`
   * 全局创建新变量
   * 管理关联内容：
   * 列表集合
   ...
   * 
      ...
      * 列表变化
         ...
         * 删除变体(请参 [阅警告](#caveats))
         * 访问变量数据(请参 `ContentVariation`阅)](#caveats)
         * 用于解析变量的快捷方式（如果指定的变量对元素不可用，则应用一些附加的特定于实施的回退逻辑）`ContentVariation`
      * **内容变化** ([ContentVariation[#$tu147])
   * 
   ...
   创建元素的新版本`ContentFragment``ContentElement``ContentVariation``Versionable`
...
### 可以调整以下内容：{#adapting-using-adaptto}
...
* `Resource` -基础Sling资源； 直接更新基础 `Resource` 需要重建对 `ContentFragment` 象。
   ...
   * `ContentElement` 可适用于：`Asset``Asset``ContentFragment`
...
* 从中创建新变 `ContentFragment` 量以更新数据结构。
   ...
   * 内容片段管理API —— 客户端 {#the-content-fragment-management-api-client-side}`ContentFragment.removeVariation()`
...
### 请参阅以下内容：{#additional-information}
...
从技术上讲，所有编辑操 *作都是* 对实时内容进行的，就像所有其他AEM编辑一样。 启动编辑会话时，将创建当前未编辑状态的某个版本。 如果用户取消编辑，则恢复该版本。 如果用户单击“保 **存**”，则不会执行任何特定操作，因为所有编辑都对实 *时内容执* 行，因此所有更改已保留。 此外，单击保 **存** 将触发一些后台处理（例如创建全文搜索信息和／或处理混合媒体资产）。
...
### 要实现此目标，您可以调整表示API的资源以：{#example-accessing-an-existing-content-fragment}

`com.adobe.cq.dam.cfm.ContentFragment`

`com.adobe.cq.dam.cfm.ContentFragment`例如：
...
节点： `<conf-root>/settings/dam/cfm/jcr:content`](/help/assets/content-fragments/content-fragments-managing.md#save-cancel-and-versions)

* 属性名称: `autoSaveInterval`
* 类型: `Long`
* 默认： `600` （10分钟）; 定义于 `/libs/settings/dam/cfm/jcr:content`

* 如果要设置5分钟的自动保存间隔，您需要定义节点上的属性； 例如：`600``/libs/settings/dam/cfm/jcr:content`
...
## 有关更多信息，请参阅{#components-for-page-authoring}

[核心组件——内容片段组件[#$tu206] （推荐）
