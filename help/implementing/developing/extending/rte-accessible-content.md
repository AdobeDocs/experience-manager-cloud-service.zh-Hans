...
description: 了解如何配置富文本编辑器，以在 [!DNL Adobe Experience Manager].
contentOwner: AG
...
有关RTE的典型配置的了解，请参阅 [配置RTE](rich-text-editor.md) 和 [为特定功能配置RTE插件](configure-rich-text-editor-plug-ins.md).

使用RTE插件配置来配置和自定义与辅助功能相关的功能。 例如，使用 `paraformat` 添加额外的块级语义元素，包括扩展基本支持的标题级别数 `H1`, `H2` 和 `H3` 默认提供。 可以使用创作用户界面中的许多组件进行富文本编辑。 常用组件有文本、图像、下载等。

RTE功能在许多组件中都可用。 主组件是 `Text` 组件。

对于 `Text` 组件 [!DNL Experience Manager]，则以下屏幕截图会显示启用了一系列插件(包括 `paraformat`:
...
要激活某个插件的一些或所有功能，请在相应的 `rtePlugins` 子分支CRXDE Lite。
...
1. [启用段落选择字段](rich-text-editor.md) by [激活插件](rich-text-editor.md#enable-rte-functionalities-by-activating-plug-ins).
1. [指定段落选择字段中要提供的格式](rich-text-editor.md).
1. 然后，内容作者可以从RTE的选择字段中使用段落格式。

使用RTE中通过段落格式选项提供的结构元素， [!DNL Experience Manager] 为开发无障碍内容提供了良好的基础。 内容作者不能使用RTE设置字体大小、颜色或其他相关属性的格式，从而阻止创建内联格式。 相反，作者可以选择相应的结构元素（如标题），并使用从“样式”选项中选择的全局样式，以确保使用自己的样式表和正确结构化内容进行浏览的用户能够获得干净的标记和更大的选项。

## 使用源编辑功能 {#use-of-the-source-edit-feature}

在某些情况下，内容作者会发现需要检查和调整使用RTE创建的HTML源代码。 例如，在RTE中创建的一段内容可能需要更多标记，以确保符合WCAG 2.0。这可以通过 [源编辑](rich-text-editor.md#aboutplugins) 选项。 您可以指定 [`sourceedit` 功能 `misctools` 插件](rich-text-editor.md#aboutplugins).

>[!CAUTION]
>
>使用 `sourceedit` 功能。 任何键入错误和不受支持的功能都可能导致问题。
