...
|---|---|---|---|
| 名称 | 功能 | 功能 | 功能 |
| 类型 | 字符串 | `String` (多字符串；将类型设置为 `String` 并单击 `Multi` CRXDE Lite) | 字符串 |
| 值 | `*` （星号） | 设置为一个或多个特征值。 | - |
...
单击“查找”(Find)后，“查找和替换”(Find and Replace)对话框将变得透明，单击“替换”(Replace)后，对话框将变得不透明。 行为允许作者查看要替换的文本。 如果用户单击“全部替换”，则对话框将关闭并显示已进行替换的数量。
...
* 将剪贴板内容粘贴为纯文本。 在[!DNL Experience Manager]组件中插入之前，会删除复制内容中的所有样式和格式元素。 使用下面的`plaintext`进行配置。
...
1. 在`edit`节点中，使用以下详细信息创建属性：
   ...
   * **名称** `defaultPasteMode`
   * **类型** `String`
   * **** 值是从、或模式进行所需的 `browser`粘贴模 `plaintext`式之 `wordhtml` 一。

### 配置粘贴内容时允许的格式 {#pasteformats}

可以进一步配置粘贴为Microsoft-Word(`paste-wordhtml`)模式，以便当从其他程序（如[!DNL Microsoft Word]）粘贴到[!DNL Experience Manager]中时，可以明确允许使用一些样式。

例如，如果在[!DNL Experience Manager]中粘贴时只允许使用粗体格式和列表，则可以过滤掉其他格式。 这称为可配置粘贴过滤，可用于以下两项：
...
要配置从其他程序将文本粘贴到[!DNL Experience Manager]时允许使用的格式，请执行以下操作：
...
| `fallbackBlockTag` | `String` | 定义块标记，该标记用于任何具有未包含在`allowBlockTags`中的块标记的块。 通常，`p`就足够了。 |
| `table` | `nt:unstructured` | 定义粘贴表时的行为。 此节点必须具有属性allow（类型Boolean）来定义是否允许粘贴表。 如果将allow设置为false，则必须指定属性ignoreMode（类型字符串）以定义如何处理粘贴的表内容。 ignoreMode的有效值为`remove`以删除表内容，而`paragraph`以将表单元格转换为段落。 |
| `list` | `nt:unstructured` | 定义粘贴列表时的行为。 必须具有属性`allow`（类型Boolean）来定义是否允许粘贴列表。 如果将`allow`设置为`false`，请指定属性`ignoreMode`（类型`String`）以定义如何处理粘贴的任何列表内容。 ignoreMode的有效值为`remove`（用于删除列表内容）和`paragraph`（用于将列表项转换为段落）。 |
...
富文本编辑器使用ID为`CQrte`的容器DOM元素，该元素提供不同的样式以进行查看和编辑：
...
使用[!DNL Experience Manager]创作日语内容的作者可以对字符应用样式，以避免在不需要中断的情况下出现换行符。 这允许作者让句子在所需位置中断。 此功能的样式基于在CSS样式表中预定义的CSS类。
...
>[!NOTE]
>
>如果无法将块标记（例如`<hr>`标记）分配给段落，则它对于`paraformat`插件不是有效用例。

首次启用“段落格式”插件后，不提供默认的“段落格式”。 弹出列表为空。 要为作者提供段落格式，请执行以下操作：

* 启用[!UICONTROL Format]弹出选择器列表。
* 从弹出式菜单中指定可选择为段落格式的块标记。
...
>[!NOTE]
>
>如果未进一步配置此插件，则启用的默认格式为“段落”(`<p>`)、“标题1”(`<h1>`)、“标题2”(`<h2>`)、“标题3”(`<h3>`)。

>[!CAUTION]
>
>配置RTE的段落格式时，请勿删除段落标记&lt;p>作为格式选项。 如果删除了`<p>`标记，则内容作者将无法选择[!UICONTROL 段落格式]选项，即使配置了其他格式也是如此。
...
1. 在`paraformat`节点下创建节点，以保存格式列表：
...
在标准[!DNL Experience Manager]安装中，当为特殊字符(`specialchars`)启用`misctools`插件时，将立即提供默认选项供使用；例如，版权和商标符号。
...
1. 在`table`节点下，根据需要创建以下节点：
   ...
   * 要定义整个表的样式（位于&#x200B;**[!UICONTROL 表属性]**&#x200B;下）：

      * **名称** `tableStyles`
      * **类型** `cq:WidgetCollection`
   * 要定义单个单元格的样式（位于&#x200B;**[!UICONTROL 单元格属性]**&#x200B;下），
...
>[!NOTE]
标准字典与相应的自述文件一起位于`/libs/cq/spellchecker/dictionaries`。 请勿修改文件。
...
1. 在`../items/text`节点的组件对话框定义中，创建属性：
...
在[!DNL Experience Manager]中添加链接时，您可以定义要使用的CSS样式以及自动接受的协议。 要配置如何从其他程序在[!DNL Experience Manager]中添加链接，请定义HTML规则。

1. 使用CRXDE Lite，找到项目的文本组件。
1. 在与`<rtePlugins-node>`相同的级别创建节点，即在`<rtePlugins-node>`的父节点下创建该节点：
   ...
   * **值** `richtext`
   ...
   `../items/text`节点的位置可能会有所不同，具体取决于对话框的结构。 `/apps/myProject>/components/text/dialog/items/text`和`/apps/<myProject>/components/text/dialog/items/panel/items/text`是两个示例。
