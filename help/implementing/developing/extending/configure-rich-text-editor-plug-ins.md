...
### 对于以后（重新）的配置，例如要添加更多格式，请仅按照说明的相关部分操作。{#formatselectorlist}

启用格式下拉选择器 {#formatselectorlist}

1. 首先启用参数格式插件：`<rtePlugins-node>/paraformat`[](#activateplugin)
1. 在您的组件中，导航到节点 `<rtePlugins-node>/paraformat`。 如果节点不存在，则创建这些节点。 有关详细信息， [请参阅激活插件](#activateplugin)。

   * 在节 `features` 点上创建 `paraformat` 属性：`features`
   * **名称** `features`
   * **类型** `String`

>**值**`*` （星号）
[!NOTE]
* 如果插件未进一步配置，则启用以下默认格式：`<p>`
* 段落 ( `<p>`)
* 标题 1 ( `<h1>`)
* 标题 2 ( `<h2>`)
...
1. 段落格式可通过以下方式进行选择：`<rtePlugins-node>/paraformat`[](#styleselectorlist)
1. 在组件定义中，导航到节 `<rtePlugins-node>/paraformat`点，如启 [用格式下拉选择器中所创建](#styleselectorlist)。

   * 在节 `paraformat` 点下创建新节点，以保存格式列表:**`formats`
   * **名称** `formats`

1. **类型** `cq:WidgetCollection`

   * 在节点下创建新节 `formats` 点，它保留单个格式的详细信息：**
   * **名称**，您可以指定名称，但该名称应适合格式（例如，myparagraph、myheading1）。`nt:unstructured`

1. **类型** `nt:unstructured`

   * **对于此节点，添加属性以定义使用的块标记：**`tag`
   * **名称** `tag`
   * **类型** `String`
...
1. 您无需输入分界角括号。
   ...
   * **对于同一节点，添加另一个属性，以便说明性文本显示在下拉列表中：**`description`
   * **名称** `description`
   * **类型** `String`

1. **值** ：此格式的描述性文本； 例如，“段落”、“标题1”、“标题2”等。 此文本将显示在“格式”选择列表中。

   保存更改。

>[!CAUTION]为每种所需格式重复这些步骤。
[!CAUTION]`<h1>``<h2>``<h3>``<p>`

## 如果定义自定义格式，则会删`<p>`除默 `<h1>`认格 `<h2>`式( `<h3>`、和)。 重新创 `<p>` 建格式，因为它是默认格式。

配置特殊字符 {#spchar}`specialchars`

在标准AEM安装中，当 `misctools` 为特殊字符启用插件时()`specialchars`默认选择会立即可用； 例如，版权和商标符号。

>[!CAUTION]您可以配置RTE，使您自己选择的字符可用； 通过定义不同的字符或整个序列。
[!CAUTION]

### 添加自己的特殊字符将覆盖默认选择。 如果需要，（重新）在您自己的选择中定义这些字符。{#definesinglechar}

1. 定义单个字符 {#definesinglechar}[](#activateplugin)
1. 在您的组件中，导航到节点 `<rtePlugins-node>/misctools`。 如果节点不存在，则创建这些节点。 有关详细信息， [请参阅激活插件](#activateplugin)。

   * 在节 `features` 点上创建 `misctools` 属性：`features`
   * **名称** `features`
   * **类型** `String[]`

      **值** `specialchars`

1.     (或 `String / *` 者，如果应用此插件的所有功能)

   * 在“ `misctools` 创建节点”下，可保存特殊字符配置：**`specialCharsConfig`
   * **名称** `specialCharsConfig`

1. **类型** `nt:unstructured`

   * 在“ `specialCharsConfig` 创建其他节点”下，保存字符列表:**`chars`
   * **名称** `chars`

1. **类型** `nt:unstructured`

   * 在“ `chars` 添加新节点”下，可保存单个字符定义：**
   * **名称** ，您可以指定名称，但应反映字符； 例如，一半。`nt:unstructured`

1. **类型** `nt:unstructured`

   * **要添加此节点，请添加以下属性：**`entity`
   * **名称** `entity`
   * **类型** `String`

1. **评估** 所需字符的HTML表示形式； 例如， `&189;` 对于分数半。

保存更改。

![在CRXDE中，保存属性后，将显示所表示的字符。 请参见下面的“一半”示例。 重复上述步骤，使作者能够使用更多特殊字符。](assets/chlimage_1-106.png "")

### ![在CRXDE中，添加要在RTE工具栏中可用的单](assets/chlimage_1-106.png "个字符在CRXDE中，添加要在RTE工具栏中可用的单个字符")

1. 定义字符范围 {#definerangechar}](#definingasinglecharacter)
1. 使用定义单个字符 [中的步骤1到3](#definingasinglecharacter)。

   * 在“ `chars` 添加新节点”下，可保存字符范围的定义：**
   * **名称** ，您可以指定名称，但应反映字符范围； 比如铅笔。`nt:unstructured`

1. **类型** `nt:unstructured`

   * **在此节点下（根据特殊字符范围命名）添加以下两个属性：**`rangeStart`      **名称** `rangeStart`
...
1. 
   ...
   ![例如，定义9998 - 10000范围可提供以下字符。](assets/chlimage_1-107.png)
...
>[!NOTE]样式通常应用于文本，但也可以对表或几个表单元格应用一组单独的样式。 从“单元格属性”或“表属性”对话框的“样式”选择器框中，作者可以使用这种样式。 当在文本组件（或衍生组件）中编辑表时，样式可用，而在标准表组件中则不可用。
[!NOTE]

>[!NOTE]您只能为经典UI定义表和单元格的样式。
[!NOTE]

1. 在RTE组件中或从RTE组件中复制和粘贴表取决于浏览器。 并非所有浏览器都支持开箱即用。 根据表结构和浏览器，您可能会获得不同的结果。 例如，当您在经典UI和触屏UI中复制和粘贴Mozilla Firefox中RTE组件中的表时，不会保留表的布局。`<rtePlugins-node>/table`[](#activateplugin)
1. 在组件中，导航到节点 `<rtePlugins-node>/table`。 如果节点不存在，则创建这些节点。 有关详细信息， [请参阅激活插件](#activateplugin)。

   * 在节 `features` 点上创建 `table` 属性：`features`
   * **名称** `features`
   * **类型** `String`
   >**值** `*`
   [!NOTE]
   * 如果不想启用所有表功能，可以创建属 `features` 性：**`String[]`
...
1. `cellprops` 以允许编辑单元格属性，包括样式。](#locationofstylesheet)[](#textstyles)
1. 定义CSS样式表的位置以引用这些样式表。 请参 [阅指定样式表的位置](#locationofstylesheet) ，因为这与定义文本样 [式时相同](#textstyles)。 如果您定义了其他样式，则可以定义位置。

   * 在节点 `table` 下，创建以下新节点（根据需要）:**

      * 定义整个表的样式(可在“表属性” **下找到**):`tableStyles`
      * **名称** `tableStyles`
   * **类型** `cq:WidgetCollection`

      * 定义单个单元格的样式(在“单元格属性” **下可用**):`cellStyles`
      * **名称** `cellStyles`


1. **类型** `cq:WidgetCollection`

   * 创建一个新节点(在相应的 `tableStyles` 或节 `cellStyles` 点下)以表示单个样式：
   * **名称** ，您可以指定名称，但应反映样式。`nt:unstructured`
...
1. **类型** `nt:unstructured`
   ...
   * 在此节点上创建属性：
      ...
      * **定义要引用的CSS样式**`cssName`
      * **名称** `cssName`
      * **类型** `String``cssClass``.cssClass`
   * **值** CSS类的名称(例如，不 `.`带前面 `cssClass` 的名 `.cssClass`称)

      * **定义要在下拉选择器中显示的描述性文本**`text`
      * **名称** `text`
      * **类型** `String`
...
* `hiddenHeaderEditingCSS`为了增强此类情况下的辅助功能，RTE支持隐藏的标题单元格。 此外，它还提供与表中隐藏的标题相关的配置设置。 这些设置允许您在编辑和预览模式下对隐藏的标题应用CSS样式。 要帮助作者在编辑模式下识别隐藏的标题，请在代码中包含以下参数：
...
* `hiddenHeaderClassName`要帮助作者在预览模式中对隐藏的标题应用CSS，您可以在代码中包含以下参数：
* `hiddenHeaderClassName`: 指定在预览模式下应用于隐藏标题单元格的CSS类的名称。

`hiddenHeaderStyle`: 指定在预览模式下应用于隐藏标题单元格的样式字符串。

## 如果在代码中指定CSS和样式字符串，则CSS类优先于样式字符串，并可能覆盖样式字符串所做的任何配置更改。{#adddict}

为拼写检查器添加词典 {#adddict}`/de/`
...
1. 如需添加更多词典，请按照以下步骤操作。[-ERR:REF-NOT-FOUND-
1. 

   >[!CAUTION]选择所需的语言并下载包含拼写定义的ZIP文件。 解压文件系统上的存档内容。
   [!CAUTION]

1. 仅支持OpenOffice. `MySpell` org v2.0.1或更早版本格式的词典。 由于词典现在是存档文件，建议您在下载后验证存档文件。`de_de.dic`
1. 找到。aff和。dic文件。 将文件名保留为小写。 例如， `de_de.aff` 和 `de_de.dic`。

>加载存储库中的。aff和。dic文件 `/apps/cq/spellchecker/dictionaries`。
[!NOTE]
RTE拼写检查器可按需使用。 它不会在您开始键入文本时自动运行。
要运行拼写检查器，请点按／单击工具栏中的拼写检查器按钮。 RTE检查单词拼写并突出显示拼写错误的单词。

## 如果包含拼写检查器建议的任何更改，则文本更改和拼写错误的单词的状态不再突出显示。 要运行拼写检查器，请再次点按／单击“拼写检查器”按钮。{#undohistory}

为撤消和重做操作配置历史记录大小 {#undohistory}

1. RTE允许作者撤消或重做上次所做的几项编辑。 默认情况下，50个编辑存储在历史记录中。 您可以根据需要配置此值。`<rtePlugins-node>/undo`[](#activateplugin)
1. 在组件中，导航到节点 `<rtePlugins-node>/undo`。 如果这些节点不存在，请创建它们。 有关详细信息， [请参阅激活插件](#activateplugin)。

   * 在节 `undo` 点上创建属性：**`maxUndoSteps`
   * **名称** `maxUndoSteps`
   * **类型** `Long`

1. **值** ，您希望在历史记录中保存的撤消步骤数。 默认为 50。使 `0` 用完全禁用撤消／重做。
...
>[!NOTE]启用缩进（默认）后，您可以定义缩进大小：
[!NOTE]

1. 此缩进大小仅应用于文本的段落（块）; 它不影响实际列表的缩进。`<rtePlugins-node>/lists`[](#activateplugin)
1. 在组件中，导航到节点 `<rtePlugins-node>/lists`。 如果这些节点不存在，请创建它们。 有关详细信息， [请参阅激活插件](#activateplugin)。

   * 在节 `lists` 点上创建 `identSize` 参数：`identSize`
   * **名称**: `identSize`
   * **类型**: `Long`
...
1. 您可以定义组件对话框中显示的可编辑空间的高度：`../items/text`

   * 在组件 `../items/text` 对话框定义的节点上，创建新属性：**`height`
   * **名称** `height`
   * **类型** `Long`
   >**以像素** 为单位对编辑画布的高度进行值。
   [!NOTE]
...
1. 要配置如何从其他项目在AEM中添加链接，请定义HTML规则。
1. 使用CRXDE Lite找到项目的文本组件。`<rtePlugins-node>``<rtePlugins-node>`

   * 在与创建节点相同的级别 `<rtePlugins-node>`创建新节点，即在以下节点的父节点下创建节点 `<rtePlugins-node>`:`htmlRules`
   * **名称** `htmlRules`
   >**类型** `nt:unstructured`
   [!NOTE]
   * 该 `../items/text` 节点具有以下属性：**`xtype`
   * **名称** `xtype`
   * **类型** `String`
   **值** `richtext`
...
1. `/apps/<myProject>/components/text/dialog/items/panel/items/text`
   ...
   * 在 `htmlRules`下，创建新节点。**`links`
   * **名称** `links`

1. **类型** `nt:unstructured`

   * 在节点 `links` 下，根据需要定义属性：

      * **内部链接的CSS样式：**`cssInternal`
      * **名称** `cssInternal`
      * **类型** `String``.cssClass`
   * **值** CSS类的名称（没有前面的“.”）; for example, `cssClass` instead of `.cssClass`)

      * **外部链接的CSS样式**`cssExternal`
      * **名称** `cssExternal`
      * **类型** `String``.cssClass`
   * **值** CSS类的名称（没有前面的“.”）; for example, `cssClass` instead of `.cssClass`)

      * 有效协议 **的阵列** (包括https://、https:// file://、mailto：等)`protocols`
      * **名称** `protocols`
      * **类型** `String[]`
   * **值**（一个或多个）协议****
   ...
   * **值**（一个或多个）默认协议
      ...
      * **如何处理链接的目标属性的定义。 创建新节点：**`targetConfig`
      * **名称** `targetConfig`
      **类型** `nt:unstructured`
      ...
      * 在节点上 `targetConfig`: 定义所需的属性：
         ...
         * **类型** `String`)
            ...
            * **值**:
               ...
               `auto`: 表示已选择自动目标`targetInternal`

            * (由外部链 `targetExternal` 接或内部链接的 `targetInternal` 属性指定)。
            * `manual`: 不适用于此上下文
      * `blank`: 不适用于此上下文

         * **内部链接的目标:**`targetInternal`
         * **名称** `targetInternal`
         * **类型** `String`
      * **为内部** 链接的目标值(仅在“”模式为时使 `auto`用)

         * **外部链接的目标:**`targetExternal`
         * **名称** `targetExternal`
         * **类型** `String`
