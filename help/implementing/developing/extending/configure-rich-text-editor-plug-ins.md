...
有关其他RTE配置的详细信息，请参见 [配置富文本编辑器](/help/implementing/developing/extending/rich-text-editor.md).
...
|---|---|---|---|
| 名称 | 功能 | 功能 | 功能 |
| 类型 | 字符串 | `String` (多字符串；将Type设置为 `String` 并单击 `Multi` CRXDE Lite) | 字符串 |
| 价值 | `*` （星号） | 设置为一个或多个特征值。 | - |
...
* 将剪贴板内容粘贴为纯文本。 在插入之前，它会从复制的内容中剥离所有样式和格式元素 [!DNL Experience Manager] 组件。 配置方式 `plaintext` 下面的。

* 从MS Word复制时，粘贴包含格式设置的文本（包括表格）。 不支持从其他源（如网页或MS Excel）复制和粘贴文本，并且仅保留部分格式。 配置方式 `wordhtml` 下面的。

1. 在组件中，导航到 `<rtePlugins-node>/edit` 节点。 如果节点不存在，则创建节点。 有关更多信息，请参阅 [激活插件](#activateplugin).
1. 在 `edit` 节点使用以下详细信息创建属性：

   * **名称** `defaultPasteMode`
   * **类型** `String`
   * **值** 是中所需的粘贴模式之一 `browser`， `plaintext`，或 `wordhtml` 模式。

### 配置粘贴内容时允许的格式 {#pasteformats}

粘贴为Microsoft-Word (`paste-wordhtml`)模式可进行进一步配置，以便在粘贴时显式允许一些样式 [!DNL Experience Manager] 来自其他程序，例如 [!DNL Microsoft Word].

例如，如果粘贴时只允许使用粗体格式和列表 [!DNL Experience Manager]中，您可以筛选掉其他格式。 这称为可配置的粘贴筛选，可以对以下两项执行：
...
配置在将文本粘贴到时允许的格式 [!DNL Experience Manager] 其他程序：

1. 在组件中，导航到节点 `<rtePlugins-node>/edit`. 如果节点不存在，则创建节点。 有关更多详细信息，请参阅 [激活插件](#activateplugin).
...
| `fallbackBlockTag` | `String` | 定义块标记，该标记用于具有块标记但未包含到的任何块 `allowBlockTags`. 通常， `p` 就足够了。 |
| `table` | `nt:unstructured` | 定义粘贴表时的行为。 此节点必须具有属性allow（类型Boolean）才能定义是否允许粘贴表。 如果allow设置为false，则必须指定属性ignoreMode（类型String）以定义如何处理粘贴的表内容。 ignoreMode的有效值为 `remove` 删除表内容和 `paragraph` 将表格单元格转换为段落。 |
| `list` | `nt:unstructured` | 定义粘贴列表时的行为。 必须具有属性 `allow` （键入布尔值）以定义是否允许粘贴列表。 如果 `allow` 设置为 `false`，指定属性 `ignoreMode` (类型 `String`)以定义如何处理任何粘贴的列表内容。 ignoreMode的有效值为 `remove` 删除列表内容和 `paragraph` 将列表项转换为段落。 |

有效示例 `htmlPasteRules` 架构如下：
...
作者可以应用样式来更改部分文本的外观。 样式基于您在CSS样式表中预定义的CSS类。 风格化的内容包含在 `span` 标记使用 `class` 属性以引用CSS类。 例如：
...
>[!NOTE]
>
>还可以为以下对象定义样式 [表或表单元格](configure-rich-text-editor-plug-ins.md#tablestyles). 这些配置需要单独的过程。

### 启用样式下拉选择器列表 {#styleselectorlist}

启用样式插件即可完成此操作。

1. 在组件中，导航到节点 `<rtePlugins-node>/styles`. 如果节点不存在，则创建节点。 有关更多详细信息，请参阅 [激活插件](#activateplugin).
...
富文本编辑器使用一个容器DOM元素，其ID为 `CQrte` 提供了不同的样式来查看和编辑：
...
1. 在节点下 `styles`，创建一个节点(也称为 `styles`)，以保存可用的列表：
...
作者使用 [!DNL Experience Manager] 要创作日语内容，可以将样式应用于字符以避免在不需要换行符时换行。 这允许作者在所需的位置处使句子断开。 此功能的样式基于CSS样式表中预定义的CSS类。

要创建作者可以应用于日语文本的样式，请执行以下步骤：

1. 在“样式”节点下创建一个节点。 参见 [指定样式](#stylesindropdown).
   * 名称: `jpn-word-wrap`
   * 类型: `nt:unstructure`
...
>[!NOTE]
>
>如果块标记，例如 `<hr>` 标记，无法分配给段落，它不是有效的用例 `paraformat` 插件。

首次启用段落格式插件时，没有默认的段落格式可用。 弹出列表为空。 要向作者提供段落格式，请执行以下操作：

* 启用 [!UICONTROL 格式] 弹出窗口选择器列表。
* 指定可从弹出菜单中选择为段落格式的块标记。

对于以后的重新配置（如要添加更多格式），请仅遵循说明中的相关部分。

### 启用“格式”下拉选择器 {#formatselectorlist}

要启用 `paraformat` 插件，请按照以下步骤操作：

1. 在组件中，导航到节点 `<rtePlugins-node>/paraformat`. 如果节点不存在，则创建节点。 有关更多详细信息，请参阅 [激活插件](#activateplugin).
...
>[!CAUTION]
>
>配置RTE的段落格式时，请勿删除段落标记 &lt;p> 作为格式选项。 如果 `<p>` 标记被删除，那么内容作者无法选择 [!UICONTROL 段落格式] 选项，即使配置了其他格式也是如此。
...
1. 在 `paraformat` 节点创建一个节点，保存格式列表：
...
在标准中 [!DNL Experience Manager] 安装，当 `misctools` 插件已启用特殊字符(`specialchars`)默认选项可立即使用；例如，版权和商标符号。

您可以配置RTE以使您选择的字符可用；通过定义不同的字符或整个序列。

>[!CAUTION]
添加特殊字符会覆盖默认选项。 如果需要，可在所选内容中重新定义这些字符。

### 定义单个字符 {#definesinglechar}

1. 在组件中，导航到节点 `<rtePlugins-node>/misctools`. 如果节点不存在，则创建节点。 有关更多详细信息，请参阅 [激活插件](#activateplugin).
...
1. 在组件内，导航到节点 `<rtePlugins-node>/table`. 如果节点不存在，则创建节点。 有关更多详细信息，请参阅 [激活插件](#activateplugin).
...
1. 在 `table` 节点根据需要创建以下节点：

   * 为整个表定义样式(可在 **[!UICONTROL 表属性]**)：

      * **名称** `tableStyles`
      * **类型** `cq:WidgetCollection`
   * 为各个单元格定义样式(可在 **[!UICONTROL 单元格属性]**)，

      * **名称** `cellStyles`
      * **类型** `cq:WidgetCollection`


1. 创建一个节点(在 `tableStyles` 或 `cellStyles` 节点)，以表示单个样式，
...
>[!NOTE]
标准词典位于 `/libs/cq/spellchecker/dictionaries`，以及相应的ReadMe文件。 请勿修改文件。

如果需要，请按照以下步骤添加更多词典。

1. 导航到页面 [https://extensions.openoffice.org/](https://extensions.openoffice.org/).
1. 选择所需的语言并下载包含拼写定义的ZIP文件。 提取文件系统中存档的内容。
...
1. 在组件中，导航到节点 `<rtePlugins-node>/keys`. 如果节点不存在，则创建节点。 有关更多详细信息，请参阅 [激活插件](#activateplugin).
1. 在 `keys` 节点创建属性：
...
您可以定义组件对话框中显示的可编辑空间的高度。 该配置仅适用于在对话框中使用RTE的情况。 该配置不会更改对话框窗口的高度。

1. 在 `../items/text` 节点，在组件的对话框定义中，创建一个属性：
...
在中添加链接时 [!DNL Experience Manager]，您可以定义要使用的CSS样式和要自动接受的协议。 配置链接在中添加的方式 [!DNL Experience Manager] 在另一个程序中，定义HTML规则。
...
1. 在与相同的级别创建节点 `<rtePlugins-node>`，即，在父节点下创建节点 `<rtePlugins-node>`：
   ...
   * **值** `richtext`
   ...
   的位置 `../items/text` 节点可能因对话框的结构而异。 两个示例为 `/apps/myProject>/components/text/dialog/items/text` 和 `/apps/<myProject>/components/text/dialog/items/panel/items/text`.
...
1. 在 `links` 节点根据需要定义属性：
   ...
   * 外部链接的CSS样式
      ...
      * **类型** `String`
      * **值** CSS类的名称(不带前缀“。”；例如， `cssClass` 而不是 `.cssClass`)
   * 有效数组 **[!UICONTROL 协议]** 包括 `https://`， `https://`， `file://`， `mailto:`和其他人，
