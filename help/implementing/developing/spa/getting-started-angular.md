...
# AEM SPA入门（使用角度） {#getting-started-with-spas-in-aem-using-angular}
...
>[!NOTE]
>
>本文基于角框架。 有关React框架的相应文档，请参 [阅AEM中SPA入门- React](getting-started-react.md)。
...
收 `MainComponent` 集页面模型的JSON表示形式并处理内容以包装／装饰页面的每个元素。 有关此项的更 `Page` 多详细信息，请参 [阅文档SPA Blueprint](blueprint.md)。
