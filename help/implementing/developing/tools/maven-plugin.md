...
将所构建的包部署到AEM由Adobe内容包Maven插件执行，并允许自动执行通常使用AEM执行的任务 [包管理器：](/help/implementing/developing/tools/package-manager.md)
...
本文档详细介绍如何使用Maven管理这些任务。 但是，了解以下内容也很重要 [AEM项目及其包的结构方式。](#aem-project-structure)

>[!NOTE]
>
>包 **创建** 现在归 [Apache Jackrabbit FileVault包Maven插件。](https://jackrabbit.apache.org/filevault-package-maven-plugin/)
>
...
| 名称 | 类型 | 必需 | 默认值 | 描述 | 目标 |
|---|---|---|---|---|---|
| `failOnError` | `boolean` | 否 | `false` | 值 `true` 导致构建在出错时失败。 值 `false` 导致生成忽略该错误。 | 除外的所有目标 `package` |
| `name` | `String` | `build`：是， `install`：否， `rm`：是 | `build`：无默认值， `install`：的值 `artifactId` Maven项目的属性 | 要执行操作的包的名称 | 除外的所有目标 `ls` |
| `password` | `String` | 是 | `admin` | 用于通过AEM进行身份验证的密码 | 除外的所有目标 `package` |
| `serverId` | `String` | 否 | 从中检索用于身份验证的用户名和密码的服务器ID | 除外的所有目标 `package` |
| `targetURL` | `String` | 是 | `http://localhost:4502/crx/packmgr/service.jsp` | AEM包管理器的HTTP服务API的URL | 除外的所有目标 `package` |
| `timeout` | `int` | 否 | `5` | 与包管理器服务通信的连接超时（以秒为单位） | 除外的所有目标 `package` |
| `useProxy` | `boolean` | 否 | `true` | 值 `true` 会导致Maven使用找到的第一个活动代理配置，将请求代理到包管理器。 | 除外的所有目标 `package` |
| `userId` | `String` | 是 | `admin` | 用于通过AEM进行身份验证的用户名 | 除外的所有目标 `package` |
| `verbose` | `boolean` | 否 | `false` | 启用或禁用详细日志记录 | 除外的所有目标 `package` |
...
除了以下参数之外，请参见 [常用参数](#common-parameters) 部分。

| 名称 | 类型 | 必需 | 默认值 | 描述 |
|---|---|---|---|---|
| `artifact` | `String` | 否 | 的值 `artifactId` Maven项目的属性 | 表单的字符串 `groupId:artifactId:version[:packaging]` |
| `artifactId` | `String` | 否 | 无 | 要安装的工件的ID |
| `groupId` | `String` | 否 | 无 | 此 `groupId` 要安装的工件 |
| `install` | `boolean` | 否 | `true` | 确定在上传包时是否自动解压缩包 |
| `localRepository` | `org.apache.maven.artifact.repository.ArtifactRepository` | 否 | 的值 `localRepository` 系统变量 | 本地Maven存储库，由于始终使用系统属性，因此无法使用插件配置对其进行配置 |
| `packageFile` | `java.io.File` | 否 | 为Maven项目定义的主要构件 | 要安装的软件包文件的名称 |
| `packaging` | `String` | 否 | `zip` | 要安装的工件的包装类型 |
| `pomRemoteRepositories` | `java.util.List` | 是 | 的值 `remoteArtifactRepositories` 为Maven项目定义的属性 | 此值不能使用插件配置进行配置，必须在项目中指定。 |
...
有关rm目标的所有参数的说明，请参见 [常用参数](#common-parameters) 部分。
...
创建内容包。 程序包目标的默认配置包括保存已编译文件的目录的内容。 程序包目标的执行要求编译构建阶段已完成。 包目标将绑定到Maven构建生命周期的包阶段。

#### 参数 {#parameters-5}

除了以下参数之外，请参见 `name` 中的参数 [常用参数](#common-parameters) 部分。
...
| `failOnMissingEmbed` | `boolean` | 是 | `false` | 值 `true` 导致在项目依赖关系中未找到嵌入工件时生成失败。 值 `false` 导致生成忽略此类错误。 |
| `filterSource` | `java.io.File` | 否 | 无 | 此参数定义一个文件，该文件指定工作区过滤器的源。 在配置中指定并通过嵌入或子包注入的过滤器将与文件内容合并。 |
| `filters` | `com.day.jcr.vault.maven.pack.impl.DefaultWorkspaceFilter` | 否 | 无 | 此参数包含定义包内容的过滤器元素。 执行时，这些过滤器包含在 `filter.xml` 文件。 请参阅 [使用过滤器](#using-filters) 部分。 |
| `finalName` | `java.lang.String` | 是 | 此 `finalName` 在Maven项目中定义（构建阶段） | 生成的包ZIP文件的名称，不带 `.zip` 文件扩展名 |
...
| `properties` | `java.util.Map` | 否 | 无 | 这些参数定义了可在 `properties.xml` 文件。 这些属性无法覆盖以下预定义属性： `group` (使用 `group` 要设置的参数)， `name` (使用 `name` 要设置的参数)， `version` (使用 `version` 要设置的参数)， `description` （从项目描述中设置）， `groupId` (`groupId` Maven项目描述符的)， `artifactId` (`artifactId` Maven项目描述符的)， `dependencies` (使用 `dependencies` 要设置的参数)， `createdBy` (值 `user.name` 系统属性)， `created` （当前系统时间）、 `requiresRoot` (使用 `requiresRoot` 要设置的参数)， `packagePath` （自动从组和包名称生成） |
...
| `workDirectory` | `java.io.File` | 是 | Maven项目中定义的目录（构建阶段） | 包含要包含在包中的内容的目录 |

#### 使用过滤器 {#using-filters}

使用filters元素定义资源包内容。 这些过滤器将添加到 `workspaceFilter` 中的元素 `META-INF/vault/filter.xml` 包的文件。
...
此 `mode` 元素定义在导入包时内容对存储库有何影响。 可以使用以下值：

* **合并：** 将添加存储库中尚未存在的包内容。 包和存储库中的内容保持不变。 不会从存储库中删除任何内容。
...
源文件可以位于文件系统中的任何位置。 在POM文件中，定义生成资源以将源文件复制到 `target/vault-work/META-INF` 包含在包中的路径。

以下POM代码会将文件添加到 `META-INF` 包的项目源的文件夹：
...
以下POM代码仅将缩略图图像添加到包。 必须命名缩略图图像 `thumbnail.png`，并且必须位于 `META-INF/vault/definition` 包的文件夹。 在此示例中，源文件位于 `/src/main/content/META-INF/vault/definition` 项目的文件夹：
