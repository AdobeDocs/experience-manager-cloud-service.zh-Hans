...
包允许导入和导出存储库内容。 您可以使用包来安装新内容、在实例之间传输内容以及备份存储库内容。
...
为AEMas a Cloud Service应用程序创建的内容包必须在不可变内容和可变内容之间实现完全分离。 因此，包管理器只能用于管理包含内容的包。 任何代码都必须通过Cloud Manager部署。
...
| 字段 | 描述 | 示例/详细信息 |
|---|---|---|
| 测试方式 | 此产品包的目标产品名称和版本或与之兼容。 | `AEMaaCS` |
...
[Software Distribution](https://downloads.experiencecloud.adobe.com) 提供AEM包，以供在本地开发AEM SDK中使用。 在Software Distribution中提供的AEM包，除非获得Adobe支持的明确批准，否则不得在AEMaaCS云环境中安装。
