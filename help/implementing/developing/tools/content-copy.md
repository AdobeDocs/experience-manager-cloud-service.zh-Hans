...
内容复制工具使用户能够根据需要将可变内容从AEMas a Cloud Service上的生产环境复制到较低环境，以进行测试。
...
* 如果内容在目标环境中已被修改，则在路径相同的情况下，它会被源中的内容覆盖。
* 如果路径不同，则源中的内容将与目标中的内容合并。

## 权限 {#permissions}

要使用内容复制工具，源和目标环境中都需要某些权限。
...
1. 如果必须优化或限制内容集，则可以排除子路径。
   ...
   1. 在包含的路径列表中，单击 **添加排除子路径** 位于要限制的路径旁边。
   1. 输入要在所选路径下排除的子路径。
   1. 点按或单击&#x200B;**排除路径**。
...
创建内容集后，您可以使用它来复制内容。 请按照以下步骤操作，以便您可以复制内容。

>[!NOTE]
> 不使用Content Copy时的环境 [内容传输](/help/journey-migration/content-transfer-tool/using-content-transfer-tool/overview-content-transfer-tool.md) 操作正在该环境中运行。
...
* 内容复制工具没有版本控制功能，并且自上次内容复制操作以来，无法自动检测内容集中的源环境中已修改的内容或新创建的内容。
   * 如果只想使用内容更改更新目标环境，则自上次内容复制操作以来，您必须创建一个内容集。 然后，指定自上次内容复制操作以来在其中进行了更改的源实例上的路径。
* 版本信息不包含在内容副本中。
