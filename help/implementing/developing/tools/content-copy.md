---
title: 内容复制工具
description: 内容复制工具允许用户按需将可变内容从其AEM as a Cloud Service上的生产环境复制到较低环境中，以进行测试。
...
1. 如果必须优化或限制内容集，则可以排除子路径。
   ...
   1. 在所包含路径列表中，单击要限制的路径旁边的&#x200B;**添加排除子路径**&#x200B;选项。
   1. 输入要从所选路径中排除的子路径。
   1. 选择&#x200B;**排除路径**。
...
1. 遵循与创建内容步骤时类似的步骤。不要单击&#x200B;**添加内容集**，请从控制台中选择现有内容集，然后从省略号菜单中选择&#x200B;**编辑**。

![编辑内容集](assets/edit-content-set.png)

1. 编辑内容集时，您可以展开配置的路径以显示排除的子路径。
...
1. 在&#x200B;**复制内容活动**&#x200B;页面上，从省略号菜单中，为要审阅的复制过程单击&#x200B;**日志**。 然后选择环境。

![正在访问复制内容进程的日志](assets/copy-content-logs.png)

日志将下载到您的本地计算机。

1. 如果未开始下载，请检查弹出窗口阻止程序设置。
...
* 版本信息不包含在内容副本中。
* [内容片段模型](/help/sites-cloud/administering/content-fragments/content-fragment-models.md#data-types)可以根据通用唯一ID (UUID)指定引用字段。 此类UUID特定于存储库，因此在复制内容片段时，内容复制工具会在目标环境中重新计算这些UUID。
