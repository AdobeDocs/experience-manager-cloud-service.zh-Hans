---
title: 内容复制工具
description: 内容复制工具使用户能够按需将可变内容从其AEMas a Cloud Service生产环境复制到较低级别的环境中，以便进行测试。
...
内容复制工具使用户能够按需将可变内容从其AEMas a Cloud Service生产环境复制到较低级别的环境中，以便进行测试。

## 简介 {#introduction}

当前的真实数据对于测试、验证和用户验收很有价值。内容复制工具允许您将内容从生产AEM环境复制到暂存、开发或 [快速开发环境(RDE)](/help/implementing/developing/introduction/rapid-development-environments.md) 进行此类测试的环境。
...
1. 在&#x200B;**复制内容**&#x200B;对话框中，指定内容复制操作的源和目标。
   ...
   * 内容只能从较高的环境复制到较低的环境，或在环境层次结构如下（从最高到最低）的开发/RDE环境之间复制：
      * 生产
      * 暂存
      * 开发/RDE
...
| 失败 | 内容复制操作失败 |
| 已完成 | 内容复制操作成功完成 |
| 已取消 | 用户启动内容复制操作后会取消该操作 |

### 取消复制进程 {#cancelling}

如果启动内容复制操作后需要中止该操作，则可以选择取消该操作。

为此，请在 **复制内容活动** 页面，选择 **取消** 复制过程的省略号菜单中的操作。

![取消内容复制](assets/content-copy-cancel.png)

>[!NOTE]
>
>取消内容复制操作时，可能会导致目标环境中的内容出现部分副本。 这会使目标环境处于不可用状态。
>
>如果您的环境因取消而处于此类状态，请联系Adobe客户关怀团队以寻求帮助。
...
* 内容复制工具没有版本控制能力，并且无法自动检测自上次内容复制操作以来在内容集中的源环境上修改或新创建的内容。
   * 如果您希望仅使用上次内容复制操作后的内容更改来更新目标环境，则需要创建内容集，并指定自上次内容复制操作后在源实例上进行更改的路径。
* 版本信息不包含在内容副本中。
