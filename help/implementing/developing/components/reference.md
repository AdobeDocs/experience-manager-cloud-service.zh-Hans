...
* **重要属性**：
   * `jcr:title`  — 组件标题；例如，在列表中列出组件时用作标签 [组件浏览器](/help/sites-cloud/authoring/page-editor/editor-side-panel.md#components-browser) 和 [组件控制台](/help/sites-cloud/authoring/components-console.md)
...
此 [组件控制台](/help/sites-cloud/authoring/components-console.md#component-details) 显示如何定义特定组件的图标。
...
| `componentGroup` | `String` | 这表示可在其中选择组件的组 [组件浏览器](/help/sites-cloud/authoring/page-editor/editor-side-panel.md#components-browser). 以开头的值 `.` 用于无法从UI中选择的组件，例如其他组件继承自的基本组件。 |
...
对话框是组件的关键元素，因为它们为作者提供了一个界面，用于在内容页面上配置组件并提供该组件的输入。 请参阅 [创作文档](/help/sites-cloud/authoring/page-editor/edit-content.md) 以了解有关内容作者如何与组件交互的详细信息。
...
“设计”对话框类似于用于编辑和配置内容的对话框，但它们为模板作者提供了界面，以便预配置页面模板上的该组件，并提供其设计详细信息。 然后，内容作者使用页面模板来创建内容页面。 请参阅 [模板文档](/help/sites-cloud/authoring/sites-console/templates.md) 以了解有关如何创建模板的详细信息。

[编辑页面模板时，使用设计对话框](/help/sites-cloud/authoring/sites-console/templates.md)，尽管并非所有组件都需要这些插件。 例如， **标题** 和 **图像组件** 两者都具有设计对话框，而 **社交媒体共享组件** 不会。
...
请参阅 [模板文档](/help/sites-cloud/authoring/sites-console/templates.md) 以了解有关如何创建模板的详细信息。
...
然后，此Markdown将显示在 [组件控制台](/help/sites-cloud/authoring/components-console.md).
