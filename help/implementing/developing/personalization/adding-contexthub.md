...
* 首先，使用[geolocation](sample-stores.md#contexthub-geolocation-sample-store-candidate)存储来确定用户的纬度。
* 然后，[surferinfo存储区](sample-stores.md#contexthub-surferinfo-sample-store-candidate)的月份数据项确定该纬度的季节。
...
ContextHub提供了[诊断页面](contexthub-diagnostics.md) ，您可以在其中看到ContextHub框架的概述。
