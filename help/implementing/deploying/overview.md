...
对于以下情况，最好在OSGI工厂配置中采用手工编码显式内容创 `repoinit` 建语句的方法：
...
>[!WARNING]
>
>对于为下面的节点定 `/apps` 义的 `/libs` ACL，或者重新指示执行将开始空的存储库。 软件包在重新指向后安装，因此语句不能依赖包中定义的任何内容，但必须定义先决条件，如下面的父结构。
...
如果对索引进行了更改，则Blue版本应继续使用其索引，直到其终止，而Green版本应使用其自己修改过的索引集。 开发者应遵循本文描述的索 [引管理技术](/help/operations/indexing.md)。
...
## 源代码控制中的维护任务配置 {#maintenance-tasks-configuration-in-source-control}

维护任务配置必须在源代码控制中保留， **因为Cloud环境中将** 不再提供“工具”>“操作”屏幕。 这有利于确保有意地坚持改革，而不是反动地应用，或许会被遗忘。 请参阅维护 [任务文章](/help/operations/maintenance.md) ，以了解详细信息。
