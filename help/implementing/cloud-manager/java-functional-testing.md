---
title: Java 功能测试
description: 了解如何为 AEM as a Cloud Service 编写 Java 功能测试
...
1. Cloud Manager中的测试使用技术管理员用户运行。
...
| 内存 | 0.5Gi | 分配给测试的内存量，以 gibibytes 为单位的值 |
| 超时 | 30m | 测试终止的持续时间。 |
...
但是，在运行这些测试时，必须设置以下各项所需的各种系统属性： `aem-testing-clients` （和底层Sling测试客户端）库。

系统属性如下。

| 属性 | 描述 | 示例 |
|-------------------------------------|------------------------------------------------------------------|-------------------------|
| `sling.it.instances` | 匹配云服务的实例数量应设置为 `2` | `2` |
