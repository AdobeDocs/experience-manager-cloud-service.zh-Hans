---
title: Java &amp；trade；功能测试
description: 了解如何为AEM as a Cloud Service编写Java &amp；trade；功能测试
...
自定义功能测试的代码是位于 `it.tests` 文件夹中的 Java™ 代码。 它应该生成一个包含所有功能测试的 JAR。 如果构建生成多个测试 JAR，那么选择哪个 JAR 是不确定的。 如果它生成零个测试 JAR，则默认通过测试步骤。 有关示例测试，请参阅[AEM项目原型](https://github.com/adobe/aem-project-archetype/tree/develop/src/main/archetype/it.tests)。
...
即将更改用于执行功能测试的容器化基础结构，需要将自定义功能测试中的[aem-cloud-testing-clients](https://github.com/adobe/aem-testing-clients)库更新为版本&#x200B;**1.2.1**&#x200B;或更高版本。 确保您的`it.tests/pom.xml`文件中的依赖项已相应地更新。
...
| 属性 | 描述 | 示例 |
|-------------------------------------|------------------------------------------------------------------|-------------------------|
| `sling.it.instances` | 要匹配云服务的实例数应设置为`2`。 | `2` |
| `sling.it.instance.url.1` | 设置为作者URL。 | `http://localhost:4502` |
| `sling.it.instance.runmode.1` | 第一个实例的运行模式。 设置为`author`。 | `author` |
| `sling.it.instance.adminUser.1` | 设置为作者管理员用户。 | `admin` |
| `sling.it.instance.adminPassword.1` | 设置为作者管理员密码。 |                         |
| `sling.it.instance.url.2` | 设置为发布URL。 | `http://localhost:4503` |
| `sling.it.instance.runmode.2` | 第二个实例的运行模式。 设置为`publish`。 | `publish` |
| `sling.it.instance.adminUser.2` | 设置为发布管理员用户。 | `admin` |
| `sling.it.instance.adminPassword.2` | 设置为发布管理员密码。 |                         |
