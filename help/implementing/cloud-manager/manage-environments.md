...
1. 请耐心等待。Cloud Manager 将不断尝试恢复该区域，并且该区域可能随时变得可用。
1. 如果该问题在若干小时后仍然存在，您可以删除其他发布区域并重新添加它（同一区域或另一个区域）以触发完整部署。

在执行其他操作之前等待系统自行恢复的时长取决于该区域的故障对系统的影响。

请注意，在任何情况下，[流量将始终路由到其他最近的在线区域。](/help/operations/additional-publish-regions.md) 如果您仍然遇到问题，请联系 Adobe 客户关怀部门。
