...
| | 用例 | 描述 |
| --- | --- | --- |
| 1 | **Adobe托管证书(DV)** | Cloud Manager允许用户配置来自Adobe的DV（域验证）证书，以进行快速域设置。 DV证书是最基本级别的SSL证书，通常用于测试目的或用于通过基本加密保护网站。 DV证书在[生产和沙盒程序](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/program-types.md)中均可用。 |
