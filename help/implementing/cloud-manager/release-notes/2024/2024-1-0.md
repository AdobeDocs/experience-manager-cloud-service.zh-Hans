...
description: AEM as a Cloud Service中的Cloud Manager 2024.1.0发行说明。
...
AEM as a Cloud Service中的Cloud Manager 2024.1.0版的发布日期是2024年1月10日。 下一个版本计划于 2024 年 2 月 16 日发布。

## 新增功能 {#what-is-new}

* Cloud Manager现在不仅验证主[证书](/help/implementing/cloud-manager/managing-ssl-certifications/introduction-to-ssl-certificates.md)的过期日期，还验证中间证书的过期日期。
...
* 修复了以下错误：如果配置文件位置设置不正确，则配置管道在构建步骤中失败，并显示不明确的消息。 新的错误消息现在指示用户应检查配置文件的位置是否正确。
