...
* 在某一特定时间段内未检测到活动时，沙盒程序内的环境将休眠。 在Cloud Manager中未观察到此状态。 但是，可以通过开发人员控制台观察此状态。 此功能将在即将发布的版本中介绍。

* 直接从 Cloud Manager 链接到开发人员控制台不会显示用于将沙盒程序的环境解除休眠/休眠的选项。要解决此问题，请在开发人员控制台中，将模式 `#release-cm-p1234-e5678` 添加到 URL 的末尾，其中 *1234* 是项目 ID，*5678* 是环境 ID。此功能将在即将发布的版本中介绍。
