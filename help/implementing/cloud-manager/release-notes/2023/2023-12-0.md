...
description: AEM as a Cloud Service中的Cloud Manager 2023.12.0发行说明。
...
* 通过 [Cloud Manager 自定义权限](/help/implementing/cloud-manager/custom-permissions.md)，可创建具有可配置的权限的自定义权限配置文件，以限制 Cloud Manager 用户访问项目、管道和环境。
   * 此功能计划分阶段推出，预计2024年2月发布的Cloud Manager将分阶段完成。
   * 如果您希望更早启用，请通过与Adobe ID关联的电子邮件地址向`Grp-CloudManager-custom-permissions@adobe.com`发送电子邮件。
* 生成容器现在支持[前端管道](/help/implementing/developing/introduction/developing-with-front-end-pipelines.md)的Node.js版本18。
* 对于新创建的 Cloud Manager 程序，默认情况下不激活[关联的 New Relic 子帐户](/help/implementing/cloud-manager/user-access-new-relic.md)。
   ...
   * 如果要使用New Relic子帐户，必须通过Cloud Manager选择加入。
...
如果您使用 GitHub 管理存储库，[现在可以通过 Cloud Manager 直接在 GitHub 存储库中验证代码](/help/implementing/cloud-manager/managing-code/private-repositories.md)。此集成使您无需始终与 Adobe 存储库同步代码，并可在将拉取请求合并到主分支之前对其进行验证。
