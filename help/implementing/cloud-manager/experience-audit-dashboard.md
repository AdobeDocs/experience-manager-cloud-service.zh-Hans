...
1. 根据要配置的管道类型，执行以下操作之一：
   ...
   * 添加新的[生产管道](/help/implementing/cloud-manager/configuring-pipelines/configuring-production-pipelines.md)以定义希望审核评估的路径。
   * 如果要在前端管道或开发全栈管道上启用审核，请添加新的[非生产管道](/help/implementing/cloud-manager/configuring-pipelines/configuring-non-production-pipelines.md)。
...
* 在&#x200B;**[最慢的5页](#view-slowest-pages)**&#x200B;对话框中单击&#x200B;**`View full report`**。
* 查看管道](#results)的[执行时单击&#x200B;**`View full report`**。
...
如果单击某个时间点的图表，将打开一个弹出窗口，其中包含该扫描的详细信息。 单击&#x200B;**打开体验审核扫描**&#x200B;将该扫描结果加载到&#x200B;**[体验审核扫描结果](#scan-results)**&#x200B;部分。
...
**扫描的页面**&#x200B;部分提供了所有扫描页面上的分数的详细信息。 使用&#x200B;**上一个**&#x200B;和&#x200B;**下一个**&#x200B;按钮逐页浏览结果，并选择显示的页数。

![扫描的页面](assets/experience-audit-scanned-pages.png)

单击特定页面的链接可更新&#x200B;[**页面得分 — 趋势**&#x200B;部分](#trend)的&#x200B;**选择**&#x200B;筛选器，并显示选定页面的&#x200B;**得分和推荐**&#x200B;选项卡。

![页结果](assets/experience-audit-page-results.png)

**原始报告**&#x200B;选项卡为您提供了每次页面审核的得分。 单击&#x200B;**Lighthouse报告**&#x200B;列中的报告日期，以检索原始数据的JSON文件。

![原始报告](assets/experience-audit-raw-reports.png)

新选项卡将在您的浏览器中打开，引导您转到`https://googlechrome.github.io/lighthouse/viewer/`。 它会自动加载一个签名的URL，其中包含所选页面的Lighthouse原始JSON报告，以便进行详细检查。
...
要运行按需扫描，请导航到&#x200B;**报告**&#x200B;选项卡以查看完整的审核报告，然后单击&#x200B;**运行扫描**&#x200B;按钮。
...
如果您配置的](#configuration)要审核的[页面不可用或审核过程中存在其他错误，则体验审核会反映这一事实。
...
* 不要延迟加载折叠上方的图像 — 在浏览器中无需向下滚动即可看到的内容。
...
* 审核会扫描来自发布者的[配置的体验审核页面路径](#configuration)的源(`.com`)域，以模拟真实的用户体验，帮助您做出更好的网站管理和优化决策。
...
* 体验审核通过处理每个页面的原始报告来估计潜在的改进。 它将浪费的字节数或毫秒数与见解相关联，从而分配对性能得分的加权影响。 审核会提供此信息以及受影响的页面，以帮助决定要执行哪项建议。
有关更多详细信息，请参阅[常规性能提示部分](#performance-tips)。
* 前端管道可以部署到现有环境，并且多个前端管道可以针对同一环境。 由于扫描结果是在环境级别汇总的，因此得分、趋势和推荐是一致的。 无论哪个管道触发了扫描，这些结果都会显示在选定的环境中。
