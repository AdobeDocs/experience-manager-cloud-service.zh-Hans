...
# 配置AEM AI助手 — 管理员设置 {#aem-ai-asst-admin-setup}

管理员必须配置访问权限、权限和设置，组织内的用户才能使用AEM AI Assistant中的功能。 本文介绍了如何为您的组织启用AI助手、设置所需凭据以及保存配置更改。

**AEM AI Assistant配置过程概述**

配置过程包含以下步骤：

1. 在Adobe Admin Console中创建新的产品配置文件。
1. 启用“AI助理产品知识”权限。
1. 创建或使用现有用户组。
1. 将用户添加到用户组。
1. 将产品配置文件分配给用户组。
...
1. 按照[在Adobe Admin Console中创建新产品配置文件](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/access-control/ui/create-profile)中的详细说明操作，可找到Experience Platform文档。

1. 创建新的产品配置文件时，请使用以下示例来表示AI助手可以使用的值。
...
## 2 — 启用“AI助理产品知识”权限{#enable-permission}
...
1. 在&#x200B;**编辑AI助理的权限**&#x200B;页面中，打开&#x200B;**AI助理产品知识**&#x200B;切换开关。
...
## 3 — 创建用户组（或使用现有用户组）{#create-user-group}
...
1. 在&#x200B;**将用户添加到此用户组**&#x200B;页面上，搜索并选择需要访问AEM AI助手的用户。
