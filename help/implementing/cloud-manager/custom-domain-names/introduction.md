...
* Cloud Manager支持为站点程序提供发布和预览服务的自定义域名。 不支持创作服务的自定义域。
* 每个Cloud Manager环境最多可托管每个环境500个自定义域。
* 当当前正在运行的管道已附加到这些环境时，无法将域名添加到这些环境。
* 同一域名不能在多个环境中使用。
* 一次只能添加一个域名。
* AEMas a Cloud Service不支持通配符域，例如 `*.example.com`.
* 在添加自定义域名之前，必须为您的程序安装包含自定义域名（通配符证书有效）的有效SSL证书。 请参阅 [添加SSL证书](/help/implementing/cloud-manager/managing-ssl-certifications/add-ssl-certificate.md) 以了解更多。
