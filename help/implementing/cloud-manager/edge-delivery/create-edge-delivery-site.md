...
一键创建您的第一个 Edge Delivery Site，旨在帮助您自动完成 Cloud Manager 中 Edge Delivery Sites 的加入和部署。 只需单击一个按钮即可，这大大简化了流程。 单击即可配置所需的基础架构、与GitHub集成以进行版本控制，以及在Google Drive中配置文档和资产存储。

这种自动化有助于减少设置初始 Site 所需的手动工作量。 它可确保无缝的工作流和可扩展性，并在管理边缘内容时提高团队的绩效。

>[!IMPORTANT]
>
>一键式Edge Delivery站点配置针对概念验证站点和快速原型进行了优化。 对于生产工作负载，Adobe建议您将站点迁移到专门的Edge Delivery Services组织。
...
1. 单击&#x200B;**立即创建站点**。
   ...
   当Cloud Manager完成站点预配和验证时，**站点名称**（您之前输入的项目名称）将出现在Edge Delivery页面的&#x200B;**Edge Delivery站点**&#x200B;列表框中。 **已验证**&#x200B;状态列的左侧也出现一个绿色圆点。
...
1. 在 Edge Delivery 页面上，执行以下任一操作：
   ...
   | 发布 Site | <ul><li> 在 **Edge Delivery Sites** 列表框中，在 Site 名称的最右侧，单击![更多图标](https://spectrum.adobe.com/static/icons/workflow_18/Smock_More_18_N.svg)以打开下拉菜单。</li><li>在下拉菜单中单击![发布检查图标](https://spectrum.adobe.com/static/icons/workflow_18/Smock_PublishCheck_18_N.svg) **发布站点**。<br>将显示一个toast消息，通知您已成功开始发布站点。</li></ul> |
...
* 来自Cloud Manager **的** -
   ...
   1. 在&#x200B;**概述**&#x200B;页面的&#x200B;**发布投放**&#x200B;选项卡的&#x200B;**环境**&#x200B;信息卡中，单击高亮显示的![信息或信息图标](https://spectrum.adobe.com/static/icons/ui_18/InfoMedium.svg)。
