...
Web 层配置管道部署 HTTPD/Dispatcher 配置。 有关此类型管道的详细信息，请参阅 [CI/CD 管道。](/help/implementing/cloud-manager/configuring-pipelines/introduction-ci-cd-pipelines.md#web-tier-config-pipeline)
...
1. 在&#x200B;**源代码**&#x200B;选项卡上，必须定义以下选项。
   ...
   * **Git 分支** – 此选项定义管道应从中检索代码的所选存储库的分支。
   * **代码位置** – 此选项定义管道应从中检索代码的所选存储库分支中的路径。
      * 对于 Web 层配置管道，该路径通常包含 `conf.d`、`conf.dispatcher.d` 和 `opt-in` 目录。
