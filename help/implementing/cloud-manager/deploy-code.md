...
* **验证** – 此步骤可确保将管道配置为使用当前可用的资源。 例如，测试配置的分支是否存在并且环境是否可用。
...
>[!NOTE]
>
>每次部署时都会清除Dispatcher缓存。 然后，在新发布节点接受流量之前，它会先加热。
...
要识别某个执行是否为重新执行执行，可以检查触发器字段。 其值为 *重新执行(_E)*.
...
提交 *PUT* 对此端点的请求会导致 *201* 响应（如果成功），并且响应正文是新执行的表示形式。 这类似于通过 API 开始常规执行。
