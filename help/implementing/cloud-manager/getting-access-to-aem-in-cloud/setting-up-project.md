...
* Git 存储库的根目录中必须有一个 `pom.xml` 文件。 此 `pom.xml` 文件可以根据需要引用尽可能多的子模块（这些子模块又可能包含其他子模块，等等）。
...
应通过查找 `CM_BUILD` [环境变量](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/build-environment-details.md).  同样，只在 Cloud Manager 构建环境之外使用的配置文件应该通过查找是否缺少此变量来激活。
...
>[!NOTE]
>
>由于通过此机制部署的代码目前没有在 Cloud Manager 的质量关卡中实现的所有[代码质量规则](/help/implementing/cloud-manager/custom-code-quality-rules.md)中运行，因此只能非常谨慎地使用受密码保护的 Maven 存储库中的工件。 因此，它只应在少数情况下使用，并且适用于与 AEM 无关的代码。 建议同时部署Java源和整个项目源代码以及二进制文件。

若要在 Cloud Manager 中使用受密码保护的 Maven 存储库，请执行以下操作：

1. 将密码（也可以是用户名）指定为密码 [管道变量](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/build-environment-details.md).
