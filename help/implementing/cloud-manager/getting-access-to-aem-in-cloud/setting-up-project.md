...
应通过查找`CM_BUILD` [环境变量来激活 Cloud Manager 构建环境中的 Maven 配置文件。](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/build-environment-details.md) 同样，只在 Cloud Manager 构建环境之外使用的配置文件应该通过查找是否缺少此变量来激活。
