...
Cloud Manager中专用存储库的配置包括两个步骤：

1. [将专用存储库](#add-repo)添加到所选计划。
1. 然后，[验证专用存储库](#validate-ownership)的所有权。

### 向项目群添加专用存储库 {#add-repo}
...
1. 在每个相应的字段中，提供有关存储库的以下详细信息：
   ...
   | --- | --- |
   | 存储库名称 | 新存储库的表现性名称。 |
   | 存储库 URL | 专用存储库的URL，必须以`.git`结尾。<br>例如，*`https://github.com/org-name/repo-name.git`* （URL路径仅供说明之用）。 |
   | 描述（可选） | 存储库的详细说明。 |

1. 选择&#x200B;**保存**。
现在您可以[验证私有存储库的所有权](#validate-ownership)。
...
**要将专用存储库与Cloud Manager结合使用，请执行以下操作：**
