...
Managed Services和AEMas a Cloud Service中的Dispatcher配置结构存在差异。 下面提供了有关如何从AMS Dispatcher配置版本2迁移到AEMas a Cloud Service的分步指南。

## 如何将AMS转换为AEM as a Cloud Service Dispatcher配置

以下部分提供了有关如何转换AMS配置的分步说明。 它假定您有一个存档，其结构与中所述的结构相似 [Cloud Manager Dispatcher配置](https://experienceleague.adobe.com/docs/experience-manager-cloud-manager/using/getting-started/dispatcher-configurations.html)

### 提取存档并删除最后的前缀

将存档解压缩到一个文件夹，并确保直接子文件夹的开头为 `conf`， `conf.d`，
`conf.dispatcher.d` 和 `conf.modules.d`. 如果没有，则将其在层次结构中上移。
...
`lc` 或 `flush` 以它的名义。 中的所有虚拟主机文件 `conf.d/available_vhosts` 也可以删除未链接到的。
...
移除任何名为的文件 `base_rewrite.rules` 和 `xforwarded_forcessl_rewrite.rules` 并记得移除 `Include` 虚拟主机文件中引用它们的语句。

如果 `conf.d/rewrites` 现在包含单个文件，应将其重命名为 `rewrite.rules` 还有，别忘了改编 `Include` 语句也在虚拟主机文件中引用该文件。
...
移除任何名为的文件 `ams_default.vars` 并记得移除 `Include` 虚拟主机文件中引用它们的语句。

如果 `conf.d/variables` 现在包含单个文件，应将其重命名为 `custom.vars` 还有，别忘了改编 `Include` 语句也在虚拟主机文件中引用该文件。

但是，如果该文件夹包含多个特定于虚拟主机的文件，则应将其内容复制到 `Include` 在虚拟主机文件中引用它们的语句。

### 删除允许列表

删除文件夹 `conf.d/whitelists` 并移除 `Include` 虚拟主机文件中的语句引用了该子文件夹中的某个文件。
...
如果您看到未列入允许列表的Apache指令，请删除它们。

### 删除所有未发布的场

删除中的任何场文件 `conf.dispatcher.d/enabled_farms` 具有 `author`， `unhealthy`， `health`，
`lc` 或 `flush` 以它的名义。 中的所有场文件 `conf.dispatcher.d/available_farms` 也可以删除未链接到的。

### 重命名场文件

中的所有场 `conf.d/enabled_farms` 必须重命名以匹配模式 `*.farm`，例如，一个名为的场文件 `customerX_farm.any` 应重命名 `customerX.farm`.
...
如果 `conf.dispatcher.d/cache` 为空，复制文件 `conf.dispatcher.d/cache/rules.any`
从标准Dispatcher配置复制到此文件夹。 可在文件夹中找到标准Dispatcher配置 `src` 此SDK的。 别忘了改编
`$include` 语句引用 `ams_*_cache.any` 场文件中的规则文件。

如果 `conf.dispatcher.d/cache` 现在包含一个带有后缀的文件 `_cache.any`，应将其重命名为 `rules.any` 还有，别忘了改编 `$include` 场文件中引用该文件的语句。

但是，如果文件夹包含多个使用该模式且特定于场的文件，则应将其内容复制到 `$include` 场文件中引用它们的语句。

移除任何带有后缀的文件 `_invalidate_allowed.any`.

复制文件 `conf.dispatcher.d/cache/default_invalidate_any` 从Cloud Dispatcher配置中的默认AEM转到该位置。

在每个场文件中，删除 `cache/allowedClients` 部分，并将其替换为：
...
如果 `conf.dispatcher.d/clientheaders` 现在包含一个带有后缀的文件 `_clientheaders.any`，应将其重命名为 `clientheaders.any` 还有，别忘了改编 `$include` 场文件中引用该文件的语句。

但是，如果文件夹包含多个使用该模式且特定于场的文件，则应将其内容复制到 `$include` 场文件中引用它们的语句。

复制文件 `conf.dispatcher/clientheaders/default_clientheaders.any` 从默认AEMas a Cloud ServiceDispatcher配置转到该位置。

在每个场文件中，将如下所示的所有clientheader include语句替换：
...
如果 `conf.dispatcher.d/filters` 现在包含单个文件，该文件应重命名为
`filters.any` 还有，别忘了改编 `$include` 场文件中引用该文件的语句。

但是，如果文件夹包含多个使用该模式且特定于场的文件，则应将其内容复制到 `$include` 场文件中引用它们的语句。

复制文件 `conf.dispatcher/filters/default_filters.any` 从默认AEMas a Cloud ServiceDispatcher配置转到该位置。

在每个场文件中，将如下所示的所有filter include语句替换：
...
复制文件 `conf.dispatcher.d/renders/default_renders.any` 从默认AEMas a Cloud ServiceDispatcher配置转到该位置。

在每个场文件中，删除 `renders` 部分，并将其替换为：
...
如果 `conf.dispatcher.d/virtualhosts` 现在包含单个文件，该文件应重命名为
`virtualhosts.any` 还有，别忘了改编 `$include` 场文件中引用该文件的语句。

但是，如果文件夹包含多个使用该模式且特定于场的文件，则应将其内容复制到 `$include` 场文件中引用它们的语句。

复制文件 `conf.dispatcher/virtualhosts/default_virtualhosts.any` 从默认AEMas a Cloud ServiceDispatcher配置转到该位置。
