...
>[!IMPORTANT]
>
>**域**&#x200B;值不得包含`.adobeaemcloud.com`。 不能直接代理到adobeaemcloud.com域。 此限制可防止不需要的请求循环。 要将流量代理到您的AEM as a Cloud Service环境，请改用AEMaaCS环境中安装的[自定义域](#proxying-to-aemaacs)作为原始后端。
...
您不能在CDN配置中直接使用`adobeaemcloud.com`域作为来源。 这样做被拒绝（域不得包含`.adobeaemcloud.com`）以防止不需要的请求循环。 当从为Edge Delivery Site安装的域进行路由时，这也适用。

如果您的自定义域(`www.example.com`)已安装到AEMaaCS环境，则默认路由将路由到AEM后端，而不具有任何CDN规则。 当您需要路由跨环境（例如，从`pXXXX-eYYYY`到`pXXXX-eZZZZ`）或从Edge Delivery站点到AEMaaCS环境时，请使用源选择器。

在这些情况下，要将流量代理到您的AEM as a Cloud Service环境(例如，将特定路径（如`/graphql`）路由到后端)，请在AEMaaCS环境中安装自定义域，并在CDN配置中使用该自定义域作为来源。

**示例：**&#x200B;如果AEM发布层可在`publish-pXXXXX-eYYYYY.adobeaemcloud.com`访问，则不要在`originSelectors`中使用该域。 相反：

1. 在AEMaaCS环境中安装指向发布服务的自定义域（例如`aem-publish-origin.example.com`）。
2. 在您的CDN配置中，定义具有自定义域的源，并将所需路径（例如，`/graphql`）路由到它。
