...
>[!NOTE]
>有关云中Dispatcher以及如何下载Dispatcher工具的更多信息，请参阅云中的[Dispatcher](/help/implementing/dispatcher/disp-overview.md)页面。
...
强烈建议您从旧版模式迁移到灵活模式（如迁移部分[从旧版模式迁移到灵活模式](#migrating-flexible)中所述），**是**。 使用灵活模式还会导致SDK和运行时以改进的方式验证和部署配置。
...
3. 检查Dispatcher SDK配置文件的子集（如[文件结构部分](##legacy-mode-file-structure)中所述）是否未修改。 这是一项新检查，它随AEM SDK版本v2021.1.4738引入，其中还包含Dispatcher工具版本2.0.36。在此更新之前，客户可能错误地假定这些不可变文件的任何本地SDK修改也将应用于云环境。
...
此阶段也可以通过`validator full -d out src/dispatcher`独立运行，该生成下一个命令`bin/docker_run.sh out host.internal.docker:4503 8080`所需的外目录。
...
此阶段也可以通过`validator full -d out src/dispatcher`独立运行，该生成下一个命令`bin/docker_immutability_check.sh out`所需的外目录。

## 调试Apache和Dispatcher配置 {#debugging-apache-and-dispatcher-configuration}

请注意，您可以使用`./bin/docker_run.sh out docker.for.mac.localhost:4503 8080`在本地运行apache Dispatcher。
...
在Cloud Manager 2021.7.0版本中，新的Cloud Manager程序会生成AEM原型为28或更高的Maven项目结构，其中包括文件&#x200B;**opt-in/USE_SOURCES_DIRECTLY**。 这消除了以前旧版模式对文件数量和大小的限制，也会导致SDK和运行时以改进的方式验证和部署配置。 如果您的调度程序配置没有此文件，强烈建议您迁移。 使用[灵活模式](/help/implementing/dispatcher/validation-debug.md#migrating)页面中描述的方法。
