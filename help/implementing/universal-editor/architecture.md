...
通用编辑器由四个基本构建基块组成，这些构建基块进行交互，允许内容作者编辑任何实施中任何内容的任何方面，以便您提供卓越的体验、提高内容速度并提供一流的开发人员体验。
...
>[!TIP]
>
>如果您想查看 Universal Editor 及其架构，请参阅文档 [AEM Universal Editor 快速入门](getting-started.md)，了解如何获取 Universal Editor 访问权限以及如何对第一个 AEM 应用程序插桩以使用 Universal Editor。

### 编辑器 {#editors}

* **Universal Editor** – Universal Editor 使用已插桩的 DOM 来允许就地编辑内容。 请参阅文档[属性和类型](attributes-types.md)，了解有关必要元数据的详细信息。请参阅文档 [AEM Universal Editor 快速入门](getting-started.md)，了解 AEM 中插桩示例。
* **属性边栏** – 无法在上下文中编辑组件的某些属性，例如轮播的轮换时间或应始终打开或关闭哪个折叠选项卡。为了允许编辑此类组件信息，在编辑器的侧边栏中提供了一个基于表单的编辑器。

### 远程应用程序 {#remote-app}

要在 Universal Editor 中使应用程序可在上下文中编辑，必须对 DOM 进行插桩。 远程应用程序必须在 DOM 中呈现某些属性。请参阅文档[属性和类型](attributes-types.md)，了解有关必要元数据的详细信息。请参阅文档 [AEM Universal Editor 快速入门](getting-started.md)，了解 AEM 中插桩示例。
