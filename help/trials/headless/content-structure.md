...
在我们的示例中，我们将会创建一个模型来表示旅游网站上的旅行数据结构。我们会将此模型中的旅行称为&#x200B;**冒险**。

1. 单击屏幕右上角的&#x200B;**创建**&#x200B;按钮以开始创建内容片段模型。

1. 这将会启动创建模型向导，该向导会引导您完成模型的创建。提供必要信息。

   * **模型标题** – 这是模型的简短描述，通常指明了该模型的用途。我们将新的模型称为 `Adventure`。
...
我们现在有一个叫做&#x200B;**冒险**&#x200B;的模型，代表旅游网站上的旅行，但它没有持续时间、目的地、活动等详细信息。您需要先定义模型的数据结构，之后才能使用模型。

内容片段模型编辑器是您配置用于定义模型内容的数据类型和属性的地方。

>[!TIP]
>
>我们将会为该&#x200B;**冒险**&#x200B;添加一些重要字段。在后面的模块中，我们将使用并添加到模型中，因此请遵循所提供的命名模式。
...
您已经有一个用于冒险标题的字段。现在您需要添加字段来捕获旅行的描述、价格和代表性图像。

>[!TIP]
>
>该&#x200B;**冒险**&#x200B;模型基于 AEM 的 WKND 示例站点。如果您愿意，您可以通过[在这里访问该网站](https://wknd.site/us/en/adventures/yosemite-backpacking.html)来了解更多信息，但这些学习模块不需要了解这些信息。
