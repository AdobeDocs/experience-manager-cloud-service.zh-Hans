...
1. 此时将打开翻译规则编辑器，其中包含AEM自动为 `/content` 路径。

   ![翻译规则编辑器](assets/translation-rules-editor.png)

1. 要翻译的页面属性位于 **常规** 列表中的 您可以添加或更新要明确包含在翻译中的现有属性名称。
   1. 在 **新建资产** 字段。
   1. 选项 **翻译** 和 **继承** 复选框。
   1. 点按或单击 **添加**.
   1. 对必须翻译的所有字段重复这些步骤。
...
通常，开始本地化您的内容并不需要此类功能，但您可以在 [其他资源](#additional-resources) 部分。
...
在此知识的基础上，通过下一步审阅文档，继续您的AEM Sites翻译历程 [翻译内容](translate-content.md) 您将从中了解连接器和规则如何协同工作来翻译内容。

## 其他资源 {#additional-resources}

但建议您通过审阅文档来继续进入翻译历程的下一部分 [翻译内容，](translate-content.md) 以下是一些其他可选资源，可更深入地了解本文档中提到的某些概念，但无需继续访问这些概念。
