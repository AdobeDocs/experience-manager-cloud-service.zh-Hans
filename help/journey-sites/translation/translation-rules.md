...
在 AEM Site 翻译历程的上一个文档[配置翻译连接器](configure-connector.md)中，您已了解如何安装和配置翻译连接器，现在应：
...
AEM Site 页面可以包含很多信息。根据您的项目需求，可能不必翻译页面中的所有信息。
...
AEM 会自动为 Site 页面创建翻译规则，但由于每个项目的要求不同，因此请务必了解如何根据项目要求查看和调整规则。
...
1. 翻译的页面属性位于列表的&#x200B;**常规**&#x200B;部分下。您可以添加或更新要明确包含在翻译中的现有属性名称。
   1. 在&#x200B;**新属性**&#x200B;字段中，输入该属性名称。将自动选中&#x200B;**翻译**&#x200B;和&#x200B;**继承**&#x200B;选项。
   1. 选择&#x200B;**添加**。
   1. 对您必须翻译的所有字段重复这些步骤。
   1. 选择&#x200B;**保存**。
...
>[!TIP]
>
>也可以通过点击或单击翻译配置控制台中的&#x200B;**添加上下文**&#x200B;按钮，为特定的路径或项目创建规则。该内容超出了此历程的范围。
...
在此知识的基础上继续您的 AEM Site 翻译历程，接下来查看文档[翻译内容](translate-content.md)，其中您将了解您的连接器和规则如何共同发挥作用以翻译内容。
