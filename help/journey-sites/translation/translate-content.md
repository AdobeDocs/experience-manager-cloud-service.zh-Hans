...
在 AEM Site 翻译历程的上一个文档[配置翻译规则](translation-rules.md)中，您已了解如何使用 AEM 的翻译规则来标识翻译内容。现在应：

* 了解翻译规则的用途。
* 能够定义您自己的翻译规则。

现在您的连接器和翻译规则已设置完毕，本文将引导您完成下一步，即翻译 AEM Site 内容。
...
1. 导航到&#x200B;**导航** > **项目**。
1. 选择在上一部分（根据您的情况，[根据内容路径自动创建翻译项目](#automatically-creating)或[通过选择内容而手动创建翻译项目](#manually-creating)）中创建的项目。
...
1. 选择某个行项的选择复选框以查看更多选项，例如，从作业中删除它或在 Site 控制台中查看它的选项。
...
>[!NOTE]
>
>如果您未按照[上一部分所述](#using-translation-project)停用项目选项&#x200B;**提升后删除启动项**，则已翻译项目将以&#x200B;**已删除**&#x200B;状态出现。这是正常的，因为在获得已翻译项目后，AEM 会自动丢弃翻译记录。已将翻译项目作为语言副本导入，并且仅删除了翻译记录，因为不再需要这些记录。
>
...
只需转到已完成的翻译作业，并通过点击或单击复选框而选择行项。工具栏中将显示&#x200B;**在 Site 中预览**&#x200B;图标。

![在 Site 中展现](assets/reveal-in-sites.png)
...
此项目旨在将与翻译相关的所有资源集中到一个地方，以便轻松访问和大致了解。但是，正如您通过查看已翻译项目的详细信息时所看到的那样，翻译本身将流回翻译语言的 Site 文件夹中。在此示例中，该文件夹为

```text
/content/<your-project>/es
```

如果通过&#x200B;**导航** > **Site** 导航到此文件夹，则您将看到已翻译的内容。
...
在此知识的基础上继续您的 AEM Site 翻译历程，接下来查看文档[发布已翻译内容](publish-content.md)，其中您将了解如何发布您已翻译的内容以及如何随您的语言根内容变化而更新这些译文。
