...
| 属性 | 类型 | 必需 | 默认 | 描述 |
|--- |--- |--- |--- |--- |
| `ref` | FragmentSelectorRef | 否 | | 引用`ContentFragmentSelector`实例，允许访问提供的功能，如`reload`。 |
...
| `allowedRepositoryIds` | 字符串[] | 否 | | 在内容片段选择器中筛选存储库和内容片段的存储库ID列表。 如果随存储库ID一起提供，则只有这些存储库会显示在存储库选择器中。 如果未提供或数组为空，则用户有权访问的所有存储库都将可用。 |
...
| `getImsToken` | 检索当前登录用户的身份验证`imsToken`，该身份验证可用于验证向其他服务（如生成资产演绎版）发送的请求。 |
