...
您可以从页面或Blueprint配置创建Live Copy以在站点中重用该内容。 管理继承和同步，您可以控制对内容所做的更改的传播方式。
...
1. 选择所需的Blueprint配置 — 相应的操作将在工具栏中可用：
...
* 如果您从源页面创建Live Copy，则它会 **是** 在Blueprint配置中引用，则转出命令在源/Blueprint页面上可用。
...
1. （可选）要指定用于 Live Copy 的一个或多个转出配置，请使用&#x200B;**转出配置**&#x200B;下拉列表来选择它们。选定的配置将显示在下拉选择器的下方。
1. 单击或点按&#x200B;**创建**。将显示确认消息，您可以从此处选择以下任一选项 **打开** 或 **完成**.
...
>
>为了能够创建属于关系一部分的本地页面，最佳实践是在源中创建本地页面并执行深度转出。 这将在本地创建页面作为 Live Copy。
...
选中后，将显示该对话框。 如果需要，您可以选择同步，然后确认操作。
...
1. 打开 [Live Copy概述](live-copy-overview.md#using-the-live-copy-overview) 并选择暂停的Live Copy页面。 页面显示为 **继承已取消**.
1. 从工具栏中选择&#x200B;**恢复**。
...
* 在转出时，将从Blueprint恢复组件的顺序。 如果在转出之前已将新组件添加到Live Copy，则这些新组件会与其上所添加的组件一起重新排序。
