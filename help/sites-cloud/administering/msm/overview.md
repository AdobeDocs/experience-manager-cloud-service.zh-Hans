...
* 通过MSM，您可以：
   * 一次创建内容
   * 在同一站点或其他站点的其他区域（通过[Live Copy](#live-copies)）中重复使用此内容。
...
* **跨国公司 — 全球到本地公司**
   ...
   例如，为美国客户创建了[WKND教程示例](/help/implementing/developing/introduction/develop-wknd-tutorial.md)的英文部分。 此站点中的大多数内容还可用于其他WKND站点，这些站点面向不同国家和文化的英语客户。 所有站点的核心内容保持不变，而区域性调整则可以进行。

   以下结构可用于美国和加拿大的地点。 请注意`language-masters`节点如何维护主控副本，不仅包含英语内容，还包含其他语言内容。 此内容可用作附加区域语言内容以及英语的基础。
   ...
   >
   >
   >有关此示例，请参阅[多语言站点的翻译内容](/help/sites-cloud/administering/translation/overview.md)。
...
* **创建站点** (**站点**)
   ...
   * MSM可帮助您管理共享通用内容的多个网站。 例如，网站通常为国际受众提供，这样大多数内容在所有国家/地区都是通用的，其中一部分内容是特定于各个国家/地区的。 MSM允许您[创建Live Copy，以根据您的源站点](creating-live-copies.md#creating-a-live-copy-of-a-site-from-a-blueprint-configuration)自动更新一个或多个站点。 这还有助于您实施一个通用的基本结构，在多个站点上使用通用内容，保持一个通用的外观，并集中精力管理网站之间实际不同的内容。 以这种方式创建站点：
...
* **创建Live Copy** (**站点**)
   ...
   * MSM允许您[为网站的各个页面或子分支创建点对点（一次性）Live Copy。](creating-live-copies.md#creating-a-live-copy-of-a-page) 例如，复制子分支以提供有关产品的新版本/更新版本的信息。以此方式创建Live Copy:
      * 创建点对点Live Copy（不需要Blueprint配置）。
      * 可用于（立即）创建任何页面/分支的Live Copy。
      * 需要&#x200B;**Synchronize**（不提供&#x200B;**Rollout**&#x200B;按钮）。

* **视图属性** (**站点**)

   * 如果适用，此选项可通过提供有关相关&#x200B;**Live Copy**&#x200B;或&#x200B;**Blueprint**&#x200B;的信息，帮助您[监视Live Copy](creating-live-copies.md#monitoring-your-live-copy)。

* **引用** (**站点**)

   * [引用](/help/sites-cloud/authoring/getting-started/basic-handling.md#references)边栏提供有关&#x200B;**Live Copy**&#x200B;的信息以及对相应操作的访问权限。

* **Live Copy概述** (**站点**)

   * 此控制台允许您[视图和管理您的Blueprint及其Live Copy。](live-copy-overview.md)
...
| 章 | 要包含在Live Copy中的Blueprint部分 | 这些通常是根的子页 |
...
在上一个示例中，`/content/wknd/language-masters/en`是全局主控站点（英语）。 要重复使用此站点的内容，将创建MSM Live Copy:

* `/content/wknd/language-masters/en`以下的内容是源。
* `/content/wknd/language-masters/en`下的内容被复制到`/content/wknd/us/en/`和`/content/wknd/ca/en`节点下。 这些是Live Copy。
...
根据要求，可以[synchronized](creating-live-copies.md#synchronizing-your-live-copy)进行更改。

![Live Copy合成概述](../assets/live-copy-composition.png)

#### 包含非Live-Copy页面{#live-copy-with-non-live-copy-pages}的Live Copy
...
* MSM会作为特殊情况处理的情况。 例如，当您（或进程）在源/Blueprint和Live Copy分支中创建位置和名称相同的页面时。 有关此类情况，请参阅[MSM转出冲突](rollout-conflicts.md)以了解详细信息。

![包含非Live Copy页面的Live Copy](../assets/live-copy-with-non-live-copy-pages.png)

#### 嵌套Live Copy {#nested-live-copies}

当您（或进程）在现有Live Copy](#live-copy-with-non-live-copy-pages)中创建[新页面时，还可以将此新页面设置为不同Blueprint的Live Copy。 这称为嵌套Live Copy。 在嵌套的Live Copy中，第二个或内部Live Copy的行为会通过以下方式受到第一个或外部Live Copy的影响：
...
将Live Copy创建为浅层Live Copy的子项时，它称为堆叠Live Copy。 它的行为方式与[嵌套的Live Copy](#nested-live-copies)相同。

### 来源、Blueprint和Blueprint配置{#source-blueprints-and-blueprint-configurations}
...
* [创建Blueprint配置](creating-live-copies.md#creating-a-blueprint-configuration)  — 配置会预先定义要用于创建Live Copy的页面。
* [创建页面的Live Copy](creating-live-copies.md#creating-a-live-copy-of-a-page)  — 用于创建Live Copy（源页面）的页面是Blueprint页面。源页面可能被Blueprint配置引用，也可能不被Blueprint配置引用。

### 转出和同步{#rollout-and-synchronize}

转出是与Live Copy与其源同步的中央MSM操作。 您可以手动执行转出，也可以自动执行。

* 可以定义[转出配置](#rollout-configurations)，以便特定的[事件](live-copy-sync-config.md#rollout-triggers)可导致转出自动发生。
* 创作Blueprint页面时，您可以使用&#x200B;**[Rollout](creating-live-copies.md#rolling-out-a-blueprint)**&#x200B;命令将更改推送到Live Copy。
...
转出配置可以重用，因此多个Live Copy可以使用相同的转出配置。 标准安装中包括几个[转出配置](live-copy-sync-config.md#installed-rollout-configurations)。

### 转出冲突{#rollout-conflicts}

转出可能会变得复杂，尤其是当作者正在源和Live Copy中编辑内容时。 因此，了解AEM如何处理转出过程中可能发生的任何[冲突非常有用。](rollout-conflicts.md)
...
您还可以从Live Copy](creating-live-copies.md#detaching-a-live-copy)的蓝图中分离[以删除所有连接。
...
>[!TIP]
>
>有关完整详细信息（包括对子页面和父页面的相关影响），请参阅[分离Live Copy](creating-live-copies.md#detaching-a-live-copy)。
...
1. 确定要使用的转出配置。
   ...
   1. MSM [安装了几个可满足许多用例的转出配置](live-copy-sync-config.md#installed-rollout-configurations)。
   1. （可选）如果需要，您可以[创建转出配置](live-copy-sync-config.md#creating-a-rollout-configuration)。

1. 确定需要[指定转出配置以使用](live-copy-sync-config.md#specifying-the-rollout-configurations-to-use)并根据需要进行配置的位置。
1. 如果需要，[创建标识Live Copy源内容的Blueprint配置](creating-live-copies.md#creating-a-blueprint-configuration)。
1. [创建Live Copy。](creating-live-copies.md#creating-a-live-copy)
1. 根据需要更改源内容。 您应采用您的组织已建立的常规内容审阅和批准流程。
1. [滚](creating-live-copies.md#rolling-out-a-blueprint) 出蓝图，或 [将实时文](creating-live-copies.md#synchronizing-a-live-copy) 本与更改同步。

## 自定义MSM {#customizing-msm}

MSM提供多种工具，使您的实施能够适应共享内容时可能存在的特殊复杂性。

* **自定义转出配置**  — 当 [安装的转](live-copy-sync-config.md#creating-a-rollout-configuration) 出配置不满足您的要求时，创建转出配置。您可以使用任何可用的转出触发器和同步操作。
...
“[MSM最佳实践](best-practices.md)”页包含有关您的实施的重要信息。
