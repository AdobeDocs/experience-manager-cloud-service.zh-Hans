...
| 提升发布内容 | 用于提升发布页面的标准转出配置。 | 转出 | `contentUpdate`<br>`contentCopy`<br>`contentDelete`<br>`referencesUpdate`<br>`orderChildren`<br>`markLiveRelationship` |
...
| `notify` | 此操作发送已转出的页面的页面事件。要接收通知，必须首先订阅转出事件。 |  |
