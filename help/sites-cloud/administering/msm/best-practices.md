...
* 虽然 MSM 本身&#x200B;**不提供内容翻译**，但它可以与第三方翻译连接器集成。请注意以下事项：
   * MSM 让您在页面和/或组件级别取消继承。这有助于防止在下一次转出时覆盖已翻译的内容（来自 Live Copy，以及来自 Blueprint 的尚未翻译的内容）。
...
除了自定义转出操作之外，MSM 还让您自定义正在转出的节点属性。[MSM OSGi 配置让您排除](live-copy-sync-config.md#excluding-properties-and-node-types-from-synchronization)从源复制到 Live Copy 的节点类型。
