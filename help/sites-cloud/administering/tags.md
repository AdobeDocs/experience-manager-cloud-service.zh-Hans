...
1. 要创建新的命名空间，请打开[标记控制台](#tagging-console)，然后点按或单击工具栏中的&#x200B;**创建**&#x200B;按钮，然后点按或单击&#x200B;**创建命名空间**。
...
* 如果您创建一个标记而不选择命名空间或其他标记，则将有效创建一个新的命名空间。

### 发布标记 {#publishing-tags}

正如在 AEM 中创建任何其他内容一样，创建标记（或命名空间）后，它仅存在于创作环境中。要使您的标记可供用户使用，您必须发布标记。
