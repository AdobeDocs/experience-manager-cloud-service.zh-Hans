...
选择特定片段将打开一个工具栏，其中重点介绍可用于该片段的操作。 您还可以选择多个片段，操作的选择会相应地进行调整。
...
* **模型**
   * 仅供参考.
   * 可用于 [快速筛选](#fast-filtering#fast-filtering)
* **文件夹**
...
* 谓词选择；
   ...
   * 可以选择并组合一个或多个谓词以创建过滤器
* **保存**&#x200B;您的配置的机会
* 用于检索保存的搜索过滤器以供重复使用的选项

一旦选定， **过滤方式** 将显示选项（在“搜索”框下）。 可以从该处取消选择它们。 例如：
...
>[!NOTE]
>
>仅支持快速筛选 **模型**， **状态**， **修改者**， **标记**、和 **发布者** 列。

![内容片段控制台 – 筛选](assets/cfc-console-fast-filter-01.png)

选择后，它会显示为一个筛选谓词，并会相应地筛选列表：
