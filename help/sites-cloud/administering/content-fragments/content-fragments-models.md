---
title: 内容片段模型
description: 了解内容片段模型如何作为AEM中内容片段的基础。 利用这些功能，可创建结构化内容，以用于无标题交付或页面创作。
...
AEM中的内容片段模型定义内容的结构 [内容片段](/help/sites-cloud/administering/content-fragments/content-fragments.md). 这些组件可用于页面创作，或用作无头内容的基础。

要使用内容片段模型，您可以：

1. [为实例启用内容片段模型功能](/help/sites-cloud/administering/content-fragments/content-fragments-configuration-browser.md)
...
1. 导航到与您的 [配置或子配置](/help/sites-cloud/administering/content-fragments/content-fragments-configuration-browser.md).
1. 使用 **创建** 打开向导。
...
* **[片段引用](#fragment-reference-nested-fragments)** （嵌套片段）
   ...
   * 允许您包含/检索结构化数据。
      ...
      >[!NOTE]
      >
      >此方法特别有意与 [通过GraphQL使用内容片段交付无头内容](/help/sites-cloud/administering/content-fragments/content-fragments-graphql.md).
   * 可以为一个或多个引用（在生成的片段中）配置。
...
>[!NOTE]
>
>这与 [通过GraphQL使用内容片段交付无头内容](/help/sites-cloud/administering/content-fragments/content-fragments-graphql.md).
