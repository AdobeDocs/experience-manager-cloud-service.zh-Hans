...
1. 选择 **更新语言副本** 要显示翻译工具并配置属性，请执行以下操作：
...
* 连接器必须实施方法 [`storeTranslation`](https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/com/adobe/granite/translation/api/TranslationService.html).
...
1. 在项目控制台中，打开您的翻译项目。
1. 在翻译作业的标题上，选择命令菜单，然后选择 **请求范围**.
1. 当作业状态更改为 **范围已完成**，选择命令菜单，然后选择 **显示范围**.
...
如果已经翻译的源页面必须 [重命名或移动](/help/sites-cloud/authoring/fundamentals/organizing-pages.md#moving-or-renaming-a-page)，在移动后再次翻译页面可根据新的页面名称/位置创建语言副本。 基于上一个名称/位置的旧语言副本仍将存在。
