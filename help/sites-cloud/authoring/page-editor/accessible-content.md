...
或者，可以使用脚本，以便在链接本身中提供最小量的文本，但是一旦激活了位于页面顶部的适当控件，链接文本就会 *已展开* 详细介绍。 类似的方法是使用CSS执行以下操作 *隐藏* 视力正常的用户提供的完整链接，但仍会将其输出到全屏阅读器用户。 这不在本文档的范围之内，但有关如何实现此目标的更多信息，请参见 [更多信息 — 链接目的（在上下文中）(2.4.4)](#more-information-link-purpose-in-context) 部分。
