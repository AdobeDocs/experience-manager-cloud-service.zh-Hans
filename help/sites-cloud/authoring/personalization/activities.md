...
在“定位”模式下，可以使用活动来[创作目标内容](/help/sites-cloud/authoring/personalization/targeted-content.md)，您也可以在该模式下创建活动。您在「鎖定目標」模式中建立的活動會顯示在「活動」主控台中。
...
1. 如果您使用Adobe Target作為目標定位引擎，請選取您要透過此活動測量的內容。 请参阅[配置活动和设置目标](/help/sites-cloud/authoring/personalization/targeted-content.md)，以了解有关可用成功量度的更多信息。您必須至少選取一個目標。
1. 按一下或點選 **儲存**.
