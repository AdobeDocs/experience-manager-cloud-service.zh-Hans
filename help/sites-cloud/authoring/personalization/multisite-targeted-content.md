...
>[!NOTE]
>
>使用目標內容的多網站支援是一項進階功能。 要使用此功能，您应该熟悉[多站点管理器](/help/sites-cloud/administering/msm/overview.md)以及 [Adobe Target 与 AEM 的集成](/help/sites-cloud/integrating/integrating-adobe-target.md)。
...
網站或即時副本會連結至包含需要在該網站或即時副本上可用的活動的區域。 默认情况下，站点或 Live Copy 会链接到主区域，但您也可以链接主区域之外的其他区域。
...
您可以根據您的使用案例，以多種方式設定目標內容的多網站支援。 本節說明這在理論上如何搭配一個品牌運作。 此外，在[示例：根据地域定位内容](#example-targeting-content-based-on-geography)中，您可以了解在多个站点中定位内容的实际应用。
...
重新整理索引標籤後，加拿大網站會變更為新影像，我們不需要採取任何動作。 它之所以這麼做，是因為與美國共用主區域。 而英国和澳大利亚站点中的图像不会发生改变。
...
您可以將區域連結至頁面或網站。 所有子页面都会继承区域，除非子页面上的映射覆盖了这些页面。不过，通常而言，您需要在站点级别建立链接。
