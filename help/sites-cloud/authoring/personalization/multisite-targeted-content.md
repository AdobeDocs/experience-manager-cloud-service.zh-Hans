...
1. [创建新区域](#creating-new-areas) 或 [创建区域作为live copy](#creating-new-areas). 区域包含可供页面的某个“区域”**&#x200B;使用的所有活动；也就是，页面上该组件所定位的位置。如果创建新区域，则会创建一个空的区域；然而，如果创建新区域作为 Live Copy，则您可以跨站点结构继承目标内容。
...
>[!NOTE]
>
>默认情况下，当您点按或单击品牌名称时，名为主区域的默认区域会折叠 **直到** 创建另一个区域。 然后，在“活动”或“优惠”控制台中选 **择品牌****时，您会看到“区** 域 **** ”控制台。
