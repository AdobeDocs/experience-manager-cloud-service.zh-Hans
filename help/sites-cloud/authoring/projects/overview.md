...
这些图标指示表示的体验类型：网站、移动应用程序或出版物。 通过点按或单击向下V形标记并点按来添加体验 **添加体验** 和选择体验类型。
...
单击位于 **翻译作业** 卡片，用于在翻译工作流中查看资产。 翻译作业列表还会显示资产元数据和标记条目。这些条目指示资产的元数据和标记也会被翻译。
...
“任务”拼贴允许您监测任何项目相关任务（包括工作流）的状态。有关任务的详细信息，请参阅 [使用任务](/help/sites-cloud/authoring/projects/tasks.md).
...
* A [翻译项目](/help/sites-cloud/administering/translation/overview.md)  — 用于管理翻译相关活动的参考示例。 它包括三个基本角色（所有者、编辑者和观察者）。此外，它还包括两个可在工作流用户界面中访问的工作流。
...
| 所有者 | 具有此角色的用户可以启动项目。所有者可以创建项目、启动项目中的工作，以及将已批准的资产移动到“生产”文件夹。 所有者还可以查看和执行项目中的所有其他任务。 | 写入权限 `/etc/commerce` | `dam-users` 组（能够创建项目）项目管理员组（能够创建项目和移动资产） |
