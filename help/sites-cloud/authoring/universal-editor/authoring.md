...
点按[属性面板](#properties-rail)中的条目以打开模式编辑器，该模式编辑器提供较大的画布以编辑富文本。
...
通用编辑器的富文本编辑器(RTE)允许作者应用标准文本格式。 以下选项可供使用。
...
* **删除线**
* **文本颜色**
   * 打开调色板，您可以在其中选择颜色或指定十六进制值
   * 仅在模式编辑器中可用，而不在上下文中可用
* **上标**
* **下标**
* **项目符号列表**
...
根据您的后端，默认可用的选项可能有所不同。 RTE可以配置为根据作者的需要隐藏选项或显示其他选项。 有关详细信息，请参阅文档[为通用编辑器配置RTE](/help/implementing/universal-editor/configure-rte.md)。
...
1. 此时将打开组件选取器对话框。
   ...
   * 单击右列的组件名称以将其插入到容器中。
   * 如果容器中只允许使用一个组件，则会自动插入该组件。
   * 单击选取器外部可取消组件插入。

   ![组件选取器](assets/component-picker.png)
