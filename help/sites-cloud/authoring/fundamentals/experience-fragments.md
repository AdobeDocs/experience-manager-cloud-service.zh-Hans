...
体验片段无需复制/粘贴。
* 支持 headless CMS 用例。
作者希望仅将 AEM 用于创作，而不是用于提供给客户。第三方系统/接触点将使用该体验，然后交付给最终用户。
...
1. 此 **构建基块** 显示在左侧选项卡中(**本地**)，并可以选择以执行进一步操作：
...
1. 选择一个文件夹或您的片段，然后从工具栏中选择&#x200B;**属性**。
   ...
   >[!NOTE]
   >
   >文件夹上定义的个性化属性由子树中的所有子文件夹以及该子树中的体验片段（和变体）继承。 可以通过破坏继承来覆盖这些属性。
...
1. 打开&#x200B;**个性化**&#x200B;选项卡来定义和保存您的设置。 例如，在文件夹上：
   ...
   >[!CAUTION]
   >
   >将片段嵌入到站点页面中时，以及 **个性化** ，则在页面渲染时仅使用页面的个性化版本。
   >
...
>[!NOTE]
>
虽然这可以直接从浏览器获得， [主要目的是允许其他应用程序（例如，第三方Web应用程序、自定义移动实施）仅使用URL直接访问体验片段的内容](/help/implementing/developing/extending/experience-fragments.md#the-plain-html-rendition).
