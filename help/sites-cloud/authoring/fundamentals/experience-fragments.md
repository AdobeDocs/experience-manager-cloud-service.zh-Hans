...
## 对体验片段进行个性化 {#personalization-experience-fragment}

对体验片段进行个性化允许您作为营销人员，为体验片段定义一次目标受众，然后在任何页面中重复使用该片段。 此功能：

* 每次使用片段时，无需为每个受众指定所需的变量
* 维护选件的样式

您可以创建一个体验片段，该体验片段具有分组到此单个片段中的多个组件。 您还可以为每个特定受众区段创建片段的变体，然后在所需的渠道中重复使用这些体验片段。

通过定义 **个性化** 体验片段或变量，或包含片段的文件夹中的属性；这意味着继承可以覆盖个性化属性。

配置这些属性还会启用 **定位** 模式。

### 为体验片段定义个性化 {#defining-personalization-experience-fragment}
...
1. 选择文件夹或片段，然后 **属性** 中。
   ...
   >[!NOTE]
   >
   >对文件夹定义的个性化属性将由所有子文件夹通过子树向下继承，并由该子树中的体验片段（和变量）继承。 可以通过中断继承来覆盖它们。

1. 打开 **个性化** 选项卡来定义和保存设置。 例如，在文件夹上：

   ![体验片段 — 个性化属性](/help/sites-cloud/authoring/assets/xf-personalization-properties.png)

   >[!CAUTION]
   >
   >当片段嵌入到站点页面中，以及 **个性化** ，则在页面渲染时将只使用页面的个性化版本。
   >
   >对于对片段中的组件执行的定位，要在页面渲染时起作用，必须满足以下条件：
   >
   >的 **ContextHub路径** 在 **个性化** 选项卡：
   >
   >* 与为片段将呈现的页面配置的路径相同
      >或者:
   >* 路径包含在为页面配置的ContextHub中定义的存储子集
...
### 为体验片段定义定位 {#defining-targeting-experience-fragment}

配置个性化属性后，在打开片段进行编辑时，“定位”模式将可用。

![体验片段编辑器 — 定位模式](/help/sites-cloud/authoring/assets/xf-targeting-mode.png)

此模式的操作方式与页面编辑相同。 请参阅 [页面编辑器的定位模式](/help/sites-cloud/authoring/personalization/targeted-content.md) 以了解更多详细信息。
