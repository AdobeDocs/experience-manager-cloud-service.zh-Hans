...
创建新页面时，AEM将 [根据惯例验证页面名称](/help/implementing/developing/introduction/naming-conventions.md) 由AEM和JCR强加。
...
1. 粘贴对话框提供了粘贴事务的摘要以及执行以下操作的功能：
   * **新网站名称：** 更改粘贴页面的名称
   * **粘贴时不含子项：** 粘贴时忽略选定页面的子页面（默认情况下，粘贴子页面）

   ![“粘贴”对话框](/help/sites-cloud/authoring/assets/paste-dialog.png)

1. 点按或单击 **粘贴** 按钮以确认粘贴交易并创建新页面。
...
>[!NOTE]
>
>如果在选择模式下开始粘贴操作，则复制页面后将自动退出该操作。

### 移动或重命名页面 {#moving-or-renaming-a-page}

移动或重命名页面的过程基本相同，两个操作都由“移动页面”向导处理。 通过此向导，您可以：
