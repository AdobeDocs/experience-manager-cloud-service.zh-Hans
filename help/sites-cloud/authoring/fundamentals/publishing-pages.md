...
在创作环境中创建和查看内容后，目标是 [使其在您的公共网站上可用](/help/sites-cloud/authoring/getting-started/concepts.md) （您的发布环境）。
...
>[!NOTE]
>
>从编辑器中发布是一种简单的发布，也就是说，只发布选定的页面，不发布任何子页面。
...
>[!NOTE]
>
>快速发布是一种浅层发布，即仅发布选定的页面/页面，不发布任何子页面。

#### 管理发布 {#manage-publication}

**管理发布** 提供的选项数大于 **快速发布**，允许包含子页面、自定义引用和启动任何适用的工作流，并提供在以后的日期发布的选项。
...
1. 如果发布的页面与工作流相关联，则这些工作流可能会显示在发布向导的最后一个步骤&#x200B;**工作流**&#x200B;中。
   ...
   >[!NOTE]
   >
   >此 **工作流** 显示的步骤基于用户可能拥有也可能没有的权限。 请参阅本页前面关于发布权限和管理对工作流的访问权限的注释 [将工作流应用于页面](/help/sites-cloud/authoring/workflows/applying.md) 了解详细信息。
...
在编辑页面时，如果要取消发布该页面，请选择 **取消发布页面** 在 **页面信息** 菜单，你也可以这么说 [发布页面](#publishing-from-the-editor).
...
您可以使用 [管理发布](#manage-publication) 选项来执行此操作。
