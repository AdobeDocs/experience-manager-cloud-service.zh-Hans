...
此 [Web内容无障碍准则(WCAG) 2.1](https://www.w3.org/TR/WCAG/) 起草人 [万维网联盟工作组](https://www.w3.org/组/#Accessibility_Guidelines_Working_Group). 它包含一系列独立于技术的准则和成功标准，可帮助残疾人访问并使用Web内容。
...
网页上的信息可以以许多不同的非文本格式提供，例如图片、视频、动画、图表和图形。失明或有严重视力障碍的人无法看到非文本内容。 但是，用户可以通过屏幕阅读器阅读文本内容或通过盲文显示设备以触觉形式呈现文本内容。 因此，通过以图形格式提供内容的替代文本，看不到该内容的人可以访问内容提供的信息的等效版本。

另外的一个实用好处是，替换文本使非文本内容能够通过搜索引擎技术建立索引。

#### 如何达到标准 - 非文本内容 (1.1.1) {#how-to-meet-non-text-content}

对于静态图形，基本的要求是为图形提供对等的替换文本。此方法可以在以下位置完成： **替换文本** 字段。 例如，请参阅核心组件 **[图像](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/wcm-components/image.html)**.

>[!NOTE]
>
>一些开箱即用的核心组件 —  **[轮播](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/wcm-components/carousel.html)**  — 不提供 **替换文本** 用于向单个图像添加替换文本说明的字段，尽管有 **标签** 字段(**[辅助功能](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/wcm-components/carousel.html#accessibility-tab)** 选项卡)。
在为AEM实例实施这些版本时，开发团队必须配置这些组件以支持 `alt` 属性。 这样做可确保作者可以将其添加到内容中(请参阅 [添加对其他HTML元素和属性的支持](/help/implementing/developing/extending/rte-accessible-content.md#adding-support-for-additional-html-elements-and-attributes))。

默认情况下，AEM 要求填写&#x200B;**替换文本**&#x200B;字段。如果图像是纯粹的装饰并且不需要替换文本，则可以选中&#x200B;**图像具有装饰性**&#x200B;选项。

#### 创建有效的替换文本 {#creating-good-text-alternatives}

非文本内容的形式多种多样，因此替换文本的值取决于图形在网页中所起的作用。您可能会觉得有帮助的一些常规规则包括：

* 替换文本应当既简洁明了，又能清晰地表达非文本内容所提供的基本信息。
* 应当避免使用冗长的描述（超过 100 个字符）。如果替换文本需要表达更多的详细信息，则可以通过以下方法来实现：
   * 在替换文本中提供简短的描述
   * 同时，在同一页面的其他位置或在一个单独的网页中提供更加详尽的描述文本。通过将图像设为链接或在图像旁边放置文本链接来链接到此单独描述。
* 替换文本不应复制同一页面邻近位置以文本形式提供的内容。请记住，许多图像都是页面文本中已覆盖点的插图，因此可能已经存在详细的替换文本。
* 如果非文本内容是指向其他页面或文档的链接，并且同一链接中没有其他文本生成部分，则图像的替换文本必须指示链接的目标。 它不能描述图像。
* 如果非文本内容包含在按钮元素中，并且同一按钮中没有文本形成部分，则图像的替换文本必须指示按钮的功能。 它不能描述图像。
* 为图像指定空(null)替换文本是完全可以接受的，但前提是图像不需要替换文本。 例如，它是纯粹的装饰性图形，或者页面文本中存在对等文本。
...
* 说明性照片：指人物、对象或地点的图像。 重要的是要考虑照片在页面中的作用，并推荐对图像内容的描述，因为辅助型技术会声明元素类型(例如， `graphic` 或 `image`)。 它可以提高使用的清晰度 `screenshot` 或 `illustration` 替换文本描述中的替换文字，但这取决于上下文。 一致性是一个重要因素，应该为整个创作团队做出决定，并且这一点适用于整个用户体验。
...
* 纯音频和纯视频（预先录制）：对于预先录制的纯音频媒体和预先录制的纯视频媒体，均存在以下情况，除非音频或视频就是文本的替代媒体 ，且明确进行了如下标记：
   * 纯音频预先录制：基于时间的媒体的替代方法，为纯音频预先录制的内容提供等效信息。
   * 仅预录视频：基于时间的媒体的替代方案，或者为仅预录视频内容提供等效信息的音频轨道。
...
>[!NOTE]
如果音频或视频内容是作为同一网页上以其他格式存在的内容的替代内容提供的，则可能不需要额外的替代内容。
准则《[了解 WCAG 1.2.1](https://www.w3.org/WAI/WCAG21/Understanding/audio-only-and-video-only-prerecorded.html)》提供了进一步的信息。

在 AEM 网页中插入多媒体的方式与插入图像类似。但是，由于多媒体内容远不止是静态图像，因此对于控制如何播放多媒体，有各种不同的设置和选项。
...
耳聋或听力缺佳的用户无法或很难访问音频内容。 字幕是讲话和非讲话音频的对等文本，在视频播放过程中会在相应的时间显示在屏幕上。这让无法听到音频的用户可以了解正在播放的内容。
...
尽量使用隐藏式字幕，因为这样用户可以选择是否查看字幕。

对于隐藏式字幕，必须创建并提供相应格式的同步字幕文件(例如 [SMIL](https://www.w3.org/AudioVideo/))以及视频文件(有关如何执行此操作的详细信息，不在本指南的涵盖范围内，但提供了指向以下部分教程的链接： [更多信息 — 字幕（预先录制）(1.2.2)](#more-information-captions-prerecorded). 确保提供注释或在视频播放器中启用字幕功能，以便让用户知道视频有字幕可看。
...
如果视频或动画中的信息仅以可视方式提供，或者音轨提供的信息不足以让用户了解视频或动画中正在播放的内容，则失明或患有视觉障碍的用户将会遇到无障碍问题。
...
1. 为视频内容提供附加的音频描述。可以采用以下三种方式中的任何一种来实现：
   ...
   * 提供附加的可选新音轨，其中包含原始音轨，但还包括有关场景变化的额外音频信息。
...
1. 为视频或动画中的音频和视觉元素提供适当的对等文本记录。这应当包括，在适当情况下，指出讲话者，描述讲话背景、任何事件或在视觉上呈现的信息，以及语音表达。 根据其长度，您可以将转录放在与视频或动画相同的页面上，或者放在单独的页面上；如果选择后一个选项，则在视频或动画旁提供指向转录的链接。
...
遵循为以下各项提供的指南： [字幕（预先录制）](#captions-prerecorded) 上面。但是，由于媒体的实时性质，必须尽快提供字幕，并对正在发生的情况做出回应。 因此，您应该考虑使用实时字幕工具或语音转文本工具。
...
AEM 允许轻松地使用相应的 HTML 元素构建语义上有意义的 Web 内容。在RTE（文本组件）中打开页面内容，并使用 **参数格式** 菜单（段落符号）以指定相应的结构元素（例如段落、标题等）。

在适用的情况下，您可以使用以下元素来确保网页具有适当的结构：

* **标题：** 只要您启用了RTE辅助功能，AEM就会提供三个级别的页面标题。 您可以使用这些标题标识内容的章节和子章节。标题 1 是最高级别的标题，标题 3 是最低级别的标题。系统管理员可以配置系统以允许使用更多标题级别。
...
* **表**：数据表必须使用 HTML 表元素进行标识：
   ...
   * `<caption>` 元素用于为表提供可视描述。默认情况下，字幕显示在表格上方，但可以使用CSS进行适当定位。 描述采用编程方式与表相关联，因此这是一种提供内容简介的有用方法。
   * `<summary>` 元素通过总结视力正常的用户可以看到的内容，帮助失明的用户更加轻松地了解表中提供的信息。在使用复杂或非常规表格布局时，此工作流非常有用（此属性不会显示在浏览器中，只会由辅助型技术读取）。
...
* **复杂数据表**：有时，如果存在具有两级或更多级标题的复杂表，则基本的表属性可能不足以提供所有必要的结构信息。 对于此类复杂表，需要使用&#x200B;**标题**&#x200B;和 **id** 属性在标题和与之相关的单元格之间建立关系。
...
此成功标准旨在使用户代理既能够提供内容的替代呈现方式，同时还能保留理解内容含义所需的阅读顺序。能够以编程方式确定至少一个有意义的内容序列，这一点很重要。 当辅助技术以错误的顺序阅读内容，或者在应用替代样式表或其他格式更改时，不符合此成功标准的内容可能会使用户感到困惑或是不知所措。
...
设计者往往关注可视设计特征，如颜色、形状、文本样式，或者内容在展示信息时所在的绝对或相对位置。在传达信息方面，这些可能是强大的设计技术（并且可以提高具有认知无障碍需求的视力正常用户的整体无障碍性），但是失明或患有视觉障碍的用户可能无法访问需要视觉识别属性（例如位置、颜色或形状）的信息。
...
* 切勿依赖可视位置来提供信息。例如，如果您想让用户参考页面右侧的菜单，以便访问更多信息，请不要参考 *右侧的菜单*；相反，命名菜单（例如通过标题），并在文本中引用该名称。
* 切勿将文本样式（例如，粗体或斜体文本）作为传递信息的唯一方式。

>[!NOTE]
如果用户在非可视上下文中理解描述性术语的含义，则可以使用描述性术语。 例如，使用&#x200B;*上面*&#x200B;和&#x200B;*下面*&#x200B;通常是可以接受的，因为它们分别表示特定内容项之前和之后的内容；在朗读内容时，这也是可以接受的。
...
颜色是增强网页审美吸引力的有效途径，也是传递信息的有效途径。 但是，视障的表现形式也各不相同，从失明到色觉缺失，这就意味着有些人无法辨别出某种颜色。 这就使颜色编码无法可靠地提供信息。

例如，患有红绿色觉缺失的人无法区分绿色阴影和红色阴影。 他们可能会将这两种颜色视为第三种颜色（例如，棕色），在这种情况下，他们无法区分红色、绿色和棕色。

此外，使用纯文本浏览器、单色显示设备或查看页面黑白打印输出的人无法感知颜色。

进一步考虑之事项为 *已选择* 界面元素（例如，选项卡、切换按钮等）的状态，这些状态需要以颜色和视觉呈现方式以外的其他某种方式传递。 对于这些元素，在创建不依赖特定感官的全包容用户体验时，额外使用图案、形状和编程信息将很有帮助。
...
如果颜色用作提供信息的提示，则应提供额外的视觉提示，如更改样式（例如，粗体、斜体）或字体。 这样可以帮助弱视或色盲的用户识别信息。但是，不能完全依赖它，因为它对完全看不到页面的用户没有帮助。 因此，提供隐藏文本或使用编程解决方案(例如 [可访问的富互联网应用程序(ARIA)一套Web标准](https://www.w3.org/WAI/standards-guidelines/aria/)，将此信息传递给失明用户。
...
如果同时还有其他音频正在播放，则使用屏幕阅读软件的用户可能会难以听清语音输出。如果屏幕阅读器的语音输出是基于软件的（与现今大多数一样），并且是通过与声音相同的音量控制来控制的，则这种困难会加剧。 此外，一些患有认知障碍和神经系统障碍的人可能对声音敏感。 这些用户发现无法更改音频内容的音量会造成中断。
...
* 对比度（最小）：文本的可视呈现以及文本的图像至少要有 4.5:1 的对比度，以下内容除外：
   ...
   * 商标标志：文本是徽标或品牌名称的一部分，对于此类文本，没有最低对比度要求。
   ...
   >[!NOTE]
   参见 [了解非文本对比度](https://www.w3.org/WAI/WCAG21/Understanding/non-text-contrast.html) 有关更多信息，有助于确保内容作者了解有关非文本元素（包括图标、界面元素等）的其他要求。
...
如果无法满足建议的对比度级别，则必须提供指向页面的替代对等版本的链接（没有颜色对比度问题），或允许用户根据自己的要求调整页面颜色方案的对比度。
...
如果必须使用文本的图像，应使用 CSS 将文本的图像替换为 HTML 形式的对等文本，这样就可以对文本进行自定义。有关如何实现此目标的示例，请参见 [C30：使用CSS将文本替换为文本的图像，并提供用户界面控件以进行切换](https://www.w3.org/TR/2008/NOTE-WCAG20-TECHS-20081211/C30).
...
此成功标准旨在确保在可能的情况下，通过键盘或键盘接口（以便使用替代键盘）来操作内容。当内容可以通过键盘或替代键盘操作时，失明人士（无法使用鼠标等需要手眼协调的设备）和必须使用替代键盘或充当键盘模拟器的输入设备的人士可以操作。 键盘模拟器包括语音输入软件、呼吸软件、屏幕键盘、扫描软件以及各种辅助技术和备用键盘。 视力缺佳的人也可能难以跟踪指针，如果可以通过键盘控制指针，他们会发现使用软件要容易得多（或者只有可能）。
...
有时，网页的功能会将焦点限制在内容的子部分（例如，模态对话框）。在这种情况下，您应该为用户提供一种方法，使其能够退出该内容的子部分（例如，按ESC键关闭模态对话框，或按“关闭”按钮关闭模态对话框）。
...
此成功标准旨在确保残障用户有尽可能充足的时间与 Web 内容交互。失明、视力欠佳、行动不便以及认知困难等残障人士，可能需要更多时间来阅读内容或执行在线填表等操作。如果Web功能是计时的，则某些用户很难在达到时间限制之前执行所需的操作。 这可能会导致他们无法访问相关服务。设计不依赖于时间的功能可帮助残障人士成功完成这些功能。 提供禁用时间限制、自定义时间限制长度或在达到时间限制之前请求更多时间的选项，可帮助那些需要比预期更多时间的用户成功完成任务。 这些选项按对用户最有帮助的顺序列出。 禁用时间限制是最优选项，其次是自定义时间限制长度，再次是在达到时间限制之前请求更多时间。
...
* 暂停、停止、隐藏：对于移动、闪烁、滚动或自动更新的信息，符合以下情况：
   * 移动、闪烁、滚动：对于(a)自动开始、(b)持续超过五秒并且(c)与其他内容同时出现的任何移动、闪烁或滚动信息，用户有一种机制可以暂停、停止或隐藏该信息，除非移动、闪烁或滚动是活动的一部分，而该活动至关重要；
   * 自动更新：对于(a)自动启动且(b)与其他内容同时显示的任何自动更新信息，用户有一种机制可以暂停、停止或隐藏该信息，或者控制更新频率，除非自动更新属于活动的一部分，而该活动至关重要。
...
某些用户可能会认为移动的内容会让人分心，甚至让人觉得难受，而且难以将注意力集中在页面的其他部分。此外，对于无法跟上移动文本节奏的人来说，此类内容可能很难阅读。

#### 如何达到标准 - 暂停、停止、隐藏 (2.2.2) {#how-to-meet-pause-stop-hide}

根据内容的性质，您可以在创建包含移动、闪烁或闪烁内容的网页时应用以下一个或多个建议：
...
* 成功标准 2.3.1
* A 级
* 三次Flash或低于阈值：网页不包含在任何一秒的时段内闪光三次以上的任何内容，或者闪光低于常规闪光和红色闪光阈值。

>[!NOTE]
由于任何不符合此成功标准的内容都会干扰用户使用整个页面的能力，因此网页上的所有内容（无论是否用于满足其他成功标准）都必须满足此成功标准。 请参阅[符合性要求 5：不干涉](https://www.w3.org/TR/WCAG/#cc5)。
...
* 如果上述条件无法满足，则在 *小安全区* 以像素为单位。 此区域使用复杂的公式计算，详见 [G176：使闪烁区域保持足够小](https://www.w3.org/TR/2008/NOTE-WCAG20-TECHS-20081211/G176)因此，仅当需要刷新内容时才应使用此技术。
...
此成功标准旨在让按顺序浏览内容的人更直接地访问网页的主要内容。网页和应用程序通常包含也会在其他页面或屏幕上显示的内容。重复出现的内容块示例包括但不限于：导航链接、标题图形、菜单和广告画面。就本规定而言，单个单词、短语或单个链接等较小的重复部分不被视为块。
...
此成功标准旨在帮助每个人（无论是否患有任何特定障碍缺陷）无需阅读整个页面即可快速识别网页内容。当多个网页在浏览器选项卡中打开时，这非常有用，因为页面标题显示在选项卡中，因此可以快速找到。

#### 如何达到标准 - 页面带有标题 (2.4.2) {#how-to-meet-page-titled}

在 AEM 中创建新 HTML 页面时，可以指定页面标题。确保标题能够充分描述页面的内容和用途，尤其是任何独特方面，以便访客能够快速识别内容是否与其需求相关。
...
此成功标准旨在确保用户按顺序导航内容时，他们接收到信息的顺序与内容含义一致，并且这种按顺序导航的操作可通过键盘实现。这可以通过让用户构建内容的统一思维模型来减少混淆。 内容中可能存在不同的反映逻辑关系的顺序。例如，在由多个字段和/或步骤组成的在线表单中移动组件反映了内容中的逻辑关系。
...
对于所有用户（无论是否患有某方面的缺陷）而言，通过适当的链接文本清晰地指明链接方向至关重要。这有助于用户确定他们是否确实想要关注链接。 对于视力正常的用户而言，有意义的链接文本在页面上存在多个链接时非常有用（尤其是页面包含大量文本时），因为有意义的链接文本可以更清晰地指示目标页面的功能。 使用一些辅助型技术的用户可以在单个页面上生成所有链接的列表，如果链接文本是唯一的且可以提供有用信息，则用户可以轻松地脱离上下文理解链接文本。但是，如果链接无法提供足够的信息来准确描述链接目标位置，则患有认知障碍的视力正常用户可能会感到困惑。
...
在编写文本时，在使用标题属性确保页面上呈现的类似链接提供有关目标的唯一信息方面存在一些问题（例如，“阅读更多”通常是指一系列不同的目标）：

* 标题属性中包含的文本仅供鼠标用户作为工具提示弹出窗口使用，使用键盘或移动用户始终无法访问。
...
有时，如果页面上有多个链接（每个链接都以复杂但必要的详细信息提供链接方向），则可能需要提供网页的替代版本，虽然该版本显示完全相同的内容，但链接文本并不那么详细。
...
用户必须能够知道在多个元素中，哪一个元素是当前的键盘焦点所在。如果屏幕上只有一个键盘可操作控件，则应该满足成功标准，因为视觉设计只会呈现一个键盘可操作项目。

成功标准中说的“操作模式”，则是考虑到了部分平台可能无法始终显示焦点指示器的情况。通常只有一种操作模式，因此此成功标准适用。
...
>[!NOTE]
AEM针对根语言的变体提供进一步的微调；例如，美式英语 — en-us、英式英语 — en-gb和加拿大英语 — en-ca。 这种详细级别对辅助型技术来说通常是多余的，尽管它可以用于标识页面内容中的区域变化。
...
此 `lang` 属性可用于标识内容语言的更改。 例如，引用的德语（ISO 639-1 代码“de”）可以采用以下方式表示：
...
焦点可以通过键盘（例如，通过Tab键前往控件）或鼠标（例如，选择文本字段）移到控件。 将鼠标移到控件上方不会移动焦点，除非脚本会实施此行为。对于某些类型的控件，选择控件也可以激活该控件（例如按钮），这反过来又可以启动上下文中的更改。
...
此成功标准旨在确保输入数据或选择表单控件这样的操作具有可预见的效果。更改任何用户界面组件的设置将会更改该控件的某些方面，当用户不再与界面交互时，这些方面会一直存在。 因此，选中复选框、在文本字段中输入文本，或更改列表控件中的选定项会更改其设置，但激活链接或按钮则不会更改其设置。上下文更改可能会给那些不容易感知更改或容易被更改分散注意力的用户造成困惑。只有当明确响应用户的操作而发生此类更改时，上下文更改才是合适的。
...
此成功标准旨在确保采用一致的方式标识一组网页内重复出现的功能组件。使用屏幕阅读器的用户在操作网站时所使用的一种策略是，依赖他们对可能出现在不同网页上的功能的熟悉程度，且这种依赖程度很深。如果相同的功能在不同网页上具有不同的标签（或者更一般地说，具有不同的可访问名称），则网站的使用难度会显着增加。 这也可能会令人困惑，并增加认知困难人士的认知负担。因此，一致的标签设置很有帮助。

此一致性要求的适用范围也包括替换文本。如果图标或其他非文本项目具有相同的功能，则其相应的替换文本也应保持一致。

如果某个网页上有两个组件的功能与一组网页内另一个页面上的组件功能相同，则所有这 3 个组件都必须保持一致。因此，同一页面上的这两个参数是一致的。
...
此成功标准旨在确保用户知道发生了错误，并可确定错误的具体内容。 错误消息应尽可能具体和明确。如果表单提交失败，则重新显示表单并指示错误的字段并不足以让某些用户感知发生了错误。 例如，屏幕阅读器用户只有在遇到某个指示器时才知道发生了错误。 他们可能会在遇到错误指示器之前就完全放弃表单，认为页面根本无法正常工作。根据 WCAG 中的定义，[输入错误](https://www.w3.org/TR/WCAG/#dfn-input-error)是用户提供的不可接受的信息。这包括：

网页要求提供但用户忽略了的信息，或者用户虽已提供但不是要求的数据格式或允许的值。例如：

* 用户在州、省、地区等字段中未输入正确的缩写；
...
在提升界面易用性的最佳实践中，一种基本的做法是提供说明来帮助用户完成表单。这样做有助于视力或认知障碍人士，否则他们可能难以理解表单的布局以及在特定表单字段中要提供的数据类型。

##### Forms

在AEM WKND演示项目中，添加表单组件(例如 **文本字段**，以访问页面。 此默认标题取决于组件类型。 您可以在中添加自己的标题 **标题和文本** 字段的“编辑”对话框的选项卡。 应务必确保标签能够帮助用户理解与每个表单组件相关的数据。

此&#x200B;**标题**&#x200B;字段必须用于字段元素，因为该字段提供了适用于辅助型技术的标签。只在字段旁边用文本编写一个标签是不够的。

对于某些表单组件，还可以使用 **隐藏标题** 复选框。以这种方式隐藏的标签仍对辅助型技术可用，但不会显示在屏幕上。 虽然这在某些情况下可能是一个不错的方法，但最好尽可能加入可视标签，因为某些用户可能正在查看屏幕的一小部分（一次查看一个字段），并且需要标签才能正确识别该字段。
...
在WKND项目中，存在一组相关控件，例如 **单选按钮组**，可能需要组和单个控件的标题。 在 AEM 中添加一组单选按钮时，**标题**&#x200B;字段会提供此组标题，而单个标题会在创建单选按钮（**项目**）时指定。

但是，组标题和单选按钮本身之间并没有编程关联。模板编辑器需要将标题包装在必需的 `fieldset` 和 `legend` 标记中，以便创建此关联，而且该操作只能通过编辑页面源代码来完成。或者，系统管理员也可以添加对这些元素的支持，以使它们显示在&#x200B;**字段属性**&#x200B;对话框中（请参阅[添加对其他 HTML 元素和属性的支持](/help/implementing/developing/extending/rte-accessible-content.md)）。
...
成功标准 3.3.1 会提供错误通知。然而，认知困难的人士可能很难理解如何更正错误。患有视觉障碍的人士可能无法明白到底应该如何更正错误。如果表单提交失败，用户可能会放弃表单，因为他们可能不确定如何更正错误，即使他们知道发生了错误。
...
检查用户输入的数据是否有输入错误，并为用户提供一次更正错误的机会。
   * 已确认提供一种机制，用于在最终提交之前复核、确认和更正信息。

#### 用途 – 错误预防（法律、金融、数据）(3.3.4) {#purpose-error-prevention-legal-financial-data}

此成功标准旨在帮助残障用户在执行无法撤销的操作时，避免因错误而造成严重后果。例如，购买不可退款的机票，或者提交订单在经纪帐户中购买股票，这些都是可能产生严重后果的金融交易。如果用户在航空旅行日期上出错，他们最终可能会拿到一张无法更换的错误日期的机票。 如果用户在要购买的股票数量上出错，他们最终可能会购买超出预期数量的股票。 这两种错误都涉及立即发生且事后无法更改的交易，并且可能代价高昂。 同样，如果用户无意中修改或删除了存储在数据库中并且以后需要访问的数据（如旅行服务网站中自己的全部旅行档案），这也可能是一个无法挽救的错误。当涉及修改或删除“用户可控”数据时，此项标准的目的是防止大量丢失数据，例如删除一份文件或记录，而不是要求在每次保存命令时进行确认，也不是指对文档、记录或其他数据进行简单的创建或编辑。

残障用户可能更容易出错。患有阅读障碍的人可能会颠倒数字和字母，而患有运动障碍的人则可能会意外按错按键。能够撤销操作使用户能够纠正可能导致严重后果的错误。 通过查看和更正信息的能力，用户可在采取会导致严重后果的行动之前发现错误。

用户可控数据是指用户可查看的数据，用户可以通过有意的操作来改变和/或删除这些数据。用户控制此类数据的示例可能包括：更新用户帐户的电话号码和地址，或者从网站删除过去发票的记录。它不是指用户无法直接查看或与之交互的互联网日志和搜索引擎监控数据。
...
此成功标准旨在确保用户代理（包括辅助技术）能够准确地解读和分析内容。如果内容无法解析为数据结构，则不同的用户代理可能会以不同的方式呈现内容或无法解析内容。 一些用户代理会使用“修复技术”来呈现编码欠佳的内容。

由于修复技术因用户代理而异，因此作者不能假设内容被准确地解析为数据结构，或者内容被专业用户代理（包括辅助技术）正确呈现，除非内容是根据该技术的正式语法中定义的规则创建的。 在标记语言中，元素和属性语法中的错误以及未提供正确嵌套的开始/结束标签，会导致使用户代理无法可靠地解析内容的错误。因此，成功标准要求内容只能使用正式语法的规则来解析。
...
* 成功标准 4.1.2
* A 级
* 名称、角色、值：对于所有用户界面组件（包括但不限于：表单元素、链接和脚本生成的组件），名称和角色可以通过编程方式确定；状态、属性和可由用户设置的值可以通过编程方式设置；并且用户代理（包括辅助技术）可以收到这些项目的更改通知。
...
用户界面控件的一个重要状态是它是否具有焦点。 控件的焦点状态可以通过编程方式确定，并且会向用户代理和辅助技术发送关于焦点状态的变更通知。用户界面控件状态的其他示例包括：复选框或单选按钮是否已被选中，或者可折叠的树或列表节点是否已展开或折叠。
