...
* **更多标题和描述**
   ...
   * **页面标题** – 要在页面上使用的标题。通常由标题组件使用。如果为空， **标题** 已使用。
   * **导航标题**  — 您可以指定单独的标题以在导航中使用（例如，当您希望某些内容能更加简洁时）。 如果为空， **标题** 已使用。
   * **子标题** – 要在页面上使用的子标题。
...
* **开启/结束时间**
  ...
  >[!NOTE]
  >如果 **准时** 或 **关闭时间** 是过去设置的，并且配置了自动复制，然后会立即触发相关操作。
   ...
   * **关闭时间**  — 类似于，并经常与 **准时**，这会定义已发布页面在发布环境中隐藏的时间。
...
* **模板设置**
   ...
   * **允许的模板** - [定义可用的模板列表](/help/sites-cloud/authoring/features/templates.md#enabling-and-allowing-a-template-template-author) 在此支行内
...
* **SEO**
   ...
   * **规范URL**  — 可用于覆盖页面的规范URL；如果留空，则页面的URL是它的规范URL
   ...
   * **生成站点地图**  — 选中后，将为此页面及其后代生成sitemap.xml
...
* **特色图像**
   ...
   * **继承 – 从 DAM 资产中提取的值** – 如果选中，这将使用 DAM 中 `dc:description` 元数据的值填充替换文本
...
* 属于同一资源类型
* 不是 Live Copy 的一部分
   * 如果有任何页面位于Live Copy中，则当属性打开时会显示一条消息。
