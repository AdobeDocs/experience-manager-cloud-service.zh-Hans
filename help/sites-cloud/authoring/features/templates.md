...
有关如何在技术层面使用可编辑模板的详细信息，请参阅开发人员文档 [页面模板](/help/implementing/developing/components/templates.md) 了解更多信息。
...
1. 选择&#x200B;**创建**。随即会显示确认，选择 **打开** 以开始编辑模板或 **完成** 以返回到“模板”控制台。
...
>[!CAUTION]
>
>启用模板后，当模板作者开始进一步更新模板时，会显示警告。 此消息用于告知用户，该模板可能已被引用，因此所做的任何更改可能会影响引用该模板的页面。
...
>[!CAUTION]
>
>如果作者开始编辑已启用的模板，则会显示警告。 此消息用于告知用户，该模板可能已被引用，因此所做的任何更改可能会影响引用该模板的页面。
...
解锁组件后，您可以定义 [初始内容](#editing-a-template-initial-content-author) 页面时所生成的页面页面页面上的项目（选件大小、页面大小、页面大小、页面大小、页面大小、页面大小、页面间距）。 可以在生成页面上编辑这些已解锁的组件。

>[!NOTE]
>
>In **初始内容** 在模式和生成页面上，可以删除任何具有可访问父项的已解锁组件（即，布局容器中的组件）。
...
>
>如果更改此文本，并添加您自己的文本，则从模板创建页面时将使用此更新的文本。
>
...
使用&#x200B;**选择策略**&#x200B;下拉列表旁边的“复制”按钮，可复制在此下拉列表中选定的现有策略以将其作为新策略。然后，应该在&#x200B;**策略标题**&#x200B;字段中输入一个新标题。默认情况下，复制的策略的标题为 **X的副本**，其中X是复制的策略的标题。
...
>[!NOTE]
>
>对于容器组件而言，必须配置策略，因为它允许您定义容器中可用的组件。
...
这包括解锁容器组件，以便在初始内容模式或生成的页面 **中添加其他组件** 。 如果在解锁容器之前已将组件/内容添加到容器，则在中时，这些组件/内容不再显示 **结构** 模式，但它们显示在 **初始内容** 模式。 In **结构模式**，则只有容器组件本身会显示其列表 **允许的组件**.
...
>[!NOTE]
>
>布局更改会反映在 **初始内容** 模式，但未看到任何更改 **结构** 模式。
