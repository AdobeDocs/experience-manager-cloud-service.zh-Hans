...
这提供了一个段落系统，允许您在响应式网格中放置组件。 此网格可以根据设备/窗口大小和格式重新安排布局。该组件将与 [**布局** 模式](/help/sites-cloud/authoring/fundamentals/environment-tools.md#page-modes)，您可以创建和编辑依赖于设备的响应式布局。
...
通过执行此操作，您可以创建和编辑响应式网站，这些网站可通过以交互方式调整组件大小，从而根据设备/窗口大小重新排列布局。 随后，用户可以使用模拟器查看内容的呈现方式。
