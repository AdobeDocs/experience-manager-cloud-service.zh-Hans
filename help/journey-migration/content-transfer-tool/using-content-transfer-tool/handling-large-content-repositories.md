...
* 从CTT版本2.0.16开始，预复制设置会在安装包时自动完成。 此外，如果迁移集大小大于200 GB，则提取过程将自动使用预复制功能。 azcopy.config文件在crx-quickstart/cloud-migration/目录中创建。 如果您使用的是CTT版本2.0.16或更高版本，则无需手动进行预复制设置。
...
* 在过去七天内，已在源上运行数据存储垃圾收集。 有关更多详细信息，请参阅 [数据存储垃圾收集](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/deploying/data-store-config.html#data-store-garbage-collection).
...
* 本地系统的可用空间必须严格大于源数据存储大小的1/256。 例如，如果数据存储的大小为3 TB，则中必须存在大于11.72 GB的可用空间 `crx-quickstart/cloud-migration` AzCopy要工作的源上的文件夹。 源系统至少应有1 GB的可用空间。 可用空间可以通过使用获得 `df -h` Linux®实例中的command和Windows实例中的dir命令。
...
>[!NOTE]
>从CTT版本2.0.16开始，预复制设置会在安装包时自动完成。 此外，如果迁移集大小大于200 GB，则提取过程将自动使用预复制功能。 azcopy.config文件在crx-quickstart/cloud-migration/目录中创建。 如果要手动更新文件的配置，请查看以下部分。

阅读本节内容，以便了解如何设置将AzCopy用作内容传输工具的预复制步骤，以将内容迁移到AEMas a Cloud Service：
...
* 对于Mac、UNIX®系统，请对datastore目录运行du命令以获取其大小：
...
[Azcopy](https://learn.microsoft.com/en-us/azure/storage/common/storage-use-azcopy-v10) 是Microsoft提供的命令行工具®该工具必须在源实例上可用才能启用此功能。

简而言之，您要从下载Linux® x86-64二进制文件 [AzCopy文档页面](https://learn.microsoft.com/en-us/azure/storage/common/storage-use-azcopy-v10) 并将其解压缩到/usr/bin之类的位置。

>[!IMPORTANT]
>记下二进制文件的放置位置，因为需要在以后的步骤中获取该二进制文件的完整路径。
...
azCopyPath属性必须包含源AEM实例上安装azCopy命令行工具所在位置的完整路径。 如果缺少azCopyPath属性，则不执行blob预复制步骤。
...
请查看Cloud Acceleration Manager中的“引入作业”列表和引入日志，以便您能够查看进度。 与成功的AzCopy任务相关的日志条目显示如下（允许存在某些差异）。 有时检查日志可能会及早提醒您问题，并帮助您找到解决任何问题的快速方法。
...
现在，您已了解如何处理大型内容存储库，以加快内容传输活动的提取和摄取阶段以将内容移动到AEMas a Cloud Service。 您现在已准备好学习使用内容传输工具的提取流程。 参见 [在内容传输工具中从源提取内容](/help/journey-migration/content-transfer-tool/using-content-transfer-tool/extracting-content.md) 以便您了解如何从内容传输工具中提取迁移集。
