...
>[!NOTE]
>您可以通过引入作业的操作，使用……下拉列表查看或下载索引日志。 The logs will be available in the
...
对目标Cloud Service实例编制索引很可能成功。 在某些情况下，该操作可能会失败，并且引入作业行将如下所示。 在所有情况下，都可以通过将鼠标悬停在故障状态上来查找故障的一些详细信息，这些信息可能会帮助您确定后续步骤。 此时，索引作业日志可供查看或下载，以帮助发现故障源。 如果不清楚下一步骤，请联系Adobe支持，并提供摄取和索引日志的详细信息。

>[!TIP]
>
> 如果索引作业运行时间过长，请确保未通过Cloud Manager应用[IP](/help/implementing/cloud-manager/ip-allow-lists/apply-allow-list.md)，因为它会阻止Cloud Acceleration Manager访问迁移服务。
