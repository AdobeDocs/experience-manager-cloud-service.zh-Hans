...
要在AEM as a Cloud Service中使用CUG，用户必须位于创作实例上并且是相关CUG组的成员。  这可以使用包完成，或者，如果CUG用户是IMS用户，则它们可能已存在。  随后，CUG用户必须成为AEM CUG组的成员。

要在发布实例上启用CUG行为，

1. 必须激活CUG组（这会将它们及其成员复制到发布实例），
1. 必须取消发布受CUG策略保护的所有&#x200B;*页面（以清除全局CUG计数），并且*
1. 随后，必须发布受CUG策略保护的页面（这可以启用发布实例并跟踪策略）。
