...
1. 查看摄取核对清单，并确保已完成所有步骤。 这些步骤对于确保成功摄取是必需的。 继续执行 **下一个** 仅当核对清单完成时执行此步骤。
...
1. 提供创建引入所需的信息。
   ...
   * 选择目标环境。 在此环境中摄取迁移集的内容。 选择层。 （创作/发布）。 不支持快速开发环境。
   ...
   >[!IMPORTANT]
   > 以下重要注意事项适用于摄取内容：
   > 仅当属于本地环境时，才可以开始摄取到目标环境 **AEM管理员** Cloud Service创作服务上的组。 如果您无法开始引入，请参阅 [无法开始引入](/help/journey-migration/content-transfer-tool/using-content-transfer-tool/ingesting-content.md#unable-to-start-ingestion) 了解更多详细信息。
   > 如果设置 **划出** 在摄取之前启用，它会删除整个现有存储库并创建一个可摄取内容的存储库。 此工作流表示它会重置所有设置，包括目标Cloud Service实例的权限。 对于添加到的管理员用户，此重置也为true **管理员** 组。 您必须已准备好进入管理员组才能开始引入。
...
1. 单击 **(i)** 按钮，以了解有关引入作业的详细信息。 您可以通过单击查看摄取操作在运行或完成时的每个步骤的持续时间 **...**，然后单击 **查看持续时间**. 此外，还显示提取中的信息以实现所摄取的内容。
...
>[!NOTE]
>初始内容传输完成后，建议在云服务上线之前，经常对差异内容进行增补，以缩短最终差异内容传输的内容冻结期。如果您已使用预复制步骤进行第一次完整摄取，则可以跳过预复制以进行后续增补摄取（如果增补迁移集大小小于200 GB）。 原因是这可能会增加整个过程的时间。
...
从创建引入作业开始，并确保 **划出** 在摄取阶段被禁用，如下所示：
...
自动检索迁移令牌可能会由于各种原因而失败，其中包括您 [通过Cloud Manager设置IP允许列表](/help/implementing/cloud-manager/ip-allow-lists/apply-allow-list.md) 在目标Cloud Service环境中。 在此类场景中，当您尝试开始引入时，您将看到以下对话框：

![图像](/help/journey-migration/content-transfer-tool/assets-ctt/troubleshooting-token.png)

通过单击对话框中的“获取令牌”链接，手动检索迁移令牌。 打开另一个选项卡以显示令牌。 然后，您可以复制令牌并将其粘贴到 **迁移令牌输入** 字段。 现在，您应该能够开始引入。
...
仅当属于本地环境时，才可以开始摄取到目标环境 **AEM管理员** Cloud Service创作服务上的组。 如果您不属于AEM管理员组，则在尝试开始引入时会看到如下所示的错误。 您可以要求管理员将您添加到本地 **AEM管理员** 或者询问令牌本身，然后将其粘贴到 **迁移令牌输入** 字段。
...
此消息表示Cloud Acceleration Manager无法访问目标环境的迁移服务以开始引入。 出于各种原因，这种情况可能会发生。

>[!NOTE]
> 
> 显示了“迁移令牌”字段，因为在少数情况下，实际上不允许检索该令牌。 通过允许手动提供，它可允许用户快速开始引入，而无需任何其他帮助。 如果提供了令牌，但消息仍然显示，则检索令牌并不是问题。
...
* 如果 [已应用IP允许列表](/help/implementing/cloud-manager/ip-allow-lists/apply-allow-list.md) 它通过Cloud Manager阻止Cloud Acceleration Manager访问迁移服务。 无法为引入添加IP地址，因为其地址是动态的。 目前，唯一的解决方案是在摄取运行时禁用IP允许列表。
* 可能还有其他需要调查的原因。 如果摄取仍然失败，请联系Adobe客户关怀团队。

### 仍启用通过Release Orchestrator进行自动更新

Release Orchestrator通过自动应用更新来自动使环境保持最新。 如果在执行摄取时触发更新，则可能会导致不可预测的结果，包括环境损坏。 一个很好的理由是，在开始引入之前记录客户支持工单（请参阅上面的“注意”），以便可以计划暂时禁用Release Orchestrator。

如果在开始引入时仍在运行Release Orchestrator，则用户界面会显示此消息。 无论如何您都可以选择继续，通过检查字段并再次按按钮来接受风险。
...
AEM中的每个节点都必须具有一个唯一的uuid。 此错误表示正在摄取的节点的uuid与目标实例上其他路径下的节点具有相同的uuid。
...
完成将内容摄取到Target中后，您可以查看每个步骤（提取和摄取）的日志并查找错误。 参见 [查看迁移集的日志](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/migration-journey/cloud-migration/content-transfer-tool/viewing-logs.html?lang=zh-Hans) 了解更多信息。
