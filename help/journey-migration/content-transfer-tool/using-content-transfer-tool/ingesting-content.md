...
>[!NOTE]
>您可以运行可选的预复制步骤，以显着加快摄取阶段。 预复制步骤对于第一次完全提取和摄取最有效。 请参阅 [使用AzCopy引入](/help/journey-migration/content-transfer-tool/using-content-transfer-tool/handling-large-content-repositories.md#ingesting-azcopy) 了解更多详细信息。
...
1. 提供创建新引入所需的信息。
   ...
   * 选择目标环境。 这是将摄取迁移集内容的地方。 选择层。 （创作/发布）。
   ...
   >[!IMPORTANT]
   >
   >仅当您属于本地环境时，才能够开始摄取到目标环境 **AEM管理员** Cloud Service创作服务上的组。 如果无法开始引入，请参阅 [无法开始引入](/help/journey-migration/content-transfer-tool/using-content-transfer-tool/ingesting-content.md#unable-to-start-ingestion) 了解更多详细信息。
...
1. 完成摄取后，单击屏幕右上角的(i)按钮以获取有关摄取作业的更多信息。
...
>title="增补摄取"
>abstract="使用增补功能可移动自上次内容传输活动以来修改的内容。 摄取完成后，检查日志中是否有任何错误/警告。 任何错误都应立即解决，方法是处理报告的问题或联系Adobe客户关怀部门。"
...
仅当您属于本地环境时，才能够开始摄取到目标环境 **AEM管理员** Cloud Service创作服务上的组。 如果您不属于AEM管理员组，则在尝试开始引入时，您将看到如下所示的错误。 您可以要求管理员将您添加到本地 **AEM管理员** 或者询问令牌本身，然后将其粘贴到 **迁移令牌输入** 字段。
...
Release Orchestrator通过自动应用更新来自动使环境保持最新。 如果在执行引入时触发更新，则可能会导致不可预测的结果，包括环境损坏。 这是应在开始引入之前记录支持工单的原因之一（请参阅上面的“注意”），以便可以计划临时禁用Release Orchestrator。
...
完成将内容摄取到Target中后，您可以查看每个步骤（提取和摄取）的日志并查找错误。 参见 [查看迁移集的日志](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/moving/cloud-migration/content-transfer-tool/viewing-logs.html?lang=en) 了解更多信息。
