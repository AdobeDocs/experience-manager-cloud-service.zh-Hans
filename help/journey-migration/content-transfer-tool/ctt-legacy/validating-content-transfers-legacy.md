...
# 驗證內容轉移（舊版） {#validating-content-transfers}

## 快速入门 {#getting-started}

使用者能夠可靠地判斷「內容轉移工具」擷取的所有內容是否已成功擷取到目標例項中。 此驗證功能的運作方式是比較擷取期間涉及的節點摘要，與擷取期間涉及的節點摘要。 如果擷取摘要中納入的任何節點路徑遺漏了擷取摘要，則驗證會被視為失敗，且可能需要額外的手動驗證。
...
除了包含在擷取記錄中外，您也可以從「內容轉移工具」使用者介面存取驗證報告。 若要這麼做，請選取移轉集，然後按一下 **驗證** 動作列中的按鈕：
...
使用 **驗證發佈/作者報告** 按鈕以檢視驗證報告，瞭解目標環境指定層級的最新內嵌。 請參閱以下小型發佈內嵌的範例：

![图像](/help/journey-migration/content-transfer-tool/assets/CTTvalidationreport.png)

>[!NOTE]
>
>此 **驗證發佈/作者報告** 擷取完成後，連結就會顯示。 此外，驗證報告會持續存在，因此在擷取完成後不會像擷取記錄一樣過期。
