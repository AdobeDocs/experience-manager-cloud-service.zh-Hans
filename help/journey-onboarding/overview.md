...
恭喜您选择 AEM as a Cloud Service！ 本文档是指导您整个上线历程的起点。 无论您是部署新应用程序还是迁移现有应用程序，这一入门培训历程都将为您建立团队。 它确保用户有权访问AEM as a Cloud Service。

## 简介 {#introduction}

Adobe Experience Manager 是一组功能强大的可组合内容服务，它在任何渠道上都快速地投放极具影响力的个性化体验，从而为所有人展示各种内容。**Edge Delivery Services** 是 Adobe Experience Manager 中最新的创新，通过它，可极快地投放内容并打造卓越的体验。通过查阅 [Edge Delivery Services 概述](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/edge-delivery/overview)，了解如何开始使用 Edge Delivery Services。要了解如何使用 Edge Delivery Services，请参阅[开发人员教程](https://www.aem.live/developer/tutorial)页面。
...
* **Cloud Manager** - 用于设置项目和环境等资源的工具。它也是您访问Git并创建管道以管理和部署自定义代码的地方。

这些概念会在此次上线历程中详细阐述。目标是在历程结束时，您可以执行以下操作：

* 授予必要用户访问AEM as a Cloud Service的权限。
* 为项目设置第一个云资源。
* 知道如何部署第一个代码并创作第一个内容。
...
该入门培训历程是专门为&#x200B;**系统管理员**&#x200B;以及AEM as a Cloud Service和AEM的新客户编写的。 系统管理员是在签署AEM as a Cloud Service合同后首次Adobe联系人的个人。 他们通常是第一个在 AEM as a Cloud Service 上访问和设置您的资源的人。如果您正在阅读本主题，您很可能是系统管理员。

系统管理员管理其组织的 AEMaaCS 用户的各个方面，从访问到权限。 但是，系统管理员必须在此过程中与其他角色进行交互。

| 角色 | 描述 | 历程中的角色 |
|---|---|---|
| 系统管理员 | 此历程的目标是提供云资源的初始配置，并根据用户的工作职责将其分配给适当的角色 | 管理用户从访问到权限的各个方面 |
| 内容作者 | 创建并查看 AEM 中的内容 | 一旦系统管理员授予权限，作者就可以开始自己的内容创建历程 |
...
>[!TIP]
>
>如果您不熟悉AEM as a Cloud Service并且熟悉AEM，并且正在从内部部署或AdobeManaged Services进行迁移，请查看[AEM as a Cloud Service迁移历程](/help/journey-migration/getting-started.md)。
...
[文档历程](/help/journey-documentation/documentation-journeys.md)将许多不同的、复杂的主题和功能联系在一起。 其中提供的内容可以帮助新接触 AEM 的读者从头到尾理解和解决业务问题，同时假定读者不了解先前的主题或 AEM 知识。

文档历程是围绕最佳实践原则设计的。 他们通过Adobe的最新研究、Adobe顾问提供的经验证的实施经验以及客户项目的反馈获得信息。

如果您想了解有关如何将您的团队载入到新的AEM as a Cloud Service应用程序中的Adobe建议，请从此处开始。

## 其他资源 {#additional-resources}

如果您想了解上线历程以外的内容，以下是额外的可选资源。

* [加入 AEM as a Cloud Service 入门培训](https://experienceleague.adobe.com/en/docs/experience-manager-learn/cloud-service/migration/moving-to-aem-as-a-cloud-service/onboarding) - 这段简短的视频概述 AEM 的云服务入门过程。
