...
作为[AEM的Content Advisor代理的一部分，](/help/ai-in-aem/agents/content-advisor/overview.md)内容优化代理通过应用自然语言指令创建渠道就绪的变体，来转换用户优化和调整资源的方式。 无论是为特定数字渠道生成新演绎版、调整视觉属性、更改背景还是准备资源，代理都会自动解释用户意图并执行复杂的编辑任务。 它与[内容发现代理](/help/ai-in-aem/agents/content-advisor/discovery.md)无缝协作，利用它找到的资产，并使用核心的[具有OpenAPI功能的Dynamic Media](/help/assets/dynamic-media-open-apis-overview.md)生成优化的变体，以满足品牌、渠道和促销活动要求，而无需手动设计。
...
* **[与内容发现代理无缝协作](/help/ai-in-aem/agents/content-advisor/discovery.md)**：基于内容发现代理标识的资源进行构建，通过自然对话实现端到端资源检索和优化。
...
* **生成动态内容变体**
  ...
  内容优化代理创建优化的变体作为针对不同渠道和格式类型定制的动态URL。
...
* **多变体资源优化**
  ...
  内容优化代理可以使用单个自然语言提示从内容发现代理返回的资产中生成多个优化的图像变体，使用户能够快速高效地生成适用于渠道的演绎版。

## 角色 {#personas-content-optimization-agent}

渠道营销人员是内容优化代理的关键角色，他们可以选择正确的高分辨率源内容，并请求针对其渠道和受众区段定制的优化格式。

区域营销人员和代理机构工作人员还可以使用内容优化代理快速生成渠道就绪的图像变体，从而支持更快、更一致的内容生产。

## 如何访问 {#access-content-optimization-agent}

您可以通过AI助手访问AEM中的内容优化代理。 登录到[`experience.adobe.com`](https://experience.adobe.com)，通过使用`Ask AI Assistant anything`字段以自然语言指定提示开始与AI助手交互：

![访问内容优化代理](/help/ai-in-aem/agents/content-advisor/assets/access-discovery-agent.png)

## 常见用例和示例提示 {#use-cases-prompts}

通过[内容发现代理搜索正确的资产，从而使用内容优化代理。](/help/ai-in-aem/agents/content-advisor/discovery.md)一旦相关图像浮出水面，用户就可以直接从搜索结果为一个或多个资源生成优化或特定于渠道的变体。 或者，用户可以通过在提示中指定资产UUID或资产路径来生成变体，而无需执行之前的搜索。 这一工作流程确保高质量的投入和持续更好的优化结果。 [有关详细信息，请参阅可用优化的完整列表](https://developer.adobe.com/experience-cloud/experience-manager-apis/api/stable/assets/delivery/)。
...
* **高分辨率演绎版创建**
  ...
  使用[内容发现代理](/help/ai-in-aem/agents/content-advisor/discovery.md)搜索正确的资源，然后在出现多个搜索结果时使用以下提示：
