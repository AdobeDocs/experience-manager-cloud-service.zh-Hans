...
[内容更新](/help/ai-in-aem/agents/brand-experience/experience-production/content-update.md)作业可轻松更新现有内容，包括内容片段、页面、表单和资产。 代理可以执行更新、删除、替换或添加内容元素等操作，以保持体验准确且最新。 输入可以是自然语言描述，在与Jira PDF一起使用时，屏幕截图也可以提供输入。

### 表单创建作业 {#form-creation}

[表单创建](/help/ai-in-aem/agents/brand-experience/experience-production/form-creation.md)作业使用户能够通过自然语言提示生成自适应表单，而无需依赖开发或IT团队。 此功能可加快表单开发，同时保持品牌一致性，并允许业务用户在不具备深入的技术产品知识的情况下创建表单。

### 通信创建作业 {#communications-creation}

[通信创建](/help/ai-in-aem/agents/brand-experience/experience-production/communication-creation.md)作业使业务用户能够大规模生成个性化、数据驱动的通信。 从账户报表、政策文件到账单和欢迎套件，Agent将自然语言要求转变为专业通信。
