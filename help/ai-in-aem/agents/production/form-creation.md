...
* **创建具有纯文本提示的新表单**：您可以通过以纯语言提交您的要求来创建品牌内表单体验。

* **导入PDF或图像并将其转换为表单**：您可以将现有图像或PDF文档导入并转换为表单。 代理程序会分析上传的内容，以检测字段类型、保留布局并通过响应式设计和验证逻辑增强表单。 支持的格式包括Acroform、XFA PDF、平面PDF、图像(JPG、PNG)和手绘表单照片。
...
使用AI助手创建初始表单结构后，您可以使用Forms创建扩展执行以下操作：
