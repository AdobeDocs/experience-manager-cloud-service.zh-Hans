...
2. **识别数据Source**：预填充服务确定配置的数据源（表单数据模型或草稿服务）
3. **检索数据**：系统根据上下文、参数或用户标识获取相关用户数据
4. **映射和显示**：数据使用`bindRef`属性映射到表单字段，并且已填充的表单向用户显示
...
2. 导航到&#x200B;**Adobe Experience Manager** > **Forms** > **数据集成**
3. 选择&#x200B;**创建** > **表单数据模型**
4. 选择您的&#x200B;**数据Source配置**&#x200B;并选择配置的&#x200B;**数据Source**
