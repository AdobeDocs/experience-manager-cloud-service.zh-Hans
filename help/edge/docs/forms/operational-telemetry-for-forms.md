...
操作遥测让您能够洞察访客如何与您的 Adobe Experience Manager（AEM）网站互动。 此内置工具提供有价值的数据，用以了解用户行为、诊断性能问题并衡量网站试验的有效性。 操作遥测不局限于合成测试，而是通过捕捉实际使用中的交互行为，呈现更为精准的网站性能图像。
...
此仪表板提供了有关访客如何与 Adobe Experience Manager (AEM) 网站上的表单互动的重要洞察。 通过监测这些量度，您可以确定需要改进的领域并优化表单，从而获得更佳用户体验和转化率：
