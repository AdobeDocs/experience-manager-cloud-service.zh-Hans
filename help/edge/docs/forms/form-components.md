...
表单块可帮助您轻松创建表单，以捕获和存储捕获的数据。 它支持基于所有HTML的组件5 [输入类型](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input#input_types) 例如文本、电子邮件、数字、日期等。 它还支持 [文本区域](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/textarea)， [选择](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/select)、和 [字段集](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/fieldset) 元素，并包括HTML5固有的输入验证功能。

表单块会为所有字段类型和容器（面板）生成统一的HTML结构，以确保一致性。 这种一致的结构使得更易于 [设置表单样式](/help/edge/docs/forms/style-theme-forms.md).
...
<table>
 <tbody>
  ...
  <tr>
   <td>价值</td>
   <td>所有组件</td>
   <td> 控件的初始值。 对于单选按钮和复选框组件，这是选中时提交的值/数据。 value属性始终是可选的，不过对于复选框和单选按钮来说，该值应被视为必选项。
   </td>
  </tr>
    <tr>
   <td>占位符</td>
