...
要添加、删除或移动行，请单击行的任何单元格。 打开内容浏览器 ![内容浏览器](/help/forms/assets/Smock_Layers_18_N.svg) 然后选择相应的行，它会突出显示带有工具栏选项的选定行，您可以在其中添加、删除或向上移动该行。
* 的 **[!UICONTROL 上移]** 和 **[!UICONTROL 向下移动]** 操作可上下移动选定行。

* 的 **[!UICONTROL 添加列]** 操作会在选定行的下方添加一行。

* 的 **[!UICONTROL 删除列]** 操作会删除选定的行。

![add-delete-move-row-column](assets/add-delete-move-row.png)

双击该行以配置行的属性，如名称、绑定引用、重复设置、CSS类。
![add-delete-move-row-column](assets/row-properties-image.png)


#### 添加或删除列

要添加或删除列，请单击标题部分的文本单元格，此时将打开一个工具栏，其中包含添加或删除列的选项：
...
>[!NOTE]
>
>您可以通过更改 [较少变量](https://lesscss.org//). 有关更多信息，请参阅 [AEM Forms主题](/help/forms/themes.md).
...
运行时或预览时，您将看到 **+** 和 ![“删除”按钮](/help/forms/assets/Smock_Delete.svg) 按钮来添加或删除行。

![add-delete-rows-dynamicaly](assets/add-delete-layout.png)
