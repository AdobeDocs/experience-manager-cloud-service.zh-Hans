...
数据集成支持开箱即用的OAuth2.0、基本身份验证和API密钥身份验证类型，并允许实施自定义身份验证以访问Web服务。 而RESTful、基于SOAP和OData服务在 [!DNL Experience Manager] as a Cloud Service、关系数据库的JDBC和 [!DNL Experience Manager] 用户配置文件配置在 [!DNL Experience Manager] web控制台。
