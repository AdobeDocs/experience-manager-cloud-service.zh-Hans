---
title: 为自适应表单设计XML架构
description: 了解如何在自适应表单中将XML架构用作表单模型。 深入了解XML架构示例，向使用XML架构的字段添加特殊属性，并限制自适应表单组件的可接受值。
...
使用XML架构作为自适应表单的表单模型创作自适应表单需要基本了解XML架构。 此外，建议在阅读本文之前通读以下内容。
...
* 在自适应表单的创作模式下，XSD的结构在内容查找器选项卡中显示为树。 您可以将元素从XSD层次结构拖放到自适应表单中。
