...
## 转换为PDF/步骤 {#convert-pdfa}

PDF/A是一种存档格式，用于通过嵌入字体和解压缩文件来长期保存文档的内容。 因此，PDF/A文档通常比标准PDF文档大。 您可以使用 ***转换为PDF/A*** 步骤，将PDF文档转换为PDF/A格式。
...
**[!UICONTROL 输入文档]**:用于设置输入文档的属性。 此选项卡下提供的各种选项包括：
* **[!UICONTROL 使用指定DDX]**:指定与有效负载相关的输入文档、具有绝对路径、可以作为有效负载提供或存储在文档类型的变量中。
* **[!UICONTROL 从负载创建映射]**:将有效负荷文件夹下的所有文档添加到汇编程序中调用API的输入文档映射中。 每个文档的节点名称将用作映射中的键。
* **[!UICONTROL 输入文档的映射]**:选项用于使用 **[!UICONTROL 添加]** 按钮。 每个条目表示映射和文档源中文档的键。
...
**[!UICONTROL 输出文档]**:指定保存输出文件的位置。 此选项卡下提供的各种选项包括：
* **[!UICONTROL 以有效负载保存输出]**:在有效负载文件夹下保存输出文档，或覆盖有效负载（如果有效负载是文件）。
...
* **[!UICONTROL 打印机格式]**:“打印格式”值，指定在未提供XDC文件时用于生成输出流的页面描述语言。 如果提供文字值，请选择以下值之一：
   ...
   * **[!UICONTROL PostScript纯文本]**:使用选项为PostScript指定纯文本XDC文件。
   ...
   * **[!UICONTROL HP_PCL_5e]**:使用选项可支持多个Canon设备。
...
* **[!UICONTROL 区域设置]**:指定用于生成PDF文档的语言。 如果提供文字值，请从列表中选择一种语言，或选择以下任一值：
   * **[!UICONTROL 使用服务器默认值]**:（默认）使用 [!DNL AEM Forms] 服务器。 “区域设置”设置是使用管理控制台配置的。 (请参阅 [Designer帮助](http://www.adobe.com/go/learn_aemforms_designer_65_cn).)

   * **[!UICONTROL 使用自定义值]**:在文字框中键入区域设置代码，或选择包含区域设置代码的字符串变量。 有关支持的区域设置代码的完整列表，请参阅http://java.sun.com/j2se/1.5.0/docs/guide/intl/locale.doc.html 。
