...
[!DNL AEM Forms] 数据集成允许配置不同的数据源并将其与连接 [!DNL AEM Forms]. 它提供了一个直观的用户界面，用于跨连接的数据源创建业务实体和服务的统一数据表示架构。 这种统一表示法称为表单数据模型(FDM)，是JSON模式的一种扩展。 表单数据模型(FDM)中的实体称为数据模型对象。 利用表单数据模型(FDM)，您可以：
