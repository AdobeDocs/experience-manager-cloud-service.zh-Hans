...
[!DNL AEM Forms]数据集成允许配置不同的数据源并将其与[!DNL AEM Forms]连接。 它提供了一个直观的用户界面，用于跨连接的数据源创建业务实体和服务的统一数据表示架构。 这种统一表示法称为表单数据模型(FDM)，是JSON模式的一种扩展。 表单数据模型(FDM)中的实体称为数据模型对象。 利用表单数据模型(FDM)，您可以：
...
[!DNL AEM Forms]还允许您创建独立于数据源的表单数据模型(FDM)，并在以后将表单数据模型(FDM)中的数据模型对象和属性与数据源关联或绑定。 它在处理表单数据模型(FDM)时消除了对数据源的任何依赖性。
