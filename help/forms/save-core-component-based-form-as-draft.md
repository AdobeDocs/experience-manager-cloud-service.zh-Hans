...
* [为草稿和提交配置Azure Storage和统一存储连接器Forms Portal组件](#configure-azure-storage-and-unified-storage-connector-for-drafts--submissions-forms-portal-component)

### 为草稿和提交配置Azure存储和统一存储连接器Forms Portal组件

**草稿和提交**&#x200B;组件需要存储设置才能在AEM Sites页面上保存和列出草稿。 统一存储连接器提供了一个将AEM与外部存储关联的框架。 要将表单另存为草稿，请确保您拥有Azure存储帐户和访问密钥以授权对[!DNL Azure]存储帐户的访问。 拥有Azure存储帐户和访问密钥后，请执行以下步骤，以创建Azure存储配置：
...
1. 在[!DNL Azure]Azure存储帐户&#x200B;**[!UICONTROL 和]** Azure访问密钥&#x200B;**[!UICONTROL 字段中指定]**&#x200B;存储帐户的名称。

   ![Azure 存储配置](/help/forms/assets/save-form-as-draft-azure-storage.png)

   在`Connection String`文本框中输入`Azure Storage Account`，在`Azure Key`文本框中输入`Azure Access key`。

1. 单击&#x200B;**保存**。

   >[!NOTE]
   >
   > 您可以从&#x200B;**[!UICONTROL Azure Azure门户]**&#x200B;检索&#x200B;**[!UICONTROL Microsoft存储帐户]**&#x200B;和[Azure访问密钥](https://learn.microsoft.com/en-us/azure/storage/common/storage-account-keys-manage?tabs=azure-portal)。
...
>[!NOTE]
>
> 如果需要配置Azure以外的存储选项，请从官方电子邮件地址向<aem-forms-ea@adobe.com>发送一封邮件，其中包含您的详细要求。
...
1. 单击&#x200B;**[!UICONTROL 策略]**&#x200B;并选择&#x200B;**[!UICONTROL AEM原型项目名称]** - Forms和通信门户&#x200B;**[下的]草稿和提交**&#x200B;复选框。
...
1. 单击指南容器属性![指南属性](/help/forms/assets/configure-icon.svg)图标，然后打开&#x200B;**[!UICONTROL 草稿]**&#x200B;选项卡。

   ![自动保存](/help/forms/assets/auto-save.png)

1. 选中&#x200B;**[!UICONTROL 自动保存草稿]**&#x200B;复选框以启用将表单自动保存为草稿。
1. 将&#x200B;**[!UICONTROL 保存首选项]**&#x200B;配置为&#x200B;**定期保存草稿**，以便在特定时间间隔后自动保存表单<!--based on the occurrence of an event or-->。
1. 以&#x200B;**[!UICONTROL 保存间隔频率（秒）]**&#x200B;为单位指定时间间隔，以设置按定义的间隔触发表单自动保存的持续时间。
1. 单击&#x200B;**[!UICONTROL 完成]**。
...
在草稿和提交组件&#x200B;**[!UICONTROL 的]**&#x200B;配置对话框中选择&#x200B;**选择类型**&#x200B;作为[已提交的Forms](#configure-properties-of-the-drafts--submissions-component)时，将显示已提交的表单。 您可以查看已提交的表单，但无法编辑它们。
...
在下一篇文章中，让我们了解如何使用[链接Forms门户组件在“站点”页面上添加对表单的引用](/help/forms/add-form-link-to-aem-sites-page.md)。
