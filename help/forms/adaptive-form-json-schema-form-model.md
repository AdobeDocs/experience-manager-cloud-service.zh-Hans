...
description: 了解如何使用JSON架构作为表单模型来创建自适应Forms。 您可以使用现有JSON架构创建自适应Forms。 深入了解JSON模式示例、在JSON模式定义中预配置字段、限制自适应表单组件的可接受值，以及了解不受支持的结构。
...
定义键用于标识可重用的架构。 可重用架构定义用于创建片段。 <!-- It is similar to identifying complex types in XSD.--> 下面给出了具有定义的示例JSON架构：
