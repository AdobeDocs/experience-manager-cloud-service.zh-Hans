...
1. 在编辑模式下，选择一个面板，然后点按 ![cmppr](assets/cmppr.png). 在侧栏中的属性下，启用 **[!UICONTROL 使面板可重复]**. 指定值 **[!UICONTROL 最大值]** 和 **[!UICONTROL 最小值]** 字段。
...
可重复子表单类似于自适应Forms中的可重复面板。 In [!DNL AEM Forms] Designer，执行以下步骤来创建重复的子表单：
...
您可以从XML架构和任何复杂类型元素的minOccours &amp; maxOccurs属性创建可重复面板。 有关XML架构的详细信息，请参见 [使用XML架构作为表单模型创建自适应Forms](adaptive-form-xml-schema-form-model.md).
