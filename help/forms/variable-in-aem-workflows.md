...
* [创建变量](variable-in-aem-workflows.md#create-a-variable) 基于要存储在其中的信息类型的数据类型。
* [设置变量的值](variable-in-aem-workflows.md#set-a-variable) 使用设置变量工作流步骤。
* [使用变量](variable-in-aem-workflows.md#use-a-variable) 在所有 [!DNL AEM Forms] 用于检索存储值的工作流步骤，以及在OR拆分和跳转步骤中定义路由选择表达式。

以下视频演示了如何在AEM Workflow模型中创建、设置和使用变量：
...
您可以使用工作流模型Sidekick中提供的Variables部分创建变量。 AEM Workflow变量支持以下数据类型：
...
1. 为变量指定可选描述，然后点按 ![done_icon](assets/Smock_Checkmark_18_N.svg) 以保存更改。 变量显示在左窗格中可用的列表中。
...
1. 拖放 **[!UICONTROL 设置变量]** 步骤到工作流编辑器，点按该步骤并选择 ![configure_icon](assets/Smock_Wrench_18_N.svg) （配置）。
...
“转至”步骤、“OR拆分”步骤以及所有 [!DNL AEM Forms] 工作流步骤支持变量。
...
可使用表达式编辑器使用变量定义路由选择表达式。 有关对OR拆分步骤使用路由表达式的详细信息，请参阅 [OR拆分步骤](https://experienceleague.adobe.com/docs/experience-manager-65/developing/extending-aem/extending-workflows/workflows-step-ref.html#extending-aem#or-split).

在此示例中，在定义路由表达式之前，使用 [示例2](variable-in-aem-workflows.md#example2) 设置值 **totalvalue** 变量。 分支1处于活动状态： **totalvalue** 变量大于50000。 同样，您可以定义一个规则，使分支2处于活动状态，如果 **totalvalue** 变量小于50000。
...
可使用表达式编辑器使用变量定义路由选择表达式。 有关为“转至”步骤使用路由选择表达式的详细信息，请参阅 [跳转步骤](https://experienceleague.adobe.com/docs/experience-manager-65/developing/extending-aem/extending-workflows/workflows-step-ref.html#extending-aem#goto-step).
...
1. 编辑变量信息并点按 ![done_icon](assets/Smock_Checkmark_18_N.svg) 以保存更改。 您无法编辑 **[!UICONTROL 名称]** 和 **[!UICONTROL 类型]** 变量对应的字段。
