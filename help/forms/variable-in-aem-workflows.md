...
1. 为变量指定可选描述，然后选择![done_icon](assets/Smock_Checkmark_18_N.svg)以保存更改。 变量将显示在左侧窗格中可用的列表中。
...
* **文本：**&#x200B;当您知道要指定的确切值时，请使用选项。 您还可以使用选项以字符串的形式指定JSON。
...
1. 将&#x200B;**[!UICONTROL 设置变量]**&#x200B;步骤拖放到工作流编辑器中，选择该步骤并选择![configure_icon](assets/Smock_Wrench_18_N.svg) （配置）。
1. 在设置变量对话框中，选择&#x200B;**[!UICONTROL 映射]** > **[!UICONTROL 添加映射]**。
1. 在&#x200B;**映射变量**&#x200B;部分中，选择要存储数据的变量，选择映射模式，然后指定要在变量中存储的值。 映射模式因变量类型而异。
1. 映射更多变量以生成有意义的表达式。 选择![done_icon](assets/Smock_Checkmark_18_N.svg)以保存更改。
...
有关详细信息，请参阅[自适应Forms规则编辑器](rule-editor.md)。
...
转到步骤、OR拆分步骤以及所有[!DNL AEM Forms]工作流步骤都支持变量。
...
所有[!DNL AEM Forms]工作流步骤都支持变量。 有关详细信息，请参阅OSGi上的[以Forms为中心的工作流](aem-forms-workflow-step-reference.md)。
...
1. 选择要编辑的变量名称旁边的![编辑](assets/edit.svg) （编辑）图标。
1. 编辑变量信息并选择![done_icon](assets/Smock_Checkmark_18_N.svg)以保存更改。 您无法编辑变量的&#x200B;**[!UICONTROL Name]**&#x200B;和&#x200B;**[!UICONTROL Type]**&#x200B;字段。
...
1. 在编辑工作流页面上，选择工作流模型Sidekick中可用的“变量”图标。 左窗格中的变量部分显示所有现有变量。
1. 选择要删除的变量名称旁边的删除图标。
1. 选择![done_icon](assets/Smock_Checkmark_18_N.svg)以确认并删除该变量。

## 引用 {#references}

有关在[!DNL AEM Forms]工作流步骤中使用变量的更多示例，请参阅AEM工作流中的[变量](https://helpx.adobe.com/experience-manager/kt/forms/using/authoring_variables_in_aem_forms-workflow1.html)。
