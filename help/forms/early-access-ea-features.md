...
* **Forms的面板生成**：让AI完成繁重的调整。 生成根据特定数据收集需求定制的预建表单面板。 例如，生成用于收集付款信息、客户偏好设置或差旅详细信息的部分。 节省时间并确保与预定义布局和功能一致。
...
AEM Forms Workfront Fusion连接器支持Adobe Experience Manager (AEM) Forms与Adobe Workfront Fusion之间的无缝集成。 Adobe Workfront作为工作管理应用程序，集中管理整个工作生命周期，而Workfront Fusion作为集成平台，促进Workfront与各种业务应用程序之间的连接。
...
* **在发布实例上调用关联UI**：您现在可以直接在发布实例上调用关联UI。 此功能定义了所需的配置、有效负载结构和调用流，从而简化了集成并加快了跨环境的部署。
...
* 精简福利通知的政府机构
* 创建高质量、安全且符合法律规定的信件
* 管理数据驱动的交互式对等的创建、汇编和交付

准备好了吗？ 您可以在Formsas a Cloud Service部署的抢先体验计划下找到交互式通信编辑器。 若要请求访问，请将您的组织ID和计划详细信息通过电子邮件从您的官方地址发送到[aem-forms-ea@adobe.com](mailto:aem-forms-ea@adobe.com)。
...
* **辅助功能支持**：HTML5 Forms使用ARIAHTML5辅助功能标准并支持选项卡式导航，使其与JAWS和VoiceOver等常用屏幕阅读器兼容。

* **自定义功能**：您可以使用标准Web技术（如CSS和JavaScript）自定义现有小部件的外观、创建自己的自定义小部件或在表单中使用自定义样式。

* **从右到左的语言支持**：HTML5 Forms支持从右到左的语言，例如希伯来语，允许您以RTL语言显示和填写表单。

* **附件支持**：上传、预览和提交带有HTML5表单的附件，以增强数据收集功能。

* **正在保存草稿**：将HTML5表单保存为草稿，并在稍后阶段继续填写表单。

准备好了吗？ [HTML5表单简介](/help/forms/introductionhtml5.md)文档提供了此创新的全面概述和入门指南。
...
* **提供移动响应式体验**：由于固有的移动响应能力，Forms在任何设备上都保持可访问状态，从而确保桌面、平板电脑和智能手机上的流畅体验。

总体而言，Adaptive Forms中的RTL语言支持使企业能够设计真正的全球性形式，从而提高影响力、参与度和包容性。

准备好了吗？ [自适应Forms RTL文档](/help/forms/supporting-new-language-localization-core-components.md)提供了使用RTL创新的分步指南。
...
* **对文档进行数字签名**：向文档添加数字签名，以进行验证和防篡改验证。 这可用于认证目的或确保文档完整性

* **Reader扩展您的文档(应用或编辑PDF文档的使用权限)**：通过扩展具有附加使用权限的Adobe Reader的功能，使您的组织可轻松共享交互式PDF文档。

  Reader扩展（使用权限）API将使用权限添加到PDF文档。 此操作将激活使用Adobe Reader打开PDF文档时通常不可用的功能，如向文档添加注释、填写表单和保存文档。 第三方用户不需要其他软件或插件即可处理启用了权限的文档。
...
### Forms Manager中的版本控制支持

Forms Manager现在支持自适应Forms（核心组件和基础组件）、表单片段、主题、XDP模板和二进制资产的版本控制。 您可以从“Forms和文档”控制台中创建版本、查看历史记录和恢复早期状态。 请参阅[在Forms管理器中管理表单版本](/help/forms/manage-form-versions-forms-manager.md)。

### 改进了表单组件的辅助功能

自适应Forms核心组件为复选框组、单选按钮组和面板引入了与WCAG兼容的语义标记。 这些组件现在利用`<fieldset>`和`<legend>`元素在组标签及其辅助技术选项之间建立有意义的关系。 请参阅自适应Forms[中的](/help/forms/creating-accessible-adaptive-forms.md#fieldset-legend-accessibility)字段集和图例支持。
