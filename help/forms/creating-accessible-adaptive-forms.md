...
description: AEM Forms为您提供了工具和用于创建无障碍自适应Forms，并有助于遵守无障碍标准。
...
无障碍表单是每个人都可以使用的表单，包括有特殊需要的用户。 自适应Forms包含许多特性和功能，可增强具有不同功能的用户的可用性。 将辅助功能构建到自适应Forms中不仅允许尽可能广泛的内容受众，而且在要求遵守辅助功能标准的地理位置提供文档时，还要求提供这些文档。 [!DNL AEM Forms] 帮助表单开发人员遵守无障碍标准。
...
1. 将现有自适应表单中的开关和图像选择组件替换为选择（单选按钮）组件。
...
1. 删除自适应表单中的潦草签名字段（如果有）。 配置要使用的自适应表单 [!DNL Adobe Sign] 提供无缝的数字签名体验。
...
**标题** 使用此选项可让 [!DNL AEM Forms] 使用与表单字段关联的可视标签作为屏幕阅读器文本。
...
>* 对于基于XFA的自适应Forms，辅助功能选项继承自XDP中设置的辅助功能选项。 XDP中的工具提示会映射到“简短描述”，而“标题”会映射到“标题”。 其他选项可按原样运行。
...
1. 在预览模式下打开自适应表单。
1. 单击添加书签的ANDI工具图标。 ANDI工具会分析自适应表单并显示无障碍问题。 有关如何使用该工具的详细信息，请参阅 [ANDI的文档](https://www.ssa.gov/accessibility/andi/help/howtouse.html).
1. 审查并修复ANDI报告的问题。
