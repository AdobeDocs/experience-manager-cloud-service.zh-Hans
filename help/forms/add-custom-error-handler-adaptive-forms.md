...
| 版本 | 文章链接 |
| -------- | ---------------------------- |
| AEM 6.5 | [单击此处](https://experienceleague.adobe.com/docs/experience-manager-65/forms/adaptive-forms-advanced-authoring/standard-validation-error-messages-adaptive-forms.html?lang=zh-Hans) |
...
* **`Response:`**
  ...
  ![自适应表单字段的Som表达式，用于显示自定义错误处理程序中的错误响应](/help/forms/assets/custom-error-handler-somexpression.png)
...
* [添加自定义错误处理程序函数](#add-custom-error-handler-function)
...
1. [为错误处理程序添加自定义函数](#1-add-the-custom-function-for-the-error-handler)
...
1. [Clone your AEM Forms as a Cloud Service Repository](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/onboarding/journey/developers.html?lang=zh-Hans#accessing-git). 
...
让我们将以下代码添加到该 JavaScript 文件中，以在浏览器控制台中显示从 REST 服务端点接收到的响应和标头。
   ...
   >[!NOTE]
   >
   > * 若要从自定义错误处理程序调用默认错误处理程序，请使用下面一行示例代码： `guidelib.dataIntegrationUtils.defaultErrorHandler(response, headers) `
