...
[!DNL AEM Forms] 让您轻松创建、发布和取消发布表单。 此 [!DNL AEM Forms] 服务器提供两个实例：“创作”和“发布”。 创作实例用于创建和管理表单资源和资源。 发布实例用于保留可供最终用户使用的资源和相关资源。
...
[!DNL AEM Forms] 允许您一次性在服务器上发布所有相应的管理资产。 已发布的资产包括所有相应的管理资产和相关依赖项。
