...
description: 您可以启用电子签名([!DNL Adobe Sign])自适应表单的工作流，可自动执行签名工作流、简化单个和多签名流程，并以电子方式从移动设备对表单进行签名。
topic-tags: develop
...
1. 在 **[!UICONTROL 基本]** 选项卡：
   ...
   配置容器包含 [!DNL Adobe Sign] 为您的环境配置的Cloud Services。 这些服务可在自适应表单编辑器中选择。
...
1. 在 **[!UICONTROL 表单模型]** 选项卡中，选择以下选项之一：
   ...
   * 如果您没有自定义表单模板，请选择 **[!UICONTROL 生成记录文档]** 选项。 使用选项时，发送供签名的文档显示自适应表单的所有字段。

1. 点按 **[!UICONTROL 创建。]** 创建支持签名的自适应表单。 您可以添加 [!DNL Adobe Sign] 将字段发送到表单以进行签名。
...
1. 点按 **[!UICONTROL 保存并关闭]**. 自适应表单已启用 [!DNL Adobe Sign]. 现在，您可以添加 [!DNL Adobe Sign] 将字段发送到表单以进行签名。

### 添加 [!DNL Adobe Sign] 自适应表单的字段 {#addadobesignfieldstoanadaptiveform}

[!DNL Adobe Sign] 在自适应表单上可放置多个字段。 这些字段接受各种类型的数据，如签名、首字母、公司或标题，并帮助在签名期间收集额外信息和签名。 您可以使用 [!DNL Adobe Sign] 要放置的块组件 [!DNL Adobe Sign] 自适应表单中不同位置的字段。

要将字段添加到自适应表单并自定义与这些字段相关的各种选项，请执行以下操作：

1. 拖放 **[!UICONTROL Adobe Sign块]** 从组件浏览器到自适应表单的组件。 此 [!DNL Adobe Sign] 块组件具有所有受支持的 [!DNL Adobe Sign] 字段。 默认情况下，它添加 **[!UICONTROL 签名]** 字段到自适应表单。

   ![签名块](assets/sign_block_new.png)

   默认情况下， [!DNL Adobe Sign] 块在发布的自适应表单中不可见。 它仅在签名文档中可见。 您可以更改 [!DNL Adobe Sign] 从的属性阻止 [!DNL Adobe Sign] 块组件。

   >[!NOTE]
   >
   >  * 使用 [!DNL Adobe Sign] 块不是强制使用的 [!DNL Adobe Sign] 在自适应表单中。 如果您不使用 [!DNL Adobe Sign] 为收件人阻止并添加字段，则默认签名字段将显示在签名文档的底部。
   >  * 使用 [!DNL Adobe Sign] 仅阻止自动生成记录文档的自适应Forms。 如果您使用自定义XDP生成记录文档或基于表单模板的自适应表单， [!DNL Adobe Sign] 不支持块。
...
1. 点按 **[!UICONTROL Adobe Sign]** 字段 ![Adobe Sign](assets/adobesign.png) 图标。 它会显示要选择和添加的选项 [!DNL Adobe Sign] 字段。
   ...
   展开 **[!UICONTROL 类型]** 用于选择 [!DNL Adobe Sign] 字段，然后点按完成 ![保存](assets/save_icon.svg) 图标以将所选字段添加到 [!DNL Adobe Sign] 块。 此 **[!UICONTROL 类型]** 下拉字段包括签名、收件人信息和数据字段类型。 [!DNL Adobe Sign] 与AEM集成 [!DNL Forms] 中列出的支持字段 [!UICONTROL 类型] 下拉框。 有关详情，请参阅 [!DNL Adobe Sign] 字段，请参见 [Adobe Sign文档](https://helpx.adobe.com/sign/help/field-types.html).

   ![adobe-sign-block-fields-options](assets/adobe-sign-block-fields-options.png)

   必须为字段提供唯一名称。 您还可以选择必填选项来标记必填字段。 除了 **[!UICONTROL 名称]** 和 **[!UICONTROL 必需]** 选项，部分 [!DNL Adobe Sign] 字段具有更多选项。 例如，蒙版和多行。 此外，请为每个ID指定一个唯一的名称 [!DNL Adobe Sign] 字段是位于同一位置还是不同位置 [!DNL Adobe Sign] 块。
...
开箱即用， [!DNL Adobe Sign] 没有为自适应表单启用。 要启用该功能：

1. 在内容浏览器中，点按 **[!UICONTROL 表单容器]**，然后点按 **[!UICONTROL 配置]** ![配置](assets/Smock_Wrench_18_N.svg) 图标。 它可打开属性浏览器并显示自适应表单容器属性。
1. 在属性浏览器中，展开 **[!UICONTROL 电子签名]** 折叠面板，然后选择 **[!UICONTROL 启用Adobe Sign]** 选项。 它支持 [!DNL Adobe Sign] （对于自适应表单）。
...
一个文档也可以有多个收件人。 例如，信用卡申请可以有多个申请人。 银行受理申请前，必须经全体申请人签字。 对于多收件人方案，您可以选择按顺序或同时顺序对文档进行签名。
...
1. 从已配置的列表中选择Cloud Service [!DNL Adobe Sign] Cloud Services。
   ...
   如果 **[!UICONTROL Adobe Sign Cloud Service]** 列表为空，请按照 [配置 [!DNL Adobe Sign] 替换为 [!DNL AEM Forms]](adobe-sign-integration-adaptive-forms.md) 文章来配置服务。

   Cloud Services下拉列表列出了 `global` 文件夹(位于“工具”> **[!UICONTROL Cloud Services]** > **[!UICONTROL Adobe Sign]**. 此外，下拉列表还列出了您在中选择的文件夹中存在的Cloud Services **[!UICONTROL 配置容器]** 创建自适应表单时显示的字段。

1. 从中选择签名顺序 **[!UICONTROL 收件人可以填写]** 对话框。 收件人可以签署自适应表单 **[!UICONTROL 按顺序]**  — 一个接一个的收件人，或者 **[!UICONTROL 同时]**  — 按任意顺序。

   按照顺序，一位收件人每次都会收到Adobe Sign协议。 收件人完成分配的操作后，协议将发送给下一个收件人，依此类推。

   所有收件人将同时接收Adobe Sign协议，并且可以相互并行采取行动。

1. 使用“协议ID”字段将绑定与协议ID (agreementId)相关联。 它会将协议ID添加到提交数据的基于架构的表单的afBoundData部分。 协议ID还会添加到所有启用Adobe Sign的表单的提交数据的afSubmissionInfo部分。 您可以使用协议ID来跟踪使用自定义代码的协议状态（需要自定义实施）。

1. [将收件人添加到自适应表单](working-with-adobe-sign.md#addsignerstoanadaptiveform) 然后点按“完成” ![保存](assets/save_icon.svg) 图标以保存更改。

### 将收件人添加到自适应表单 {#addsignerstoanadaptiveform}

一个Adobe Sign协议可以有一个或多个收件人。 添加收件人时，您还可以配置收件人的身份验证详细信息，并选择表单填写者和收件人是否为同一人。 执行以下步骤以添加并提供有关收件人的各种详细信息：

1. 在内容浏览器中，点按 **[!UICONTROL 表单容器]**，然后点按 **[!UICONTROL 配置]** ![配置](assets/Smock_Wrench_18_N.svg) 图标。 它会打开具有自适应表单容器属性的属性浏览器。
1. 在属性浏览器中，展开 **[!UICONTROL 电子签名]** 折叠面板，然后选择 **[!UICONTROL 启用Adobe Sign]** 选项。 它支持 [!DNL Adobe Sign] （对于自适应表单）。
1. 点按 **[!UICONTROL 添加收件人]**. 它会向自适应表单添加收件人。 您可以将多个收件人添加到自适应表单。 所有收件人都会收到有关提交自适应表单的Adobe Sign协议。
...
1. 单击 **[!UICONTROL 编辑]** ![编辑](assets/Smock_Edit_18_N.svg) 图标来指定有关收件人的以下信息：
   ...
   * **[!UICONTROL 收件人角色]：** 选择收件人的角色。 用于商业和企业服务级别的Adobe Sign可以选择扩展 [协议收件人的角色](https://helpx.adobe.com/sign/using/set-up-signer-approver-roles.html)，不只是 **签名者**，以更好地匹配他们的工作流要求。

   * **[!UICONTROL 收件人电子邮件地址]：** 指定收件人的电子邮件地址。 收件人通过指定的电子邮件地址接收Adobe Sign协议。 您可以选择使用表单字段、登录用户的Experience Manager用户配置文件中提供的电子邮件地址，也可以手动输入电子邮件地址。 这是强制性步骤。

     >[!NOTE]
     >
     >确保第一个收件人或唯一收件人（如果只有一个收件人）的电子邮件地址与 [!DNL Adobe Sign] 用于配置AEM云服务的帐户。

   * **[!UICONTROL 收件人身份验证方法]：** 指定在打开Adobe Sign协议之前对收件人进行身份验证的方法。 您可以在电话、知识库、基于社交身份的身份验证和 [政府机关标识](https://helpx.adobe.com/sign/using/adobesign-authentication-government-id.html) 对象 [!DNL Adobe Acrobat Sign]. 对象 [!DNL Adobe Acrobat Sign for Government] 您可以在电话和基于知识的身份验证之间进行选择。
   ...
   * **[!DNL Adobe Sign]要填写或签名的字段：** 选择 [!DNL Adobe Sign] 收件人字段。 自适应表单可以有多个 [!DNL Adobe Sign] 字段。 您可以选择为收件人启用特定字段。 字段显示所有可用的 [!DNL Adobe Sign] 块。 选择块时，将选择块的所有字段。 您可以使用X图标取消选择字段。

   ![recipient-details](assets/signer-details.png)

   上图有两个例子 [!DNL Adobe Sign] 块：个人信息和办公室详细信息

   点按 ![保存](assets/save_icon.svg) 图标。 收件人已添加。

### 为自适应表单选择提交操作 {#selectsubmitactionforanadaptiveform}

之后，添加 [!DNL Adobe Sign] 自适应表单的字段，启用 [!DNL Adobe Sign] 在表单容器中，选择 [!DNL Adobe Sign] Cloud Service并添加Adobe Sign协议收件人，然后为自适应表单选择适当的提交操作。 有关自适应Forms提交操作的详细信息，请参阅 [配置提交操作](configuring-submit-actions.md).

表单的签署和提交是相互独立的。 在用户提交表单后创建Adobe Sign协议后，自适应表单会立即提交。 [!DNL AEM Forms] as a Cloud Service不会等待收件人签名或完成其他操作以提交自适应表单。 用户单击“提交”按钮或“摘要”步骤显示表单摘要后，就会立即提交表单。

此外， [!DNL Adobe Sign] 已启用自适应表单嵌入Adobe Sign协议ID以提交数据。 您可以使用协议ID来跟踪使用自定义代码的协议状态（需要自定义实施）。

Adobe Sign协议ID (agreementId)包含在自适应表单的提交数据中。 默认情况下，协议ID出现在 `afSubmissionInfo` 已提交数据的节点。
...
您的表单签名体验已准备就绪。 您可以预览表单以验证签名体验。 在已发布的表单上， [!DNL Adobe Sign] 当收件人收到通过电子邮件签名的表单时，会显示块字段。 当 **[!UICONTROL 何时收件人与填写表单的人相同？]** 选项标记为“是”且满足条件，则在提交后用户将被重定向到Adobe Sign协议，并且用户可以立即签署文档，而不是等待协议显示在电子邮件中。

## 为自适应表单配置云签名 {#configure-cloud-signatures-for-an-adaptive-form}

基于云的数字签名或远程签名是新一代的数字签名，可在桌面、移动设备和Web上使用，满足收件人身份验证的最高合规性和保证要求。 您可以使用基于云的数字签名对自适应表单进行签名。

晚于 [编辑Adobe Sign的自适应表单属性](working-with-adobe-sign.md#enableadobesign)，执行以下步骤以将云签名字段添加到自适应表单：
...
1. 点按 **[!UICONTROL Adobe Sign字段]** ![Adobe Sign](assets/adobesign.png) 图标。 它会显示要选择和添加的选项 [!DNL Adobe Sign] 字段。
   ...
   展开 **[!UICONTROL 类型]** 要选择的下拉字段 **[!UICONTROL 数字签名]** 然后点按 **[!UICONTROL 完成]** 图标以将所选字段添加到 [!DNL Adobe Sign] 块。
...
此 **[!UICONTROL 摘要步骤]** 组件会自动提交表单，在自定义的“摘要”页面中填充信息，并显示已提交表单的摘要。 摘要步骤组件占据可用于表单的全部宽度。 建议在包含摘要步骤组件的部分中没有任何其他组件。

## 常见问题 {#frequently-asked-questions}

**问：** 您可以将自适应表单嵌入到其他自适应表单中。 嵌入式自适应表单是否可以 [!DNL Adobe Sign] 已启用？
**Ans：** 否，Experience Manager Forms不支持使用嵌入 [!DNL Adobe Sign] 为签名启用自适应表单
...
**Ans：** 可以，您可以在文本组件中使用文本标记来添加 [!DNL Adobe Sign] 字段到已启用记录文档的自适应表单（仅限自动生成的记录文档选项）。 要了解创建文本标记的过程和规则，请参阅 [Adobe Sign文档](https://helpx.adobe.com/sign/using/text-tag.html). 另请注意，自适应Forms对文本标记的支持有限。 您可以使用文本标记仅创建满足以下条件的字段 [Adobe Sign块](working-with-adobe-sign.md#configure-cloud-signatures-for-an-adaptive-form) 支持。
...
**问题**
时间 [!DNL Adobe Sign] 服务是为自适应表单配置的，该服务无法创建 [!DNL Adobe Sign] 基础自适应表单的协议。
...
* 如果您使用多个 [!DNL Adobe Sign] Cloud Services，指向 **[!UICONTROL oAuth URL]** 所有服务中的相同项 **[!UICONTROL Adobe Sign Shard]**.

* 使用单独的电子邮件地址进行配置 [!DNL Adobe Sign] 第一个或单个收件人的帐户和。 第一个收件人或唯一收件人（如果有单个收件人）的电子邮件地址不能与 [!DNL Adobe Sign] 用于配置AEM云服务的帐户。
