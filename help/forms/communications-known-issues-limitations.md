...
存在两种类型的®字体。 一种类型是PCL支持的TrueType ®字体。 另一个是CFF®。 PDF和PostScript输出支持嵌入的Type-1、TrueType和OpenType®字体。 PCL输出支持嵌入的TrueType字体。

Type-1和®字体未嵌入到PCL输出中。 使用Type-1和®字体格式化的内容将被栅格化，并生成为位图图像，该图像可能会大而慢，无法生成。
