...
不包含XFA流的PDF文档无法呈现为PostScript、PCL或ZPL。 通信API可以将具有XFA流(即在Designer中创建的表单)的PDF文档渲染为激光和标签格式。 如果PDF文档已经过签名、认证或包含使用权限(使用AEM Forms Reader扩展服务应用)，则无法呈现这些打印格式。
...
存在两种类型的OpenType®字体。 一种类型是PCL支持的TrueType OpenType®字体。 另一个是CFFOpenType®。 PDF和PostScript输出支持嵌入的Type-1、TrueType和OpenType®字体。 PCL输出支持嵌入的TrueType字体。

Type-1和OpenType®字体未嵌入到PCL输出中。 使用Type-1和OpenType®字体格式化的内容将被栅格化，并生成为位图图像，该图像可能会大而慢，无法生成。
...
| XCI选项 | 描述 |
| ------------------------------------| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| config/present/pdf/creator | 使用文档信息词典中的创建者条目标识文档创建者。 有关此词典的信息，请参阅PDF参考指南。 |
| config/present/pdf/producer | 使用文档信息词典中的制作者条目标识文档制作者。 有关此词典的信息，请参阅PDF参考指南。 |
...
* 在打印选项列表中，您只能使用一次特定渲染类型(PDF、PRINT)。 例如，不能有两个PRINT选项，每个选项都指定PCL渲染类型。

* 对于批处理配置，只允许一个OutputType(PDF、PRINT)和RenderType(PostScript、PCL、IPL、ZPL等)值的组合实例。
...
目前，您可以使用Microsoft Azure Storage保存输入数据和生成的文档。 Microsoft Azure storage为[自动执行数据移动操作](https://docs.microsoft.com/en-us/azure/storage/common/storage-use-azcopy-v10)提供了各种选项。

**Microsoft Azure存储帐户是否包含在Experience Manager Forms Cloud Service许可证中？**

Microsoft Azure Storage帐户独立于Experience Manager Forms Cloud Service许可证。

**通信API是否在Experience Manager Forms Cloud Service服务器上存储数据？**

输入和输出数据仅保存在Microsoft Azure Storage中。

**通信API是否仅可用于Experience Manager Forms Cloud Service？ 能否在内部部署环境中获得类似的功能？**

您可以使用AEM Forms Output服务将模板(XFA或PDF)与客户数据相结合，生成PDF、PS、PCL和ZPL格式的文档。

与内部部署环境相比，Cloud Service提供了自动扩展和成本效益方面的更多优势。
