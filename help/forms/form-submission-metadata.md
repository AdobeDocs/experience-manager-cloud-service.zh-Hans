...
1. 在编辑模式下，选择一个组件，然后点按 ![字段级](assets/select_parent_icon.svg) > **[!DNL Adaptive Form Container]**，然后点按 ![cmppr](assets/configure-icon.svg).
1. 在侧栏中，单击 **[!DNL Metadata]**.
1. 在元数据部分中，单击 **[!DNL Add]**.
1. 使用“元数据”选项卡的“值”字段可添加脚本。 您添加的脚本从表单上的元素中收集数据，并计算提供给元数据的值。

   例如， **[!DNL true]** 如果输入的年龄大于21岁，则记录在元数据中，并且 **[!DNL false]** 小于21时记录。 在“元数据”选项卡中输入以下脚本：
