...
1. 在创建表单数据模型对话框中：
   ...
   * (**仅在配置了数据源时可选且适用**)点按 **[!UICONTROL 数据源配置]** 字段，然后选择云服务所在的配置节点。 它将下一页可供选择的数据源列表限制为选定配置节点中可用的数据源列表。 但是，任何JDBC数据库和 [!DNL Experience Manager] 默认情况下，会列出用户配置文件数据源。 如果不选择配置节点，则会列出来自所有配置节点的数据源。
