...
Forms as a Cloud Service — 通信提供了用于计划文档生成的按需和批处理API（异步API）：
...
* **基本身份验证**：基本身份验证是内置到HTTP协议中的简单身份验证方案。 客户端发送的HTTP请求具有Authorization标头，该标头包含单词Basic，后跟空格和base64编码的字符串用户名:password。 例如，若要授权为管理员/管理员，客户端将发送基本[base64编码的字符串用户名]： [base64编码的字符串密码]。
...
* **基于令牌的身份验证：**&#x200B;基于令牌的身份验证使用访问令牌（持有者身份验证令牌）向Experience Manager as a Cloud Service发出请求。 AEM Forms as a Cloud Service提供API以安全检索访问令牌。 要检索并使用令牌对请求进行身份验证，请执行以下操作：
  ...
  >[!IMPORTANT]
  >
  > 有关详细信息，请参阅[OAuth服务器到服务器身份验证](/help/forms/oauth-api-authetication.md)和[JWT服务器到服务器身份验证](/help/forms/jwt-api-authentication.md)。
<!-- 
...
### （仅适用于文档生成API）将模板和其他资源上传到Experience Manager实例
...
>[!NOTE]
>
> 了解调用AEM Forms Communication API的详细步骤，请参阅[使用OAuth服务器到服务器身份验证调用AEM Forms Communications API](/help/forms/aem-forms-cloud-service-communications-on-demand-processing.md)文章。
