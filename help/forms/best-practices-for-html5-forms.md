...
# HTML5 表单的最佳做法{#best-practices-for-html-forms}
...
* 启用Web压缩。 它减小了源自表单的请求和响应的大小。<!-- For details, see [Performance tuning of AEM Forms Server](https://helpx.adobe.com/cn/aem-forms/6-3/performance-tuning-aem-forms.html)-->
...
* 评估是否将大型表单拆分为多个表单。 您还可以使用表单集将所有较小的表单组合在一起，并将它们作为单个单元呈现。 表单集仅加载所需的表单。 此外，在表单集中，您可以配置不同表单中的常用字段以共享数据绑定。 数据绑定可帮助用户仅填写一次常用信息；这些信息会在后续表单中自动填写，从而显着提升性能。<!-- For more details about form sets, see [Form set in AEM forms](https://helpx.adobe.com/cn/aem-forms/6-3/formset-in-aem-forms.html).-->
...
* 如果上述建议没有产生太多改进，请评估将数据驱动表单拆分为表单集。 例如，如果表的行数超过1000，则每隔100行移至另一个表单。 这有助于改善表单的加载时间和性能。  另请注意，表单集为所有表单生成合并的提交XML。 要区分每个表单的数据，请使用不同的数据根。<!--For more information, see [Form set in AEM Forms](https://helpx.adobe.com/cn/aem-forms/6-3/formset-in-aem-forms.html).-->
