...
# 将AEM Forms与集成 [!DNL Adobe Analytics] {#integrate-aem-forms-with-adobe-analytics}
...
<span class="preview"> 本文档概述了在自适应表单上启用Adobe Analytics的手动过程。 但是，Adobe建议使用 [使用Experience Cloud设置自动为自适应表单启用Adobe Analytics](/help/forms/enable-adobe-analytics-adaptive-form-using-experience-cloud-setup-automation.md). </span>
...
除了这些开箱即用的事件之外，您还可以使用规则编辑器在自适应表单中定义自定义事件，并将这些事件映射到中的事件。 [!DNL Adobe Analytics]
...
1. 在 **[!UICONTROL Prop]** 部分，将下拉列表中可用的报表包选项映射到使用定义的数据元素 [配置数据元素](#configure-data-elements).
   ...
   1. 在 **[!UICONTROL Prop]** 部分，为报表包中可用的表单标题选择一个prop ，然后选择 ![“数据库”图标](assets/database-icon.svg) 以将其映射到在中创建的表单标题 [配置数据元素](#configure-data-elements).
...
1. 选择 **[!UICONTROL 清除变量]** 作为操作类型。 选择 **[!UICONTROL 保留更改]**. 执行这些步骤后， **[!UICONTROL 操作]** 部分显示为：
   ...
   自定义 **[!UICONTROL 操作]** 部分。 例如，您可以定义两个 **发送信标** 操作流中将数据发送到的步骤 [!DNL Adobe Analytics] 并将其视为一个步骤中的页面查看，并将数据发送到 [!DNL Adobe Analytics] 并且不要将其视为第二步中的页面查看。
...
在创建Adobe Launch配置之前，请创建 [将AdobeLaunch用作云解决方案的Adobe IMS配置](https://experienceleague.adobe.com/docs/experience-manager-learn/sites/integrations/experience-platform-launch/connect-aem-launch-adobe-io.html).
...
要使用 [!DNL Adobe Launch] 现有自适应表单中的配置：
...
1. 选择 ![选择产品](assets/select-analytics.png) 并选择 **[!UICONTROL 分析]**.
...
1. 选择 ![可视化图表](assets/visualization-icon.svg) 并将图表类型拖放到“自由格式”部分。 同样，可以向自由格式部分添加多个图表类型。
