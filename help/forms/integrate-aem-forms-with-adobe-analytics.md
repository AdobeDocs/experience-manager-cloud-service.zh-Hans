...
1. 在 **[!UICONTROL Prop]** 部分，将下拉列表中可用的报表包选项映射到使用 [配置数据元素](#configure-data-elements).
   ...
   1. 在 **[!UICONTROL Prop]** ，为报表包中可用的表单标题选择一个属性，然后点按 ![“数据库”图标](assets/database-icon.svg) 将其映射到在中创建的表单标题 [配置数据元素](#configure-data-elements).
...
1. 点按字段并选择 ![规则编辑器](assets/rule-editor-icon.svg) 打开规则编辑器页面。
...
1. 点按 ![选择产品](assets/select-analytics.png) 选择 **[!UICONTROL Analytics]**.
...
1. 点按 ![可视化图表](assets/visualization-icon.svg) 并将图表类型拖放到自由格式部分。 同样，您也可以向自由格式部分添加多个图表类型。
