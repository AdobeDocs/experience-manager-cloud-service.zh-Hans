...
规则是操作和条件的组合。 在规则编辑者中，操作包括一些活动，如隐藏、显示、启用、禁用或计算表单中某个对象的值。 条件是对表单对象的状态、值或属性执行检查和操作而计算的布尔表达式。 根据评估条件返回的值（ `True` 或 `False` ）执行操作。
...
编写When规则时，可以触发Clear Value Of操作。 清除值操作清除指定对象的值。 在 in 语句中具有明确的值作为选项，可让您使用多个字段创建复杂的条件。
