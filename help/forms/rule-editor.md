...
规则编辑者功能使表单业务用户和开发人员能够在自适应表单对象上编写规则。 这些规则定义基于预设条件、用户输入和表单上的用户操作触发表单对象的操作。 它有助于进一步简化表单填充体验确保准确性和速度。

规则编辑者提供了一个直观的简化用户界面来编写规则。 规则编辑者为所有用户提供了一个可视化编辑者。<!-- In addition, only for forms power users, rule editor provides a code editor to write rules and scripts. --> 使用规则可对自适应表单对象执行的一些关键操作包括：
...
* **已更改（事件）：** 当用户更改表单对象的输入值或选定选项时，返回 true。
* **导航（事件）：** 当用户单击导航对象时，返回 true。 导航对象用于在面板之间移动。
...
规则编辑者提供一组可用于编写规则的预定义规则类型。 我们来详细了解一下每个规则类型。 有关在规则编辑者中编写规则的详细信息，请参阅 [ 编写规则 ](rule-editor.md#p-write-rules-p) 。

### [!UICONTROL 时间] {#whenruletype}

**[!UICONTROL 当]** 规则类型遵循 **条件-操作-替代操作** 规则构造，或者有时只是 **条件-操作** 构造。在此规则类型中，您首先指定评估的条件，并在条件得到满足（ `True` ）后执行一个要触发的操作。 使用When规则类型时，您可以使用多个AND和OR运算符来创建 [嵌套表达式](#nestedexpressions).
...
>[!NOTE]
>
>此外，您还可以使用 Set 值规则从表单数据模型服务或 web 服务的输出中填充下拉列表组件中的所有值。 但是，请确保您选择的输出参数为数组类型。 数组中返回的所有值都可在指定的下拉式列表中使用。

### [!UICONTROL 显示] {#show}

**[!UICONTROL 使用显示]** 规则类型，您可以编写一个规则，以根据是否满足条件来显示或隐藏表单对象。如果条件未满足或返回 `False` ，则显示规则类型也会触发隐藏操作。
