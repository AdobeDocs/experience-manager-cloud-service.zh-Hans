...
description: 自适应Forms规则编辑器允许您添加动态行为并将复杂的逻辑构建到表单中，而无需编码或编写脚本。 开始了解选择规则结构的规则和指南。 了解有关规则编辑器中可用运算符类型和事件的更多信息。
...
规则编辑器提供了直观、简化的用户界面来编写规则。 规则编辑器为所有用户提供了一个可视编辑器。<!-- In addition, only for forms power users, rule editor provides a code editor to write rules and scripts. --> 可使用规则对自适应表单对象执行的一些关键操作包括：
...
* 当用户更改应用了时间规则的对象的值时，将评估When规则中的条件。 但是，如果您希望操作在服务器端更改时触发（如预填充值），建议编写一个When规则，以在字段初始化时触发操作。
...
* **Submission(event)出错：**  向表单数据模型提交数据失败时返回true。
...
此 **[!UICONTROL 时间]** 规则类型遵循 **condition-action-alternate action** 规则结构，或者有时只是 **condition-action** 构造。 在此规则类型中，您首先为求值指定条件，然后指定满足该条件时要触发的操作( `True`)。 使用When规则类型时，您可以使用多个AND和OR运算符来创建 [嵌套表达式](#nestedexpressions).
...
**[!UICONTROL 调用服务]** 调用在表单数据模型中配置的服务。 选择“调用服务”操作时，将显示一个字段。 点按该字段，它会显示您在上的所有表单数据模型中配置的所有服务。 [!DNL Experience Manager] 实例。 在选择表单数据模型服务时，会显示更多字段，您可以在其中使用指定服务的输入和输出参数映射表单对象。 请参阅调用表单数据模型服务的示例规则。
...
**[!UICONTROL 设置值]** 计算并设置指定对象的值。 您可以将对象值设置为字符串、另一个对象的值、使用数学表达式或函数的计算值、对象的属性值或来自已配置表单数据模型服务的输出值。 选择Web服务选项后，它将显示在上的所有表单数据模型中配置的所有服务。 [!DNL Experience Manager] 实例。 在选择表单数据模型服务时，会显示更多字段，您可以在其中使用指定服务的输入和输出参数映射表单对象。

有关在表单数据模型中配置服务的更多信息，请参阅 [[!DNL Experience Manager Forms] 数据集成](data-integration.md).

此 **[!UICONTROL 设置属性]** 规则类型允许您根据条件操作设置指定对象的属性值。

它使您能够定义规则以动态地将复选框添加到自适应表单。 您可以使用自定义函数、表单对象或对象属性来定义规则。

![设置属性](assets/set_property_rule_new.png)

要根据自定义函数定义规则，请选择 **[!UICONTROL 函数输出]** ，然后从下拉列表中拖放自定义函数 **[!UICONTROL 函数]** 选项卡。 如果满足条件操作，则自定义函数中定义的复选框数将添加到自适应表单中。
...
此 **设置值** 规则类型不适用于所有表单对象，例如面板和工具栏按钮。 标准“设置值”规则具有以下结构：
...
使用 **[!UICONTROL 显示]** 规则类型，您可以根据是否满足条件来编写显示或隐藏表单对象的规则。 Show rule类型也会在条件不满足或返回时触发Hide操作 `False`.
...
与显示规则类型类似，您可以使用 **[!UICONTROL 隐藏]** 规则类型，根据是否满足条件来显示或隐藏表单对象。 Hide规则类型还会触发Show操作，以防条件未得到满足或返回 `False`.
...
此 **[!UICONTROL 启用]** 规则类型允许您根据是否满足条件来启用或禁用表单对象。 Enable规则类型还会触发Disable操作，以防条件未得到满足或返回 `False`.
...
与启用规则类型类似， **[!UICONTROL 禁用]** 规则类型允许您根据是否满足条件来启用或禁用表单对象。 Disable规则类型还会触发Enable操作，以防条件不满足或返回 `False`.
...
此 **[!UICONTROL 验证]** 规则类型使用表达式验证字段中的值。 例如，您可以编写一个表达式，以检查用于指定名称的文本框是否不包含特殊字符或数字。
...
要根据自定义函数定义规则，请选择 **[!UICONTROL 函数输出]** ，然后从下拉列表中拖放自定义函数 **[!UICONTROL 函数]** 选项卡。 自定义函数中定义的复选框数会添加到自适应表单中。
...
1. 选择数据模型对象。
1. 从中选择数据模型对象属性 **[!UICONTROL 显示值]** 下拉列表。 自适应表单中的复选框数是从数据库中为该属性定义的实例数派生而来的。
...
有关使用可视规则编辑器的更多信息，请参见 [写入规则](rule-editor.md#p-write-rules-p).
<!-- 
...
此 **[!UICONTROL 完成]** 按钮用于保存规则。 您可以保存不完整的规则。 但是，不完整的内容无效，因此不会运行。 当您下次从同一表单对象启动规则编辑器时，会列出表单对象上保存的规则。 您可以在该视图中管理现有规则。 有关更多信息，请参阅 [管理规则](rule-editor.md#p-manage-rules-p).

此 **[!UICONTROL 取消]** 按钮会放弃对规则所做的任何更改并关闭规则编辑器。

## 写入规则 {#write-rules}

您可以使用可视规则编辑器编写规则 &lt;!> — 或代码编辑器>。 首次启动规则编辑器时，它将在可视编辑器模式下打开。 您可以切换到代码编辑器模式并编写规则。 但是，如果在代码编辑器中编写或修改规则，则除非清除代码编辑器，否则无法切换到该规则的可视化编辑器。 当您下次启动规则编辑器时，它将在您上次用于创建规则的模式下打开。
...
1. 首先，根据用户为“婚姻状况”单选按钮选择的选项，编写规则以控制“配偶薪金”字段的可见性。
   ...
   在创作模式下打开贷款申请表单。 点按 **[!UICONTROL 婚姻状况]** 组件和点按 ![edit-rules](assets/edit-rules-icon.svg). 下一步，点按 **[!UICONTROL 创建]** 以启动规则编辑器。
...
1. 拖放 **[!UICONTROL 配偶薪金]** 字段。 **[!UICONTROL 拖放对象或在此选择]** 字段。 或者，点按 **[!UICONTROL 拖放对象或在此选择]** 字段并选择 **[!UICONTROL 配偶薪金]** 字段，其中列出了表单中的所有表单对象。
...
1. 接下来，编写规则以计算贷款资格金额（占总薪金的50%），并在“贷款资格”字段中显示。 要实现此结果，请创建 **[!UICONTROL 设置值]** “贷款资格”字段规则。

   在创作模式下，点按 **[!UICONTROL 贷款资格]** 字段并点按 ![edit-rules](assets/edit-rules-icon.svg). 下一步，点按 **[!UICONTROL 创建]** 以启动规则编辑器。
...
1. 在表达式字段中：
   ...
   * 从Forms对象选项卡中选择或拖放 **[!UICONTROL 薪金]** 第一个字段中的字段 **[!UICONTROL 拖放对象或在此选择]** 字段。

   * 选择 **[!UICONTROL 加号]** 从 **[!UICONTROL 选择运算符]** 字段。

   * 从Forms对象选项卡中选择或拖放 **[!UICONTROL 配偶薪金]** 字段中的其他字段 **[!UICONTROL 拖放对象或在此选择]** 字段。
...
1. 点按 **[!UICONTROL 添加条件]** 添加When语句。
   ...
   * 从Forms对象选项卡中选择或拖放 **[!UICONTROL 婚姻状况]** 第一个字段中的字段 **[!UICONTROL 拖放对象或在此选择]** 字段。

   * 选择 **[!UICONTROL 等于]** 从 **[!UICONTROL 选择运算符]** 字段。

   * 选择另一个中的字符串 **[!UICONTROL 拖放对象或在此选择]** 字段并指定 **[!UICONTROL 已婚]** 在 **[!UICONTROL 输入字符串]** 字段。

   规则编辑器中的规则最终如下所示。  ![write-rules-visual-editor-16](assets/write-rules-visual-editor-16.png)

1. 点按 **[!UICONTROL 完成]**. 它保存规则。

1. 重复步骤7至14，定义另一条规则，以计算婚姻状况为“单身”时的贷款资格。 规则编辑器中的规则如下所示。
...
在CRX存储库中添加客户端库后，在自适应表单中使用该库。 它允许您在表单中将自定义函数用作规则。 要在自适应表单中添加客户端库，请执行以下步骤：

1. 在编辑模式下打开表单。
要在编辑模式下打开表单，请选择表单并点按 **[!UICONTROL 打开]**.
1. 在编辑模式下，选择一个组件，然后点按 ![字段级](assets/select_parent_icon.svg) > **[!UICONTROL 自适应表单容器]**，然后点按 ![cmppr](assets/configure-icon.svg).
...
1. 点按 **[!UICONTROL 创建规则]**. 使用下面显示的选项，创建一个规则以在表单的“输出”字段中保存输入的平方值。
...
当您点按表单对象并点按时，会列出该对象上的任何现有规则 ![edit-rules1](assets/edit-rules-icon.svg). 您可以查看标题并预览规则摘要。 此外，UI允许您展开和查看完整的规则摘要、更改规则的顺序、编辑规则以及删除规则。

![List-rules](assets/list-rules.png)

您可以对规则执行以下操作：

* **展开/折叠**：规则列表中的“内容”列显示规则内容。 如果整个规则内容在默认视图中不可见，请点按 ![expand-rule-content](assets/Smock_ChevronDown.svg) 以扩展它。

* **重新排序**：您创建的任何新规则都将栈叠在规则列表的底部。 规则将从上到下执行。 顶部的规则先执行，然后是相同类型的其他规则。 例如，如果您分别将When、Show、Enable和When规则放在顶部的第一、第二、第三和第四个位置，则顶部的当When规则将首先执行，紧接着在第四个位置执行When规则。 然后，执行显示和启用规则。
...
* **启用/禁用**：当必须暂时暂停使用规则时，您可以选择一个或多个规则并点按 **[!UICONTROL 禁用]** 操作工具栏中的以禁用它们。 如果某个规则被禁用，则它不会在运行时执行。 要启用已禁用的规则，您可以选择该规则并点按操作工具栏中的启用。 规则的状态列显示规则是启用还是禁用。
...
1. 选择要将规则粘贴到的其他表单对象，然后点按 **[!UICONTROL 粘贴]**. 此外，您还可以编辑规则以对其进行更改。
...
如果用户填写的财产抵押日期是过去，则自适应表单会显示有关收入计算的说明。 以下规则将用户填写的日期与当前日期进行比较，如果用户填写的日期早于当前日期，则表单会显示文本消息（名为Income）。
...
以下是一个示例条件，如果申请人在当前地址停留的月数小于36，则该条件会显示静态文本对象。

![数字比较条件](assets/numbercomparisoncondition.png)

当用户表示在当前居住地址居住不到36个月时，表格显示可以请求更多居住证明的通知。
...
* 一个单选按钮， **[!UICONTROL 您是否为Geometrixx现有客户？]**，它提供 [!UICONTROL 是] 和 [!UICONTROL 否] 选项。 “是”的值为 **0** “否”为 **1**.

* 一个文本字段， **[!UICONTROL Geometrixx客户ID]**，以指定客户ID。
...
在采购订单表单中，您有下表，用户在该表中填写其订单。 在此表中：
...
现在，您要为所有产品在“产品数量”列中添加指定数量，并在“总数量”单元格中显示总和。 可通过在“总数量”单元格中编写“设置值”规则来实现此总和，如下所示。
...
在上一个示例中说明的采购订单表单中，您需要限制用户订购任何数量超过该10000价的产品。 要执行此验证，您可以编写验证规则，如下所示。
