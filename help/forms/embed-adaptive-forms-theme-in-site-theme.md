...
* 为站点主题配置了前端管道&#x200B;**的** AEM as a Cloud Service[&#128279;](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/developing/developing-with-front-end-pipelines.html?lang=zh-Hans)。
...
在Forms主题中，组件SCSS文件通常引用路径为`./resources/`或`url(resources/`的图像。 迁移到`theme/src/components/adaptiveform/<component>/`后，这些路径必须向上指向一个级别`adaptiveform/resources/`。
...
在替换之后，这两个对象将分别变为`url(../resources/images/...)`和`url('../resources/images/...')`。 对`adaptiveform/`下引用图像（按钮、可折叠项、向导、容器、涂写等）的每个SCSS文件重复此操作。
...
2. 通过现有[前端管道](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/developing/developing-with-front-end-pipelines.html?lang=zh-Hans)部署。 部署后，相同主题CSS将应用于站点页面和嵌入的自适应Forms。

## 疑难解答 {#troubleshooting}

| 问题 | 检查内容 |
|-------|-------------------------------|
| 生成失败：图像的“找不到文件” | 将所有表单图像放入`theme/src/components/adaptiveform/resources/images/`。 在`.scss`下的每`adaptiveform/`中，将`../resources/` （和`url(../resources/`）用于图像路径，而不是`./resources/`。 如果您的捆绑包解析来自`theme/src/`的路径，请将图像放入`theme/src/resources/images/`并在SCSS中使用`resources/images/` （无`../`）。 |
...
* [使用前端管道开发](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/developing/developing-with-front-end-pipelines.html?lang=zh-Hans)