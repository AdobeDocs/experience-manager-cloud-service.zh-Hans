...
<!-- When you sign up for the service, Adobe creates an Organization identifier for your company in the Adobe Identity Management System (IMS), where your users and their permissions can be managed. So, --> 在加入Adobe Experience Manager (AEM) Forms as a Cloud Service环境之前，请决定角色并为您的项目构建团队。 典型的[!DNL AEM Forms]项目团队具有以下角色：

* **用户体验(UX) Designer**：用户体验(UX) Designer定义[!DNL AEM Forms]资源的样式、布局和品牌化。

* **Forms从业者**：Forms从业者根据UX Designer提供的样式、布局和品牌创建自适应Forms、主题和模板。 该从业人员还创建自适应表单并将其与表单数据模型(FDM)和AEM工作流集成。 Forms从业人员通常执行前端相关任务。
...
完成服务新用户引导后，请登录到[!DNL AEM Forms] as a Cloud Service环境，打开创作和发布实例，并根据用户角色将其添加到特定于Forms的[AEM组](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/accessing/aem-users-groups-and-permissions.html?lang=zh-Hans#accessing)。 下表列出了特定于Forms的AEM组（开箱即用）以及相应的用户类型。 该表还提供了每种用户类型的AEM实例类型：
...
| 最终用户 | <ul> <li>当用户必须登录才能查看并提交自适应表单时，请将此类用户添加到[!DNL forms-users]组。 </li> <li>当访问自适应Forms不需要用户身份验证时，不要向此类用户分配任何组。 </li> </ul> | 创作和发布实例 |
...
[设置本地开发环境](setup-local-development-environment.md)。 您可以使用本地开发环境创建自适应表单和相关资产（主题、模板、自定义提交操作、预填充服务等）。 并且，无需登录到云开发环境，[将PDF forms转换为自适应Forms](https://experienceleague.adobe.com/docs/aem-forms-automated-conversion-service/using/introduction.html?lang=zh-Hans)。
