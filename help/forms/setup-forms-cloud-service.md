...
完成服务入门后，请登录到[!DNL AEM Forms]as a Cloud Service环境，打开Author和Publish实例，并根据用户的角色将用户添加到特定于Forms的[AEM组](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/accessing/aem-users-groups-and-permissions.html#accessing)。 下表列出了特定于Forms的AEM组（现成可用）以及相应的用户类型。 该表还提供了每种用户类型的AEM实例类型：
