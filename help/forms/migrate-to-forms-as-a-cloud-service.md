---
title: 如何从AEM 6.5 Forms迁移到AEM Forms as a Cloud Service？
description: 迁移到历程快速入门| Adobe Experience Manager。 从 [!DNL AEM Forms] （内部部署和AMS环境）迁移到 [!DNL AEM Forms] as a Cloud Service环境。
...
| 版本 | 文章链接 |
| -------- | ---------------------------- |
| AEM 6.5 | [单击此处](https://experienceleague.adobe.com/docs/experience-manager-65/forms/upgrade-aem-forms/upgrade.html?lang=zh-Hans) |
...
让我们开始迁移到AEM as a Cloud Service | Adobe Experience Manager。 运行Migration Utility时，将更新以下资产：
...
## 迁移到Forms as a Cloud Service的注意事项 {#consideration}
...
* （仅适用于AEM 6.5 Forms之前的版本）基于AEM 6.3 Forms中提供的现成模板和主题的自适应Forms或以前的版本在[!DNL AEM Forms] as a Cloud Service上不受支持。

* 与Adobe Experience Manager Forms 6.5 Forms（内部部署和Adobe托管服务）环境相比，Adobe Experience Manager as a Cloud Service对现有功能进行了一些显着更改。 在继续迁移到服务之前，[了解这些可注释的更改](notable-changes.md)和[功能级别差异](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/migration-journey/cloud-migration/best-practices-analyzer/using-best-practices-analyzer.html?lang=zh-Hans#viewing-report)，以决定根据组织所需的功能进行迁移。
...
* 为您的Forms Cloud Service程序启用[Forms — 数字注册](https://experienceleague.adobe.com/docs/experience-manager-cloud-manager/using/getting-started/setting-up-program.html?lang=zh-Hans&#editing-program)选项，并[运行管道](https://experienceleague.adobe.com/docs/experience-manager-cloud-manager/using/how-to-use/deploying-code.html?lang=zh-Hans)。

  ![练习结果](assets/enable-add-on.png)

* 在Cloud Service环境中，迁移实用程序可与内容传输工具配合使用。 迁移实用程序使[!DNL AEM Forms]资源与Cloud Service兼容，内容传输工具将内容从[!DNL AEM Forms]环境迁移到[!DNL AEM] as a Cloud Service环境。 在使用迁移实用程序之前，请了解[移至AEM as a Cloud Service](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/moving/home.html?lang=zh-Hans)的过程。 该流程使用以下工具：
   * [内容传输工具](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/moving/cloud-migration/content-transfer-tool/overview-content-transfer-tool.html?lang=zh-Hans&#cloud-migration)：内容传输工具可帮助您准备内容并将内容从现有环境传输到Cloud Service环境。 它有助于用户轻松从AEM Forms升级到云环境。
* 在[!DNL AEM Forms] as a Cloud Service和您的本地[!DNL AEM Forms]环境中具有管理员权限的帐户。
* 从[!DNL AEM Forms]软件分发门户[下载并安装Best Practice Analyzer、内容传输工具和](https://experience.adobe.com/#/downloads/content/software-distribution/en/aemcloud.html)迁移实用程序。

* 运行[Best Practices Analyzer](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/moving/cloud-migration/best-practices-analyzer/overview-best-practices-analyzer.html?lang=zh-Hans#cloud-migration)工具并修复报告的问题。 有关从Adobe Experience Manager Forms迁移到Adobe Experience Manager Forms as a Cloud Service时可能出现的问题，请参阅[适用于Forms as a Cloud Service的AEM模式检测](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/migration-journey/cloud-migration/best-practices-analyzer/using-best-practices-analyzer.html?lang=zh-Hans#viewing-report)。


<!-- * Download the latest [compatibility package](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases.html?lang=zh-Hans#aem-65-forms-releases) for your [!DNL AEM Forms] version. -->




## 将[!DNL AEM 6.5 Forms]资产迁移到AEM Cloud Service {#use-the-migration-utility}

执行以下步骤以使您的[!DNL AEM Forms]资源与Cloud Service兼容，并将它们传输到[!DNL AEM] as a Cloud Service环境。

1. 创建现有[环境的](https://experienceleaguecommunities.adobe.com/t5/adobe-experience-manager/correct-method-to-clone-the-aem-environment/qaq-p/363487?profile.language=zh-Hans)克隆[!DNL AEM Forms]。
...
1. 在克隆的环境中从[软件分发门户](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/moving/cloud-migration/content-transfer-tool/overview-content-transfer-tool.html?lang=zh-Hans&#cloud-migration)下载并安装[!DNL AEM Forms]内容传输工具[和](https://experience.adobe.com/#/downloads/content/software-distribution/en/aemcloud.html)as a Cloud Service迁移实用程序。 您可以使用AEM包管理器来安装该工具和实用程序。
...
1. 逐个使用选项以使您的[!DNL AEM Forms]资源与[!DNL AEM] as a Cloud Service兼容：
   ...
   AEM Form迁移到Cloud Service，请使用[包管理器](https://experienceleague.adobe.com/docs/experience-manager-65/administering/contentmanagement/package-manager.html?lang=zh-Hans#contentmanagement)将可重用的函数（客户端库）导出到包。

1. [将](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/deploying/overview.html?lang=zh-Hans#deploying-content-packages-via-cloud-manager-and-package-manager)可重用的函数（客户端库）包、[自定义代码、组件、配置](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/cloud-manager/devops/deploy-code.html?lang=zh-Hans#cloud-manager)以及特定于自定义区域设置的库部署到您的[!DNL AEM] as a Cloud Service环境。

   <!-- 1. Install the latest [Compatibility Package](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/moving/cloud-migration/content-transfer-tool/overview-content-transfer-tool.html?lang=zh-Hans&#cloud-migration) to your cloned [!DNL AEM Forms] environment. -->

1. 运行[内容传输工具](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/moving/cloud-migration/content-transfer-tool/overview-content-transfer-tool.html?lang=zh-Hans&#cloud-migration)。 在&#x200B;**[!UICONTROL 创建迁移集]**&#x200B;屏幕上指定参数时，请将自适应Forms、主题、模板、表单数据模型(FDM)、Cloud Services、自定义组件和其他特定于AEM Forms的资源路径指定为&#x200B;**[!UICONTROL 要包含的路径]**&#x200B;选项。 它将指定的[!DNL AEM Forms]资源添加到迁移集。
...
* **自适应表单主题和客户端库**：主题的默认路径为` /content/dam/formsanddocuments-themes/`，客户端库的默认路径为`/etc/clientlibs/fd/theme`。 例如，对于标题为WKND主题的模板，在` /content/dam/formsanddocuments-themes/wkndtheme`处为主题添加路径`/etc/clientlibs/reference-themes/wkndtheme-3-0`和客户端库。 您还可以在其他自定义路径拥有主题和客户端库。
...
您可以添加下面列出的顶级文件夹路径或如下所述的特定文件夹路径。 当您从AEM forms 6.5升级到云服务时，它使您能够一次迁移特定资源以及所有资源和表单。
...
将AEM工作流模型从AEM Forms 6.5迁移到Cloud Service时，请指定以下路径：
...
* [对现有Adobe Experience Manager 6.5 Forms用户的重要更改](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/forms-overview/notable-changes.html?lang=zh-Hans)
* [加入AEM Forms as a Cloud Service](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/setup-configure-migrate/setup-forms-cloud-service.html?lang=zh-Hans)
* [在Cloud Service上创建您的第一个自适应表单](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/adaptive-forms-authoring/authoring-adaptive-forms-foundation-components/create-an-adaptive-form-on-forms-cs/creating-adaptive-form.html?lang=zh-Hans)

## 附加信息

迁移实用程序可帮助您迁移基于基础组件的自适应Forms。 此外，Forms as a Cloud Service支持自适应Forms核心组件。 因此，您可以：
...
要了解有关AEM Forms as a Cloud Service的更多信息，请参阅：

* [AEM Forms Cloud Service简介](/help/forms/home.md)
* [AEM Forms Cloud Service中的创新](/help/forms/latest-innovations.md)