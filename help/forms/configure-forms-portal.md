...
* 在自定义布局中列出表单。 开箱即用地提供列表视图和卡片视图布局。 您可以创建自己的自定义布局。
...
## 导入Forms门户组件 {#import-forms-portal-components-aem-archetype}
...
1. **创建 [!DNL Experience Manager Forms] as a [Cloud Service] 项目：** 创建 [!DNL Experience Manager Forms] as a [Cloud Service] 项目基于 [AEM原型27](https://github.com/adobe/aem-project-archetype/releases/tag/aem-project-archetype-27) 或更高版本。 原型可帮助开发人员轻松开始开发 [!DNL AEM Forms] as a Cloud Service。 它还包含一些示例主题和模板，可帮助您快速入门。
   ...
   项目准备就绪后，更新 `<core.forms.components.version>x.y.z</core.forms.components.version>` 属性位于顶级 `pom.xml` 原型项目的URL来反映最新版本的 [core-forms-components](https://github.com/adobe/aem-core-forms-components) 在您的 `AEM Archetype` 项目。
...
[[!DNL Experience Manager Forms] 数据集成](data-integration.md) 提供 [!DNL Azure] 用于集成表单的存储配置 [!DNL Azure] 存储服务。 表单数据模型可用于创建与交互的自适应Forms [!DNL Azure] 服务器启用业务工作流。
...
要在Adobe Experience Manager (AEM)站点中使用任何核心组件（包括现成的门户组件），您必须创建一个代理组件并为您的站点启用它。 有关创建代理组件和启用门户组件的信息，请参阅 [使用核心组件](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/get-started/using.html?lang=en#create-proxy-components).
...
要添加组件，请将组件从“组件”窗格拖放到页面上的布局容器中，或点按布局容器上的添加图标，然后从 [!UICONTROL 插入新组件] 对话框。

### 配置草稿和提交组件 {#configure-drafts-submissions-component}

草稿和提交组件显示另存为草稿的表单，以供以后填写和提交表单。 要配置，请点按组件，然后点按 ![“配置”图标](assets/configure_icon.png). 在 [!UICONTROL 草稿和提交] 对话框，请指定标题以将表单列表指示为草稿或已提交的表单。 另选择组件应采用卡片或列表格式列出草稿表单还是提交的表单。
...
1. 在 [!UICONTROL 显示] 选项卡，配置以下内容：
   * In **[!UICONTROL 标题]**，指定Search &amp; Lister组件的标题。 指示性标题使用户能够在表单列表中执行快速搜索。
   * 从 **[!UICONTROL 版面]** 列表时，选择布局以卡片或列表格式表示表单。
   * 选择 **[!UICONTROL 隐藏搜索]** 和 **[!UICONTROL 隐藏排序]** 隐藏搜索并按功能排序。
   * In **[!UICONTROL 工具提示]**，提供将鼠标悬停在该组件上时显示的工具提示。
1. 在 [!UICONTROL 资产文件夹] 选项卡，指定从中提取表单并将其列在页面上的位置。 您可以配置多个文件夹位置。
1. 在 [!UICONTROL 结果] 选项卡，配置每页显示的最大表单数。 默认为每页八个表单。
...
1. 在 [!UICONTROL 显示] 选项卡，提供链接标题和工具提示，以便于识别链接表示的表单。
...
1. 在创作实例中，在编辑模式下打开自适应表单。
1. 从左窗格中，点按属性图标，然后展开 **[!UICONTROL 电子签名]** 选项。
1. 选择 **[!UICONTROL 启用Adobe Sign]**. 将显示各种配置选项。
1. 在 [!UICONTROL 提交表单] 部分，选择 **[!UICONTROL 在每个收件人完成签名仪式后]** 用于配置“提交表单”操作的选项，其中表单将首先发送给所有收件人进行签名。 所有收件人签署表单后，才提交表单。
...
* 启用自动保存功能，该功能将表单保存为指定的事件或配置的时间间隔。
...
1. 在 [!UICONTROL 时间] 部分，选择“已单击”，然后在 [!UICONTROL 则] 部分，选择“保存表单”选项。
...
* In [!UICONTROL 触发器]，配置以根据事件的发生或在特定时间间隔后触发自动保存。
