...
1. [!DNL AEM Forms]在 https:// [*hostname*] 处登录创作实例： [*端口*] /aem/forms.html。
1. 按 **一下创建 > 自适应表单片段** 。
1. 指定片段的标题、名称、描述和标记。

   >[!NOTE]
   >
   >请确保为片段指定唯一的名称。 如果已存在具有相同名称的其他片段，则无法创建片段。
...
1. 单击&#x200B;**确定**。
   ...
   该面板将保存在存储库中的指定位置或默认地点。 在自适应表单中，面板被替换为片段的快照。 如下所示，一般信息面板及其子面板、个人信息和地址都将保存为片段。

   要编辑片段，请单击 **[!UICONTROL 面板工具栏中的编辑资源]** 。 片段在编辑模式下的新选项卡或窗口中打开。
...
1. 按 **一下侧栏中 Assets** ![ Assets 浏览器 ](assets/assets-browser.png) 。 在 Assets 浏览器中，从下拉列表中选择 **自适应表单片段** 。
...
要本地化包含自适应表单片段的自适应表单，您需要单独本地化片段和表单。 其目的是对片段进行一次本地化，然后在多个自适应 Forms 中重复使用。
