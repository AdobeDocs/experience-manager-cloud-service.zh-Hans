...
* FORMS-7483：AEM Forms JSON 架构解析器现在支持 JSON 架构 (2020-12)。
...
* SITES-20149： RTC： `[cq-wcm-launches-core]`为CF导出启动项的新API。
...
* SITES-20342： `[Backend]`在文件夹级别发布 — 将过滤器添加到仅发布CF。
...
* SITES-20495： `[BE]`获得在文件夹级别发布的权限的能力。
...
* SITES-21043：对Cloud Service的`[CF][launches]`侧端口性能改进。
* SITES-21044： `[CF][launches]`到Cloud Service的侧端口异步编辑有效负载实现。
...
* SITES-20559： `[MSM][XF][Lufthansa]`将体验片段从母版/语言转出到国家/地区时，不会更新引用。
...
* SITES-21233：升级到15860后，为GS1美国断开了`[CoreCmp]`可折叠项。
...
* 某些用户在`Cloud Service version 2024.5.16461`上创建表单时可能会遇到问题，问题是，当用户创建表单时，创建对话框中会弹出以下错误消息：
  ...
  出现此问题的原因是，作者发布表单时没有先发布表单中使用的模板，这会导致向`jcr:uuid`节点添加`<template-path>/initial/jcr:content`和其他受保护且系统生成的属性。 这会导致后续表单创建失败。
...
* Assets-31058：在“演绎版”选项卡的AEM Assets UI中显示智能裁剪演绎版，并在用户单击这些演绎版时生成智能裁剪图像。
