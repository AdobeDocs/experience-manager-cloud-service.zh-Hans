...
* SKYOPS-65368：FACT 工具（错误处理）- 工具陷入无休止的包含循环，最终在循环嵌入 Clientlibs 的情况下超时。
...
* SITES-14899：体验片段：为 Target 中的 XF 变体创建多个产品建议。
...
* Forms-10416：对于Headless自适应表单，当包含“:type”属性时，多行输入组件显示为常规单行文本输入组件。
