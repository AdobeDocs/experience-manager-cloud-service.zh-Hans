...
* Assets-28634： Adobe Stock中的Omnisearch字段不会自动添加资产数据。
...
* Assets-29106：在启用了Unified Shell的Adobe上查看股票时，会引发错误AEM。
...
* Assets-29515： Assets /具有“jcr:lastModifiedBy”：“workflow-process-service”的节点在列表视图中显示为“外部用户”。
* Assets-29579：非管理员用户无法创建图像集。
* Assets-29631：使用dam:roles进行安全交付/搜索。
* Assets-29689：应该从AEM端筛选dc:roles（以及新属性dam:roles）。
...
* Assets-30167：适用于adobe_dam:restrictions的HTML在上传资源后中断。
