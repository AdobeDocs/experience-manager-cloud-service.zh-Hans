...
- SITES-13636 — 报告了以下问题：某些客户缺少图像和轮播站点clientlib的clientlib依赖项。 如果客户嵌入了另一个clientlib（其中包括图像或轮盘式客户端库），则不会解析依赖关系。
- SITES-13475 - 2.22.10核心组件版本中PanelContainer.js的向后兼容性中断问题。
...
- SITES-12573 — 如果未指定一个变量，则使用筛选器中的变量的GraphQL查询将失败。 如果您将 GraphQL 与 AEM as a Cloud Service 结合使用，请不要更新到此版本。
