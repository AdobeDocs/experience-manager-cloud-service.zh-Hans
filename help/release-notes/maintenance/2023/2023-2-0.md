...
- GRANITE-44542 — 已报告未为包筛选器中包含的文件夹指定包节点类型（通过提供带有jcr:primaryType的.content.xml）的客户遇到的问题。 此问题导致这些文件夹被视为nt:folder，从而在不同情况下造成问题。
- SKYOPS-56928 - Apache HTTPD 回归可能导致 404 错误。如果您遇到这些问题，出于安全原因，建议回滚到上一版本并避免在该时段内运行任何管道。

### 嵌入的技术 {#embedded-tech-11382}
