...
2023.7.0功能激活提供了此维护版本的完整功能集。 有关更多信息，请参阅[ Experience Manager 发布路线图](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/update-releases-roadmap.html)。
...
- SLING-11974 — 修复了SlingHttpServletRequest#getUserPrincipal中对未经身份验证的请求的回归。 该修复可确保返回主体，甚至对于未经身份验证的请求也是如此。

### 已知问题 {#known-issues-12790}

- GRANITE-46601 — 如果在没有SDK的情况下，快速入门SDK将无法在jdk 11.0.20上启动 `-Djdk.util.zip.disableZip64ExtraFieldValidation=true` Java™选项
...
- SITES-11757： MSM：对于子页面，从父项继承转出配置不会恢复
- SITES-14073：Sites 管理员：选择不导出任何属性时，CSV 报告失败并显示 500 错误
- Forms-7648：无法验证数值框组件中的最大位数。 验证脚本不起作用。
...
- Forms-8668：呈现表单的“PDF预览”时，错误日志中会显示一些非必需的Java™栈栈转储。 但是，渲染表单时没有问题。
...
- SKYOPS-61385：通过最新的Dispatcher更新，可以使用以前静默忽略的某些无效正则表达式。 `libpcre1` 不再为更新接受 `libpcre2` 在部署期间。 Dispatcher配置检查将很快更新，以便更早地识别这些内容。
