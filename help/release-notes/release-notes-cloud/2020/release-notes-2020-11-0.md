...
* **[启动层次结构管理](/help/sites-cloud/authoring/launches/managing-pages.md) 和 [未来时间扭曲](/help/sites-cloud/authoring/launches/preview.md)**：新的UI用于在启动项中添加/删除页面，并且使用时间扭曲功能浏览网站会显示启动项的未来状态。

* **[扩展的内容片段模型和编辑器](/help/assets/content-fragments/content-fragments-models.md)**：新增了各种数据类型的输入验证选项、改进了枚举数据类型（包含新表单可视化），以及内容片段模型名称可在Assets UI中显示和搜索。

* **对可用于转出的Live Copy页面进行排序**：新增了用于对可用于转出的Live Copy页面进行排序的选项 [!UICONTROL 名称]， [!UICONTROL 上次修改日期]、和 [!UICONTROL 上次转出日期] 属性。 此 [!UICONTROL 上次转出日期] 对于页面，引入了新属性。
...
* 对中的辅助功能进行了以下增强 [!DNL Experience Manager Assets] 在此版本中。 有关更多信息，请参阅 [中的辅助功能 [!DNL Assets]](/help/assets/accessibility.md).
   ...
   * 使用键盘导航时间轴时，Esc键可以折叠“全部显示”选项而不会失去焦点。
   ...
   * 将指针悬停在选项上时，会显示一些提示，例如选择和下载。 由于这些提示，使用屏幕放大镜的用户可能无法看到文件缩略图。 现在，可以使用删除选项来保留焦点 `Escape` 键。
   * 从页面中显示的网格中选择网格单元格时，焦点会移动到屏幕上显示的操作栏。
...
* 以下辅助功能增强功能现已在以下位置提供： [!DNL Dynamic Media]：
   ...
   * 改进了“发布”页面、“编辑资产”页面、“编辑智能裁剪”页面和“图像集编辑器”页面，以符合Web标准。 辅助技术(AT)用户现在可以轻松导航这些页面并采取裁切图像等操作。
   * 改进了查看器，让用户可以使用键盘导航。
   * 键盘和屏幕阅读器用户可以使用裁切功能。
   * 键盘用户可更好地管理热点。
...
新 [“httpd -t”语法](/help/implementing/dispatcher/disp-overview.md#local-validation) 检查在Cloud Manager构建期间执行的Apache和Dispatcher配置，该配置也可以使用AEMas a Cloud ServiceSDK的Dispatcher工具运行。

## 内容传输工具 {#content-transfer-tool}

阅读本节内容，了解的新增功能和更新 [内容传输工具](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/moving/cloud-migration/content-transfer-tool/overview-content-transfer-tool.html) 版本1.1.12。
...
* Cloud Readiness Analyzer现在更名为Best Practices Analyzer (BPA)。 BPA可以为您当前的AEM实施提供最佳实践评估，并帮助您评估从现有AEM实例迁移到AEMas a Cloud Service的准备工作。

* 添加了新检测器以检测使用 `java.io.InputStream`，在AEMas a Cloud Service中使用可能会导致问题。

### 错误修复 {#bpa-bug-fixes}

* 错误导致与相关的阳性 *textfield foundation* 组件已修复。
