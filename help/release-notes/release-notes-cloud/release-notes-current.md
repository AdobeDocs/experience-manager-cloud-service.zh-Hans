...
>[!VIDEO](https://video.tv.adobe.com/v/3480409/?captions=chi_hans&quality=12)
...
>[!IMPORTANT]
>
>Beta版本可能包含缺陷，并“按原样”提供，无任何类型的担保。 Adobe没有义务维护、更正、更新、更改、修改或以其他方式支持(通过Adobe支持服务或其他方式)Beta版。 Adobe建议客户谨慎使用，不要依赖beta版的正确功能或性能，也不应依赖任何随附的文档或材料。 测试版中的功能和API如有更改，恕不另行通知。 因此，使用任何Beta版本的风险完全由客户自行承担。

**参与的优势**
通过抢先使用Adobe正在开发的功能，客户和合作伙伴可以提供反馈并指导产品开发。 它还有助于客户在公开发行之前做好采用新功能的准备。
...
借助基于属性的访问控制(ABAC)，管理员可使用元数据驱动的规则控制对带有OpenAPI资源的Dynamic Media的访问。 管理员可以根据资源元数据定义用户组的规则，以确定哪些资源对特定组可见。 当资源的元数据与定义的条件匹配时，将自动授予访问权限。 此功能可帮助组织实施更好的管理，确保用户只能查看和使用Dynamic Media以及与他们的角色或权限相关的OpenAPI资产。
...
#### 使用开发代理进行代码质量管道故障诊断 {#devagent-codequality}
...
详细了解[开发代理](/help/ai-in-aem/agents/brand-experience/development/development.md)和管道疑难解答。

### [!DNL Experience Manager]作为[!DNL Cloud Service] Foundation重要声明 {#foundation-notices}

#### Java API弃用 {#java-api-deprecation}

代码中不应再使用面向2026年2月26日删除的已弃用API。 要防止部署块，请在2026年3月30日之前删除API使用情况。 重要日期：
...
#### Cloud Manager MCP Server（Beta版计划） {#cm-mcp-server}

>[!VIDEO](https://video.tv.adobe.com/v/3480352/?captions=chi_hans&quality=12)

现代IDE使用模型上下文协议(MCP)来启用大型语言模型(LLM)以调用MCP服务器公开的工具。 开发人员无需直接与低级API规范集成，而只需使用自然语言描述其意图即可。

Cloud Manager MCP服务器现在已推出Beta版，该版本允许您使用提示直接从IDE与Cloud Manager API交互。 支持的方案包括执行管道、检查环境状态等。
...
#### 用于AEM Java和Dispatcher开发的IDE AI工具（测试版计划） {#ai-dev-beta}
...
#### 将AEM 6.5的IDE AI工具迁移到AEM Cloud Service(Alpha程序) {#cm-ide-migration}

通过使用IDE AI工具来执行[最佳实践分析器报告](/help/journey-migration/best-practices-analyzer/overview-best-practices-analyzer.md)的建议，加速从AEM 6.5到AEM as a Cloud Service (Java stack)的迁移。

有关详细信息，请发送电子邮件至[aem-devagent@adobe.com](mailto:aem-devagent@adobe.com)。
