...
以下版本 (2022.1.0) 的发布日期为 2022 年 1 月 27 日。
...
* 在配置远程 DAM 和 Sites 部署之间的连接后，远程 DAM 上的资源即可用于 Sites 部署。您现在可以执行 [更新、删除、重命名和移动操作](../../assets/use-assets-across-connected-assets-instances.md) 在远程DAM资产或文件夹上。 更新会在 Sites 部署中自动提供，但会有一些延迟。
...
* **[!DNL AEM Forms as a Cloud Service - Communications]**：[Communication API](https://experienceleague.adobe.com/docs/experience-manager-forms-cloud-service/forms/using-communications/aem-forms-cloud-service-communications.html) 可帮助您组合模板和 XML 数据，以生成各种格式的打印文档。该服务允许您以同步模式和批处理模式生成文档。API 使您能够创建应用程序，这些应用程序允许您：
