...
# [!DNL Adobe Experience Manager]的202278.0发行说明as a Cloud Service {#release-notes}

以下部分概述了[!DNL Experience Manager]as a Cloud Service2022.7.0版本的功能发行说明。
...
* **[转换为PDF/A - AEM Workflow步骤](/help/forms/aem-forms-workflow-step-reference.md##convert-pdfa)**：PDF/A是一种用于长期保存文档内容的存档格式，所有字体都将嵌入，并且文件是未压缩的。 现在，您可以使用 AEM 工作流中的“转换为 PDF/A”步骤，将任意格式的文档或文件转换为 PDF/A 格式。
...
* Sling内容分发(SCD)现在支持显式“无效”操作，以便在不发布内容的情况下使内容无效。 有关更多详细信息，请参阅[在AEM as a Cloud Service中缓存](/help/implementing/dispatcher/caching.md#explicit-invalidation)页面。
* AEM as a Cloud Service 现在提供 mod_macro。有关支持的Apache模块的列表，请参阅[此表](/help/implementing/dispatcher/disp-overview.md)。
