...
# Content Hub 概述 {#overview-content-hub}
...
## 为什么选择AEM Assets Content Hub？

AEM Assets Content Hub提供以下主要优势：
...
## 如何访问AEM Assets Content Hub？ {#access-content-hub}
...
## 常见问题解答 {#faqs-content-hub-overview}

### 什么是AEM Assets Content Hub？ {#what-is-content-hub}

AEM Assets Content Hub是一项Adobe Experience Manager as a Cloud Service功能，它使范围更广的团队能够通过直观的门户轻松发现相关的已批准资源，并快速调整这些资源以符合其需求。 它支持大规模分发资产，并有助于创建品牌内内容变体以提高营销敏捷性。

### 访问AEM Assets Content Hub的先决条件是什么？ {#prerequisites-for-content-hub}

Content Hub需要Experience Manager as a Cloud Service的生产创作环境，尤其是2024.6版本或更高版本(最低版本2024.6.16799)。

### AEM Assets Content Hub如何改善品牌认可资产的搜索体验？ {#content-hub-improves-search-experience}

AEM Assets Content Hub以平面层次结构显示所有批准的资源，从而更便于通过直观的门户查找和共享品牌批准的资源。 这种设置简化了搜索过程，并确保用户能够有效地找到他们需要的资产。

### 谁可以配置AEM Assets Content Hub用户界面，并且可以配置哪些方面？ {#content-hub-configuration}

管理员可以配置AEM Assets Content Hub用户界面，包括搜索筛选器、用于添加或导入资源的字段、资源属性以及用于品牌化的横幅内容。 这允许根据组织要求进行自定义。

### AEM Assets Content Hub如何让非创意人员编辑和重新混合内容？ {#content-hub-edit-remix-content}

Content Hub使非创意人员能够使用易于使用的工具、模板和品牌元素编辑现有内容并创建新品牌变量。 如果用户拥有Adobe Express权限，他们还可以利用Adobe Firefly GenAI功能创建高级内容。

### 用户如何访问AEM Assets Content Hub？ {#content-hub-access}

用户可以通过直接链接(https://experience.adobe.com/#/assets/contenthub)访问AEM Assets Content Hub，也可以登录experience.adobe.com并从快速访问部分中选择Experience Manager Assets Content Hub来访问。
...
<td>
   ...
   <p>
      <em> 内容模块提供了对资产的宝贵洞察，解决了营销利益相关者经常遇到的共同挑战</em>
   </p>
</td>
</table>
