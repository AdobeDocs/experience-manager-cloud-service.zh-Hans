...
>[!NOTE]
>
>删除对默认表单的自定义更改后，“元数据架构”界面上的锁定图标会重新显示在该表单之前，以指示该表单已恢复到其默认状态。
...
1. 单击/点按 **[!UICONTROL 应用到文件夹]** 以将自定义元数据应用到文件夹。
1. 选择要应用修改后架构的文件夹，然后单击/点按 **[!UICONTROL 应用]**.
...
您可以在文件夹级别定义必填字段，这会在上传到文件夹的资产上强制实施。 如果上传的资产缺少之前定义的必填字段的元数据，卡片视图中的资产上会显示缺少元数据的视觉指示。

>[!NOTE]
>
>可根据其他字段的值将元数据字段定义为必填字段。 在“信息卡”视图中，Experience Manager不会显示有关此类必填元数据字段缺少元数据的警告消息。
...
1. 单击“**[!UICONTROL 保存]**”。修改后的表单将列在 **[!UICONTROL 元数据架构Forms]** 页面。 选择表单，然后单击或点按 **[!UICONTROL 应用到文件夹]** 以将自定义元数据应用到文件夹。
1. 导航到文件夹并上传一些缺少元数据的资产，这些资产属于您添加到自定义表单中的必填字段。 资产的“卡片”视图上会显示一条消息，指出缺少必填字段的元数据。
1. （可选）访问 `https://[server]:[port]/system/console/components/`. 配置和启用 `com.day.cq.dam.core.impl.MissingMetadataNotificationJob` 默认禁用的组件。 设置Experience Manager检查资源上元数据有效性的频率。

   此配置添加了一个属性 `hasValidMetadata` 到 `jcr:content` 个资源。 使用此属性，Experience Manager可以筛选搜索中的结果。

   >[!NOTE]
   >
   >如果在计划检查后添加资产，则该资产不会标记为 `hasValidMetadata` 直到下一次计划检查。 资产不会显示在中间搜索结果中。
