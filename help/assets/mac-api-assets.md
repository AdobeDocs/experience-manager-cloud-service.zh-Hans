...
资产HTTP API允许对数字资产（包括元数据、演绎版和注释）以及使用[!DNL Experience Manager]内容片段的结构化内容执行创建——读取——更新——删除(CRUD)操作。 它在`/api/assets`上公开，并作为REST API实现。 它包含对内容片段[的支持。](/help/assets/content-fragments/assets-api-content-fragments.md)
...
有关详细信息，请参阅Experience Manager资产HTTP API](/help/assets/content-fragments/assets-api-content-fragments.md)中的[内容片段支持。

## 数据模型{#data-model}

资产HTTP API公开两个主要元素、文件夹和资产（对于标准资产）。

此外，它还针对描述内容片段中结构化内容的自定义数据模型显示更详细的元素。 有关详细信息，请参阅[内容片段数据模型](/help/assets/content-fragments/assets-api-content-fragments.md#content-models-and-content-fragments)。
...
有关如何使用API创建资产的信息，请参阅[资产上传](developer-reference-material-apis.md)。 不建议使用HTTP API创建资产。

## 更新资产二进制{#update-asset-binary}

有关如何使用API更新资产二进制文件的信息，请参阅[资产上传](developer-reference-material-apis.md)。 不建议使用HTTP API更新资产二进制文件。
