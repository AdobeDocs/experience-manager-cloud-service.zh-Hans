...
对于全文搜索，请将谓 `Fulltext` 词添加到表单。 使用“属性”谓词可搜索与您指定的单个属性匹配的资产。 使用“选项”谓词可搜索与特定属性的一个或多个值匹配的资产。 添加“日期范围”谓词以搜索在指定日期范围内创建的资产。
...
您不必为CRX存储库中的选项手动创建节点结构，而是可以通过指定相应的键值对在JSON文件中定义选项。 在“属性名称”字段中指定JSON文 **[!UICONTROL 件的路径]** 。 例如，您可以定义键值对、 `image/bmp`、 `image/gif`、 `image/jpeg`和 `image/png` ，并指定它们的值，如以下示例JSON文件中所示。 在“属 **[!UICONTROL 性名称]** ”字段中，可以指定此文件的CRX路径。
...
1. 执行下列操作之一：
   ...
   * 点 ![](assets/do-not-localize/aem_assets_add_icon.png) 按“选项”字段旁边的，以指定要在“滤镜”面板中提供的选项的显示文本和值。 要添加其他选项，请点按／单 ![](assets/do-not-localize/aem_assets_add_icon.png) 击并重复该步骤。
...
此谓 `Multi Value Property` 词允许您搜索资产中的多个值。 请考虑在AEM资产中有多个产品的图像，并且每个图像的元数据包括与产品关联的SKU编号的情况。 您可以使用此谓词根据多个SKU编号搜索产品图像。
...
此谓 `Tags` 词允许您对资产执行基于标记的搜索。 默认情况下，AEM资产会根据您指定的标记搜索资产中一个或多个标记匹配项。 换句话说，搜索查询使用指定的标记执行OR操作。 但是，您可以使用“匹配所有标记”选项来搜索包含您指定的所有标记的资产。
...
<table>
 <tbody>
  ...
  <tr>
   <td><p>全文</p> </td>
   <td>此搜索谓词用于对整个资产节点执行全文搜索。 <code>jcr</code> 它映射为 <code>contains</code>:运算符。 如果要对资产节点的特定部分执行全文搜索，则可以指定相对路径。</td>
   <td>
    <ul>
     <li>标签</li>
     <li>占位符</li>
  ...
  <tr>
   <td><p>路径</p> </td>
   <td><p>使用它可以按位置筛选结果。 可以指定不同的路径作为选项。</p> </td>
   <td>
    <ul>
     <li>标签</li>
     <li>路径</li>
...
1. 点按 **[!UICONTROL 工具]** 栏中的删 ![除](assets/do-not-localize/deleteoutline.png) “删除”图标。
