...
对于全文搜索，请将 `Fulltext` 谓词。 使用属性谓词，可搜索与您指定的单个属性匹配的资产。 使用“选项”谓词，可搜索与特定属性的一个或多个值匹配的资产。 添加“日期范围”谓词，以搜索在指定日期范围内创建的资产。
...
1. 执行下列操作之一：
   ...
   * 点按 ![“资产”添加图标](assets/do-not-localize/aem_assets_add_icon.png) 在“选项”字段旁边，指定要在“过滤器”面板中提供的选项的显示文本和值。 要添加其他选项，请点按/单击 ![“资产”添加图标](assets/do-not-localize/aem_assets_add_icon.png) 然后重复该步骤。
...
的 `Tags` 此谓词允许您对资产执行基于标记的搜索。 默认情况下， [!DNL Assets] 根据您指定的标记，搜索资产中一个或多个标记匹配项。 换句话说，搜索查询使用指定的标记执行OR操作。 但是，您可以使用“匹配所有标记”选项来搜索包含您指定的所有标记的资产。
...
1. 点按 **[!UICONTROL 删除]** ![删除图标](assets/do-not-localize/deleteoutline.png) 中。
