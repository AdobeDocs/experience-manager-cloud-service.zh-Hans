...
* **使用标记组织资源：**
为资产添加标记可增强搜索性、收藏集创建和搜索排名。 Adobe AI使用自学习算法来进行精确标记，从而实现快速资源检索。 Adobe AI还可以识别和分配相关标记（包括自定义标记）到资产，通过自动描述性标记简化资产管理。
...
最佳做法是使用智能裁剪创建图像配置文件。 在配置文件中，您可以定义各种屏幕大小并让Adobe AI执行其余操作，确保您的图像和视频始终针对查看者的设备而优化。
