...
* **使用标记组织资源：**
为资产添加标记可增强搜索性、收藏集创建和搜索排名。 Adobe AI采用自学习算法来进行精确标记，从而实现快速资源检索。 Adobe AI还可以识别相关标记（包括自定义标记）并将它们分配给资产，通过自动的描述性标记来简化资产管理。
...
智能裁剪是Dynamic Media中的一项功能，它使用Adobe AI、Adobe的AI和机器学习框架来自动裁剪图像和视频。 它智能地检测并关注图像或视频中的主要主题或兴趣点。 这种智能功能可确保焦点在台式计算机和移动设备上的各种屏幕尺寸中得以保持。

最佳做法是使用智能裁剪创建图像配置文件。 在配置文件中，您可以定义各种屏幕大小并让Adobe AI完成其余步骤，以确保您的图像和视频始终针对查看者的设备而优化。
