...
>[!NOTE]
>
>您必须先发布资产，然后才能将其添加到AEM中的页面。 See [Publishing Dynamic Media Assets](/help/assets/dynamic-media/publishing-dynamicmedia-assets.md).
...
1. 在左窗格中，点按组 **[!UICONTROL 件图]** 标，然后筛选Dynamic Media。
...
* **[!UICONTROL 启用长宽比匹配]**-选择此选项可让Dynamic Media选取宽高比与原始图像的长宽比最匹配的智能裁剪再现。
...
* 宽高比为2:1。
* 使用关键字 `equirectangular` 或(`spherical` + `panorama`)或(`spherical` + `panoramic`)标记。 请参 [阅使用标记](/help/sites-cloud/authoring/features/tags.md)。
...
有 [关使用Dynamic Media帐户](/help/assets/dynamic-media/http2faq.md) HTTP/2入门的完整详细信息，请参阅内容的HTTP2投放。
