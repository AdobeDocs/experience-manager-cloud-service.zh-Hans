...
360度视频同时在每个方向记录一个视图。 它们使用全方位相机或一组相机拍摄。 在播放时，在平面显示器上，用户可以控制观看角度；在移动设备上播放通常应用其内置的陀螺仪控件。

Dynamic Media包含对360个视频资源投放的本机支持。 默认情况下，查看或回放不需要任何其他配置。 您可以使用标准视频扩展（如。mp4、.mkv和。mov）交付360视频。 最常见的编解码器是H.264。

您可以使用360/VR视频查看器来渲染等长形视频。 结果是，您可以沉浸式地欣赏房间、房产、位置、风景、医疗程序等。
...
点按[空间站360](http://mobiletest.scene7.com/s7viewers/html5/Video360Viewer.html?asset=Viewers/space_station_360-AVS)打开浏览器窗口并观看360度视频。 在视频播放过程中，将指针拖动到新位置以更改观看角度。
...
1. [将自适应视频用户档案应用到文件夹](/help/assets/dynamic-media/video-profiles.md#applying-a-video-profile-to-folders)。
   ...
   您可以使用现成的自适应视频用户档案，它已随Dynamic Media提供。 但是，对于使用非360视频查看器渲染的设置进行编码的非360视频，其视频质量明显低于360。 因此，如果需要高质量的360视频，请执行以下操作：

   * 理想情况下，原始360视频内容具有以下分辨率之一：

      * 1080p - 1920 x 1080，称为全高清或全高清分辨率，或
      * 2160p - 3840 x 2160，称为4K、UHD或UltraHD分辨率。 这种大屏幕分辨率通常出现在高级电视机和计算机显示器上。 2160p分辨率通常称为“4K”，因为宽度接近4000像素。 换句话说，它优惠1080p像素的4倍。
...
请参阅[在网页上嵌入视频查看器或图像查看器](/help/assets/dynamic-media/embed-code.md)。请参阅[将 URL 关联到您的 Web 应用程序](/help/assets/dynamic-media/linking-urls-to-yourwebapplication.md)。如果您的交互式内容包含相对URL的链接，特别是指向“Experience Manager站点”页面的链接，则无法使用基于URL的链接方法。
请参阅[将Dynamic Media资产添加到页面。](/help/assets/dynamic-media/adding-dynamic-media-assets-to-pages.md)

**预览360个视频**

1. 在&#x200B;**[!UICONTROL 资产]**&#x200B;中，导航到您已创建的现有360视频。 要在预览模式下打开视频，请点按360视频资产。
...
1. 在预览页面的左上角附近，点按下拉列表，然后选择&#x200B;**[!UICONTROL 查看器]**。
   ...
   * 要更改静态场景的观看角度，请在视频上拖动指针。
   * 要开始播放，请点击视频的&#x200B;**[!UICONTROL 播放]**&#x200B;按钮。 播放视频时，在视频上拖动指针以改变观看角度。
...
要使用360视频，您必须发布它。 发布360视频时，将激活URL和嵌入代码。 它还将360视频发布到Dynamic Media云，该云与CDN集成以实现可扩展和高性能投放。
...
另请参阅[将URL关联到您的Web应用程序](/help/assets/dynamic-media/linking-urls-to-yourwebapplication.md)。 如果您的交互式内容包含相对URL的链接，特别是指向“Experience Manager站点”页面的链接，则无法使用基于URL的链接方法。
