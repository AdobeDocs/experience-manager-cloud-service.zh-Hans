...
您可以在Adobe Experience Manager中通过导航到&#x200B;**[!UICONTROL 工具]**（锤子图标）> **[!UICONTROL 资产] > [!UICONTROL 查看器预设]**&#x200B;来添加、编辑、删除、发布、取消发布和预览查看器预设。

![6_5_tools-assets-viewerpresets](assets/6_5_tools-assets-viewerpresets.png)

>[!NOTE]
>
>默认情况下，当您在资产的详细信息视图中选择查看器时，系统会显示15个查看器预设。 您可以提高此限制。请参阅[增加显示](#increasing-the-number-of-viewer-presets-that-display)的查看器预设数。

### 查看器支持响应式设计的网页 {#viewer-support-for-responsive-designed-web-pages}

不同的网页有不同的需求。 例如，有时您希望网页提供一个链接，以在单独的浏览器窗口中打开HTML5查看器。 在其他情况下，需要直接将HTML5查看器嵌入到托管页面。 在后一种情况下，网页具有静态布局。 或者，它是“响应式”的，并在不同设备上或针对不同浏览器窗口大小显示不同。 为了满足这些需求，Dynamic Media附带的所有预定义现成HTML5查看器都支持静态网页和响应式设计的网页。

有关如何将响应式查看器嵌入网页的更多信息，请参阅&#x200B;*Dynamic Media图像提供和渲染API帮助*&#x200B;中的[响应式静态图像库](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/responsive-static-image-library/c-about-responsive-static-image-library.html#about-responsive-image-library)。
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><p>全景图像和全景VR查看器可渲染球面全景图像，以让用户沉浸在房间、属性、位置或景观的360°观看体验中。</p> <p>要使上传的图像符合球面全景图的条件，该图像必须具有以下任一项或两项：</p>
    <ul>
     <li>宽高比为2:1。</li>
     <li>使用关键字<code>equirectangular</code>、<code>spherical</code>和<code>panorama</code>或<code>spherical </code>和<code>panoramic</code>进行标记。 请参阅<a href="/help/sites-cloud/authoring/features/tags.md">使用标记</a>。</li>
  ...
  <tr>
   ...
   <td><p>使用360/VR视频查看器可渲染等长形视频，以体验房间、属性、位置、景观或医疗程序的沉浸式观看体验。</p> <p>在平面显示器上播放期间，用户可以控制观看角度。 移动设备上的播放使用其内置的陀螺仪控件。</p> <p>查看器包含对交付360个视频资产的本机支持。 默认情况下，查看或播放不需要任何其他配置。 您使用标准视频扩展名(如.mp4、.mkv和.mov)交付360视频。 最常见的编解码器是H.264。</p> </td>
  </tr>
  <tr>
...
Experience Manager在从&#x200B;**[!UICONTROL 详细信息视图]** > **[!UICONTROL 查看器]**&#x200B;查看资产时，会显示各种查看器预设。 您可以增加或减少显示的查看器数量。
...
通过创建查看器预设，您可以应用各种设置来查看资产并与之交互。 但是，您不需要创建查看器预设。 如果您愿意，可以使用Experience Manager资产附带的默认现成查看器预设。
...
1. 在Experience Manager左上角，选择Experience Manager徽标，然后在左边栏中，转到&#x200B;**[!UICONTROL 工具]**（锤子图标）> **[!UICONTROL 资产]** > **[!UICONTROL 查看器预设]**。
...
1. 执行下列操作之一：
   ...
   * 如果您在单独的CSS文件中定义了样式设置，则可以将CSS文件上传到Experience Manager资产。 要查找上传的CSS文件并将其与查看器预设关联，请选择&#x200B;**[!UICONTROL 选定类型]**&#x200B;下拉菜单下的&#x200B;**[!UICONTROL 导入CSS]**（如有必要，请向上滚动可视编辑器以查看）。
...
1. 从&#x200B;**[!UICONTROL 选定类型]**&#x200B;下拉菜单中，选择要更改其行为的组件。
   ...
   >
   >
   >在文本字段中键入值后，请在用户界面的其他位置选择以提交更改并关闭虚拟键盘。 如果选择&#x200B;**[!UICONTROL Enter]**，则不会执行任何操作。
...
创建或编辑交互式视频查看器预设时，您可以选择要使用的显示模式设置。 当您从&#x200B;**[!UICONTROL Behavior]**&#x200B;选项卡下的&#x200B;**[!UICONTROL 选定组件]**&#x200B;下拉菜单中选择`InteractiveSwatches`时，会出现此选择。 您选择的“显示”模式会影响视频播放时缩略图的显示方式和显示时间。您可以选择 `segment` 显示模式（默认）或 `continuous` 显示模式。
...
<table>
 <tbody>
  ...
  <tr>
   <td>区段</td>
   <td><p><code>Segment </code>是现成的交互式视频查看器预设<code>Shoppable_Video_light</code>和<code>Shoppable_Video_dark</code>以及您自己创建的任何交互式视频查看器预设的默认显示模式。</p> <p>在此模式下，假定分配给视频区段的缩略图数量少于显示面板中的可见点数。 在这种情况下，下一个或上一个子区段的缩略图不是</i>提取到<i>，以填充面板中的任何空白位置。 即，它保留分配给特定视频区段的色板的显示。</i></p> </td>
  </tr>
  <tr>
   <td>连续</td>
   <td><p>在<code>continuous </code>显示模式下，假设区段中缩略图的数量小于面板中可见缩略图的数量。 在这种情况下，查看器会自动包含显示下一个区段或显示最后一个缩略图的上一个区段的缩略图。</p> <p>本主题</a>中的<a href="/help/assets/dynamic-media/interactive-videos.md">视频是<code>continuous </code>显示模式的一个示例。</a></p> </td>
  </tr>
 </tbody>
</table>
...
但是，要&#x200B;*传送*&#x200B;带有查看器预设的资产，还必须发布查看器预设。 必须激活&#x200B;*和*&#x200B;并发布所有查看器预设，才能获取资产的URL或嵌入代码。 激活并发布Dynamic Media附带的所有现成查看器预设。 您创建和添加的自定义查看器预设将自动激活，但也必须发布。
