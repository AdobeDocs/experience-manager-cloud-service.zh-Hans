...
1. 发布新查看器预设。 有必要发布预设，以便您能够在网站上使用其生成的URL。
   ...
   >[!IMPORTANT]
   >
   >对于使用自适应比特率流配置文件的旧视频，URL会继续照常播放（与HLS流播放），直到您 [重新处理视频资产](/help/assets/dynamic-media/about-image-video-profiles.md#reprocessing-assets). 重新处理后，同一URL将继续工作，但现在与 *both* 已启用DASH和HLS流。
