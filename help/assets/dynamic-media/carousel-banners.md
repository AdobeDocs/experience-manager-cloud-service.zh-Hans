...
1. 可选：根 [据需要创建传送集查看器预设](/help/assets/dynamic-media/managing-viewer-presets.md)。
...
If you need to edit Carousel Sets, see [editing Carousel Sets.](#editing-carousel-sets) 此外，您还可以查看和编辑传送 [集属性](/help/assets/manage-digital-assets.md#editing-properties)。
...
1. 通过点按或单击“行为”和“外观”选项卡，并调整传送横幅的外观或特定组件的行为，可以更改传送集的可视设计和行为。 有关如 [何使用查看器编辑器的更多信息](/help/assets/dynamic-media/viewer-presets.md) ，请参阅管理查看器预设。
...
>[!NOTE]
>
>在交互式图像或传送横幅中向图像添加热点时 [](/help/assets/dynamic-media/interactive-images.md) ，热点信息将存储在相对于图像位置和madhis的同一元数据位置，而不管它是交互式图像还是传送横幅。 此功能意味着您可以在任一查看器中轻松地重复使用同一图像及其定义的热点数据。
...
1. 在“操作”选项卡中执行下列 **[!UICONTROL 操作]** 之一：
   ...
   * 点按 **[!UICONTROL 概览]**。
      ...
      * 如果您是AEM Sites，请点 <!-- and Ecommerce customer-->按产品选取器图标（放大镜）以打开选择产品页面。 点按要使用的产品，然后点按页面右上角的复选标记以返回到传送横幅编辑器。
...
有关 [如何发布传送横幅的信息](/help/assets/dynamic-media/publishing-dynamicmedia-assets.md) ，请参阅发布Dynamic Media资产。
