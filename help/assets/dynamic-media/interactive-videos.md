...
1. **（可选）识别概览变量** — 首先识别由您现有的概览实施使用的动态变量。 在创建交互式视频时，可使用变量将产品缩略图映射到对应的产品概览。 请参阅[（可选）标识概览变量](#optional-identifying-quickview-variables)。
   ...
   * 要通过触发快速视图向视频添加交互性。
   * 您的Experience Manager实施&#x200B;*不*&#x200B;使用电子商务集成框架。 它不会将产品数据从®WebSphere®Commerce、Elastic Path、SAP Hybris或Intershop等解决方案拉入Experience Manager。
...
>
>* 要通过触发快速视图向视频添加交互性。
>* 您的Experience Manager设置不使用电子商务集成框架。 它不会从®WebSphere®Commerce、Elastic Path、SAP Hybris或Intershop中提取产品数据。
>
