...
在Dynamic Media中使用3D资源不需要额外安装。

![鞋](/help/assets/dynamic-media/assets/3d-dimensional-viewer-quickpublish-url-embed2a.png)
...
| USDZ | 通用场景描述Zip存档 | model/vnd.usdz+zip | *仅支持摄取；没有可用的查看或交互。* USDZ是专有的3D格式，Safari或iOS可以本机查看。 |

## 快速开始:Dynamic Media中的3D资源{#quick-start-three-d}

以下工作流分步说明旨在帮助您在Dynamic Media中快速设置并运行3D资源。

在Dynamic Media中处理3D资产之前，请确保您的Experience Manager管理员已启用并配置了Dynamic MediaCloud Services。
...
1. 在3D资产的“详细信息”页面上，执行下列任一操作：
   ...
   * **缩放相机**  — 缩放相机以移入和移出3D场景的区域。
      * _鼠标_:滚轮。
      * _触摸屏_:两指捏。
   * **重新输入您的相机**  — 将您的相机重新输入到3D场景中某个对象上的某个点。
      * _鼠标_:多次单击。
      * _触摸屏_:多次点击。
...
一个完全交互式的3D资产，如    ![3D资产在3D媒体组件内显示在预](/help/assets/dynamic-media/assets/3d-asset-in-3d-mediaa.png)
览模式中显示的完全交互式3D **** 资产。
...
Dynamic Media包含一个Dynamic Media 3D媒体组件，您可以在Experience Manager站点中使用它来在您的网页上实现3D模型的交互式查看。
...
如果您使用Experience Manager作为Web内容管理系统，则可以通过3D媒体组件将3D资产添加到网页。
...
1. 要在页面编辑器中打开页面，请点按&#x200B;**[!UICONTROL 编辑]**（铅笔）图标。 确保在页面的右上角附近选择了&#x200B;**[!UICONTROL 编辑]**&#x200B;模式。
...
>[!NOTE]
>
>当网页处于“Experience Manager站点”**[!UICONTROL “编辑”]**&#x200B;模式时，3D Media组件会显示3D资产，但无法与资产进行交互。 要使资产具有交互性，您可以使用&#x200B;**[!UICONTROL 预览]**&#x200B;功能在页面编辑器中视图网页，以完全访问3D媒体组件的功能。
...
1. 从3D资产文件名左侧的下拉列表，点按&#x200B;**[!UICONTROL 演绎版]**。
...
1. 要显示可在网页上复制和使用的3D资产的直接生产URL，请点按&#x200B;**[!UICONTROL URL]**。
...
* **[!UICONTROL 嵌入]**  — 当 **** 您希望使用维查看器视图嵌入到网页中的Dynamic Media 3D资源时，请使用“嵌入”。将嵌入代码复制到剪贴板，以便将其粘贴到网页中。 不允许在&#x200B;**[!UICONTROL 嵌入]**&#x200B;对话框中编辑代码。
   ...
   请参阅[在网页上嵌入Dynamic Media Video、Image Viewer或Dimensional查看器](/help/assets/dynamic-media/embed-code.md#embedding-the-video-or-image-viewer-on-a-web-page)。
