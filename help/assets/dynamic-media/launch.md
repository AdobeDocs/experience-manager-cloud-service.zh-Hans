...
如集成 [的使用案例中所述](#use-cases-for-the-integration)，可以通过AEM Sites中新的Adobe Launch集成和嵌入代码跟踪Dynamic Media查看器。
...
您必须完成配置Adobe Analytics和配 [置Adobe Launch部分](#configuring-adobe-analytics-for-the-integration)[中的配置步骤](#configuring-adobe-launch-for-the-integration) 。 但是，不需要执行与AEM相关的配置步骤。
...
1. 首先登录Adobe Launch，获取Adobe Launch库的嵌入代码(请参阅配 [置Adobe Launch](#configuring-adobe-launch-for-the-integration))。
...
* (可选. 仅当需要视频跟踪时才需要) *Adobe Media Analytics for Audio和Video扩展*
