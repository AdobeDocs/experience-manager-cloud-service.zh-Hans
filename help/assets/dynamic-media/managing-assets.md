---
title: 管理 Dynamic Media 资产
description: 了解如何使用预览、下载或发布等工作流处理和管理Dynamic Media资源。
contentOwner: Rick Brough
feature: Asset Management
role: Admin,User
badgeSaas: label="AEM Assets" type="Positive" tooltip="适用于AEM Assets)。"
exl-id: 0dc6d577-a8ee-4dd5-8643-28862bb1394c
source-git-commit: a641933d1049cd07ee8935672c8ef357a5bbf18c
workflow-type: tm+mt
source-wordcount: '134'
ht-degree: 19%

---

# 管理 Dynamic Media 资产 {#managing-assets}

{{work-with-dynamic-media}}

管理Dynamic Media资产（图像、视频和交互式资产）上传后，需要执行许多任务，包括预览、下载或发布。

在预览时，您还可以应用图像预设（仅适用于图像）或查看器预设（适用于视频和图像）。 不能将两者都应用于资源。 请参阅[应用查看器预设](viewer-presets.md)和[应用图像预设](image-presets.md)。

有关管理资源的一般信息可在[使用触屏UI管理Assets](/help/assets/manage-digital-assets.md)中找到。

以下主题介绍了如何管理Dynamic Media资源：

* [优化图像质量的最佳做法](best-practices-for-optimizing-the-quality-of-your-images.md)
* [预览 Dynamic Media 资产](previewing-assets.md)
* [发布Dynamic Media Assets](publishing-dynamicmedia-assets.md)
* [使用选择器](working-with-selectors.md)
