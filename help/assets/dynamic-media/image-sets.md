...
1. [创建图像集。](#creating-image-sets)
   ...
   您还可以通过批量集预设自动 [创建图像集](/help/assets/dynamic-media/config-dm.md#creating-batch-set-presets-to-auto-generate-image-sets-and-spin-sets)。
...
>
>您还可以通过批量集预设自动 [创建图像集](/help/assets/dynamic-media/config-dm.md#creating-batch-set-presets-to-auto-generate-image-sets-and-spin-sets)。
>****重要说明：批量集由IPS（图像生产系统）创建，作为资产摄取的一部分。
