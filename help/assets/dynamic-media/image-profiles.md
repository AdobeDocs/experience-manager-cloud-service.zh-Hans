...
创建图像配置文件时，您可以使用 **[!UICONTROL 钝化蒙版]** 用于微调最终缩减取样图像的锐化滤镜效果的选项。 您可以控制效果的强度、效果的半径（以像素为单位）以及被忽略的对比度阈值。 此效果使用与Adobe Photoshop的“钝化蒙版”滤镜相同的选项。
...
中介绍了锐化 [锐化图像](/help/assets/dynamic-media/assets/sharpening_images.pdf).
...
要为其它资产类型定义高级处理参数，请参阅 [配置资产处理](config-dm.md#configuring-asset-processing).
...
如果文件夹已具有您后来更改的现有“图像配置文件”，您可以重新处理该文件夹中的资产。 请参阅 [编辑文件夹中用于处理资产的配置文件后，重新处理该文件夹中的资产](/help/assets/dynamic-media/about-image-video-profiles.md#reprocessing-assets).

### 将Dynamic Media图像配置文件应用到特定文件夹 {#applying-image-profiles-to-specific-folders}

您可以从将图像配置文件应用到文件夹 **[!UICONTROL 工具]** 菜单，或者如果您在文件夹中，则从 **[!UICONTROL 属性]**.

如果文件夹已经分配了配置文件，则文件夹名称正下方会显示配置文件的名称。

如果文件夹已具有您后来更改的现有视频配置文件，您可以重新处理该文件夹中的资产。 请参阅 [编辑文件夹中用于处理资产的配置文件后，重新处理该文件夹中的资产](/help/assets/dynamic-media/about-image-video-profiles.md#reprocessing-assets).
...
1. 选择Experience Manager徽标并导航到 **[!UICONTROL 资产]**.
...
1. 下 **[!UICONTROL 图像配置文件]**，来自 **[!UICONTROL 配置文件名称]** 下拉列表，选择要应用的配置文件。
...
>[!IMPORTANT]
>
>当您手动重新对齐资源的智能裁切窗口或调整其大小时，即使您稍后决定重新处理该资源，也会维护并保留该编辑。 但是，如果您在 **[!UICONTROL 响应式图像裁切]** 区域，则该资产需要进行重新处理。
>请参阅 [重新处理文件夹中的Dynamic Media资源](/help/assets/dynamic-media/about-image-video-profiles.md#reprocessing-assets).
...
1. 在 **[!UICONTROL 编辑智能裁剪]** 页面，执行以下任一操作：
   ...
   * 调整智能裁剪框的大小。 执行以下任一操作：
     ...
     ![调整图像的智能裁剪的大小](assets/edit_smart_crops-resize.png).
