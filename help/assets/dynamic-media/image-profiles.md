...
>[!NOTE]
>
>USM只应用于PTIFF（金字塔tiff）内缩减采样率超过50%的缩减格式副本。 钝化蒙版不会影响ptiff中大小最大的格式副本。 而更改更小的演绎版（例如缩略图）（并显示钝化蒙版）。
...
<table>
 <tbody>
  ...
  <tr>
   <td>阈值</td>
   <td><p>确定在应用钝化蒙版滤镜时要忽略的对比度范围。 此选项控制锐化的像素与其周围有多大的差异，才能被视为边缘像素。 只有达到此阈值的像素才会锐化。 为避免引入噪声，请尝试使用0-255之间的值。</p> </td>
  </tr>
 </tbody>
</table>
...
* 为特定文件夹和子文件夹创建按页面/资源类型的图像配置文件，而不是创建应用于所有文件夹或所有资源的通用智能裁剪配置文件。
* 应用于子文件夹的图像配置文件将覆盖应用于该文件夹的图像配置文件。
* 不允许包含重复的智能裁剪维度的图像配置文件。
* 不允许设置智能裁剪选项的重复命名图像配置文件。

您可以选择两个图像裁切选项：像素裁切和智能裁切。 您还可以选择自动创建颜色和图像色板，或保留所有目标分辨率的裁切内容。
...
1. 输入配置文件名称，以及钝化蒙版、裁切或色板（或两者）的值。
   ...
   >[!TIP]
   >
   >使用专用于其目标用途的配置文件名称。 例如，假设您要创建一个仅生成样本的配置文件。 也就是说，智能裁切将禁用（关闭），而颜色和图像色板将启用（打开）。 在这种情况下，您可以使用配置文件名称“智能色板”。
...
已分配配置文件的文件夹会在文件夹名称的正下方显示配置文件的名称。
...
1. 选择&#x200B;**[!UICONTROL 将处理配置文件应用到文件夹]**，然后选择一个或多个要用来接收新上传资源的文件夹，然后选择&#x200B;**[!UICONTROL 应用]**。 已分配配置文件的文件夹会在文件夹名称的正下方显示配置文件的名称。
...
>[!IMPORTANT]
>
>手动调整资源的智能裁剪窗口时，将保存所做的更改。 即使您稍后重新处理资产，这些编辑仍会保持不变。 但是，如果您在图像配置文件的&#x200B;**[!UICONTROL 响应式图像裁切]**&#x200B;区域中编辑宽度和/或高度，则该资产需要重新处理。
...
1. 执行以下任一操作：
   ...
   * 在图像的右上角附近，选择&#x200B;**[!UICONTROL 还原]**&#x200B;以撤消所有编辑并恢复原始裁切或色板。
...
>[!IMPORTANT]
>
>当您手动调整多个资源的智能裁剪窗口时，您的更改将会保存。 即使您稍后重新处理资产，这些编辑仍会保持不变。 不过，如果您在图像配置文件的&#x200B;**[!UICONTROL 响应式图像裁切]**&#x200B;区域中编辑宽度和/或高度，则这些资源需要重新处理。
>请参阅[重新处理文件夹](/help/assets/dynamic-media/about-image-video-profiles.md#reprocessing-assets)中的Dynamic Media资源。
