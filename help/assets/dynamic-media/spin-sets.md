...
1. [创建旋转集。](#creating-spin-sets)
   ...
   >[!NOTE]
   >
   >您还可以通过批量集预设自动 [创建旋转集](/help/assets/dynamic-media/config-dm.md#creating-batch-set-presets-to-auto-generate-image-sets-and-spin-sets)。 **** 重要说明：批量集由IPS（图像生产系统）创建，作为资产摄取的一部分。
...
>[!NOTE]
>
>您还可以通过批量集预设自动 [创建旋转集](/help/assets/dynamic-media/config-dm.md)。 **** 重要说明：批量集由IPS（图像生产系统）创建，作为资产摄取的一部分。
>
...
您可以在用户界面中创建旋转集，也可以使用批量集预 [设自动创建旋转集](/help/assets/dynamic-media/config-dm.md)。 但是，使用批量集预设创建的集 *不会* 显示在用户界面中。 您可以通过三种不同方式访问通过批集预设创建的集。 （即使您在用户界面中创建了旋转集，这些方法也可用）。
