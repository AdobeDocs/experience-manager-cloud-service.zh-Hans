...
>[!NOTE]
>
>有关Assets用户界面的信息，请参阅 [使用触屏UI管理资源](/help/assets/manage-digital-assets.md) 并将其应用到上传图像集资源的新文件夹。
...
1. [上传图像供多个视图使用](#uploading-assets-for-spin-sets).
   ...
   请参阅 [Dynamic Media — 支持的栅格图像格式](/help/assets/file-format-support.md#image-support-dynamic-media) 有关旋转集支持的格式的列表。

1. [创建旋转集](#creating-spin-sets).

   要创建旋转集，请选择 **[!UICONTROL 创建]** > **[!UICONTROL 旋转集]** 然后命名该集，选择资源，并选择图像的显示顺序。
...
可在用户界面中创建旋转集，也可使用 [批次集预设](/help/assets/dynamic-media/config-dm.md). 但是，使用批次集预设创建的集，请 *非* 显示在用户界面中。 您可以通过三种不同的方式访问通过批次集预设创建的批次集。 （即使已在用户界面中创建旋转集，这些方法也可用）。
...
1. 打开单个资产的属性时。 属性指示所选资产所属的集(在 **[!UICONTROL 集成员]**)。 要查看整个集，请选择集的名称。
