...
1. 可选. [创建批次集预设](/help/assets/dynamic-media/batch-set-presets-dm.md) 并将其应用到新的资源文件夹。
...
1. [上传图像以供多个视图使用](#uploading-assets-for-spin-sets).
   ...
   参见 [Dynamic Media — 支持的栅格图像格式](/help/assets/file-format-support.md#image-support-dynamic-media) 有关旋转集支持的格式的列表。

1. [创建旋转集](#creating-spin-sets).

   要创建旋转集，请选择 **[!UICONTROL 创建]** > **[!UICONTROL 旋转集]** 然后命名该集，选择资源，然后选择图像的显示顺序。

   参见 [使用选择器](/help/assets/dynamic-media/working-with-selectors.md).

1. 设置 [旋转集查看器预设](/help/assets/dynamic-media/managing-viewer-presets.md)（根据需要）。
...
1. [将URL链接到您的Web应用程序](/help/assets/dynamic-media/linking-urls-to-yourwebapplication.md) 或 [嵌入视频查看器或图像查看器](/help/assets/dynamic-media/embed-code.md).
   ...
   参见 [将旋转集链接到网页](/help/assets/dynamic-media/linking-urls-to-yourwebapplication.md) 和 [嵌入视频查看器或图像查看器](/help/assets/dynamic-media/embed-code.md).
...
对于一维旋转集，至少需要8到12张项目快照。 拍摄必须定期进行，以提供项目正在旋转和翻转的印象。 例如，如果一维旋转集包含12个镜头，则每个镜头应将项目旋转30° (360/12)。

参见 [Dynamic Media — 支持的栅格图像格式](/help/assets/file-format-support.md#image-support-dynamic-media) 有关旋转集支持的格式的列表。

您可以像上传一样上传旋转集的图像 [在Experience Manager Assets中上传任何其他资源](/help/assets/manage-digital-assets.md).
...
>
>请参阅中的“创建批次集预设以自动生成图像集和旋转集” [配置Dynamic Media](/help/assets/dynamic-media/config-dm.md).
...
可在用户界面中创建旋转集，也可使用 [批次集预设](/help/assets/dynamic-media/config-dm.md). 但是，使用批次集预设创建的集可以 *非* 显示在用户界面中。 您可以通过三种不同的方式访问通过批集预设创建的集。 （即使已在用户界面中创建旋转集，也可以使用这些方法）。
...
1. 打开单个资产的属性时。 属性指示所选资产所属的集(位于 **[!UICONTROL 集成员]**)。 要查看整个集，请选择集的名称。
...
1. 执行以下任一操作：
   ...
   * 选择一个旋转集资源，然后选择 **[!UICONTROL 编辑]** （铅笔图标）。
