...
>[!NOTE]
>
>有关资产用户界面的信息，请参阅[使用触屏UI管理资产](/help/assets/manage-digital-assets.md)，并将其应用到上传图像集资产的新文件夹。

## 快速开始:旋转集{#quick-start-spin-sets}

要快速设置并运行旋转集，请执行以下步骤：

1. 可选。[创建批集预设](/help/assets/dynamic-media/batch-set-presets-dm.md) 并将其应用到新的资产文件夹。
...
1. [将 URL 关联到您的 Web 应用程序](/help/assets/dynamic-media/linking-urls-to-yourwebapplication.md)或者[嵌入视频查看器或图像查看器](/help/assets/dynamic-media/embed-code.md)。
   ...
   Adobe Experience Manager资产会在您发布旋转集后为旋转集创建URL调用并将其激活。 预览资产时，您可以复制这些 URL。或者，您也可以将这些 URL 嵌入到网站中。
...
如有必要，您可以[编辑旋转集](#editing-spin-sets)。 此外，您还可以视图和修改[旋转集属性](/help/assets/manage-digital-assets.md#editing-properties)。

## 上传旋转集{#uploading-assets-for-spin-sets}的资产

对于一维旋转集，一个项目至少需要8-12张照片。拍照必须以固定的间隔拍摄，以给人以项目正在旋转和翻动的印象。 例如，如果一维旋转集包含12个镜头，则对每个镜头将项目旋转30°(360/12)。

您可以像上传Experience Manager资产](/help/assets/manage-digital-assets.md)中的任何其他资产一样，为旋转集上传图像。[

### 捕获旋转集{#guidelines-for-shooting-spin-set-images}的图像的指南

以下是关于旋转集图像的一些最佳实践。一般而言，旋转集中的图像越多，图像的旋转效果越好。但是，在旋转集中包含很多图像也会使加载图像所需的时间变长。Experience Manager建议拍摄用于旋转集的图像时遵循以下准则：

* 在一维旋转集中至少使用8-12幅图像，在二维旋转集中至少使用16-24幅图像。要转向360°，最少需要8幅图像。 一维旋转集比较常见，因为创建二维旋转集非常繁琐。
* 使用无损格式；建议使用 TIFF 和 PNG。
* 对所有图像使用蒙版，以使项目显示在纯白或其他高对比度的背景中。或者，也可以添加阴影。
* 确保充分突出产品细节，使其成为焦点。
* 拍摄时装的旋转图像时，使用模特道具或真人模特。通常，模特道具会被遮罩（使用玻璃材质的模特道具），或者图像中会显示风格化的模特道具/道具。 您可以通过定义多个角度来创建真人模特展示旋转集。可使用卷尺在地面上标出各个角度，以指导模特行走并目视每个拍摄方向。
...
>[!NOTE]
>
>旋转集中的图像显示顺序很重要。请务必订购它们，使旋转保持360度的顺畅视图。
...
1. 在旋转集编辑器的&#x200B;**[!UICONTROL 标题]**&#x200B;字段中，输入旋转集的名称。 该名称会显示在旋转集的横幅中。（可选）输入说明。
   ...
   >[!NOTE]
   >
   >创建旋转集时，您可以更改旋转集缩略图，或允许Experience Manager根据旋转集中的资产自动选择缩略图。 要选择缩略图，请单击&#x200B;**[!UICONTROL 更改缩略图]**&#x200B;并选择任何图像（您也可以导航到其他文件夹以查找图像）。 如果您选择了缩略图，然后决定要Experience Manager从旋转集中生成缩略图，请选择&#x200B;**[!UICONTROL 切换到自动缩略图]**。
...
您可以在用户界面中创建旋转集，也可以使用[批集预设](/help/assets/dynamic-media/config-dm.md)自动创建旋转集。 但是，使用批集预设创建的集，请执行&#x200B;*not*&#x200B;操作，以在用户界面中显示。 您可以通过三种不同方式访问通过批集预设创建的集。 （即使您在用户界面中创建了旋转集，这些方法也可用）。
...
1. 打开单个资产的属性时。 属性指示所选资产是其成员（位于“集成成员”]**下）。**[!UICONTROL &#x200B;要查看整个集，请点按该集的名称。
...
1. 从搜索中，您可以选择&#x200B;**[!UICONTROL 过滤器]**，然后展开 **[!UICONTROL Dynamic Media]**，并选择&#x200B;**[!UICONTROL 集]**。
   ...
   搜索会返回在UI中手动创建或通过批集预设自动创建的匹配集。 对于自动集，搜索查询使用与基于使用`Contains`搜索条件的Experience Manager搜索不同的`Starts with`搜索条件进行。 将筛选器设置为&#x200B;**[!UICONTROL Sets]**&#x200B;是搜索自动集的唯一方法。
