...
1. 为最大限度地提高智能成像的性能，Adobe建议将生存时间(TTL)设置为24小时或更长。 TTL定义CDN缓存资源的时长。 要更改此设置：
   ...
   1. 如果您使用Dynamic Media，请按照 [以下说明](config-dm.md)。 将“过 **[!UICONTROL 期]** ”值设置为24小时或更长。
...
要使用智能成像头，您需要在请求中添`cache=off`加修饰符。 请参[阅Dynamic](https://docs.adobe.com/content/help/en/dynamic-media-developer-resources/image-serving-api/image-serving-api/http-protocol-reference/command-reference/r-is-http-cache.html) Media图像服务和渲染API中的缓存。
