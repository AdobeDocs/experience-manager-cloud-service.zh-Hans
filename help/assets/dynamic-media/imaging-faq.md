...
要激活智能成像，贵公司的Dynamic Media Classic或Experience Manager上的Dynamic Media帐户必须将Adobe捆绑的CDN（内容分发网络）作为许可证的一部分提供。
...
是。智能成像可自动调整质量百分比。 此质量使用Adobe开发的机器学习算法来确定。 此百分比不特定于范围。
