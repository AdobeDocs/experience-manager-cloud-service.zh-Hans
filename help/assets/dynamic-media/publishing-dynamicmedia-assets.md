...
description: 了解如何发布Dynamic Media资源。
...
Dynamic Media您可以通过选择已上传的资源并选择 **[!UICONTROL Publish]** 或 **[!UICONTROL 快速发布]**. 发布Dynamic Media资源后，您可以通过URL或在页面中嵌入代码的方式将其包含在网页中。

您还可以即时发布您上传的资产，无需任何用户干预。 或者，您也可以有选择地发布这些资源。 参见 [配置Dynamic Media](config-dm.md). 或者，您也可以选择将资源发布到Dynamic Media或Adobe Experience Manager（二者相互排斥），并使用 **[!UICONTROL 选择性发布]** 在文件夹级别。 参见 [在Dynamic Media中使用“选择性发布”功能](/help/assets/dynamic-media/selective-publishing.md).

在 **[!UICONTROL 卡片视图]**，资产名称的正下方以及日期和时间左侧会显示一个小地球图标，指示资产已发布。 在&#x200B;**[!UICONTROL 列表视图]**&#x200B;中，**[!UICONTROL 已发布]**&#x200B;列显示已发布的资产和未发布的资产。

>[!NOTE]
>
>如果资产已发布，然后您将资产移动到另一个文件夹，并从其新位置重新发布，则原始已发布的资产位置以及新重新发布的资产仍然可用。 但是，原始已发布的资产会被Experience Manager“丢失”，无法取消发布。 因此，最佳做法是先取消发布资产，然后再将资产移动到其他文件夹。
...
参见 [HTTP/2内容交付常见问题解答](/help/assets/dynamic-media/http2faq.md).
