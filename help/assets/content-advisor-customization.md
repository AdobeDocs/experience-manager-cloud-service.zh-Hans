---
title: 自定义内容审查程序
description: 使用函数自定义应用程序中的内容审查程序。
role: Admin, User
...
# 内容审查程序自定义 {#content-advisor-customization}

Content Advisor允许您根据首选项、要求或功能需求定制各种组件。 您可以自定义以下组件[微前端内容顾问](#overview-asset-selector.md)：
...
* [dragOptions属性](#drag-options-property)
...
内容审查程序允许您控制已过期资源的使用情况。 您可以使用即将过期的&#x200B;**徽章**&#x200B;自定义已过期的资产，该徽章可帮助您提前了解将在当前日期起30天内过期的资产。 此外，还可根据需要自定义标记。 您还可以允许在画布上选择已过期的资源，反之亦然。 可以通过多种方式使用某些代码片段来自定义已过期的资源：
...
您可以自定义已过期资源的使用情况，使其成为可选或不可选。 您可以自定义是否允许将过期的资源拖放到内容审查程序画布上。 要实现此目的，请使用以下参数使资产在画布上不可选择：
...
内容审查程序允许您添加标记选取器过滤器。 它支持将所有相关标记组合到特定标记组的标记组。 此外，您还可以通过它选择与要查找的资源对应的其他标记。 此外，您还可以在上下文调用过滤器下设置您最常用的默认标记组，以便您能够随时访问它们。
...
## 在内容审查器中上载 {#upload-in-content-advisor}

您可以从本地文件系统将文件或文件夹上载到内容审查程序。 要使用本地文件系统上传文件，您通常需要使用Content Advisor微型前端应用程序提供的上传功能。 在内容审查器中调用上载所需的`upload`各种代码片段包括：
...
*更多属性包括`metadataSchema`、`onMetadataFormChange`、`targetUploadPath`、`hideUploadButton`、`onUploadStart`、`importSettings`、`onUploadComplete`、`onFilesChange`、`uploadingPlaceholder`*。 有关详细信息，请参阅[内容审查程序属性](/help/assets/content-advisor-properties.md)。
