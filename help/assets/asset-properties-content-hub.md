...
在AEM Assets Content Hub中预览资源及其属性可让用户密切查看资源详细信息，这对于高效的资源分发和管理至关重要。 随着数字信息的增长，仅依赖文件名和缩略图变得无法扩展。 查看详细属性有助于对资源进行分类，使其更易于访问、更易于操作，并确保所有用户的信息都完整。

### 如何在AEM Assets Content Hub中查看资产的资产并与之交互？

要在AEM Assets Content Hub中查看资源的属性，请导航到资源或搜索该资源，然后单击以打开其属性页面。 在这里，您可以放大或缩小预览、撤消缩放、移动到上一个或下一个资源、下载资源、使用Adobe Express编辑资源、将其添加到收藏集或关闭预览。 属性页面可显示标题、格式、大小、分辨率、标记、颜色标记和智能标记等详细信息。

### AEM Assets Content Hub中有哪些派生属性，以及如何生成这些属性？

上传和批准资源时，会自动生成AEM Assets Content Hub中的派生属性。 示例包括资源的大小、智能标记和颜色标记。 智能标记使用Adobe AI的智能内容服务来自动识别并应用相关标记，从而提高资产的可发现性。 颜色标记还可以使用AI自动识别，帮助用户根据其突出颜色识别资产。

### 管理员是否可以自定义在AEM Assets Content Hub中可见的资源属性？

是，管理员能够配置在AEM Assets Content Hub中为每个资源显示哪些属性。 既可以为资产预览用户界面执行此操作，也可以为搜索结果或收藏集中的资产卡执行此操作，以确保用户根据要求看到最相关的信息。

### 在AEM Assets Content Hub中预览资源时，支持哪些文件格式？

AEM Assets Content Hub支持的文件格式包括：用于图像的JPEG和PNG；用于视频的Quicktime、MP4和MPEG；用于文档的TXT、DOC/DOCX和XML；以及用于打印媒体的PDF。
