...
处理配置文件可以包含FPO（仅用于放置）演绎版。 请参阅[!DNL Adobe Asset Link] [文档](https://helpx.adobe.com/enterprise/using/manage-assets-using-adobe-asset-link.html)以了解是否需要为处理配置文件启用它。 有关详细信息，请参阅[Adobe Asset Link完整文档](https://helpx.adobe.com/enterprise/using/adobe-asset-link.html)。
...
*图：使用[!UICONTROL 服务参数]字段将添加的信息传递到自定义应用程序中的预定义参数。 在此示例中，在上传促销活动图像时，将使用`Jumanji`字体中的`Arial-BoldMT`文本更新图像。*
