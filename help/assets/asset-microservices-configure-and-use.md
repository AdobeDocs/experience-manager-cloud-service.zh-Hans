...
资产微服务允许您处理各种[文件类型](/help/assets/file-format-support.md)，这些文件类型的开箱即用格式比以前版本的[!DNL Experience Manager]所能提供的格式多。 例如，现在可以提取PSD和PSB格式的缩略图，而以前需要的第三方解决方案（如ImageMagick）就可以了。
...
### 自定义配置文件{#custom-profile-example}的示例
...
要配置在资产微服务处理完成后要为系统中上传或更新的资产执行的后处理工作流模型，需要配置自定义工作流运行器服务。

Adobe CQ DAM自定义工作流运行程序(`com.adobe.cq.dam.processor.nui.impl.workflow.CustomDamWorkflowRunnerImpl`)是OSGi服务，提供了两个配置选项：

* 按路径(`postProcWorkflowsByPath`)划分的后处理工作流：可以根据不同的存储库路径列出多个工作流模型。 路径和模型应使用冒号分隔。 支持简单的存储库路径，应该将其映射到`/var`路径中的工作流模型。 例如：`/content/dam/my-brand:/var/workflow/models/my-workflow`。
