...
## 如何在我的组织中评估Content Hub ？ {#how-can-i-evaluate-content-hub}
...
## 如何选择一个存储库用于仅查看选定环境的资产？ {#select-repository-multiple-environments}

当您为同一个程序的生产环境和其他较低环境配置了 Content Hub 后，您可以选择存储库并显示所选环境的资产。执行以下步骤：
...
## Content Hub 可以如何显示 .ZIP 文件类型的缩略图预览？ {#thumbnail-preview-zip-file}

要在 Content Hub 中提供 .ZIP 等文件类型的缩略图预览，您可以将名为 `cq5dam.preview.jpg` 或者 `cq5dam.preview.png` 的演绎版添加到 AEM as a Cloud Service 创作环境中可用的 .ZIP 的路径根目录中。
