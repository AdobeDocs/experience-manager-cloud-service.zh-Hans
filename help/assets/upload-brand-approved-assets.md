...
>[!VIDEO](https://video.tv.adobe.com/v/3445390/?captions=chi_hans&learn=on){transcript=true}
...
### 我可以从何处将哪些类型的资源上传到AEM Assets Content Hub？ {#asset-types-upload-to-content-hub}

有权添加资产的AEM Assets Content Hub用户可以从其本地文件系统上传品牌批准的资产。 所有上传的资源（无论其原始文件夹结构如何）将显示在Content Hub的顶级，以增强搜索功能。

### AEM Assets Content Hub如何增强资产搜索和整理功能？ {#search-content-hub}

AEM Assets Content Hub通过允许用户定义每次上传的关键详细信息（如促销活动名称、关键字、渠道、时间范围和区域等），增强了资源搜索和组织能力。 此外，它还会为每个资源自动生成其他属性（例如文件大小、格式和分辨率），并使用Adobe AI来应用智能标记，从而更加轻松快速地查找相关资源。

### 如何将资源从我的本地文件系统上传到AEM Assets Content Hub？ {#upload-assets-content-hub}

要将资源从本地文件系统上传到AEM Assets Content Hub，请单击&#x200B;**添加Assets**&#x200B;以打开上传对话框。 您可以拖放文件或文件夹，或手动浏览以选择它们。 您必须按营销活动名称对资源进行分组，建议填写其他字段，如关键字、渠道、时间范围和区域以更好地进行组织。 准备就绪后，单击&#x200B;**上传**，查看详细信息，并确认开始上传。
...
### 我能否配置在将资源上传到AEM Assets Content Hub时为必填或可选字段？ {#available-fields-while-uploading-assets-to-content-hub}

管理员可以使用配置用户界面定义在将资源上传到AEM Assets Content Hub时必填或可选字段。

### 如果上传的资源没有在AEM Assets Content Hub中自动显示，我应该怎么做？ {#assets-do-not-display-in-content-hub}

如果资源没有在AEM Assets Content Hub中自动显示，则意味着自动审批切换处于禁用状态。 资源位于Assets as a Cloud Service环境的&#x200B;**hydrated-assets**&#x200B;文件夹中。 您需要将其状态批量编辑为&#x200B;**已批准**，以便它们显示在Content Hub中。
