...
## 如何使用OpenAPI保护资产交付和搜索Dynamic Media？ {#secure-asset-delivery}
...
| [支持的图像修饰符集，如宽度、高度、旋转、翻转、质量和格式](/help/assets/deliver-assets-apis.md) | 丰富的可用图像修改器集 |
...
| 启用使用资产UID的安全、模糊的URL，而不会影响SEO。 | URL 混淆仅适用于 URL 查询参数。URL 中的资产 ID（资产名称）是可识别的。 |
...
## 如何测试实验修饰符？ {#modifiers-not-generally-available}

您可以测试任何修饰符，但通常无法通过实验API获得该修饰符。 例如，&lt;/adobe/experimental/advancememodifiers-expires-YYYYMMDD/assets>
