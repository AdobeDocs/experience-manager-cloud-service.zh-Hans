...
该消息建议您检查 [引用](/help/sites-cloud/authoring/getting-started/basic-handling.md#references) 要进一步调查的小组：
...
为了解决这些问题，内容片段模型包括 *锁定* 一旦发布，就会进入作者的只读模式。 表示为 **已锁定**:

![锁定内容片段模型的卡片](assets/cfm-model-locked.png)

当模型为 **已锁定** （在“只读”模式下），您可以查看模型的内容和结构，但无法编辑它们。

您可以管理 **已锁定** 从控制台或模型编辑器中选择模型：

* 控制台

   从控制台中，您可以使用 **解锁** 和 **锁** 操作：
