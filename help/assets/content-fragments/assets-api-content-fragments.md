...
例如， [单页应用程序(SPA)](/help/implementing/developing/hybrid/introduction.md)、基于框架或自定义的内容需要通过HTTP API提供的内容，通常采用JSON格式。
...
* 也被视为原子元素，也就是说，元素和变体作为片段属性的一部分显示，而不是作为链接或子实体显示。 这允许高效地访问片段的有效负载。
