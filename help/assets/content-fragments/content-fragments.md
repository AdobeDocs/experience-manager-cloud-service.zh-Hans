...
>
>创作内容片段有两个编辑器；尽管基本功能相同，但存在一些差异。 本节介绍原始编辑器，主要从&#x200B;**Assets**&#x200B;控制台访问。 有关新编辑器（主要从[内容片段](/help/sites-cloud/administering/content-fragments/authoring.md)控制台访问）的详细信息，请参阅站点文档&#x200B;**内容片段 — 创作**。 两个编辑器的顶部工具栏中都有一个切换开关，用于提供对另一个编辑器的快速访问。
...
* **关联的内容**
   ...
   * 资产通过收藏集[与片段](/help/assets/content-fragments/content-fragments-assoc-content.md)关联；关联的收藏集允许作者决定在创作页面时要使用的资产。
...
* 可用于处理内容片段[时的](/help/sites-cloud/authoring/fragments/content-fragments.md)页面编辑器。
...
>[!CAUTION]
>
>如果要使用MSM（这将创建内容片段的副本），则应该从各自&#x200B;**内容片段模型**&#x200B;中使用的任何数据类型中删除任何[唯一](/help/assets/content-fragments/content-fragments-models.md)约束。
