...
>[!IMPORTANT]
>
> 自2022年6月起，Adobe发布了一个新的原生集成，用于将Workfront与Adobe Experience Manager Assetsas a Cloud Service连接。 这种集成已成为连接这两种解决方案的必需方法。 未来任何新实施的增强型连接器（1.9.8及更高版本）都会被阻止，这些连接器用于将Workfront与AEM Assetsas a Cloud Service连接。
...
1. 单击启用自动创建链接的文件夹。 如果您返回到“事件订阅”选项卡，您会看到现在有一个创建事件。
