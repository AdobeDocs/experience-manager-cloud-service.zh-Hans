...
1. 选择 [!DNL Experience Manager] 徽标，然后转到 **[!UICONTROL 工具]** > **[!UICONTROL 资产]** > **[!UICONTROL 文件夹元数据架构]**.
...
1. 在“架构Forms”页面中，选择您创建的表单，然后选择 **[!UICONTROL 编辑]** 图标。
1. 在“文件夹元数据架构编辑器”页中，选择 **[!UICONTROL +]** 图标，以将选项卡添加到表单。 要重命名选项卡，请选择默认名称，并在下指定新名称 **[!UICONTROL 设置]**.
...
要编辑表单项的属性，请选择该组件并在中编辑以下属性的全部或子集 **[!UICONTROL 设置]** 选项卡。 建议仅将一个字段映射到元数据架构中的给定属性。 否则，系统会选取映射到属性的最新添加字段。
...
1. 从 [!DNL Experience Manager Assets] 用户界面，选择 **[!UICONTROL 创建]** 工具栏中。
1. 指定文件夹的标题和名称。
1. 从文件夹元数据架构列表中，选择所需的架构。 然后，选择 **[!UICONTROL 创建]**.
...
在各个字段中输入元数据值并选择 **[!UICONTROL 保存]** 以存储值。 您指定的值存储在CRX存储库的文件夹节点中。
