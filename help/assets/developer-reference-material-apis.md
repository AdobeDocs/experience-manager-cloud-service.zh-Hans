---
title: '作为Cloud Service,Adobe Experience Manager用于数字资产管理的Assets API '
...
# 作为Cloud ServiceAPI的资产 {#assets-cloud-service-apis}
...
* 二进制文件不会通过AEM，现在只需将上传过程与为部署配置的二进制云存储进行协调
...
第一步是将HTTPPOST请求提交到应创建或更新资产的文件夹；包括选择 `.initiateUpload.json` 器以指示请求将开始二进制上传。 例如，应创建资产的文件夹的路径为 `/assets/folder`。 POST请求为 `POST https://[aem_server]:[port]/content/dam/assets/folder.initiateUpload.json`。
...
启动上传的输出将包括一个或多个上传URI值。 如果提供了多个URI，则客户端有责任将二进制文件“拆分”为各个部分，并按顺序将每个URIPOST。 必须使用所有URI，且每个部件必须大于最小大小并小于启动响应中指定的最大大小。 这些请求将由CDN边缘节点前端，以加速二进制文件的上传。
...
* POST字节范围10,000 —— 上传URI列表中二进制到第二个URI的19,999
...
对于Adobe Experience Manager作为Cloud Service，仅支持新的上传API。 已弃用Adobe Experience Manager6.5的API。 与上传或更新资产或演绎版（任何二进制上传）相关的方法在以下API中已弃用：
