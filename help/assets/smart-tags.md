...
在创建标记模型并培训服务之前，请确定一组唯一的标记，这些标记最能描述业务环境中图像中的对象。 确保精选集中的资产符合培 [训准则](#training-guidelines)。
...
**覆盖**:培训中的图像应该有足够的多样性。 我们的想法是提供几个但相当多样化的例子，这样AEM就能学会专注于正确的事情。 如果要对视觉上不相似的图像应用同一标签，请至少包含每种类型的五个示例。 例如，对于标记下 *模式姿势*，为服务包括更多与下面突出显示的图像相似的培训图像，以便在标记过程中更准确地识别类似图像。
