...
AEM中的资源搜索支持以下用例，本文介绍这些用例的使用、概念、配置、限制和疑难解答。
...
当您开始键入关键字时，Experience Manager会建议可能的搜索关键字或短语。 这些建议基于Experience Manager中的资源。 Experience Manager会对所有metadata字段编制索引以帮助搜索。 为了提供搜索建议，系统使用以下几个元数据字段的值。 要提供搜索建议，请考虑使用相应的关键字填充以下字段：
...
资产选择器（在早期版本中称为资产选取器） [!DNL Adobe Experience Manager])允许您以特殊方式搜索、筛选和浏览DAM资源。 资源选择器位于 `https://[aem_server]:[port]/aem/assetpicker.html`. 您可以使用资源选择器获取所选资源的元数据。 您可以使用支持的请求参数启动它，例如资源类型（图像、视频、文本）和选择模式（单选或多选）。 这些参数为特定搜索实例设置资源选择器的上下文，并在整个选择过程中保持不变。
...
* **标记**：标记可帮助您对可以更高效地浏览和搜索的资源进行分类。 标记有助于将相应的分类传播给其他用户和工作流。 [!DNL Experience Manager] 提供了一些方法，可使用Adobe Sensei的人工智能服务自动标记资产，从而更好地通过使用和培训来标记资产。 搜索资产时，智能标记会考虑在内。 它与内置搜索功能配合使用。 参见 [搜索行为](#searchbehavior). 要优化搜索结果的显示顺序，您可以 [提升搜索排名](#searchrank) 少数精选资源的。
...
谓词用于创建Facet。 管理员可以使用预配置的谓词自定义“筛选器”面板中的搜索彩块化。 可以使用叠加自定义这些谓词。 参见 [创建自定义谓词](/help/assets/search-facets.md).
...
对于在单个文件夹或收藏集中可用的资产，更易于执行以下操作 [批量更新元数据](/help/assets/manage-metadata.md#manage-assets-metadata) 而不使用搜索功能。 对于跨文件夹可用的资源或符合通用标准的资源，通过搜索批量更新元数据会更快一些。
