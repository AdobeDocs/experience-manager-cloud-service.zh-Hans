---
title: 要与 [!DNL Adobe Creative Cloud]集成的最佳实践
...
>[!NOTE]
>
>Experience Manager到Creative Cloud的文件夹共享现已弃用，并且不再包括以下内容。 Adobe建议使用Adobe Asset Link或Experience Manager桌面应用程序等较新的功能，以便创意用户能够访问Experience Manager中管理的资源。

## Collaboration创意人员、营销人员和DAM用户的需求 {#collaboration-need-of-creatives-marketers-and-dam-users}
...
| 用例 | Adobe Asset Link | Experience Manager桌面应用程序 | 备注或备用方法 |
|----------------------------------------------------------|-----------------------------------------------------------------------------------|--------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------|
...
| 使用 — 打开资源 | 是 | 是 — 适用于任何应用程序 | 从Web界面[或从Finder打开](/help/assets/manage-digital-assets.md#previewing-assets) |
...
| 杂项 — 网络和访问 | 需要通过网络从用户桌面访问Experience Manager部署 | 需要通过网络从用户桌面访问Experience Manager部署 | Adobe Asset Link不共享网络代理环境。 |
...
* [Assets Web界面](/help/assets/manage-digital-assets.md)，其中包含外部参与方的区域，这些区域由Experience Manager访问控制设置进行保护，并包含必要的IT/网络配置调整，从而为这些外部用户授予访问Experience Manager的权限。
...
下面简要概述了Experience Manager与Creative Cloud集成的最佳实践。 请阅读本文档的其余部分以了解这些内容。

* **对于在Photoshop、InDesign或Illustrator中工作的创意用户：** Adobe Asset Link提供了最佳用户体验，包括对从Experience Manager中签出的正在进行的资源的简洁处理
* **为简化从桌面访问任何通用文件格式或应用程序资产的操作：**&#x200B;请使用Experience Manager桌面应用程序
...
[Experience Manager与Adobe Stock集成](/help/assets/aem-assets-adobe-stock.md)为Experience Manager用户提供了从Adobe Stock搜索、预览、许可和保存资源到Experience Manager中的功能。 已许可并保存的Adobe Stock资源已选择Stock元数据，这些元数据可用于通过额外的筛选器搜索这些资源。

此集成的一些要点：

* Adobe Stock中的资产保存到Experience Manager后，它们会成为常规的Experience Manager Assets，并将二进制文件保存到Experience Manager存储库。 Experience Manager中为该资源保存了某些与Adobe Stock相关的元数据，否则摄取过程将与任何其他文件相同。 例如，如果智能标记处于活动状态，则会在保存时将标记添加到这些资源。
* 保存到Experience Manager的资源是副本，而不是链接回Adobe Stock。

**在Creative Cloud中处理从Adobe Stock保存到Experience Manager中的资源**。 此集成独立于Adobe Asset Link，但Adobe Asset Link可以识别通过这种方式从Stock中保存的这些资源，并在Photoshop、Illustrator或InDesign的Adobe Asset Link扩展UI中，在这些资源上显示其他元数据和库存图标。 这些文件可用于浏览、打开等，因为它们是保存到Experience Manager中的常规Experience Manager资源。
在具有Creative Asset Link扩展的Creative Cloud应用程序中工作的Adobe用户，除了可以从Adobe Stock访问Experience Manager中已授予许可的资源外，还可以使用Creative Cloud Libraries面板来搜索、预览和许可Adobe Stock资源。
从Adobe Stock许可并保存到Experience Manager中的Assets可供访问Experience Manager Assets部署的更广泛团队使用，而创意人员通过Creative Cloud Libraries面板从Adobe Stock中许可资产，则默认情况下只能在其Creative Cloud帐户中为自己使用。
...
#### 从Experience Manager处理资源的工作正在进行中 {#work-in-progress-on-assets-from-aem}

如本文档中所述，建议对资源执行重大更新，有时也称为正在进行中的工作，并且不要将保存到本地文件的所有编辑也作为更改上传到Experience Manager。 这可以加快桌面用户的工作，限制使用的网络带宽，保持资产时间线整洁并专注于受控的重大更新。

Adobe Asset Link为以下用例提供了良好的支持：

* 当Photoshop、InDesign或Illustrator中的用户打算编辑文件时，他们对给定资源执行签出操作
* 在后台下载资产，通过Creative Cloud桌面应用程序将其放入与磁盘同步的Creative Cloud帐户中，并在资产的Experience Manager中切换签出标记以最大限度地减少编辑冲突
...
* 当创意用户完成更改后，他们可以在其Creative Cloud应用程序中对该文件执行签入操作，并提供可选评论。 Experience Manager中的相应资产将进行版本控制，并使用新二进制文件更新为。 营销人员或LOB用户等Experience Manager用户可通过Experience Manager资源时间线UI访问主要资源更改或里程碑。

Experience Manager桌面应用程序为在本机应用程序中打开的资产提供网络共享。 默认情况下，在本地完成的所有更改都会在短时间内自动上传到Experience Manager。 有了这样的配置，在进行中阶段频繁进行的保存操作都将上传到Experience Manager并进行版本控制，从而产生大量网络流量并带来潜在的可扩展性挑战，更不用说Experience Manager中不必要的版本了。

此处推荐的方法是使用Experience Manager桌面应用程序中的选项来关闭自动更新，并使用应用程序的资产状态UI中的上传更改操作手动将资产更改上传到Experience Manager。
...
如果您使用网络文件共享来管理数字资源，则只需使用Experience Manager桌面应用程序映射的网络共享即可成为一种方便的替代方式。 从网络文件共享过渡时，Experience Manager Web界面提供了一组丰富的数字资产管理功能，这些功能远远超出了网络共享上可提供的功能（搜索、收藏集、元数据、协作、预览等），而Experience Manager桌面应用程序则提供用于将服务器端DAM存储库与桌面上的工作关联的方便链接。

避免使用Experience Manager桌面应用程序直接在Experience Manager Assets的网络共享中管理资产。 例如，避免使用Experience Manager桌面应用程序移动/复制多个文件。 请改用Experience Manager Assets Web UI将文件夹从Finder/Explorer拖到网络共享或使用Experience Manager Assets文件夹上传功能。
