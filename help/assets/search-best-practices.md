...
**方案2：使用`classic car`文件夹中的`automobile`关键字搜索所有文件。**
...
如果要检索除某些关键字以外的结果，可以使用NOT运算符。 NOT运算符使用连字符(-)符号指示AEM搜索要从搜索结果中排除的内容。 例如，指定包含`car - classic`但不包含`car`的元数据的搜索查询`classic`。
