...
## 常见问题解答 {#faqs-content-hub-approved-assets}

### 在Experience Manager Content Hub as a Cloud Service中批准AEM Assets的资产有何用途？ {#approving-assets-content-hub}

批准资源可确保只有最新和批准的版本才能在AEM Assets Content Hub中使用，从而在所有渠道和应用程序中保持严格的品牌一致性。 这种受控流程可简化品牌经理和营销人员的资产管理。

### 批准AEM Assets Content Hub的资产需要什么先决条件？

您必须具有AEM Assets as a Cloud Service的访问权限和写入权限，才能编辑资源元数据，尤其是资源属性中的&#x200B;**状态**&#x200B;字段。

### 如何使用AEM as a Cloud Service中的Assets视图批准单个资源，以便该资源在AEM Assets Content Hub中可用？

选择资产，单击工具栏中的&#x200B;**详细信息**，导航到&#x200B;**基本**&#x200B;选项卡，从&#x200B;**状态**&#x200B;下拉列表中选择&#x200B;**已批准**，然后单击&#x200B;**保存**。 该资源在AEM Assets Content Hub中可用。

### 能否为AEM Assets Content Hub批量批准资源？如果是，如何批准？

可以，资产可以批量批准。 在Assets视图中，选择多个资源，单击&#x200B;**批量元数据编辑**，在“属性”下的&#x200B;**状态**&#x200B;字段中选择&#x200B;**已批准**，然后单击&#x200B;**保存**。 在AEM Assets Content Hub中，所有选定的资源都可用。

### AEM Assets Content Hub中的资源批准流程的工作原理 {#asset-approval-content-hub}

如果启用了自动审批切换，则使用AEM Assets Content Hub上传的资源会自动可用。 如果已禁用，则上传的资产将放在Assets as a Cloud Service的&#x200B;**hydrated-assets**&#x200B;文件夹中，您需要手动将其状态批量编辑为&#x200B;**已批准**，以便它们在Content Hub中显示。

### AEM Assets视图中的批准目标字段是什么，它如何影响资源发布？

通过“资产详细信息”页面上的&#x200B;**批准目标**&#x200B;字段，可选择已批准资产的发布位置。 选项包括&#x200B;**投放**(已发布到带有OpenAPI和Content Hub的Dynamic Media)或仅限&#x200B;**Content Hub**。 如果未选择任何选项，则会应用Assets as a Cloud Service环境的默认值。 有关详细信息，请参阅[已批准资产的默认批准目标和发布目标](#default-approval-target-options-publish-destinations)。


### 如果您在AEM Assets查看资源详细信息页面上看不到批准目标字段，会发生什么情况？

如果“Assets查看资源详细信息”页面上缺少&#x200B;**批准目标**&#x200B;字段，则应编辑您的元数据表单，将&#x200B;**批准**&#x200B;字段从可用组件拖到表单中，然后单击&#x200B;**保存**。 这允许您设置资源的批准目标。

### 如何在AEM Assets管理视图中自动批准新摄取的资源？

在创作环境中创建文件夹，导航到&#x200B;**工具** > **Assets** > **元数据配置文件**，创建和编辑元数据配置文件。 添加单行文本字段，将其标记为&#x200B;**已批准的Assets**，并将其映射到&#39;。/jcr:content/metadata/dam:status&#39;，并将其默认值设置为`approved`。 将元数据配置文件应用到文件夹。 这会自动批准添加到文件夹的新资产。

### 谁可以在AEM Assets Content Hub中访问批准的资源，以及已实施哪些控制？

在AEM Assets Content Hub中，属于同一组织的用户可以使用批准的资源。 严格的控制确保只能访问最新的批准版本，从而帮助维护品牌一致性和安全性。
