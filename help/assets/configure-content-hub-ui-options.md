...
在Content Hub上![配置UI上传详细信息](assets/filter-new.png)
...
1. 指定筛选器的标签，使用&#x200B;**[!UICONTROL 元数据]**&#x200B;字段将其映射到属性，并为新筛选器选择适当的输入类型。 输入类型标签将更新以与Adobe Experience Manager (AEM)保持一致。 可用的输入类型包括“文本”、“标记”、“日期”、“数字”、“下拉列表”、“多值文本”、“智能标记”和“智能颜色标记”。 对于某些元数据字段(xcm:colorDistribution， xcm:machineKeywords)，输入类型是自动分配的，无法修改。
...
#### 批量搜索 {#bulk-search-configuration}

要在[!DNL Content Hub]中同时搜索多个资源，请执行以下步骤：

1. 在[配置](#access-configuration-options-content-hub)用户界面上，单击&#x200B;**[!UICONTROL 筛选器]**。

1. 单击每个可用筛选器旁边的![编辑图标](assets/do-not-localize/edit_icon.svg)。

1. 启用&#x200B;**[!UICONTROL 批量搜索]**&#x200B;切换。 默认分隔符`[ , | \t | \r\n | \r | \n ]`会自动显示。 此外，您还可以添加其他分隔符。 为此，请在以`pipe symbol (|)`分隔的输入框中指定分隔符。

   ![批量搜索配置](assets/edit-filter-new.png)

1. 单击&#x200B;**[!UICONTROL 确认]**&#x200B;以保存更改。 请参阅[Content Hub中的批量搜索](search-assets-content-hub.md#bulk-search)的实际操作。
...
1. [从[!UICONTROL 徽标图像]部分更改徽标图像](#Change-the-logo-image)
...
#### 更改徽标图像{#change-the-logo-image}

在![品牌](/help/assets/assets/ColorPalette.svg) **[!UICONTROL 品牌]**&#x200B;页面上，执行以下步骤以更改[!DNL Content Hub]部署的徽标图像：

1. 单击![选择图像](/help/assets/assets/Browse.svg) **[!UICONTROL 选择图像]**&#x200B;以使用Asset Selector对话框选择徽标图像。 资源选择器仅显示批准的图像。
1. 选择图像，单击&#x200B;**[!UICONTROL 选择]**，然后单击&#x200B;**[!UICONTROL 保存]**&#x200B;以将其显示为您的[!DNL Content Hub]部署的徽标图像。
   ![横幅图像](/help/assets/assets/logo-image-content-hub1.png)
...
>[!NOTE]
>
> * **横幅图像**&#x200B;的建议大小为`height = 200 to 450px`和`width = 1920 to 2560px`。
> * **徽标图像**&#x200B;的建议大小为`height = 80 to 120px`和`width = 120 to 200px`。
> * 横幅和徽标图像的&#x200B;**支持的MIME类型**&#x200B;为`'JPG', value: 'image/jpeg'`、`'PNG', value: 'image/png'`、`'WEBP', value: 'image/webp'`、`'TIFF', value: 'image/tiff'`、`'SVG', value: 'image/svg+xml'`、`'GIF', value: 'image/gif'`。
...
## 常见问题解答 {#faqs-content-hub-upload-assets}

### 谁可以访问和配置AEM Assets Content Hub用户界面设置？

只有具有Content Hub管理员权限的用户才能访问和配置Content Hub用户界面设置。 在访问配置界面之前，必须通过Adobe Admin Console授予管理权限。

### 管理员可以在AEM Assets Content Hub UI中配置哪些内容？

管理员可以在AEM Assets Content Hub配置用户界面中配置以下设置：

* 导入元数据字段（包括必填字段和自动审批设置）

* 搜索页面上显示的搜索筛选器

* 用于索引的可搜索元数据字段

* 资产信息卡元数据（信息卡上最多显示六个字段）

* 资源详细信息视图属性

* 品牌元素（徽标、横幅文本、颜色、横幅图像）
...
这些设置控制最终用户在Content Hub中的视觉体验和功能行为。


### 管理员如何在AEM Assets Content Hub中为资源上传添加新的元数据字段？

管理员可以通过以下方式为资源上传添加新元数据字段：在&#x200B;**导入**&#x200B;选项卡中单击&#x200B;**添加元数据**，指定标签，将其映射到元数据架构中的相应属性，选择输入类型，并确定该字段是必需字段还是可选字段。 确认并保存之后，新字段将在资源上传期间供用户使用。

### 什么是AEM Assets Content Hub中资产的自动审批设置，它的工作原理是什么？

启用自动审批设置后，新资源可在上传后立即在Content Hub上使用。 默认情况下，此设置处于关闭状态，要求AEM作者或管理员手动批准新资源，然后才能将其显示在Content Hub门户中。

### 管理员如何在AEM Assets Content Hub中为资源搜索配置筛选器？

管理员可以通过单击&#x200B;**添加筛选器**、指定筛选器标签、将其映射到相应的元数据属性、选择数据类型以及确认添加来配置用于资源搜索的筛选器。 当至少有一个资源与筛选器条件匹配时，筛选器即可在界面中变得可用。 单击&#x200B;**保存**&#x200B;可应用更改。

### “资源详细信息”视图中可以显示哪些资源属性，以及在AEM Assets Content Hub中如何配置这些属性？

资源属性（如文件名、标题、格式、大小和自定义字段，如类别）可以显示在资源详细信息视图中。 管理员可通过以下方式添加这些属性：单击&#x200B;**添加元数据**，指定标签，将其映射到元数据架构，选择数据类型，确认并保存更改。

### 管理员如何在AEM Assets Content Hub中自定义资源卡片上显示的元数据详细信息？

管理员可以自定义资源卡上显示的元数据详细信息，方法是单击资源卡设置中的&#x200B;**添加元数据**，指定标签（例如文件格式），将其映射到元数据架构中的相应属性，并确认添加。 资产信息卡上最多可以显示6个元数据字段。

### 管理员能否自定义您的AEM Assets Content Hub门户网站以满足贵组织的品牌要求？

是。 管理员可以通过编辑主横幅的标题和正文文本，通过拾色器或十六进制代码选择文本颜色，以及为背景、文本和图标设置次要颜色和主要颜色，来个性化AEM Assets Content Hub的外观。
