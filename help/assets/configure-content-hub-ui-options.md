...
在Content Hub UI中，资源级别视图显示已批准资源的&#x200B;**资源卡**。 资产卡会显示用户的关键资产详细信息。 资产卡具有资产缩略图、资产元数据和其他可操作选项。 Content Hub管理员在存储库级别添加元数据字段，以便一次性在所有资源的资源卡中显示元数据名称及其元数据值。 资产卡接受六个元数据字段。 元数据字段包括元数据名称和元数据属性。 资源卡显示元数据及其元数据值。 管理员指定元数据名称并将其映射到正确的元数据属性。 元数据属性在资产卡中呈现元数据值。 元数据属性链接到资源的属性，因此呈现的元数据值在所有资源卡上有所不同。 例如，管理员在资产卡中添加元数据&#x200B;**已许可**，并将此&#x200B;**已许可**&#x200B;元数据映射到正确的元数据属性。 Content Hub UI上的资源卡显示此&#x200B;**已授予许可**&#x200B;元数据及其元数据值（**是**&#x200B;或&#x200B;**否**）。 在此示例中，具有许可证的Assets在资产卡中显示&#x200B;**是**，而无许可证的资产显示&#x200B;**否**。

Content Hub上的![资源属性](/help/assets/assets/metadata-on-asset-card.png)

#### 在资产信息卡下添加、编辑或删除元数据 {#add-edit-delete-metadata}

要将元数据添加到资源，请参阅以下说明：

1. 在[配置](#access-configuration-options-content-hub)用户界面上，单击&#x200B;**资产卡**。
2. 单击&#x200B;**添加元数据**。 显示&#x200B;**添加资源卡元数据**&#x200B;对话框。
3. 在&#x200B;**标签**&#x200B;字段中指定元数据名称，并在&#x200B;**元数据**&#x200B;字段中选择元数据属性。 例如，将元数据&#x200B;**Licensed**&#x200B;命名为，并选择&#x200B;**dc：license**&#x200B;作为元数据属性。 所选元数据属性将映射到在Content Hub UI中与元数据一起显示的值。 在此示例中，**dc：license**&#x200B;属性映射到许可证ID。 此属性遵循指定的资源(PDF许可证)路径以查找相应的资源，并根据资源的存在在UI中相应地呈现布尔值（**是**&#x200B;或&#x200B;**否**）。 如果找到许可证PDF，则显示&#x200B;**是**。 如果没有，则显示&#x200B;**否**。 因此，在Content Hub UI中，具有许可证ID的资源显示&#x200B;**是**，而没有许可证ID的资源显示&#x200B;**否**。
