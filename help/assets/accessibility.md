...
# [!DNL Adobe Experience Manager Assets]中的辅助功能为[!DNL Cloud Service] {#accessibility-in-aem-assets}
...
* 对于内容设计人员和创作人员，有一些功能可用于创建和发布供其客户和网站访客依次使用的辅助内容。 残障人士可以借助辅助型技术使用内容。 有关详细信息，请参阅[Web辅助功能指导原则](/help/onboarding/accessibility/web-accessibility.md)。
...
许多可单击或可操作指针的用户界面元素也可以使用键盘。 使用键盘，用户可以专注于UI元素并采取相应的操作。 用户可以直接使用键盘快捷键触发命令或操作，而无需关注UI元素并使用键盘触发它。 例如，用户可以使用键盘浏览到用户界面控件，然后选择`Return`并选择`Alt + 2`键盘快捷键，从左侧打开资产的时间轴。
...
| [!DNL Assets] 控制台 | `Alt + 5` | 在所选文件夹内调用搜索和搜索。 |
...
登录后，DAM用户可以使用键盘在[!DNL Assets]用户界面中导航。 用户界面元素(如左边栏、菜单、用户用户档案、搜索栏、文件和文件夹)以及管理和配置设置可通过键盘进行导航。 键盘导航顺序为从左到右和从上到下。 使用键盘导航时，焦点时的可操作选项会以更好的颜色对比度高亮显示，并由屏幕阅读器解说。 如果适用，屏幕阅读器会宣布菜单中重点选项的状态，例如，展开、折叠和混合状态。 此外，可操作选项的目的由屏幕阅读器来宣布，而不是由屏幕阅读器来宣布外观或UI位置。

如果用户从菜单中展开帮助或用户用户档案选项，屏幕阅读器会宣布相应的选项或状态。 如果用户展开用户用户档案选项，则可以使用键盘选择可用选项。 例如，管理员可以模拟其他用户。 如果用户从[!UICONTROL Help]选项中搜索字符串，则旁白将宣布“搜索帮助”以指示正在进行搜索。
...
许多资产管理任务（如CRUD操作、下载资产、添加元数据）都可以在不同程度上进行访问。 [!DNL Assets] 可让您使用各种辅助技术（尤其是屏幕阅读器和键盘）完成任务。
...
>[!MORELIKETHIS]
>
>* [发行说明中对每个版本中的增强功能进行了说明](/help/release-notes/release-notes-cloud/release-notes-current.md)。
>* [[!DNL Adobe Experience Manager] 辅助功能指导](/help/onboarding/accessibility/web-accessibility.md)。
