...
* **元数据属性：** 仔细识别和定义每个元数据属性。 确保每个资产都有明确的纳入理由，并且与目标和目的相关联。

为确保整个存储库获得一致的结果，请仔细规划该战略。 了解有关 [元数据架构](metadata-schemas.md).
...
例如，不要使用重复名称，如 *campaign_image_001* 和 *campaign_image_002*，实施系统化的命名约定，例如 *event_promotion* 和 *product_launch*，确保标识明确有序地进行。

* **有效使用受控词汇：** 通过使用标签的标准化术语来实现受控词汇。 了解如何实施 [AEM标记框架](/help/implementing/developing/introduction/tagging-framework.md) 有效。  <br>
...
* **关键词优化：** 通过优化与资源关联的关键字提高元数据可搜索性。 您可以在中提高特定资产的关键字相关性 [!UICONTROL 资产管理器] 按以下步骤操作：

   1. 转到 **[!UICONTROL 资产]** > **[!UICONTROL 文件]** > **[!UICONTROL [资产文件夹]]**.
   1. 选择要更新其元数据的资源，然后单击 **[!UICONTROL 属性]**.
   1. 导航至 **[!UICONTROL 高级]** 选项卡，然后单击 **[!UICONTROL 添加]** 在 **[!UICONTROL 提升搜索关键词]**. <br>必须使用默认元数据架构提升搜索关键词。
   1. 输入要增加搜索的关键字，然后单击 **[!UICONTROL 添加]**.<br>
您可以添加多个关键字，并根据优先级排列关键字。
   1. 单击 **[!UICONTROL 保存并关闭]**.
使用添加的关键字搜索资源。 该资源会出现在排名最前的搜索结果中。

  了解如何 [提升Experience Manager搜索](https://experienceleague.adobe.com/docs/experience-manager-learn/assets/search-and-discovery/search-boost.html).

* **自定义元数据字段：** 自定义元数据字段以捕获有关资源的其他信息。 例如，为项目详细信息、版权信息或任何其他增强搜索功能的相关数据添加特定字段。 学习 [如何编辑或添加自定义元数据](meta-edit.md) 在Experience Manager Assets中。
...
检查内容供应链以查找元数据源，并让所有利益相关者从头开始参与，以获得全面的最佳实践方法。 让不同的工作人员参与，以确保整个组织的全面支持。 <br>在各个阶段合并元数据，以分担在上传期间提供资产详细信息的责任。 例如，集成 [!DNL Experience Manager Assets] 和 [!DNL Workfront] 在元数据管理、提高内容创建和管理方面的效率和协作方面提供了巨大的好处。 此集成确保链接资源的有效元数据同步，并在发生更改时自动更新项目详细信息 [!DNL Workfront].
