...
### 用户如何访问AEM Assets Content Hub？可以分配哪些权限？

可以通过Adobe Admin Console将用户添加到AEM Assets Content Hub，方法是将用户分配到Content Hub的相关产品配置文件。

以下权限可供用户使用：

* Content Hub用户可以在Content Hub门户上访问品牌批准的资产。

* 除了访问品牌批准的资源、将资源上传到Content Hub以及通过Adobe Express集成编辑图像(如果您拥有Content Hub权限)之外，Content Hub管理员还可以访问Adobe Express上的配置用户界面。

* 有权添加资产的Content Hub用户除了能够在Content Hub门户上访问品牌批准的资产外，还可以将资产上传到Content Hub。

* 有权混合资产的Content Hub用户除了可以在Content Hub门户上访问品牌批准的资产外，还可以访问Adobe Express(如果您拥有Adobe Express权限)。

### 在AEM Assets Content Hub上，不同类型的用户可以使用哪些不同的产品配置文件？

AEM Assets Content Hub上的不同类型用户可以使用产品配置文件：

* Content Hub用户： AEM Assets有限用户

* Content Hub管理员：AEM Assets有限用户+ AEM管理员

* 有权添加资产的Content Hub用户： AEM Assets有限用户+ AEM用户

* 有权重新组合资源的Content Hub用户： AEM Assets有限用户+ AEM用户

### 管理员如何为其组织启用AEM Assets Content Hub？

要为组织启用AEM Assets Content Hub，管理员必须登录Cloud Manager，选择（或创建）其项目，在解决方案和加载项选项卡下启用Assets和Content Hub，并更新项目。 这会在Adobe Admin Console中创建一个Content Hub实例，以便管理用户访问权限。

### AEM Assets中包含多少个Content Hub受限用户？ {#content-hub-limited-users-with-aem-assets}

[Assets Ultimate](/help/assets/assets-ultimate-overview.md)和Assets as a Cloud Service各自包含250名Content Hub受限用户，而[Assets Prime](/help/assets/assets-prime.md)包含50名Content Hub受限用户。

### 我的Content Hub许可证中有多少个AEM Assets积分？

可用的Content Hub积分数取决于您的AEM Assets许可证：

* Assets Ultimate包括三个Content Hub积分。

* Assets Prime包含一个Content Hub点数。

* 现有Assets as a Cloud Service客户可获得一个Content Hub信用。

### 如何使用AEM Assets Content Hub积分？

每个启用Content Hub的环境都会消耗一个Content Hub点数。 例如，在生产、开发和暂存环境中启用Content Hub需要三个积分。

### 我可以在较低级别的环境中启用Content Hub吗？

是。如果您有可用的AEM Assets Content Hub积分，则可以在较低环境（例如开发或暂存）上启用Content Hub 。 启用的每个较低环境都会消耗一个点数。

### 如何拥有在AEM Assets Content Hub中访问已批准资源的权限？

AEM Assets Content Hub用户可以在Content Hub门户上访问品牌批准的资产。 您必须添加到AEM受限用户产品配置文件才能成为Content Hub用户。

### 如何拥有在AEM Assets Content Hub上上传资源的权限？

有权添加资产的AEM Assets Content Hub用户除了可以在Content Hub门户上访问品牌批准的资产外，还可以将资产上传到Content Hub。 您必须添加到AEM Limited Users和AEM Users产品配置文件，才能成为具有添加资产权限的Content Hub用户。

### 如何拥有在AEM Assets Content Hub上访问配置用户界面的权限？

AEM Assets Content Hub管理员除了可以访问Content Hub上的配置用户界面，还可以访问品牌批准的资源、将资源上传到Content Hub，以及Adobe Express集成以编辑图像(如果您拥有Adobe Express权限)。 您必须添加到AEM有限用户和AEM管理员产品配置文件，才能成为Content Hub管理员。

### 如何拥有在AEM Assets Content Hub中使用Adobe Express编辑图像的权限？

有权混合资产的AEM Assets Content Hub用户除了可以在Content Hub门户上访问品牌批准的资产外，还可以访问Adobe Express(如果您拥有Adobe Express权限)。 您必须添加到AEM Limited Users和AEM Users产品配置文件，才能成为有权重新混合资源的Content Hub用户。
