...
### 用户如何访问Content Hub以及可以分配哪些权限？

可以通过Adobe Admin Console将用户分配到Content Hub的相关产品配置文件，将其添加到Content Hub。
...
### Content Hub上不同类型的用户可以使用哪些不同的产品配置文件？

Content Hub上的不同类型用户可以使用产品配置文件：
...
### 管理员如何为其组织启用Content Hub？

管理员必须登录Cloud Manager，选择（或创建）其项目，在解决方案和加载项选项卡下启用Assets和Content Hub，并更新项目。 这会在Adobe Admin Console中创建一个Content Hub实例，以便管理用户访问权限。
...
是。如果您有可用的Content Hub积分，则可以在较低环境（如开发或暂存）上启用Content Hub。 启用的每个较低环境都会消耗一个点数。

### 如何拥有在Content Hub中访问已批准资源的权限？

Content Hub用户可以在Content Hub门户上访问品牌批准的资产。 您必须添加到AEM受限用户产品配置文件才能成为Content Hub用户。

### 如何拥有在Content Hub上上传资源的权限？

有权添加资产的Content Hub用户除了能够在Content Hub门户上访问品牌批准的资产外，还可以将资产上传到Content Hub。 您必须添加到AEM Limited Users和AEM Users产品配置文件，才能成为具有添加资产权限的Content Hub用户。

### 如何拥有在Content Hub上访问配置用户界面的权限？

除了访问品牌批准的资源、将资源上传到Content Hub以及通过Adobe Express集成编辑图像(如果您拥有Content Hub权限)之外，Content Hub管理员还可以访问Adobe Express上的配置用户界面。 您必须添加到AEM有限用户和AEM管理员产品配置文件，才能成为Content Hub管理员。

### 如何拥有在Content Hub中使用Adobe Express编辑图像的权限？

有权混合资产的Content Hub用户除了可以在Content Hub门户上访问品牌批准的资产外，还可以访问Adobe Express(如果您拥有Adobe Express权限)。 您必须添加到AEM Limited Users和AEM Users产品配置文件，才能成为有权重新混合资源的Content Hub用户。
