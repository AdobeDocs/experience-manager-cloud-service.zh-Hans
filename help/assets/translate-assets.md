...
## 准备要转换的资源{#prepare-assets-for-translation}
...
每个语言文件夹都称为语言副本。 语言副本的根文件夹（称为语言根）标识语言副本中内容的语言。 例如，`/content/dam/it`是意大利语语言副本的意大利语根。 语言副本必须使用[正确配置的语言根](#create-a-language-root)，以便在执行源资源翻译时瞄准正确的语言。
...
1. 创建主语言的语言根。 例如，示例文件夹层次结构中英语副本的语言根目录为`/content/dam/en`。 确保根据[创建语言根](#create-a-language-root)中的信息正确配置语言根。
...
例如，示例层次结构的意大利语语言副本的根页面具有`it`作为“名称”属性。 名称属性用作存储库中资产节点的名称，因此会确定资产的路径。 (*&lt;server>:&lt;port>/assets.html/content/dam/it/*)
...
1. 要开始资产的转换，请单击／点按&#x200B;**[!UICONTROL 转换作业]**&#x200B;拼贴上的箭头，然后从列表中选择&#x200B;**[!UICONTROL 开始]**。 系统会显示一条消息，通知翻译作业的开始。
...
1. 请参阅[添加到现有翻译项目](#add-to-existing-translation-project)的步骤9-14以完成其余步骤。
