...
您可以通过 [静态引用](#create-a-collection-with-static-references) 或基于 [基于搜索条件的过滤器](#create-a-smart-collection). 您还可以从灯箱创建收藏集。
...
1. 将资产拖到 **[!UICONTROL 放入收藏集]** 区域。 或者，点按/单击 **[!UICONTROL 收藏]** 图标。
...
1. 在“全源搜索”框中输入搜索关键词，然后选择 `Enter`. 点按/单击GlobalNav图标以显示“过滤器”面板，并从“搜索”面板中应用搜索过滤器。
...
1. 指定集合的名称。 选择 **[!UICONTROL 公共]** 将具有查看者角色的DAM用户组添加到智能收藏集。
   ...
   >[!NOTE]
   >
   >如果您选择 **[!UICONTROL 公共]**，则创建智能收藏集后，具有所有者角色的每个人都可以使用该智能收藏集。 如果取消 **[!UICONTROL 公共]** 选项，则DAM用户组将不再与智能收藏集关联。
...
您可以将资产添加到包含引用资产或文件夹列表的收藏集。

>[!NOTE]
>
>智能收藏集使用搜索查询来填充资产。 因此，对资产和文件夹的静态引用不适用于这些资产和文件夹。

1. 在资产UI中，导航到要添加到收藏集的资产所在的位置。
1. 选择资产，然后点按/单击 **[!UICONTROL 收藏]** 图标。 或者，您也可以将资产拖到 **[!UICONTROL 放入收藏集]** 区域。 当拖放区域变为活动状态且其标签更改为 **[!UICONTROL 拖放以添加]**.
...
收藏集中的搜索功能同时支持这两个功能 [搜索收藏集](#search-collections) 和 [搜索收藏集中的资产](#search-within-collections).

### 搜索收藏集 {#search-collections}

您可以从“收藏集”控制台中搜索收藏集。 在Omnisearch框中使用关键词搜索时， [!DNL Experience Manager Assets] 搜索收藏集名称、元数据和添加到收藏集的标记。
...
AdobeEnterprise Manager Assets允许您批量编辑许多收藏集的元数据。 使用 [!UICONTROL 属性] 页面以对多个收藏集执行元数据更改，例如，将元数据属性更改为公用值，或添加或修改标记。

自定义元数据 [!UICONTROL 属性] 页面（包括添加、修改、删除元数据属性）使用架构编辑器。

>[!NOTE]
>
>批量编辑方法适用于收藏集中可用的资产。 对于跨文件夹提供的资产或符合常用条件的资产，可以 [搜索后批量更新元数据](/help/assets/search-assets.md#metadata-updates).
...
1. 在各种选项卡下修改选定收藏集的元数据属性。
   ...
   >[!NOTE]
   >
   >您为选定收藏集添加的元数据将覆盖这些收藏集之前的元数据（标记除外）。 您在 **[!UICONTROL 标记]** 字段，将附加到元数据中的现有标记列表。
...
1. 从收藏集控制台中，选择所需的收藏集或收藏集组，然后点按或单击 **[!UICONTROL 收藏]** 中。
...
1. 在Omnisearch字段中使用光标，选择 `Enter` 键。
