...
收藏集是Adobe Experience Manager Assets中的一组资源。 使用收藏集可在用户之间共享资产。集合可以是静态集合或基于搜索结果的动态集合。
...
1. 向收藏集添加成员并分配相应的权限。或者，选择&#x200B;**[!UICONTROL 公共收藏集]**，以允许所有用户访问该收藏集。
   ...
   >[!NOTE]
   >
   >若要使成员能够与其他用户共享收藏集，请在路径`dam-users`上提供`home/users`组读取权限。 将权限授予位于`/content/dam/collections`位置的用户以允许这些用户在弹出列表中查看收藏集。 或者，将用户设为`dam-users`组的一部分。
...
1. 将资源拖到收藏集&#x200B;**[!UICONTROL 区域中的]**&#x200B;拖放位置。 或者，从工具栏中选择&#x200B;**[!UICONTROL 目标收藏集]**&#x200B;图标。
...
1. 选择资产并从工具栏中选择&#x200B;**[!UICONTROL 到收藏集]**&#x200B;图标。 或者，您可以将资产拖到收藏集&#x200B;**[!UICONTROL 区域中的]**&#x200B;放置。 当放置区域变为活动状态并且其标签更改为&#x200B;**[!UICONTROL 放置到“添加”]**&#x200B;时，释放鼠标按钮。
...
>[!NOTE]
>
>批量编辑方法适用于收藏集中可用的资产。 对于跨文件夹可用的资源或与通用条件匹配的资源，可在搜索[后](/help/assets/search-assets.md#metadata-updates)批量更新元数据。
