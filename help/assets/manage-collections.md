...
打开 **[!UICONTROL 收藏集]**，选择Experience Manager徽标。 从导航页面，转到 **[!UICONTROL 资产]** > **[!UICONTROL 收藏集]**.

## 创建收藏集 {#create-a-collection}

您可以使用以下任一方式创建收藏集 [静态引用](#create-a-collection-with-static-references) 或基于 [基于搜索条件的过滤器](#create-a-smart-collection). 您还可以从Lightbox创建收藏集。
...
1. 选择 **[!UICONTROL 创建]**，然后选择 **[!UICONTROL 确定]** 以关闭对话框。 具有指定标题和属性的集合将在“收藏集”控制台中打开。
   ...
   要将资源添加到收藏集，请导航到资源用户界面。 有关详细信息，请参阅 [将资产添加到收藏集](#add-assets-to-a-collection).
...
1. 将资源拖动到 **[!UICONTROL 放入收藏集]** 区域。 或者，选择 **[!UICONTROL 目标收藏集]** 图标。
1. 在 **[!UICONTROL 添加到收藏集]** 页面上，选择 **[!UICONTROL 创建收藏集]** 图标。 如果要将资源添加到现有收藏集，请从页面中选择它，然后选择 **[!UICONTROL 添加]**. 默认情况下，将选择最近更新的集合。
1. 在&#x200B;**[!UICONTROL 创建新收藏集]**&#x200B;对话框中，指定收藏集的名称。如果希望所有用户都可以访问该收藏集，请选择&#x200B;**[!UICONTROL 公共收藏集]**。
1. 选择 **[!UICONTROL 继续]** 以创建收藏集。

### 创建智能收藏集 {#create-a-smart-collection}

智能收藏集使用搜索条件动态填充资产。 您可以创建仅使用文件的智能收藏集，而不使用文件夹或文件和文件夹。

1. 导航到资产UI，然后选择 **[!UICONTROL Search]** 图标。
1. 在“全搜索”框中输入搜索关键字并选择 `Enter`. 选择GlobalNav图标以显示“筛选器”面板，并从“搜索”面板应用搜索筛选器。
...
1. 指定收藏集的名称。 选择 **[!UICONTROL 公共]** 将具有查看器角色的DAM用户组添加到智能收藏集。
   ...
   >[!NOTE]
   >
   >如果您选择 **[!UICONTROL 公共]**&#x200B;之后，该智能收藏集将可供所有具有所有者角色的用户使用。 如果您取消 **[!UICONTROL 公共]** 选项，DAM用户组不再与智能收藏集关联。

1. 选择 **[!UICONTROL 保存]** 创建智能收藏集，然后关闭消息框以完成该过程。 新的智能收藏集也会添加到 **[!UICONTROL 保存的搜索]** 列表。
“创建智能选 **[!UICONTROL 择”按钮的标签会变]** 为“编 **[!UICONTROL 辑智能选择”]**。 要编辑智能收藏集的设置，请从“文件和文 **[!UICONTROL 件夹]** ”列 **[!UICONTROL 表中选择“文件]** ”。 然后，选择 **[!UICONTROL 编辑智能选择]** 按钮。
...
1. 在Assets UI中，导航到要添加到收藏集的资产的位置。
1. 选择资产并选择 **[!UICONTROL 目标收藏集]** 图标。 或者，您可以将资产拖到 **[!UICONTROL 放入收藏集]** 区域。 当放置区域变为活动状态并且其标签变为 **[!UICONTROL 放置以添加]**.
...
1. 在Assets用户界面中，选择 **[!UICONTROL Search]** 图标。
...
1. 选择&#x200B;**[!UICONTROL 保存]**。此 **[!UICONTROL 编辑智能收藏集]** 出现对话框。
1. 选择 **[!UICONTROL 覆盖]** 将原始智能收藏集替换为已编辑的收藏集。 或者，选择 **[!UICONTROL 另存为]** 以单独保存已编辑的收藏集。
...
1. 从收藏集控制台中，选择一个收藏集，然后选择 **[!UICONTROL 属性]** 图标。
1. 在&#x200B;**[!UICONTROL 收藏集元数据]**&#x200B;页面中，从&#x200B;**[!UICONTROL 基本]**&#x200B;和&#x200B;**高级**&#x200B;选项卡中查看收藏集元数据。
1. 根据需要修改元数据，然后选择 **[!UICONTROL 保存并关闭]** 以保存更改。
...
收藏集中的搜索功能同时支持两者 [搜索收藏集](#search-collections) 和 [在收藏集中搜索资源](#search-within-collections).

### 搜索收藏集 {#search-collections}

您可以从“收藏集”控制台中搜索收藏集。 当您在Omnisearch框中搜索关键字时， [!DNL Experience Manager Assets] 搜索收藏集名称、元数据和添加到收藏集的标记。
...
1. 选择一个收藏集，然后选择 **[!UICONTROL 设置]** 图标。 或者，使用 **[!UICONTROL 设置]** 收藏集缩略图中的快速操作。
...
1. 从工具栏中选择下载图标。
1. 在 **[!UICONTROL 下载]** 对话框，选择 **[!UICONTROL 下载]**. 如果要下载收藏集中资产的演绎版，请选择 **[!UICONTROL 节目]**. <!-- Select the **[!UICONTROL Email]** option to send an email notification to the owner of the collection. -->
...
通过Enterprise Manager AssetsAdobe，可以批量编辑多个收藏集的元数据。 使用 [!UICONTROL 属性] 对多个收藏集执行元数据更改的页面，例如，将元数据属性更改为公共值或者添加或修改标记。
...
1. 在工具栏中，选择 **[!UICONTROL 属性]** 以打开 [!UICONTROL 属性] 页面中选定的收藏集。
1. 修改各种选项卡下所选收藏集的元数据属性。

   >[!NOTE]
   >
   >为所选收藏集添加的元数据将覆盖这些收藏集的先前元数据，但标记除外。 您在中添加的任何标记 **[!UICONTROL 标记]** 字段)会附加到元数据中的现有标记列表中。
...
1. 从收藏集控制台中，选择所需的收藏集或收藏集组，然后选择 **[!UICONTROL 目标收藏集]** 工具栏中。
...
1. 选择 **[!UICONTROL 添加]**. 此时将显示一条消息，确认该收藏集已添加到中的目标收藏集 **[!UICONTROL 选择目标]** 页面。 关闭消息以完成该过程。
...
1. 选择GlobalNav图标以显示“筛选器”面板。
1. 从 **[!UICONTROL 保存的搜索]** 列表，选择 **[!UICONTROL 删除]** ，该页面位于要删除的智能收藏集旁边。
