---
title: 管理资产收藏集
description: 了解AEM资产中的集合概念。 了解如何与其他用户一起收藏、管理、编辑和收藏集。
contentOwner: AG
mini-toc-levels: 1
...
### 使用Dropzone创建集合 {#create-collections-using-dropzone}

您可以将资产从资产UI拖动到集合。 您还可以创建收藏集的副本，并将资产拖动到该集合。

1. 从资产UI中，选择要添加到收藏集的资产。
1. 将资产拖至“放 **[!UICONTROL 入收藏集”区域]** 。 或者，点按／单击工 **[!UICONTROL 具栏中的]** “到集合”图标。
1. 在添加到 **[!UICONTROL 收藏集页面中]** ，点按／单击工具 **[!UICONTROL 栏中的创建收藏集图标]** 。 如果要将资产添加到现有收藏集，请从页面中选择它，然后点按／单击添 **[!UICONTROL 加]**。 默认情况下，将选择最近更新的集合。
...
1. 点按／单 **[!UICONTROL 击保存]** ，以创建智能收藏集，然后关闭消息框以完成该过程。 The new smart collection is also added to the **[!UICONTROL Saved Searches]** list.
...
1. 导航到资产用户界面，然后点按／单击工 **[!UICONTROL 具栏上]** 的搜索。
1. 在“Omnisearch”框中显示光标，按Return键。
...
## 在集合上运行工作流 {#run-a-workflow-on-a-collection}

您可以为集合中的资产运行工作流。 如果集合包含嵌套集合，则该工作流也会在嵌套集合中的资产上运行。 但是，如果集合和嵌套集合包含重复的资产，则此类资产只运行一次工作流。

1. 从“收藏集”控制台中，选择要运行工作流的收藏集。
1. 点按／单击GlobalNav图标，然后从列 **表中选择** “时间轴”。
1. 在时间轴中，单击或点按底部的尖角图标，然后点按／单击开始工 **作流**。
1. 在“开始 **工作流** ”(Start Workflow)部分，从列表中选择工作流模型。 例如，选择 **DAM更新资产模型** 。
1. 输入工作流的标题，然后点按／单击开 **始**。
1. 在对话框中，点按／单击继 **续**。 该工作流将在集合中的所有资产上运行。
