...
1. 为元数据配置文件（例如，示例元数据）输入标题，然后点按 **[!UICONTROL 提交]**. 此时会显示元数据配置文件的“编辑表单”。
...
编辑 **[!UICONTROL 描述]** 组件：
   ...
   * **[!UICONTROL 映射到属性]**  — 此属性的值提供资产节点在存储库中保存的相对路径/名称。值应始终以开头 `./` 因为它表示路径位于资产的节点下。
      ...
      您为&#x200B;**[!UICONTROL 映射到属性]**&#x200B;指定的值会作为属性存储在资产的元数据节点下。例如，如果您指定`/jcr:content/metadata/dc:desc` 作为的名称 **[!UICONTROL 映射到属性]**, [!DNL Adobe Experience Manager Assets] 存储值 `dc:desc` ，位于资产的元数据节点。
