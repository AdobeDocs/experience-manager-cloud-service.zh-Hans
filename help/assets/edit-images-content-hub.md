...
## 常见问题解答 {#faqs-edit-images-content-hub}

### 谁可以在AEM Assets Content Hub中编辑图像？

有权访问Adobe Express和AEM Assets Content Hub以及有权将资源重新混合到新变体的用户可以使用AEM Assets Content Hub编辑图像。

### 如何在AEM Assets Content Hub中使用Adobe Express编辑图像？

要在AEM Assets Content Hub中编辑图像，请单击该图像资产卡上的&#x200B;**在Adobe Express中打开**，或者打开该图像的详细信息并单击Adobe Express徽标。 这会在AEM Assets Content Hub中加载嵌入式Adobe Express编辑器，使您能够在不离开平台的情况下进行编辑。

### Adobe Express在AEM Assets Content Hub中提供了哪些编辑功能？

Adobe Express在AEM Assets Content Hub中提供了多种图像编辑功能，包括调整图像大小、删除或更改背景颜色、裁切图像，以及将其与AI生成的图像或文本组合等。

### 在AEM Assets Content Hub中，可以将编辑后的图像另存为哪些文件格式？

您可以在AEM Assets Content Hub中以PNG（质量高）、JPG（适用于小文件）或PDF（适用于文档）格式保存编辑后的图像。

### 在AEM Assets Content Hub中保存编辑的图像时，我应该填写哪些字段？

在AEM Assets Content Hub中保存编辑的图像时，您应在&#x200B;**另存为**&#x200B;字段中指定资源的名称，并在&#x200B;**促销活动名称**&#x200B;字段中指定促销活动名称。 Adobe还建议在关键字、渠道、时间范围和区域等其他字段中指定值以增强可搜索性和组织性。

### 能否在AEM Assets Content Hub中将编辑内容另存为新资源？

是，编辑后，您可以单击&#x200B;**另存为新资源**，以在AEM Assets Content Hub中将更改另存为新资源。

### 在将资源上传到AEM Assets Content Hub时，管理员能否自定义这些字段？

可以，管理员可以配置在向AEM Assets Content Hub添加资源时，必须填写或可选的字段，例如促销活动名称、关键字和渠道，以满足组织的需求。 使用配置UI上的&#x200B;**导入**&#x200B;选项卡配置字段。
