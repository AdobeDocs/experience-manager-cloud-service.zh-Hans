...
中的多站点管理器(MSM)功能 [!DNL Adobe Experience Manager] 使用户能够重复使用一次创作，并在多个Web位置重复使用的内容。 对于数字资产，可使用相同的功能，其名称为MSM [!DNL Assets]. 将MSM用于 [!DNL Assets]，您可以：
...
>[!NOTE]
>
>的MSM [!DNL Assets] 功能包括内容片段，其存储为 [!DNL Assets] （尽管被认为是Sites功能）。

>[!CAUTION]
>
>MSM用于内容片段仅在通过使用内容片段时可用 **[!UICONTROL 资产]** 控制台。
>
...
要了解在多个Web位置上重用相同内容（文本和资产）的使用方案，请参阅 [可能的MSM方案](/help/sites-cloud/administering/msm/overview.md). [!DNL Experience Manager] 维护原始资产与其链接副本之间的链接，称为活动副本(LC)。 维护的链接允许将集中更改推送到多个活动副本。 这样可以在消除管理重复副本的限制的同时实现更快的更新。 变化的传播是无差错的，并且是集中化的。 该功能为仅限选定活动副本的更新留出了空间。 用户可以分离链接（即中断继承），并进行本地编辑，以在下次更新主副本并转出更改时不会覆盖这些编辑。 分离操作适用于几个选定的元数据字段或整个资源。 它允许灵活地在本地更新最初从主副本继承的资产。
...
|---|---|---|
| [!UICONTROL 引用] 边栏 | 选择源后快速同步。 | 请参阅 [源的“引用”边栏中的快速操作](#ref-rail-source) |
...
1. 要启用项目的继承，请选择 **重新启用继承** 图标（位于所需项目旁）：
...
* 的MSM不支持在页面属性上配置MSM锁定 [!DNL Assets].
* 对于MSM [!DNL Assets]，仅使用 **[!UICONTROL 标准转出配置]**. 其他转出配置不适用于MSM的 [!DNL Assets].

>[!NOTE]
>
>请记住，内容片段的MSM(通过 **[!UICONTROL 资产]** 控制台)使用资产功能；这是因为资产作为资产存储（尽管被视为站点功能）。
