...
使用高级联网时，连接数受到限制，以确保跨环境的稳定性并防止较低环境耗尽可用连接。

每个AEM实例的连接数限制为1000，当数量达到750时，将向客户发送警报。

##### 连接限制是否仅适用于非标准端口的出站通信或所有出站通信？

此限制仅适用于使用高级网络（非标准端口上的出口，使用专用出口IP或VPN）的连接。

##### 在传出连接数方面，我们看不到任何显着差异。 为什么现在才收到通知？

如果客户动态创建连接（例如，每个请求有一个或多个），则流量的增加可能会导致连接激增。

##### 我们过去有没有可能在没有收到警报的情况下经历了类似的情况？

仅当达到软限制时才发送警报。

##### 如果达到最大限制会发生什么情况？

当达到硬限制时，将丢弃通过Advanced Networking（非标准端口上的出口，使用专用出口IP或VPN）从AEM发出的新出口连接，以防御DoS攻击。

##### 能否提高限制？

不需要，如果连接数量很大，则会对性能造成重大影响，并会导致Pod和环境之间的DoS。

##### 在一段时间后，AEM系统是否自动关闭连接？

是，连接在JVM级别关闭，网络基础架构中的不同点也关闭。 但是，对于任何生产服务来说，这都将为时已晚。 当不再需要连接时，应显式关闭连接，或在使用连接池时将其返回到池。 否则，资源消耗会太高，可能导致资源耗尽。

##### 如果达到最大连接限制，是否会影响任何许可证并导致额外成本？

否，此限制无相关许可证或成本。 这是一个技术限制。

##### 我们离极限有多近？ 最大限制是多少？

当连接数超过750时会触发警报。 最大限制是每个AEM实例1000个连接。

##### 此限制是否适用于VPN？

可以，该限制适用于使用高级网络（包括VPN）的连接。

##### 如果我们使用专用出口IP，此限制是否仍适用？

是，如果使用专用出口IP，则该限制仍然适用。
