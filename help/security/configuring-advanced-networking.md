...
请注意，以下几点作为信息性指南提供，并包含故障排除的最佳实践。 这些建议旨在帮助有效地诊断和解决问题。

### 连接池 {#connection-pooling-advanced-networking}

连接池是一种量身定制的技术，用于创建和保持连接存储库，随时可供任何需要连接的线程立即使用。 可以跨各种在线平台和资源找到多种连接池技术，每种技术都有其独特的优点和注意事项。 我们鼓励客户研究这些方法，以确定与其系统体系结构最兼容的方法。

实施适当的连接池策略是纠正系统配置中常见监督的主动措施，这通常会导致性能缺佳。 通过正确建立连接池，Adobe Experience Manager (AEM)可以提高外部调用的效率。 这不但减少了资源消耗，而且减少了服务中断的风险，减少了与上游服务器通信时遇到失败请求的概率。

根据此信息，我们建议您重新评估您当前的AEM配置，并考虑将连接池与高级联网设置结合使用。 通过实施这些措施，解决方案旨在防止代理过载，从而降低请求无法到达外部端点的可能性。

根据此信息，我们建议您重新评估您当前的AEM配置，并考虑将连接池与高级联网设置结合使用。 通过管理并行连接的数量并尽可能减少可能的陈旧连接的发生，这些措施可以减小代理服务器达到其连接限制的风险。 因此，这一战略实施旨在降低请求无法到达外部端点的可能性。
