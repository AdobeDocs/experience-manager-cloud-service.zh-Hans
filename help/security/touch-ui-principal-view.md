...
此视图允许用户高效地搜索指定路径和所选承担者上应用的权限。 在此视图中，用户可以清楚地识别针对所选路径授予一个或多个承担者的权限类型。

通过选中专用复选框，可以选择显示组成员资格。 启用此选项后，权限评估将考虑承担者的所有组，而不仅仅是选定的承担者。

此外，“Audit View”（审核视图）提供了对有效ACL的分析。 它显示与所选路径的父节点关联的ACL，同时考虑所选主体以及任何常见主体。

![筛选视图](assets/audit-view.png)

### 存储库浏览器权限和审核视图 {#the-repository-browser-permissions-and-audit-view}
...
1. 进入存储库浏览器后，单击&#x200B;**权限**&#x200B;选项卡
   ...
   ![权限选项卡](/help/security/assets/permissions-tab.png)

1. 审核视图允许用户有效地搜索指定路径和所选承担者上应用的权限。 通过选中专用复选框，可以选择显示组成员资格。

   ![审核选项卡](/help/security/assets/audit-tab.png)
