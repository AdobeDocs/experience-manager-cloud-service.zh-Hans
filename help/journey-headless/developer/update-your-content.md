...
利用这些操作，API 让您通过向 JavaScript 前端应用程序提供内容服务，将 Adobe Experience Manager as a Cloud Service 作为 Headless CMS（内容管理系统）进行操作。或者，任何其他可以执行 HTTP 请求并处理 JSON 响应的应用程序。例如，基于框架或自定义的单页应用程序 (SPA) 需要通过 API 提供的内容（通常采用 JSON 格式）。
...
>[!NOTE]
>
>请参阅 API 参考 [Adobe Experience Manager Assets API - 内容片段](https://www.adobe.io/experience-manager/reference-materials/cloud-service/javadoc/assets-api-content-fragments/index.html)。
...
有关使用 AEM Assets REST API 的更多详细信息，请参阅以下内容：
...
* [内容片段 REST API](/help/assets/content-fragments/assets-api-content-fragments.md)
   * [API 引用](/help/assets/content-fragments/assets-api-content-fragments.md#api-reference)
