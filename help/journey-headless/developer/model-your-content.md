...
# 如何为内容建模{#model-your-content}

在[AEM Headless Developer历程](overview.md)的这一部分中，您可以了解如何对内容结构建模。 然后，使用内容片段模型和内容片段实现Adobe Experience Manager(AEM)的结构，以便跨渠道重复使用。
...
也许，也许不是，但是这肯定是一个大的&#x200B;***复杂的***&#x200B;世界，数据建模用于定义非常（非常）小的子区域的简化表示，使用特定目的所需的特定信息。
...
您要描述的内容称为&#x200B;**实体** — 基本上是我们要存储相关信息的“事物”。

我们要存储的关于这些属性的信息是&#x200B;**属性**（属性），如“姓名”和“教师资格”。

然后，实体之间有各种&#x200B;**Relationships**&#x200B;关系。 例如，一所学校通常只有一位校长，而许多教师（而校长通常也是一位教师）。

分析和定义此信息的过程，以及它们之间的关系，称为&#x200B;**内容建模**。

### 基本信息 {#basics}

通常，您需要首先绘制一个&#x200B;**概念架构**&#x200B;以描述实体及其关系。 通常情况下，这是高级别（概念性）。

稳定后，您可以将模型转换为描述实体、属性和关系的&#x200B;**逻辑架构**。 在此级别，您应仔细检查定义，以消除重复并优化设计。

>[!NOTE]
>
>有时，这两个步骤会合并，通常取决于您方案的复杂性。

例如，您是需要单独的`Head Teacher`和`Teacher`实体，还是只需在`Teacher`模型上添加一个附加属性？
...
## 使用模型创作包含内容片段{#use-content-to-author-content}的内容

内容片段始终基于内容片段模型。 模型提供了结构，片段包含内容。

### 选择相应的型号{#select-model}
...
## 一些示例{#getting-started-examples}快速入门
...
现在，您已了解如何对结构进行建模并创建基于此的内容，接下来的步骤是[了解如何使用GraphQL查询访问和检索内容片段内容](access-your-content.md)。 这将介绍和讨论GraphQL，然后查看一些示例查询，以了解实际操作情况。
