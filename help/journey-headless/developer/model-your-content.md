...
开始时 [了解CMS无头开发](learn-about.md) 涵盖了无标题内容交付以及应使用它的原因。 然后 [AEM Headless入门as a Cloud Service](getting-started.md) 在您自己的项目上下文中描述了AEM Headless。
...
然后，可以创建内容片段模型并定义结构。 此操作可在 **工具** -> **常规** -> **内容片段模型**. 例如：
...
* [使用内容片段](/help/sites-cloud/administering/content-fragments/content-fragments.md)  — 内容片段的前置页面
