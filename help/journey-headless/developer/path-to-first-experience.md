...
在建立了内容模型和必须呈现内容的各种客户端的清晰视图后，您需要确保将与访问各种内容模型关联的GraphQL/API发布到需要此内容的所有客户端。 关于访问某些内容的方式，提供了不同的选项。您可以请求一段特定的静态内容，以便缓存内容并提高性能。您还可以请求动态生成的内容，而这将需要进行更多的处理。确保客户使用的API在满足其业务需求方面最有效。
...
* 如果在模型中设置根文件夹，则可以简化新内容片段的内联内容片段编辑器创建。然后，从业者不必选择位置，只需提供一个名称，即可开始编辑新引用。
