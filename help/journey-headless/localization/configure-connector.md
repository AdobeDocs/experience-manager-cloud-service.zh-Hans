...
在AEM无头本地化历程的上一个文档中，[开始使用AEM无头本地化](learn-about.md)您学习了如何组织无头内容以及AEM本地化工具的工作方式，您现在应该：
...
AEM翻译集成框架与第三方翻译服务集成以编排AEM内容的翻译。 这涉及到三个基本步骤。
...
第一步是选择要使用的翻译服务。 对于AEM提供的人文和机器翻译服务，有许多选择。 有关可用选项的选项选择，请参阅[其他资源](#additional-resources)部分。

大多数提供程序将提供要安装的翻译程序包。 为了完成此历程，我们将使用AEM提供开箱即用试用许可证的Microsoft Translator。 有关此提供程序的更多信息，请参阅[其他资源](#additional-resources)部分。

如果选择其他提供商，则需要按照翻译服务提供的说明安装连接器包。

>[!NOTE]
>
>在AEM中使用现成的Microsoft Translator不需要额外的设置，而且无需额外的连接器配置即可按原样工作。
>
>如果您选择使用Microsoft Translator连接器进行测试，则无需执行后两节中的步骤，但建议您阅读这些步骤，以便您熟悉何时需要连接首选连接器。
>
>Microsoft Translator连接器的试用许可证不用于生产目的，如果您决定许可该连接器，则需要按照本文档末尾的[其他资源](#additional-resources)部分中详述的后续步骤来配置该许可证。

## 创建翻译集成配置 {#create-config}

首先，您需要创建翻译集成框架配置以指定如何翻译内容。 配置包括以下信息：
...
1. 提供以下信息。
   ...
   1. **翻译方法**  — 根据您的 **翻** 译提供 **商，选** 择“机器翻译”或“人工翻译”。在此历程中，我们将假定是机器翻译。
...
AEM是一款灵活而强大的工具，通过多个连接器和多个配置支持多个同时翻译服务。 此设置不在此历程的范围之内，但意味着您需要指定应使用哪些连接器和配置来翻译内容。
...
在此知识的基础上，继续您的AEM无头本地化历程，方法是接下来查看文档[配置翻译规则](translation-rules.md)，您将在其中了解如何定义要翻译的内容。

## 其他资源 {#additional-resources}

虽然建议您通过查看文档[配置翻译规则](translation-rules.md)进入无头本地化历程的下一部分，但是以下是一些额外的可选资源，可更深入地了解本文档中提到的一些概念，但不需要继续访问无头本地化历程。
