...
1. 在全局导航菜单中，选择 **工具** > **Cloud Service** > **翻译Cloud Service**.
1. 导航到要在内容结构中创建配置的位置。 这一般为项目专属的位置，但也可为全局位置。
...
1. 选择 **创建** 和 **编辑配置** 此时将显示一个窗口，您可以在其中配置配置属性。

1. 请记住，内容片段作为资源存储在 AEM 中。选择 **资产** 选项卡。
...
1. 提供以下信息。
   ...
   1. **禁用仅更新翻译** – 选中此选项后，更新翻译项目将提交所有可翻译的字段以供翻译，而非仅提交自上次翻译以来更改的字段。在该历程的后面部分中会讨论如何更新您的翻译项目。
   1. **为翻译启用内容模型字段** – 选中此选项可让翻译配置自动根据&#x200B;**可翻译**&#x200B;标志识别内容模型中的字段。
...
1. 转到全局导航，然后转到 **导航** > **资产** > **文件**.
1. 在资产控制台中，选择要配置的语言根，然后选择 **属性**.
1. 选择&#x200B;**云服务**&#x200B;选项卡。
1. 下 **Cloud Service配置** 在 **添加配置** 从下拉列表中，选择您的连接器。 当您将其软件包安装为 [如前所述。](#connect-translation-provider)
1. 下 **Cloud Service配置** 在 **添加配置** 下拉列表中，也选择您的配置。
1. 选择 **保存并关闭**.
...
在此知识的基础上继续您的AEM Headless翻译历程，接下来查看文档 [翻译内容，](translation-rules.md) 在这里，您可以了解目前为止如何使用您的配置来实际翻译您的内容。
