...
需要本地开发环境才能完成本教程。 这包括已配置并连接到Adobe Commerce实例的AEM运行实例。 查看 [使用AEMas a Cloud ServiceSDK设置本地开发](../develop.md).
...
负责呈现店面主题/样式的CSS和JavaScript由 [客户端库](/help/implementing/developing/introduction/clientlibs.md) 或clientlibs。 客户端库提供了一种机制，用于在项目代码中组织CSS和Javascript，然后将其交付到页面。
...
*编译的CSS和Javascript将从 `ui.frontend` 模块 `ui.apps` 模块作为Maven内部版本期间的客户端库*
...
1. 在 `ui.apps` 模块检查文件： `ui.apps/src/main/content/jcr_root/apps/venia/clientlibs/clientlib-site/css/site.css`:
   ...
   复制的 `site.css` 文件 `ui.apps` 项目。 它现在是名为 `clientlib-site` 具有 `venia.site`. 文件成为 `ui.apps` 模块可将其部署到AEM。
...
1. 接下来，检查由项目生成的其他客户端库：
   ...
   这些客户端库不由 `ui.frontend` 模块。 相反，这些客户端库包含由Adobe提供的CSS和JavaScript依赖项。 这些clientlibraries的定义位于 `.content.xml` 文件。
   ...
   **clientlib-grid**  — 这包括启用AEM响应式网格功能所需的CSS。 使用AEM网格可启用 [布局模式](/help/sites-cloud/authoring/features/responsive-layout.md) 在AEM编辑器中，允许内容作者重新调整组件大小。 类别为 `venia.grid` 并嵌入在 `venia.base` 库。
...
1. 选择 **页面信息** 菜单，单击 **查看已发布的项目**:
   ...
   这将打开页面，且未加载任何AEM作者javascript，就像它显示在已发布的网站上一样。 请注意，url具有查询参数 `?wcmmode=disabled` 已附加。 在开发CSS和Javascript时，最好使用此参数来简化页面，而不使用AEM作者提供的任何内容。
...
1. 查看页面源，您应该能够识别其中包含的多个客户端库：
   ...
   客户端库在交付到页面时以为前缀 `/etc.clientlibs` 和通过 [代理](/help/implementing/developing/introduction/clientlibs.md) 避免在 `/apps` 或 `/libs`.
...
有关如何包含客户端库的选项有多种。 接下来，检查生成的项目是如何包含 `clientlib-site` 库通过 [页面模板](/help/implementing/developing/components/templates.md).
...
1. 这将打开登陆页面模板的页面策略：
   ...
   * `venia.site`  — 这是 `clientlib-site` 该 `ui.frontend` 模块生成。
   ...
   请注意，其他模板使用相同的策略， **内容页面**, **登陆页面**&#x200B;等……通过重新使用相同的策略，我们可以确保所有页面上都包含相同的客户端库。
...
1. 打开新的终端窗口并导航到 `ui.frontend` 文件夹。 运行命令 `npm start`:
   ...
   >[!CAUTION]
   >
   > 如果出现与Sass相关的错误，请停止服务器并运行命令 `npm rebuild node-sass` 并重复上述步骤。 如果的 `npm` 和 `node` 然后在项目中指定 `aem-cif-guides-venia/pom.xml`.
...
使用 [AEM样式系统](/help/sites-cloud/authoring/features/style-system.md) 创建两个样式，以便内容作者可以打开/关闭这些样式。 [用风格体系发展](https://experienceleague.adobe.com/docs/experience-manager-learn/getting-started-wknd-tutorial-develop/style-system.html) 包括有关如何完成此操作的详细步骤和信息。
