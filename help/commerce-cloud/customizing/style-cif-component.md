...
需要本地开发环境才能完成本教程。 这包括已配置并连接到AEM实例的Magento实例。 查看[使用AEM作为Cloud ServiceSDK](../develop.md)设置本地开发的要求和步骤。
...
负责呈现店面主题/样式的CSS和JavaScript在AEM中由[Client library](/help/implementing/developing/introduction/clientlibs.md)或clientlibs来管理。 客户端库提供了一种机制，用于在项目代码中组织CSS和Javascript，然后将其交付到页面。
...
1. 在`ui.apps`模块中，检查文件：`ui.apps/src/main/content/jcr_root/apps/venia/clientlibs/clientlib-site/css/site.css`:
   ...
   这会将复制的`site.css`文件复制到`ui.apps`项目中。 它现在是名为`clientlib-site`且类别为`venia.site`的clientlibrary的一部分。 文件成为`ui.apps`模块的一部分后，即可将其部署到AEM。
...
1. 接下来，检查由项目生成的其他客户端库：
   ...
   **clientlib-grid**  — 这包括启用AEM响应式网格功能所需的CSS。使用AEM网格可在AEM编辑器中启用[布局模式](/help/sites-cloud/authoring/features/responsive-layout.md)，并使内容作者能够重新调整组件的大小。 类别为`venia.grid`，并嵌入在`venia.base`库中。
...
1. 查看页面源，您应该能够识别其中包含的多个客户端库：
   ...
   客户端库在交付到页面时以`/etc.clientlibs`为前缀，并通过[proxy](/help/implementing/developing/introduction/clientlibs.md)提供，以避免公开`/apps`或`/libs`中任何敏感内容。
...
有关如何包含客户端库的选项有多种。 接下来，通过[页面模板](/help/implementing/developing/components/templates.md)检查生成的项目如何包含`clientlib-site`库。
...
在上一个练习中，对`ui.frontend`模块中的Sass文件进行了更新，然后在执行Maven内部版本后，更改将部署到AEM。 接下来，我们将研究如何利用Webpack-dev-server来快速开发前端样式。
...
1. 打开新的终端窗口，然后导航到`ui.frontend`文件夹。 运行命令`npm start`:
   ...
   >[!CAUTION]
   >
   > 如果出现与Sass相关的错误，请停止服务器并运行命令`npm rebuild node-sass`并重复上述步骤。 如果具有不同版本的`npm`和`node`，然后在项目`aem-cif-guides-venia/pom.xml`中指定，则可能会发生这种情况。
