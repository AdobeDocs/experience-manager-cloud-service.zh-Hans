...
需要本地开发环境才能完成本教程。 这包括已配置并连接到AEM实例的Magento实例。 查看[使用AEM作为Cloud ServiceSDK](../develop.md)设置本地开发的要求和步骤。 要完整地学习本教程，您需要拥有将[属性添加到Magento中产品](https://docs.magento.com/user-guide/catalog/product-attributes-add.html)的权限。
...
查看&#x200B;**New**&#x200B;标记的功能，该标记已在Product Teaser中实施。 尝试为作者添加一个额外的复选框，以控制何时应显示&#x200B;**Eco Friendly**&#x200B;标记。 您需要更新位于`ui.apps/src/main/content/jcr_root/apps/venia/components/commerce/productteaser/_cq_dialog/.content.xml`的组件对话框。
