...
需要本地开发环境来完成本教程。 这包括正在运行的AEM实例，该实例已配置并连接到Adobe Commerce实例。 查看的要求和步骤 [使用AEMas a Cloud ServiceSDK设置本地开发](../develop.md). 要完全遵循本教程，您需要具有添加教程的权限 [产品的属性](https://docs.magento.com/user-guide/catalog/product-attributes-add.html) 在Adobe Commerce中。
...
1. 更新GraphQL查询以包含 `eco_friendly` 部分查询中的属性：
   ...
   在上面的代码中，`addCustomSimpleField` 用于检索 `eco_friendly` 属性。 这说明了如何查询作为Adobe Commerce架构一部分的任何自定义属性。
...
AEM组件的一种常见扩展是修改组件生成的标记。 这是通过覆盖 [HTL脚本](https://experienceleague.adobe.com/docs/experience-manager-htl/using/overview.html?lang=zh-Hans) 组件用于呈现其标记的内容。 HTML模板语言(HTL)是一种轻量级模板语言，AEM组件使用它根据创作的内容动态呈现标记，从而允许重复使用组件。 例如，产品Teaser可以重复使用，以显示不同的产品。
...
1. 修改 `productteaser.html` 以调用 `isEcoFriendly` 在上一个练习中实施的方法：
   ...
   有关以下内容的更多信息 `data-sly-test` 和其他 [可以在此处找到HTL块语句](https://experienceleague.adobe.com/docs/experience-manager-htl/using/htl/block-statements.html#test).
