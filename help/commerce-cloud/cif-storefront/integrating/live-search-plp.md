...
Adobe Commerce Live Search可提供快速、相关和直观的搜索体验，而无需支付额外费用。 由Adobe AI提供支持的实时搜索使用人工智能和机器学习算法来对汇总的访客数据进行深入分析。 此数据与Adobe Commerce目录结合使用后，可提供相关且个性化的购物体验。
...
PLP组件要求安装[[!DNL Live Search] Popover CIF组件](/help/commerce-cloud/cif-storefront/integrating/live-search-popover.md)。 PLP构件需要由弹出窗口生成的浏览器会话变量。
