---
title: Adobe Experience Manager云服务的SEO和URL管理最佳实践
seo-title: Adobe Experience Manager云服务的SEO和URL管理最佳实践
...
# Adobe Experience Manager云服务的SEO和URL管理最佳实践{#seo-and-url-management-best-practices-for-aem}

搜索引擎优化(SEO)已成为许多营销人员的一个关键问题。 因此，许多Adobe Experience Manager(AEM)需要将SEO问题作为云服务项目加以解决。

本文档首先介绍了在 [AEM上作为云服务实施实现这些功能的一些](#seo-best-practices) SEO最佳实践和建议。 然后，本文档将更深入地介绍第一部分中提 [出的一些更复](#aem-configurations) 杂的实施步骤。
...
* 尽可能避免使用查询参数。 必要时，将其限制为两个或更少。
   ...
   * 如果可用，请使用目录结构指示信息架构。
   * 如果目录结构不是选项，请在URL中使用Sling选择器，而不是查询字符串。 除了它们提供的SEO值之外，sling选择器还将使调度程序的页面可缓存。
...
近年来的趋势是删除这些URL，以便使URL更易读。 在许多平台上，这涉及在Web服务器或内容投放网络(CDN)上实施重定向，但Sling使这变得简单明了。 Sling选择器：

* 提高URL可读性。
* 允许您在调度程序上缓存页面，并经常提高安全性。
* 允许您直接对内容进行地址处理，而不是使用检索内容的通用servlet。 这样，您就可以从应用到存储库的ACL和应用到调度程序的过滤器中受益。
...
**Bin** Servlets遵循许多开发者在J2EE编程中使用的模式。 Servlet在特定路径中注册，在AEM通常处于下方的情况下 `/bin`，您可以从查询字符串中提取所需的请求参数。
...
* 必须（仔细）配置调度程序才能公开 `/bin/myApp/myServlet`。 仅公开 `/bin` 即允许访问某些不应对站点访客开放的Servlet。
...
在AEM中，所有网页都存储在下面 `/content/my-brand/my-content`。 虽然从存储库数据管理的角度来看，这可能很有用，但并不一定是您希望客户如何查看您的网站，并且可能与SEO指导相冲突，以尽可能缩短URL。 此外，您可能正在从同一AEM实例和不同域名中提供多个网站。
...
对页面名称进行本地化的难题是，AEM平台上的许多可用本地化工具都依赖于让页面名称在不同区域设置之间匹配以保持内容同步。
...
>[!NOTE]
>
>在编 `sling:alias` 辑页面属性时，可以使用 [Alias属性设置该属性](/help/sites-cloud/authoring/fundamentals/page-properties.md#advanced)。
...
虽然这是官方AEM和Sling文档中记录的方法，但与直接使用的选项相比，此实施提供的常规表达式支持在范围上有限 `SlingResourceResolver` 。 此外，以这种方式实施映射可能会导致调度程序缓存失效问题。
...
1. **SlingResourceResolver规则**
   ...
   * **Apache Sling Resource Resolver Factory**
   ...
   建议您构建将URL缩短为常规表达式所需的映射，然后在OsgiConfignode（包含在您的构建中）下定义这些配置。 `config.publish`
...
1. **映射页面上的URL输出**
   ...
   例如，如果您正在实现一个自定义导航组件，该组件列表了当前页面的子项，则可以使用映射方法，如下所示：
...
拼图的最后一部分是当这些缩短的URL进入调度程序时处理它们，这就是 `mod_rewrite` 开始发挥作用的地方。 使用URL的最大优 `mod_rewrite` 点是，在将URL发送到调度程序模块之前， *URL* 会映射回长表单。 这意味着调度程序将从发布服务器请求长URL并相应地缓存它。 因此，从发布服务器传入的任何调度程序刷新请求都将能够成功使此内容失效。
...
规范的URL标记是放置到HTML文档标题中的链接标记，用于阐明搜索引擎在为内容编制索引时应如何对待页面。 他们优惠的好处是，确保（不同版本）页面的索引与相同，即使页面的URL可能包含差异也是如此。

例如，如果站点要优惠打印机友好版本的页面，则搜索引擎可能会将此页面与常规版本的页面分开索引。 标准标签将告诉搜索引擎它们是相同的。
...
要以编程方式生成sitemap，请注册一个侦听调用的Sling Servlet `sitemap.xml` 。 Servlet随后可以使用通过Servlet API提供的资源查看当前页面及其子页面，输出XML。 然后，XML将缓存在调度程序中。 此位置应在文件的sitemap属性中引 `robots.txt` 用。 此外，需要实施自定义刷新规则，以确保在激活新页面时刷新此文件。

>[!NOTE]
>
>可以注册一个Sling Servlet以侦听带扩展 `sitemap` 的选择器 `xml`。 这将导致Servlet在请求URL时处理请求，该URL以下结束：
