...
>[!NOTE]
>
> 本文介绍的某些产品实例和产品配置文件可能仅出现在新创建的环境中。请参阅[为现有环境添加产品轮廓部分](#adding-product-profiles-for-existing-environments)，了解如何使环境符合现代化要求。
...
2024 年 4 月初之前创建的环境可能会缺少上述部分描述的组织级产品实例以及某些产品轮廓。现有的产品轮廓也会缺少服务切换功能。建议更新这些产品轮廓，这是访问某些未来 API 的前提条件。
