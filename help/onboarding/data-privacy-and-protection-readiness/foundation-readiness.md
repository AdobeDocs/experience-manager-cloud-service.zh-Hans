...
description: '了解Adobe Experience manager作为各种数据保护和数据隐私法规的云服务基础支持；包括欧盟一般数据保护规定(GDPR)、加利福尼亚州消费者隐私法以及在将新AEM作为云服务项目实施时如何遵守这些规定。 '
...
在AEM Foundation级别，存储的个人数据保留在用户配置文件中。 因此，本文中的信息主要介绍如何访问和删除用户配置文件，以分别解决访问和删除请求。

## 访问用户配置文件 {#accessing-a-user-profile}

### 手动步骤 {#manual-steps}

1. 通过浏览至“工具”-“安全”-“用户” **[!UICONTROL ，或直接浏览至]** “ `https://<serveraddress>:<serverport>/security/users.html`
...
2. 将鼠标悬停在用户上并单击选择图标。 配置文件将变为灰色，表示已选择配置文件。
...
4. 最后，确认操作。
   ...
   用户界面随后将指示已通过注销和向配置文件卡添加锁定来取消激活用户帐户：
...
>[!NOTE]
>
> 对于AEM作为云服务，在UI中没有可用于删除用户配置文件的手动过程，因为CRXDE无法访问。
