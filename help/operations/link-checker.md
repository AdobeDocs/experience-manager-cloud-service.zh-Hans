...
* 该页包含指向`/content/wknd/us/en/adventures/extreme-ironing`文本组件中的[的链接。](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-core-components/using/wcm-components/text)
...
* 您创建页面`/content/wknd/us/en/adventures/ski-touring`
* 该页包含指向`https://bunwarmerthermalunderwear.com`文本组件中的[的链接。](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-core-components/using/wcm-components/text)
...
1. 然后，**Adobe AEM Link Checker服务**&#x200B;会定期执行以检查缓存中的条目是否为有效语法。
1. 经过语法验证的链接随后显示在[外部链接检查器窗口中。](#external-using)，但是它们将处于&#x200B;**待处理**&#x200B;状态。
1. 然后，定期执行&#x200B;**Adobe AEM链接检查器任务**，以通过进行GET调用来验证链接。
1. **Adobe AEM链接检查器任务**&#x200B;随后将使用GET调用的结果更新[外部链接检查器窗口](#external-using)中的条目。
...
* **Adobe AEM Link Checker Info Storage Service** — 此服务定义存储库中Link Checker缓存的大小。
* **Adobe AEM链接检查器服务** — 此服务对外部链接的语法执行异步检查。
   * 除其他选项外，您还可以定义检查时段以及检查器跳过的链接类型。
* **Adobe AEM链接检查器任务** — 此服务执行外部链接的GET验证。
   * 它允许单独定义间隔，以检查其他选项中的坏链接和好链接。
* **Adobe AEM链接检查器转换器** — 此服务根据用户定义的规则集转换链接。
