...
查询性能工具可通过Cloud Manager[中的](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/developer-console.html?lang=zh-Hans#queries)Developer Console访问。 与AEM 6.x版本相比，AEM as a Cloud Service的查询性能工具可提供更多有关查询执行细节的信息。
...
有时需要将自定义属性添加到现有索引中，以使用该索引查询其他约束。有关详细信息，请参阅文档[内容搜索和索引](/help/operations/indexing.md#changing-an-index)。本文档的[JCR查询备忘表](#jcr-query-cheatsheet)部分描述了索引上的属性定义必须如何支持特定的查询类型。
...
本文档的[JCR查询备忘表](#jcr-query-cheatsheet)部分列出了可用的约束，并概述了索引定义的外观必须如何，才能选取它。 使用[查询性能工具](#query-performance-tool)测试查询并确保使用了正确的索引，并且查询引擎不需要评估索引之外的约束。
...
如果必须完全处理潜在的大型结果集，请参阅本文档的[包含大型结果集的查询](#queries-with-large-result-sets)部分。

## 查询性能工具 {#query-performance-tool}

查询性能工具(位于`/libs/granite/operations/content/diagnosistools/queryPerformance.html`，可通过Cloud Manager[中的](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/developer-console.html?lang=zh-Hans#queries)Developer Console使用)提供 — 
...
这些表可帮助识别未完全编制索引的查询(请参阅[使用索引](#use-an-index)或读取的节点过多（另请参阅[存储库遍历](#repository-traversal)和[索引遍历](#index-traversal)）。 此类查询将突出显示，并将相应的关注领域标示为红色。

提供了`Reset Statistics`选项以删除表中收集的所有现有统计信息。 这允许执行特定查询（通过应用程序本身或Explain查询工具）并分析执行统计信息。
...
* 前20个查询结果的路径（如果已选中`Read first page of results`复选框）
...
* 所有3个限制都由索引处理 — 
   ...
   * 属性限制
      * 因为`+jcr:content/metadata/dc:title:My Title`出现在Lucene查询中。
* 排序由索引处理
   * 因为`ordering:[{ propertyName : jcr:created, propertyType : UNDEFINED, order : ASCENDING }]`出现在Lucene查询中。

此类查询可能执行良好，因为从索引查询返回的结果不会在查询引擎中进一步过滤（除了访问控制过滤）。 但是，如果不遵循最佳实践，此类查询仍可能执行缓慢 — 请参阅下面的[索引遍历](#index-traversal)。
...
* 属性限制`jcr:content/metadata/myProperty = "My Property Value"`不是在索引上执行，而是作为查询引擎筛选对基础Lucene查询的结果应用。
   * 这是因为`+jcr:content/metadata/myProperty:My Property Value`未出现在Lucene查询中，因为该属性未在该查询使用的`damAssetLucene-9`索引中编制索引。

此查询执行计划将导致`/content/dam`下的每个资产都从索引中读取，然后由查询引擎进一步筛选（这将仅包括那些与结果集中的非索引属性限制匹配的资产）。

即使只有一小部分资源与限制`jcr:content/metadata/myProperty = "My Property Value"`匹配，查询也必须读取大量节点以（尝试）填充请求的结果“页面”。 这可能会导致查询性能不佳，查询性能工具中显示为`Read Optimization`得分很低)，并可能导致出现警告消息，指示正在遍历大量节点（请参阅[索引遍历](#index-traversal)）。

要优化此第二个查询的性能，请创建`damAssetLucene-9`索引的自定义版本(`damAssetLucene-9-custom-1`)并添加以下属性定义 — 
...
* 对于具有现有索引（如`dam:Asset`或`cq:Page`）的节点类型，除了添加新索引外，还首选扩展OOTB索引。
   * 强烈建议不要在`dam:Asset`节点类型上添加新索引，尤其是全文索引（请参阅[此注释](/help/operations/indexing.md##index-names-index-names)）。
* 添加新索引时
   * 始终定义“lucene”类型的索引。
   * 在索引定义（和关联的查询）和`selectionPolicy = tag`中使用索引标记以确保该索引仅用于预期的查询。
   * 请确保同时提供了`queryPaths`和`includedPaths`（通常使用相同的值）。
   * 使用`excludedPaths`排除不包含有用结果的路径。
   * 仅在需要时使用`analyzed`属性，例如，您需要仅对该属性使用全文查询限制时。
   * 始终指定`async = [ async, nrt ] `、`compatVersion = 2`和`evaluatePathRestrictions = true`。
   * 仅在需要节点作用域全文索引时指定`nodeScopeIndex = true`。

>[!NOTE]
>
>有关详细信息，请参阅[Oak Lucene索引文档](https://jackrabbit.apache.org/oak/docs/query/lucene.html)。

自动Cloud Manager管道检查将强制实施上述某些最佳实践。
...
| 原因 | 缓解 |
|----------|--------------|
| `p.guessTotal`的省略（或使用非常大的guessTotal）会导致QueryBuilder迭代大量结果计数结果 | 为`p.guessTotal`提供适当的值 |
| 在查询生成器中使用较大限制或无限制限制（即`p.limit=-1`） | 为`p.limit`使用适当的值（最好为1000或更低） |
