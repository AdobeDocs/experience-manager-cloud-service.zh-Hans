...
<table style="table-layout:auto">
 <tbody>
  ...
  <tr>
    <td>版本清除</td>
    <td>Adobe</td>
    <td>对于现有环境（2024年2月4日之前创建的环境），清除处于禁用状态，并且将来不会启用，除非客户明确启用此功能，届时，他们还可以使用自定义值来配置此功能。<br><br> <!--Alexandru: leave the two line breaks in place, otherwise spacing won't render properly-->新环境（从2024年2月4日开始创建的环境）默认情况下将启用清除，但值如下，客户可以使用自定义值配置。
     <ol>
  ...
  <tr>
    <td>审核日志清除</td>
    <td>Adobe</td>
    <td>对于现有环境（2024年2月4日之前创建的环境），清除处于禁用状态，并且将来不会启用，除非客户明确启用此功能，届时，他们还可以使用自定义值来配置此功能。<br><br> <!-- See above for the two line breaks -->新环境（从2024年2月4日开始创建的环境）在下的默认情况下将启用清除 <code>/content</code> 节点，具体行为如下：
     <ol>
